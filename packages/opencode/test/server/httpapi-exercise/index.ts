...
const scenarios: Scenario[] = [
  ...
  http.protected.get("/settings/agents", "settings.agent.list").inProject({ git: false }).json(200, array, "status"),
  http.protected
    .post("/settings/agents/import", "settings.agent.import")
    .inProject({ git: false })
    .mutating()
    .seeded((ctx) =>
      Effect.promise(async () => {
        const source = path.join(ctx.directory!, "exercise-agent.md")
        await Bun.write(
          source,
          [
            "---",
            "name: Exercise Agent",
            "description: Imported through the settings API exerciser.",
            "mode: subagent",
            "---",
            "",
            "You are an imported exercise agent.",
            "",
          ].join("\n"),
        )
        return source
    ...
    .at((ctx) => ({
      path: "/settings/agents/import",
      headers: ctx.headers(),
      body: { source: ctx.state },
    }))
    .json(200, (body) => {
      object(body)
      check(body.imported === 1, "agent import should import the source markdown file")
      check(body.skipped === 0, "agent import should not skip a valid source markdown file")
      array(body.items)
    }),
  http.protected.get("/settings/skills", "settings.skill.list").inProject({ git: false }).json(200, array, "status"),
  http.protected
    .post("/settings/skills/import", "settings.skill.import")
    .inProject({ git: false })
    .mutating()
    .seeded((ctx) =>
      Effect.promise(async () => {
        const source = path.join(ctx.directory!, "SKILL.md")
        await Bun.write(
          source,
          [
            "---",
            ...
            "---",
            "",
            "Use this skill only for HTTP API import validation.",
            "",
          ].join("\n"),
        )
        return source
    ...
    .at((ctx) => ({
      path: "/settings/skills/import",
      headers: ctx.headers(),
      body: { source: ctx.state },
    }))
    .json(200, (body) => {
      object(body)
      check(body.imported === 1, "skill import should import the source SKILL.md file")
      check(body.skipped === 0, "skill import should not skip a valid source SKILL.md file")
      array(body.items)
    }),
  ...
  http.protected.get("/powersnexus/status", "powersnexus.status").status(400),
  http.protected.get("/powersnexus/changes", "powersnexus.changes").status(400),
  http.protected
    .post("/powersnexus/changes", "powersnexus.createChange")
    .at((ctx) => ({
      path: "/powersnexus/changes",
      headers: ctx.headers(),
      body: {
        actionID: "httpapi-powersnexus-create",
        expectedRevision: 0,
        changeName: "httpapi-change",
        level: "L2",
      },
    }))
    .status(400),
  http.protected
    .post("/powersnexus/bind", "powersnexus.bind")
    .at((ctx) => ({
      path: "/powersnexus/bind",
      headers: ctx.headers(),
      body: {
        actionID: "httpapi-powersnexus-bind",
        expectedRevision: 0,
        changeName: "httpapi-change",
    ...
    .status(400),
  http.protected
    .post("/powersnexus/actions", "powersnexus.action")
    .at((ctx) => ({
      path: "/powersnexus/actions",
      headers: ctx.headers(),
      body: {
        changeName: "httpapi-change",
        actionID: "httpapi-powersnexus-action",
        expectedRevision: 0,
    ...
    .status(400),
  http.protected
    .post("/powersnexus/verify", "powersnexus.verify")
    .at((ctx) => ({
      path: "/powersnexus/verify",
      headers: ctx.headers(),
      body: {
        actionID: "httpapi-powersnexus-verify",
        expectedRevision: 0,
        bindingID: "pnb_httpapi_missing",
        steps: [],
      },
    }))
    .status(400),
  http.protected
    .get("/powersnexus/runs/{id}", "powersnexus.run")
    .at((ctx) => ({ path: "/powersnexus/runs/pnr_httpapi_missing", headers: ctx.headers() }))
    .json(),
  http.protected
    .post("/powersnexus/runs/{id}/cancel", "powersnexus.runCancel")
    .at((ctx) => ({
      path: "/powersnexus/runs/pnr_httpapi_missing/cancel",
      headers: ctx.headers(),
      body: { actionID: "httpapi-powersnexus-cancel" },
    }))
    .json(),
  http.protected
    .post("/powersnexus/runs/{id}/retry", "powersnexus.runRetry")
    .at((ctx) => ({
      path: "/powersnexus/runs/pnr_httpapi_missing/retry",
      headers: ctx.headers(),
      body: { actionID: "httpapi-powersnexus-retry" },
    }))
    .status(400),
  http.protected
    .get("/powersnexus/runs/{id}/log", "powersnexus.runLog")
    .at((ctx) => ({
      path: "/powersnexus/runs/pnr_httpapi_missing/log?stepID=build&stream=stdout&offset=0&limit=1024",
      headers: ctx.headers(),
    }))
    .status(400),
  http.protected
    .get("/powersnexus/evidence", "powersnexus.evidence")
    .at((ctx) => ({ path: "/powersnexus/evidence?runID=pnr_httpapi_missing", headers: ctx.headers() }))
    .status(404),
  http.protected
    .post("/powersnexus/archive", "powersnexus.archive")
    .at((ctx) => ({
      path: "/powersnexus/archive",
      headers: ctx.headers(),
      body: { actionID: "httpapi-powersnexus-archive", expectedRevision: 1, bindingID: "pnb_httpapi_missing" },
    }))
    .status(404),
  http.protected.get("/powersnexus/version", "powersnexus.version").status(400),
  http.protected
    .post("/powersnexus/update/check", "powersnexus.update.check")
    .at((ctx) => ({
      path: "/powersnexus/update/check",
      headers: ctx.headers(),
      body: { requestID: "httpapi-powersnexus-check", channel: "stable" },
    }))
    .status(400),
  http.protected
    .post("/powersnexus/update/install", "powersnexus.update.install")
    .at((ctx) => ({
      path: "/powersnexus/update/install",
      headers: ctx.headers(),
      body: {
        requestID: "httpapi-powersnexus-install",
        targetDigest: "a".repeat(64),
        expectedActiveDigest: "b".repeat(64),
      },
    }))
    .status(400),
  http.protected
    .post("/powersnexus/update/activate", "powersnexus.update.activate")
    .at((ctx) => ({
      path: "/powersnexus/update/activate",
      headers: ctx.headers(),
      body: {
        requestID: "httpapi-powersnexus-activate",
        targetDigest: "a".repeat(64),
        expectedActiveDigest: "b".repeat(64),
      },
    }))
    .status(400),
  http.protected
    .post("/powersnexus/update/rollback", "powersnexus.update.rollback")
    .at((ctx) => ({
      path: "/powersnexus/update/rollback",
      headers: ctx.headers(),
      body: { requestID: "httpapi-powersnexus-rollback", expectedActiveDigest: "b".repeat(64) },
    }))
    .status(400),
  http.protected
