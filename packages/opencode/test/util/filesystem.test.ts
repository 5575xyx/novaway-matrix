...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "novaway.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "novaway.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "novaway.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "novaway.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["novaway.json", "novaway.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "novaway.json"),
        path.join(tmp.path, "novaway.jsonc"),
        path.join(project, "novaway.json"),
        path.join(project, "novaway.jsonc"),
      ])
    })
  })
