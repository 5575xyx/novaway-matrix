...
describe("skill", () => {
  it.live("includes root utility skills as built-in skills", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const skill = yield* Skill.Service
          const list = yield* skill.all()
          const find = list.find((item) => item.name === "find-skills")
          const create = list.find((item) => item.name === "skill-creator")

          expect(find).toBeDefined()
          expect(find!.location).toBe("<built-in>")
          expect(find!.description).toContain("discover and install")
          expect(find!.content.startsWith("# Find Skills")).toBe(true)

          expect(create).toBeDefined()
          expect(create!.location).toBe("<built-in>")
          expect(create!.description).toContain("Create new skills")
          expect(create!.content.startsWith("# Skill Creator")).toBe(true)
        }),
      { git: true },
    ),
  )

  it.live("discovers skills from .novaway/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".novaway", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => !s.builtIn)
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".novaway", "skill", "dir-skill", "SKILL.md"),
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".novaway", "skill", "dir-skill"))
  ...
  it.live("discovers multiple skills from .novaway/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".novaway", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".novaway", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".novaway", "skill", "no-frontmatter", "SKILL.md"),
              `# No Frontmatter
...
`,
          ...
          const skill = yield* Skill.Service
          expect((yield* skill.all()).filter((s) => !s.builtIn)).toEqual([])
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".novaway", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".novaway", "skill", "opencode-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .novaway/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".novaway", "skill", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .novaway/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".novaway", "skills", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .novaway/skills directory.
---
