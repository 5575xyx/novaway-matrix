...
describe("tool.write", () => {
  describe("new file creation", () => {
    ...
    it.instance("uses project-relative permission path for .novaway plans when worktree is root", () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const filepath = path.join(test.directory, ".novaway", "plans", "test-plan.md")
        let patterns: readonly string[] = []
        yield* run(
          { filePath: filepath, content: "plan content" },
          {
            ...ctx,
            ask: (input) =>
              Effect.sync(() => {
        ...
        expect(patterns.map((item) => item.replaceAll("\\", "/"))).toEqual([".novaway/plans/test-plan.md"])
      }),
    )
  })
