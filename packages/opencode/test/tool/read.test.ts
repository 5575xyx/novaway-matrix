...
root_type Monster;`
  ...
  it.live("extracts Office document text before binary rejection", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped()
      const docx = yield* Effect.promise(() =>
        zip({
          "word/document.xml":
            "<w:document><w:body><w:p><w:r><w:t>办公项目方案</w:t></w:r></w:p></w:body></w:document>",
        }),
      )
      yield* put(path.join(dir, "proposal.docx"), docx)

      const result = yield* exec(dir, { filePath: path.join(dir, "proposal.docx") })

      expect(result.output).toContain("<type>office:docx</type>")
      expect(result.output).toContain("办公项目方案")
    }),
  )
})
