...
type TestConfigInput = { compaction?: Config.Info["compaction"]; memory?: Config.Info["memory"] }

function isTestConfigInput(input: Config.Info["compaction"] | TestConfigInput | undefined): input is TestConfigInput {
  return !!input && ("memory" in input || "compaction" in input)
}

function cfg(input?: Config.Info["compaction"] | TestConfigInput) {
  const base = Schema.decodeUnknownSync(Config.Info)({}) as Config.Info
  const options = isTestConfigInput(input) ? input : { compaction: input }
  return TestConfig.layer({
    get: () => Effect.succeed({ ...base, compaction: options?.compaction, memory: options?.memory }),
  })
}
...
function compactionProcessLayer(options?: CompactionProcessOptions) {
  ...
  return Layer.mergeAll(
    SessionCompaction.layer.pipe(Layer.provide(processor), Layer.provide(Memory.defaultLayer)),
    processor,
    bus,
    status,
    Memory.defaultLayer,
...
describe("session.compaction.process", () => {
  ...
  itCompaction.instance(
    "creates pending memory review candidates before compacting dropped history",
    Effect.gen(function* () {
      const ssn = yield* SessionNs.Service
      const memory = yield* Memory.Service
      const session = yield* ssn.create({})
      yield* createUserMessage(session.id, "remember that compaction should preserve durable project preferences")
      yield* createUserMessage(session.id, "recent turn stays in the tail")
      yield* createSummaryCompaction(session.id)
      ...
      const candidates = yield* memory.listReviewCandidates({ sessionID: session.id, status: "pending", limit: 10 })
      expect(candidates).toHaveLength(1)
      expect(candidates[0].content).toBe("compaction should preserve durable project preferences")
      expect(candidates[0].tags).toContain("compaction")
    }).pipe(
      withCompaction({
        config: cfg({
          compaction: { tail_turns: 1, preserve_recent_tokens: 10_000 },
          memory: { enabled: true, review_enabled: true },
        }),
      }),
    ),
  )
  ...
  itCompaction.instance(
    "skips compaction memory review when memory review is disabled",
    Effect.gen(function* () {
      const ssn = yield* SessionNs.Service
      const memory = yield* Memory.Service
      const session = yield* ssn.create({})
      yield* createUserMessage(session.id, "remember that disabled compaction review must not create candidates")
      yield* createUserMessage(session.id, "recent turn stays in the tail")
      yield* createSummaryCompaction(session.id)
      ...
      withCompaction({
        config: cfg({
          compaction: { tail_turns: 1, preserve_recent_tokens: 10_000 },
          memory: { enabled: true, review_enabled: false },
        }),
      }),
    ),
  )
