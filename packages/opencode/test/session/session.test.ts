...
describe("Session", () => {
  ...
  it.instance(
    "archives session after creating pending memory review candidates",
    () =>
      Effect.gen(function* () {
        const session = yield* SessionNs.Service
        ...
        yield* session.updatePart({
          ...
          sessionID: info.id,
          type: "text",
          text: "remember that archived sessions should preserve durable facts",
        })

        yield* session.setArchived({ sessionID: info.id, time: Date.now() })

        const candidates = yield* memory.listReviewCandidates({
        ...
        expect(candidates[0].content).toBe("archived sessions should preserve durable facts")
        expect(candidates[0].tags).toContain("session-end")

        yield* session.remove(info.id)
      }),
    { config: { memory: { enabled: true, review_enabled: true } } },
  )

  it.instance(
    "archives session after creating pending evolution candidates",
    () =>
      Effect.gen(function* () {
        const session = yield* SessionNs.Service
        ...
        yield* session.updatePart({
          ...
          sessionID: info.id,
          type: "text",
          text: "please improve that future reviews should include verification commands before delivery",
        })

        yield* session.setArchived({ sessionID: info.id, time: Date.now() })

        const candidates = yield* evolution.list({
          projectID: ProjectID.global,
          sessionID: info.id,
          status: "pending",
          limit: 10,
        ...
        expect(candidates).toHaveLength(1)
        expect(candidates[0].content).toBe("future reviews should include verification commands before delivery")
        expect(candidates[0].tags).toContain("session-end")
