  ...
  if (!imageUrl && bgIR.children) {
    ...
    if (imgChild) {
      ...
      if (existsSync(imgPath)) {
        ...
        const imgOpacity = imgChild.styles?.opacity !== undefined ? parseFloat(imgChild.styles.opacity) : 1
  ...
  if (backgroundImage && backgroundImage !== "none") {
    ...
    if (bottomGrad && bottomGrad.stops && bottomGrad.stops.length > 0) {
      ...
      if (visibleStops.length >= 2) {
        ...
        if (bottomGrad.type === "linear" && visibleStops[0] !== bestStop && visibleStops[0].color !== bestStop.color) {
...
export function buildTextElement(node) {
  ...
  if (s.color && extractCssAlpha(s.color) < 0.05) {
    ...
    const hasStroke =
      (stroke && stroke !== "none" && !stroke.startsWith("0")) || (strokeWidth && parseFloat(strokeWidth) > 0)
  ...
  const shortTextW = shortText ? pxToInch(Math.max(b.w, fontSizePx * text.trim().length * 0.7) + bufferPx) : null
  ...
  if (!textRuns && (padTop > 0 || padRight > 0 || padBottom > 0 || padLeft > 0)) {
    options.margin = [pxToPt(padTop), pxToPt(padRight), pxToPt(padBottom), pxToPt(padLeft)]
  }
...
function hasVisualDecoration(node) {
  ...
  if (
    ...
    (s.backgroundImage.includes("gradient") || s.backgroundImage.includes("url("))
  )
    return true
  if (
...
export function buildShapeElement(node) {
  ...
  const isCircular =
    r > 0 &&
    Math.abs(b.w - b.h) < Math.max(b.w, b.h) * 0.1 &&
    (s.borderRadius?.includes("%") ? parseFloat(s.borderRadius) >= 50 : (r / Math.min(b.w, b.h)) * 100 >= 50)
  const hasGradFill = s.backgroundImage && s.backgroundImage !== "none" && s.backgroundImage.includes("gradient")
  const isLargeRadius = b.w >= 200 && b.h >= 200
  const isOffCanvas = b.x < -10 || b.y < -10 || b.x + b.w > _currentCanvasW + 10 || b.y + b.h > _currentCanvasH + 10
  ...
  const hasGradientBgImage = s.backgroundImage && s.backgroundImage !== "none" && s.backgroundImage.includes("gradient")
  ...
  if (hasGradientBgImage && _currentGradientHandler) {
    ...
    if (grad && grad.stops.length >= 2) {
      ...
      const isEllipseShape = (() => {
        ...
        const radiusPct = s.borderRadius?.includes("%") ? parseFloat(s.borderRadius) : (r / Math.min(b.w, b.h)) * 100
  ...
  if (gradientApplied) {
    ...
    const allRgba = [
      ...s.backgroundImage.matchAll(/rgba\(\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)\s*\)/g),
    ]
    let bestMatch = null
    let bestAlpha = 0
  ...
  if (radius > 0) {
    ...
    const radiusPercent = s.borderRadius?.includes("%")
      ? parseFloat(s.borderRadius)
      : (radius / Math.min(node.bounds.w, node.bounds.h)) * 100
...
export function flattenIRToElements(node, deckDir, parentBorderRadius = 0, parentBgColor = null) {
  ...
  if (hasVisualDecoration(node) && tag !== "IMG") {
    ...
    if (s.backgroundImage && s.backgroundImage !== "none" && !isGradientText(node)) {
      ...
      if (bgUrlMatch) {
        ...
        try {
          bgImgPath = decodeURIComponent(bgImgPath)
        } catch {
          /* keep as-is */
        }
  ...
  const currentBgHex = s.backgroundColor && !isTransparent(s.backgroundColor) ? cssColorToHex(s.backgroundColor) : null
...
export function buildSlideFromIR(pptx, ir, deckDir) {
  ...
  if (_currentGradientHandler) {
    const candidates = [ir.wrapperBgImage, ir.bodyBgImage, ir.bg?.styles?.backgroundImage].filter(
      (s) => s && s !== "none" && s.includes("gradient") && !s.includes("url("),
    )
    ...
    for (const src of candidates) {
      ...
      if (!bottomLaid && radialLayers.length > 0) {
        const bottomColor = cssColorToHex(ir.wrapperBgColor) || cssColorToHex(ir.bodyBgColor) || "FFFFFF"
      ...
      for (const rl of radialLayers) {
        ...
        const visible = r.stops.find((st) => !st.isTransparent && (!st.rawColor || extractCssAlpha(st.rawColor) > 0.02))
        if (!visible) continue
