...
function mapBar(option, ChartType) {
  ...
  if (!chartColors && Array.isArray(option.color)) {
    chartColors = option.color
      .map((c) => (typeof c === "string" ? c.replace("#", "").toUpperCase() : null))
      .filter(Boolean)
  }
...
function mapGauge(option, ChartType) {
  ...
  if (items.length === 0) return null
  const first = items[0]
  const value = typeof first === "number" ? first : first && typeof first === "object" ? Number(first.value) : 0
...
function mapCombo(option, ChartType) {
  ...
  const valFmt = extractValueAxisFormatCode(option, "yAxis")
  const valFmt2 =
    Array.isArray(option.yAxis) && option.yAxis[1] ? formatterToCode(option.yAxis[1].axisLabel?.formatter) : null
  const catRotate = extractAxisLabelRotate(option, "xAxis")
...
export function echartsOptionToPptx(option, ChartType) {
  ...
  try {
    switch (type) {
      case "bar":
        return mapBar(option, ChartType)
      case "line":
        return mapLine(option, ChartType)
      case "area":
        return mapArea(option, ChartType)
      case "pie":
        return mapPie(option, ChartType, false)
      case "doughnut":
        return mapPie(option, ChartType, true)
      case "radar":
        return mapRadar(option, ChartType)
      case "scatter":
        return mapScatter(option, ChartType)
      case "funnel":
        return mapFunnel(option, ChartType)
      case "gauge":
        return mapGauge(option, ChartType)
      case "combo":
        return mapCombo(option, ChartType)
      default:
