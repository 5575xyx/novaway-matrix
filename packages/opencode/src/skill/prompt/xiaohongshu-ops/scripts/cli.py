...
def cmd_inject_cookies(args: argparse.Namespace) -> None:
    """注入 cookie 到浏览器（从 JSON 文件或 stdin 读取）。
    支持的 JSON 格式：
    - 数组：[{"name": "xxx", "value": "yyy", "domain": ".xiaohongshu.com", ...}]
    - 对象：{"cookies": [...]} 或直接 cookie 字段
    """
    ...
    if args.file:
        ...
        raw = json_mod.load(sys.stdin)
    else:
        _output({"error": "请通过 --file 或 stdin 提供 cookie JSON 数据"}, exit_code=2)
        return
    ...
    if isinstance(raw, list):
        ...
        cookies = raw.get("cookies") or raw.get("cookie_list") or [raw]
    else:
        _output({"error": "无效的 cookie 格式"}, exit_code=2)
        return
    ...
    if not valid_cookies:
        _output({"error": "没有有效的 cookie（每个 cookie 必须包含 name 和 value）"}, exit_code=2)
        return

    # 注入到浏览器
    browser, page = _connect(args)
    try:
        result = page.set_cookies(valid_cookies)
        _output({
            "success": True,
            "message": f"已注入 {result.get('set', 0)} 个 cookie",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # inject-cookies
    sub = subparsers.add_parser("inject-cookies", help="注入 cookie 到浏览器（从外部登录状态同步）")
    sub.add_argument("--file", help="cookie JSON 文件路径（不指定则从 stdin 读取）")
    sub.set_defaults(func=cmd_inject_cookies)
