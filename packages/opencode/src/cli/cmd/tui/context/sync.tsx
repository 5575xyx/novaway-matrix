  ...
  init: () => {
    ...
    async function bootstrap(input: { fatal?: boolean } = {}) {
      ...
      await Promise.allSettled(blockingRequests.map((r) => r.promise))
        .then((settled) => {
          const providers = settledValue<Awaited<typeof providersPromise>>(settled[0])?.data
          const providerList = settledValue<Awaited<typeof providerListPromise>>(settled[1])?.data
          const agents = settledValue<Awaited<typeof agentsPromise>>(settled[2])?.data
          const config = settledValue<Awaited<typeof configPromise>>(settled[3])?.data
          const sessions = args.continue ? settledValue<Awaited<typeof sessionListPromise>>(settled[5]) : undefined
          ...
          batch(() => {
            ...
            if (agents) setStore("agent", reconcile(agents))
            if (config) setStore("config", reconcile(config))
    ...
    function settledValue<T>(result: PromiseSettledResult<unknown> | undefined) {
