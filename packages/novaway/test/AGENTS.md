...
- `config?: Partial<Config.Info>` - Write an `NovaWay.json` config file
...
- Directories are created in the system temp folder with prefix `NovaWay-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("NovaWay-test-")
  }),
)
```
