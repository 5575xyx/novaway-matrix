...
import { EffectFlock } from "@novaway/core/util/effect-flock"
...
async function writeManagedSettings(settings: object, filename = "novaway.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "novaway.json") {
...
test("adds default global plugins to existing config file that lacks them", async () => {
  ...
  try {
    ...
    await Bun.write(path.join(tmp.path, "novaway.json"), JSON.stringify(initial, null, 2))
...
test("creates novaway.json with default plugins when only novaway.jsonc exists", async () => {
  await using tmp = await tmpdir()
  const prev = Global.Path.config
  ;(Global.Path as { config: string }).config = tmp.path
  await clear(true)
  ...
  try {
    const initial: Record<string, unknown> = {
      $schema: "https://opencode.ai/config.json",
      plugin: ["old-plugin@1.0"],
    }
    await Bun.write(path.join(tmp.path, "novaway.jsonc"), JSON.stringify(initial, null, 2))
...
test("updateGlobal removes deleted agent permission.skill keys", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
  ...
  try {
    ...
    const parsed = ConfigParse.schema(Config.Info, ConfigParse.jsonc(content, file), file)
    expect(parsed.agent?.["test-agent"]?.permission?.skill).toEqual({
      "*": "deny",
      "skill-a": "allow",
    })
  } finally {
...
test("does not create global config when OPENCODE_CONFIG_DIR is set", async () => {
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "novaway.jsonc"))).toBe(false)
  } finally {
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "novaway.jsonc")
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["NOVAWAY_CONSOLE_TOKEN"]
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["NOVAWAY_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["NOVAWAY_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "novaway.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .novaway directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".novaway")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".novaway", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .novaway/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".novaway")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .novaway/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".novaway")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .novaway/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".novaway")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .novaway config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".novaway")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .novaway config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "novaway.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .novaway config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "novaway.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".novaway")
...
test("local .novaway config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .novaway directory config enables it
      const opencodeDir = path.join(dir, ".novaway")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "novaway.json"),
    ...
    if (urlStr.includes(".well-known/opencode")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/novaway.json",
              headers: {
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(remoteFetchedUrl).toBe("https://config.example.com/novaway.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.novaway/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.novaway/plugin/demo.ts", "file:///project/.novaway/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.novaway/plugin/demo.ts"])
  })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"]
    process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .novaway/ directories when flag is set", async () => {
    const originalEnv = process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"]
    process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .novaway directory with a command
          const opencodeDir = path.join(dir, ".novaway", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await withTestInstance({
        ...
        fn: async (ctx) => {
          ...
          // Project .novaway should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NOVAWAY_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["NOVAWAY_CONFIG_DIR"]
      process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["NOVAWAY_CONFIG_DIR"]
      } else {
        process.env["NOVAWAY_CONFIG_DIR"] = originalConfigDir
      }
    }
  })
  ...
  test("OPENCODE_CONFIG_DIR still works when flag is set", async () => {
    ...
    try {
      ...
      process.env["NOVAWAY_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["NOVAWAY_CONFIG_DIR"] = configDirTmp.path
...
describe("OPENCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NOVAWAY_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["NOVAWAY_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["NOVAWAY_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["NOVAWAY_CONFIG_CONTENT"]
      }
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.novaway.managed.test",
          PayloadType: "ai.novaway.managed",
