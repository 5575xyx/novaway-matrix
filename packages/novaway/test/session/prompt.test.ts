...
const writeBytes = Effect.fn("test.writeBytes")(function* (file: string, bytes: Uint8Array) {
  const fs = yield* AppFileSystem.Service
...
async function docxBytes(text: string) {
  ...
  await writer.add(
    "word/document.xml",
    new TextReader(`<w:document><w:body><w:p><w:r><w:t>${text}</w:t></w:r></w:p></w:body></w:document>`),
  )
...
it.instance(
  "converts uploaded office attachments into text-only prompt context",
  () =>
    Effect.gen(function* () {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const session = yield* sessions.create({})
      const bytes = yield* Effect.promise(() => docxBytes("Quarterly plan summary"))
      ...
      const msg = yield* prompt.prompt({
        ...
        noReply: true,
        parts: [
          { type: "text", text: "Create a presentation from this document" },
          {
            type: "file",
      ...
      expect(text.some((part) => part.includes("Extracted Office attachment text from plan.docx"))).toBe(true)
      expect(text.some((part) => part.includes("Quarterly plan summary"))).toBe(true)
...
it.instance(
  "converts local office files into text-only prompt context",
  () =>
    Effect.gen(function* () {
      const { directory: dir } = yield* TestInstance
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const session = yield* sessions.create({})
      const file = path.join(dir, "proposal.docx")
      const bytes = yield* Effect.promise(() => docxBytes("Client proposal details"))
      yield* writeBytes(file, bytes)
      ...
      expect(text.some((part) => part.includes("Extracted Office attachment text from proposal.docx"))).toBe(true)
      expect(text.some((part) => part.includes("Client proposal details"))).toBe(true)
      expect(stored.parts.some((part) => part.type === "file")).toBe(false)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* llm.text("packages/novaway/src/session/processor.ts")
