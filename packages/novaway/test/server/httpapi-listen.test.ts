...
async function startListener() {
  Flag.NOVAWAY_SERVER_PASSWORD = auth.password
  Flag.NOVAWAY_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  Flag.NOVAWAY_SERVER_PASSWORD = undefined
  Flag.NOVAWAY_SERVER_USERNAME = auth.username
...
describe("HttpApi Server.listen", () => {
  ...
  // the server resolved the PTY in its own cwd context 鈥?where the project
  // PTY isn't registered 鈥?and returned 404. The fix is to always pass
  ...
  testPty("PTY connect token requires matching directory across mint and connect", async () => {
    ...
    try {
      ...
      // Mint without directory 鈥?server uses its own cwd, can't find the PTY.
      ...
      // Mint with the project directory 鈥?succeeds, ticket binds to that scope.
      ...
      // Same directory on the WS upgrade 鈫?consume succeeds.
