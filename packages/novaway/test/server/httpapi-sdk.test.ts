...
import { extractOfficeDocumentText } from "../../src/util/office-document"
...
const original = {
  OPENCODE_SERVER_PASSWORD: Flag.NOVAWAY_SERVER_PASSWORD,
  OPENCODE_SERVER_USERNAME: Flag.NOVAWAY_SERVER_USERNAME,
}
...
function app(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  Flag.NOVAWAY_SERVER_PASSWORD = input?.password
  Flag.NOVAWAY_SERVER_USERNAME = input?.username
...
async function pptxTemplateBytes() {
  ...
  await writer.add(
    ...
    new TextReader(
      [
        ...
        "<a:tbl><a:tr><a:tc><a:p><a:r><a:t>H1</a:t></a:r></a:p></a:tc><a:tc><a:p><a:r><a:t>H2</a:t></a:r></a:p></a:tc></a:tr><a:tr><a:tc><a:p><a:r><a:t>A1</a:t></a:r></a:p></a:tc><a:tc><a:p><a:r><a:t>A2</a:t></a:r></a:p></a:tc></a:tr></a:tbl>",
        "</p:sld>",
      ].join(""),
    ),
  )
  await writer.add(
    ...
    new TextReader(
      '<Relationships><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image1.png"/><Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart" Target="../charts/chart1.xml"/></Relationships>',
    ),
  )
  await writer.add("ppt/media/image1.png", new TextReader("OLD_IMAGE"))
  await writer.add(
...
afterEach(async () => {
  Flag.NOVAWAY_SERVER_PASSWORD = original.OPENCODE_SERVER_PASSWORD
  Flag.NOVAWAY_SERVER_USERNAME = original.OPENCODE_SERVER_USERNAME
  await disposeAllInstances()
  await resetDatabase()
})
...
describe("HttpApi SDK", () => {
  ...
  httpapiInstance(
    "fills pptx templates through generated SDK office route",
    { serverPath: "raw", git: false },
    ({ sdk, directory }) =>
      Effect.gen(function* () {
        const template = yield* call(() => pptxTemplateBytes())
        const filled = yield* call(() =>
          sdk.office.pptxTemplate.fill({
            directory,
            filename: "filled-template.pptx",
            templateBase64: Buffer.from(template).toString("base64"),
            slides: [
              {
                slide: 1,
        ...
        const saved = yield* AppFileSystem.Service.use((fs) =>
          fs.readFile(path.join(directory, ".novaway", "office", "ppt", "filled-template.pptx")),
        )
        const text = yield* call(() => extractOfficeDocumentText({ filename: "filled-template.pptx", bytes: saved }))
        const image = yield* call(() => zipEntryBytes(saved, "ppt/media/image1.png"))
        const chart = yield* call(() => zipEntryBytes(saved, "ppt/charts/chart1.xml"))
        const artifacts = yield* call(() => sdk.office.artifact.list({ directory }))

        expect(filled.response.status).toBe(200)
        expect(filled.data?.path).toBe(path.join(".novaway", "office", "ppt", "filled-template.pptx"))
        expect(text?.text).toContain("SDK Office Title")
        expect(text?.text).toContain("SDK Office Body")
        expect(text?.text).toContain("Revenue")
        expect(text?.text).toContain("120")
        expect(Buffer.from(image).toString()).toBe("SDK_IMAGE")
        expect(Buffer.from(chart).toString()).toContain("<c:v>160</c:v>")
        expect(artifacts.data?.map((item) => item.filename)).toContain("filled-template.pptx")
      }),
  )
