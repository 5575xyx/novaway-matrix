...
describe("office platform seed workflows", () => {
  test("seeds three differentiated office workflows", () => {
    ...
    expect(seeds.map((item) => item.id)).toEqual(["sales-weekly-report", "competitor-watch", "batch-file-cleanup"])
    expect(
      seeds.every(
        (item) => item.version === 1 && item.enabled === true && item.createdAt === now && item.updatedAt === now,
      ),
    ).toBe(true)
    ...
    expect(seeds.some((item) => item.prompt.includes("{source_path}") && item.prompt.includes("{output_dir}"))).toBe(
      true,
    )
