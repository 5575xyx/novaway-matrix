...
- The server still lives in `packages/novaway`.
...
Until enough shared service code lives outside `packages/novaway`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/novaway`

It should not import `packages/novaway` services while `packages/novaway`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/novaway` implementation details.
