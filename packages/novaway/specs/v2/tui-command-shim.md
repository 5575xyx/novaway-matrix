...
Delete `packages/novaway/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/novaway/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/novaway/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/novaway`
- TUI plugin loader tests in `packages/novaway` if runtime plugin API wiring changed
