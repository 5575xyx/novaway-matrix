...
const BROWSER_EXTRACT_FN = () => {
  ...
  function extractBounds(el, wrapperRect) {
    ...
    if (transform && transform !== "none" && /matrix|rotate/.test(transform)) {
      ...
      if (offW > 0 && offH > 0 && (Math.abs(offW - r.width) > 1 || Math.abs(offH - r.height) > 1)) {
    ...
    for (const pseudo of ["::before", "::after"]) {
      try {
        ...
        if (position === "absolute" || position === "fixed") {
          ...
          const wouldCoverParent =
            finite && w >= parentRect.width * 0.9 && h >= parentRect.height * 0.9 && bgAlpha > 0.5
          ...
          if (small) {
            ...
            const heightIsShort =
              Number.isFinite(parseFloat(heightStr)) &&
              parseFloat(heightStr) > 0 &&
              parseFloat(heightStr) < parentRect.height * 0.3
            ...
            const widthIsShort =
              Number.isFinite(parseFloat(widthStr)) &&
              parseFloat(widthStr) > 0 &&
              parseFloat(widthStr) < parentRect.width * 0.3
...
export async function extractPage(page, htmlPath) {
  const fileUrl = htmlPath.startsWith("file://") ? htmlPath : `file://${path.resolve(htmlPath)}`
