...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for NovaWay.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal NovaWay sessions
...
NovaWay acp --cwd /path/to/project
```
...
```bash
NovaWay_ENABLE_QUESTION_TOOL=1 NovaWay acp
```
...
- **Terminal Integration**: Full terminal support via NovaWay's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | NovaWay acp
```
...
ACP sessions map cleanly to NovaWay's internal session model:
