  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // NovaWay stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const NovaWayProvider = providers.find((p) => p.id === "novaway")
  if (NovaWayProvider) {
    const [best] = Provider.sort(Object.values(NovaWayProvider.models))
    if (best) {
      return {
