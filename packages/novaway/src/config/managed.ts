...
const MANAGED_PLIST_DOMAIN = "ai.novaway.managed"

// Keys injected by macOS/MDM into the managed plist that are not NovaWay config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/novaway"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "novaway")
    default:
      return "/etc/novaway"
  }
}
...
export function managedConfigDir() {
  return process.env.NovaWay_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
