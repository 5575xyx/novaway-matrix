...
const TUI_SCHEMA_URL = "https://NovaWay.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from novaway.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const NovaWay = await NovaWayFiles(input)
  for (const file of NovaWay) {
...
async function NovaWayFiles(input: { directories: string[]; cwd: string }) {
  ...
  if (Flag.NOVAWAY_CONFIG) files.push(Flag.NOVAWAY_CONFIG)
