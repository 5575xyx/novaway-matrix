...
import { InstallationLocal, InstallationVersion } from "@novaway/core/installation/version"
...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@novaway/core/schema"
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for NovaWay serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://novaway.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://novaway.ai/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@novaway/core/schema`. See the definition
...
export class Service extends Context.Service<Service, Interface>()("@NovaWay/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      if (!Flag.NOVAWAY_CONFIG && !Flag.NOVAWAY_CONFIG_DIR && !Flag.NOVAWAY_CONFIG_CONTENT) {
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://NovaWay.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "NovaWay_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/NovaWay` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/NovaWay`))
            if (!response.ok) {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://NovaWay.ai/config.json"
            const source = `${url}/.well-known/NovaWay`
        ...
        if (Flag.NOVAWAY_CONFIG) {
          yield* merge(Flag.NOVAWAY_CONFIG, yield* loadFile(Flag.NOVAWAY_CONFIG))
          log.debug("loaded custom config", { path: Flag.NOVAWAY_CONFIG })
        }

        if (!Flag.NOVAWAY_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.NOVAWAY_CONFIG_DIR) {
          log.debug("loading config from NovaWay_CONFIG_DIR", { path: Flag.NOVAWAY_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".novaway") || dir === Flag.NOVAWAY_CONFIG_DIR) {
        ...
        if (process.env.NovaWay_CONFIG_CONTENT) {
          const source = "NovaWay_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.NovaWay_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from NovaWay_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["NOVAWAY_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("NovaWay_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.NOVAWAY_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.NOVAWAY_PERMISSION))
        }
        ...
        if (Flag.NOVAWAY_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.NOVAWAY_DISABLE_PRUNE) {
