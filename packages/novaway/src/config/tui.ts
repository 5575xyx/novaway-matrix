...
import { isRecord } from "@novaway/tui/util/record"
...
import { DEFAULT_GLOBAL_PLUGINS } from "@/config/config"
...
import { makeRuntime } from "@novaway/core/effect/runtime"
import { Filesystem } from "@/util/filesystem"
import { ConfigVariable } from "@/config/variable"
import { Npm } from "@novaway/core/npm"
import { FormatError, FormatUnknownError } from "@/cli/error"
import { TuiConfig } from "@novaway/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@NovaWay/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // can sync-throw 鈥?those become defects, which orElseSucceed wouldn't catch.
  ...
  const loadFile = (filepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      // Silent-swallow non-NotFound read errors (perms, EISDIR, IO) 鈫?log + skip.
      // Matches how parse/schema/plugin failures in load() are handled 鈥?every
  ...
  // Every config dir we may read from: global config dir, any `.novaway`
  // folders between cwd and home, and NovaWay_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NOVAWAY_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit NovaWay_TUI_CONFIG override, if set.
  if (Flag.NOVAWAY_TUI_CONFIG) {
    const configFile = Flag.NOVAWAY_TUI_CONFIG
    yield* mergeFile(acc, configFile)
    yield* Effect.logDebug("loaded custom tui config", { path: configFile })
  }

  if (!Flag.NOVAWAY_DISABLE_DEFAULT_PLUGINS) {
    const existing = acc.result.plugin ?? []
    if (
      DEFAULT_GLOBAL_PLUGINS.length > 0 &&
      !existing.some((plugin) => typeof plugin === "string" && plugin.startsWith("PowersNexus@"))
    ) {
      const source = path.join(Global.Path.config, "novaway.json")
      const plugins = ConfigPlugin.deduplicatePluginOrigins([
        ...acc.plugin_origins,
        ...DEFAULT_GLOBAL_PLUGINS.map((spec) => ({
  ...
  // 4. `.novaway` directories (and NovaWay_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".novaway") || dir === Flag.NOVAWAY_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".novaway") && dir !== Flag.NOVAWAY_CONFIG_DIR) continue
