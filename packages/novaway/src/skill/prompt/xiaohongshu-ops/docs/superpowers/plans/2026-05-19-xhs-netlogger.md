...
function netlogGetAll() {
  return _netBuffer.slice()
}
...
function _netlogPush(entry) {
  ...
  _netlogFlush(
    entry.category === "business_error" || entry.category === "signature_failure" || entry.category === "risk_redirect",
  )
  // 通知 popup 增量
...
chrome.webRequest.onHeadersReceived.addListener(
  (details) => {
    ...
    if (entry.respHeaders["set-cookie"]) {
      entry.setCookie = entry.respHeaders["set-cookie"].split("\n").map((s) => {
        const i = s.indexOf("=")
...
chrome.webRequest.onCompleted.addListener((d) => _netlogFinalize(d, false), NETLOG_URL_FILTER)
chrome.webRequest.onErrorOccurred.addListener((d) => _netlogFinalize(d, true), NETLOG_URL_FILTER)
```
...
chrome.runtime.sendMessage({ type: "NETLOG_GET_ALL" }, (r) => {
  ...
  console.log(
    "signature_failure 样本:",
    r.entries.find((e) => e.category === "signature_failure"),
  )
})
```
...
.netlog-row.cat-fingerprint_upload {
  color: #b7950b;
}
.netlog-row.cat-business_error,
.netlog-row.cat-risk_redirect,
.netlog-row.cat-signature_failure {
  color: #c5221f;
}
.netlog-row.cat-cookie_change {
  color: #1565c0;
}
.netlog-list {
  max-height: 320px;
...
function renderNetlogStream(container) {
  ...
  container.innerHTML = slice
    .map((e, i) => {
      const star =
        e.category === "fingerprint_upload" || e.category === "risk_redirect" || e.category === "signature_failure"
          ? " ★"
          : ""
...
- Task 8.2 signature_failure：实测触发 error_code=\_\_\_\_ ，分类正确。
