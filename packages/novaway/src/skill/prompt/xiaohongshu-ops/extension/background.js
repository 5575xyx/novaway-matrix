...
async function handleCommand(msg) {
  ...
  switch (method) {
    ...
    case "set_cookies":
      return await cmdSetCookies(params)
    ...
    func: (targetUrl, landedUrl) => {
      ...
      if (!blockType) {
        try {
          ...
          if (s) {
            ...
            const note = s?.note?.noteDetailMap ? Object.values(s.note.noteDetailMap)[0]?.note : null
      ...
      if (blockType === "redirect_404") {
        ...
        diagnosis = {
          ...
          confidence: "high",
          how_xhs_decides: "CDN / API 网关在路由层校验 xsec_token 存在性，缺失直接短路返回 404。",
        }
        ...
        diagnosis = {
          ...
          confidence: "high",
          how_xhs_decides: "服务端用 web_session 解密 xsec_token 中的加密字段，" + "session 失效则解密失败 → 404。",
        }
        ...
        diagnosis = {
          ...
          confidence: "medium",
          how_xhs_decides: "服务端解密 xsec_token 后逐字段校验：noteId、userId、ipHash、ts、source，任一不符 → 404。",
        }
      } else {
        diagnosis = {
          ...
          confidence: "medium",
          how_xhs_decides:
            "服务端对 {IP, userId, deviceId} 三元组计算行为评分，" + "低于阈值时对内容请求返回 404，登录态保持正常。",
        }
      }
...
async function cmdNavigate({ url, _retried = false }) {
  ...
  if (/xiaohongshu\.com\/404(\?|$)/.test(finalUrl)) {
    ...
    const recent = (stored.blockEvents || []).find(
      (e) =>
        e.intercept_type === "webRequest_302" &&
        e.url &&
        url.includes(e.url.split("?")[0].split("/").pop()) &&
    ...
    if (!_retried && cause === "token" && tokenExpiredOrMissing) {
      ...
      if (noteId) {
        console.log(
          `[XHS Bridge] token 过期/缺失（error_code=${errorCode || "无"}），搜索页取新 token（noteId=${noteId}）...`,
        )
...
async function cmdSetCookies({ cookies = [] }) {
  const results = []
  for (const cookie of cookies) {
    try {
      const details = {
        url: cookie.url || `https://${cookie.domain || "xiaohongshu.com"}${cookie.path || "/"}`,
        name: cookie.name,
        value: cookie.value,
      }
      ...
      else if (cookie.expires) details.expirationDate = Math.floor(new Date(cookie.expires).getTime() / 1000)

      await chrome.cookies.set(details)
      results.push({ name: cookie.name, success: true })
    } catch (e) {
      results.push({ name: cookie.name, success: false, error: String(e) })
    }
  }
  return { set: results.length, results }
}
...
function mainWorldExecutor(method, params) {
  ...
  switch (method) {
    ...
    case "wait_for_selector": {
      ...
      return poll(() => (document.querySelector(params.selector) ? true : false), 200, timeout).catch(() => {
  ...
  switch (method) {
    ...
    case "set_file_input": {
      return new Promise((resolve) => {
        ...
        const dropTarget = el.closest('[class*="upload"]') || el.closest('[class*="Upload"]') || el.parentElement
        if (dropTarget) {
          try {
            const dt2 = makeFiles()
    ...
    case "dispatch_wheel_event": {
      ...
      target.dispatchEvent(
        new WheelEvent("wheel", { deltaY: params.deltaY || 0, deltaMode: 0, bubbles: true, cancelable: true }),
      )
      return null
    }
...
// 非 302 请求完成时清理缓存
chrome.webRequest.onCompleted.addListener((d) => _reqCache.delete(d.requestId), {
  urls: ["https://www.xiaohongshu.com/*", "https://xiaohongshu.com/*"],
})
chrome.webRequest.onErrorOccurred.addListener((d) => _reqCache.delete(d.requestId), {
  urls: ["https://www.xiaohongshu.com/*", "https://xiaohongshu.com/*"],
})
...
async function riskControlAnalyzer(extraProbeUrls) {
  ...
  ps.page_has_risk_block = !!(
    document.querySelector('[class*="risk-block"]') || document.querySelector('[class*="forbidden-page"]')
  )
  ...
  const probes = [...defaultProbes, ...extraProbeUrls.map((u) => ({ key: u, url: u, method: "GET" }))]
  ...
  if (fp.webdriver === true) issues.push({ level: "high", msg: "navigator.webdriver = true，自动化特征已暴露" })
  if (fp.plugins_count === 0) issues.push({ level: "medium", msg: "navigator.plugins 为空（自动化/无扩展环境）" })
  if (fp.is_headless_ua) issues.push({ level: "high", msg: `User-Agent 含自动化标志: ${fp.user_agent}` })
  ...
  if (fp.webgl_is_swiftshader) issues.push({ level: "high", msg: `WebGL 软件渲染: ${fp.webgl_renderer}（无头特征）` })
  if (fp.notifications_permission === "denied")
    issues.push({ level: "low", msg: "通知权限被拒绝（无头浏览器常见特征）" })
  if (fp.visibility_state !== "visible")
    issues.push({
      level: "medium",
      msg: `document.visibilityState = "${fp.visibility_state}"（visibilityState 伪装失效）`,
    })
  if (ps.has_captcha_modal) issues.push({ level: "high", msg: "检测到页面验证码/人机验证弹窗" })
  if (ps.page_is_404) issues.push({ level: "high", msg: "当前页面为 404 错误页（风控拦截跳转）" })
  if (ps.page_has_risk_block) issues.push({ level: "high", msg: "检测到风控封锁页面元素" })
  ...
  for (const [key, probe] of Object.entries(report.api_probes)) {
    if (probe.risk_blocked) issues.push({ level: "high", msg: `API ${key} 返回风控状态码 ${probe.status}` })
...
async function getOrOpenXhsTab() {
  const tabs = await chrome.tabs.query({
    url: ["https://www.xiaohongshu.com/*", "https://xiaohongshu.com/*", "https://creator.xiaohongshu.com/*"],
  })
