...
class BridgePage:
    ...
    def set_cookies(self, cookies: list[dict]) -> dict:
        """注入 cookie 到浏览器。每个 cookie 是一个 dict，至少包含 name 和 value。
        可选字段：domain, path, secure, httpOnly, sameSite, expires/expirationDate, url。
        """
        return self._call("set_cookies", {"cookies": cookies})
