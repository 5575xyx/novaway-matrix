...
export class Service extends Context.Service<Service, Interface>()("@NovaWay/MCP") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const connectTransport = (transport: Transport, timeout: number) =>
      Effect.acquireUseRelease(
        ...
        (t) =>
          Effect.tryPromise({
            try: () => {
              const client = new Client({ name: "novaway", version: InstallationVersion })
            ...
            if (isAuthError) {
              ...
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return bus
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: NovaWay mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
    ...
    const startAuth = Effect.fn("MCP.startAuth")(function* (mcpName: string) {
      ...
      if (!mcpConfig) throw new Error(`MCP server ${mcpName} not found`)
