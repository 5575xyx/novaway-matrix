...
import { NovaWayHttpApi } from "./api"
...
const QueryParameterSchemas: Record<string, OpenApiSchema> = {
  ...
  "GET /evolution/candidate limit": { type: "number" },
  "GET /memory includeArchived": QueryBooleanOpenApi,
  "GET /memory limit": { type: "number" },
  "GET /memory/review/candidate limit": { type: "number" },
}

const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for NovaWay serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(NovaWayHttpApi)
...
export const PublicApi = NovaWayHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "novaway",
    version: "1.0.0",
    description: "NovaWay api",
