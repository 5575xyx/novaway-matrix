...
import { errorData, errorMessage } from "@novaway/tui/util/error"
...
import { hasTheme, upsertTheme } from "@novaway/tui/context/theme"
import { Global } from "@novaway/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@novaway/core/util/flock"
import { Flag } from "@novaway/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@novaway/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { createCommandShim } from "@novaway/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@novaway/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".NovaWay"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".NovaWay", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".NovaWay", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.NOVAWAY_PURE ? [] : pluginOrigins
    if (Flag.NOVAWAY_PURE && pluginOrigins.length) {
    }
