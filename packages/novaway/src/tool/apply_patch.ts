...
const permissionRoot = (instance: { directory: string; worktree: string }) =>
...
export const ApplyPatchTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ApplyPatchTool.execute")(function* (
      ...
      const root = permissionRoot(instance)
      ...
      const files = fileChanges.map((change) => ({
        ...
        relativePath: path.relative(root, change.movePath ?? change.filePath).replaceAll("\\", "/"),
        type: change.type,
        patch: change.diff,
      ...
      const relativePaths = fileChanges.map((c) => path.relative(root, c.filePath).replaceAll("\\", "/"))
      yield* ctx.ask({
        permission: "edit",
      ...
      const summaryLines = fileChanges.map((change) => {
        if (change.type === "add") {
          return `A ${path.relative(root, change.filePath).replaceAll("\\", "/")}`
        }
        if (change.type === "delete") {
          return `D ${path.relative(root, change.filePath).replaceAll("\\", "/")}`
        }
        const target = change.movePath ?? change.filePath
        return `M ${path.relative(root, target).replaceAll("\\", "/")}`
      })
