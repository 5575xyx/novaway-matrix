...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "NovaWay.mode": input.resolveSession ? "local" : "attach",
      "NovaWay.initial_input": !!input.initialInput,
    ...
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "NovaWay.directory": ctx.directory,
        "NovaWay.resume": ctx.resume === true,
        "NovaWay.agent.name": state.agent,
        "NovaWay.model.provider": state.model?.providerID,
        "NovaWay.model.id": state.model?.modelID,
        "NovaWay.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "NovaWay.model.provider": model.providerID,
            "NovaWay.model.id": model.modelID,
            "NovaWay.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "NovaWay.model.id": state.model?.modelID,
                "NovaWay.model.variant": state.activeVariant,
                "NovaWay.prompt.chars": prompt.text.length,
                "NovaWay.prompt.parts": prompt.parts.length,
                "NovaWay.prompt.include_files": includeFiles,
                "NovaWay.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "NovaWay.directory": input.directory,
      "NovaWay.initial_input": !!input.initialInput,
      "NovaWay.demo": input.demo,
    },
    async () => {
