...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "NovaWay.agent.name": input.agent,
      "NovaWay.directory": input.directory,
      "NovaWay.first": input.first,
      "NovaWay.model.provider": input.model?.providerID,
      "NovaWay.model.id": input.model?.modelID,
      "NovaWay.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "NovaWay.show_exit": next.showExit,
