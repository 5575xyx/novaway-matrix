...
const AGENT_USERNAME = "NovaWay-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/NovaWay.yml"
    ...
    yield* Effect.promise(async () => {
      {
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              "",
              "   Learn more about the GitHub agent - https://novaway.ai/docs/github/#usage-examples",
            ].join("\n"),
          )
        }
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/NovaWay-agent"
          const command =
          ...
          async function getInstallation() {
            return await fetch(`https://api.novaway.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
              .then((res) => res.json())
...
jobs:
  NovaWay:
    if: |
      ...
      contains(github.event.comment.body, ' /NovaWay') ||
      startsWith(github.event.comment.body, '/NovaWay')
    ...
    steps:
      ...
      - name: Run NovaWay
        uses: anomalyco/NovaWay/github@latest${envStr}
        with:
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.novaway.ai" : "https://novaway.ai"
      ...
      try {
        ...
        console.log("NovaWay session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.novaway.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/NovaWay,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to NovaWay...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("NovaWay-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `NovaWay/${type}-${hex}-${timestamp}`
        }
        return `NovaWay/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/NovaWay-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[NovaWay session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the NovaWay infrastructure after your response",
