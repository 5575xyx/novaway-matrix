      ...
      <div class="flex min-w-0 flex-1 items-start gap-3">
        ...
        <div class="flex min-w-0 flex-col gap-1">
          <div class="flex flex-wrap items-center gap-2">
            <span class="text-14-medium text-text-strong break-words">{props.candidate.summary || props.candidate.content}</span>
...
export const SettingsMemory: Component = () => {
  ...
  const candidates = useQuery(() => ({
    ...
    queryFn: () => globalSDK.client.memory.listReviewCandidates({ status: tab(), limit: 100 }).then((x) => x.data ?? []),
    ...queryOptions,
  }))
  ...
  const modeCounts = createMemo(() =>
    Object.fromEntries(modeGroups.map((group) => [group.value, filterMemoryReviewCandidates(sourceFilteredCandidates(), "all", group.value).length])) as Record<
      ModeGroup,
      number
    >,
  )
  const filteredCandidates = createMemo(() => filterMemoryReviewCandidates(candidates.data ?? [], source(), modeGroup()))

  return (
    <SettingsPage title={language.t("settings.memory.title" as never)} description={language.t("settings.memory.description" as never)}>
      <div>
        ...
        <SettingsList>
          <div class="flex items-center justify-between gap-4 py-3 border-b border-border-weak-base">
            <div class="flex flex-col gap-1">
              <span class="text-14-medium text-text-strong">{language.t("settings.memory.config.enabled.title" as never)}</span>
              <span class="text-12-regular text-text-weak">{language.t("settings.memory.config.enabled.description" as never)}</span>
            </div>
            <Switch
          ...
          <div class="flex items-center justify-between gap-4 py-3 border-b border-border-weak-base">
            <div class="flex flex-col gap-1">
              <span class="text-14-medium text-text-strong">{language.t("settings.memory.config.review.title" as never)}</span>
              <span class="text-12-regular text-text-weak">{language.t("settings.memory.config.review.description" as never)}</span>
            </div>
            <Switch
          ...
          <div class="flex items-center justify-between gap-4 py-3 border-b border-border-weak-base">
            <div class="flex flex-col gap-1">
              <span class="text-14-medium text-text-strong">{language.t("settings.memory.config.llm.title" as never)}</span>
              <span class="text-12-regular text-text-weak">{language.t("settings.memory.config.llm.description" as never)}</span>
            </div>
            <Switch
          ...
          <div class="flex items-center justify-between gap-4 py-3">
            <div class="flex flex-col gap-1">
              <span class="text-14-medium text-text-strong">{language.t("settings.memory.config.interval.title" as never)}</span>
              <span class="text-12-regular text-text-weak">{language.t("settings.memory.config.interval.description" as never)}</span>
            </div>
      ...
      <div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-2 pb-3">
          ...
          <Button variant="secondary" icon="reset" disabled={candidates.isFetching || reviewStatus.isFetching} onClick={() => void refetchReview()}>
        ...
        <SettingsList>
          <Show when={!candidates.isLoading && !reviewStatus.isLoading} fallback={<LoadingState />}>
            <Show when={filteredCandidates().length > 0} fallback={<EmptyState message={language.t("settings.memory.review.empty" as never)} />}>
