...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.NovaWayComment &&
          (part.metadata.NovaWayComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
  ...
  test("adds synthetic assistant-only context before visible user text", () => {
    const result = buildRequestParts({
      prompt: [{ type: "text", content: "帮我做一份小学数学 PPT", start: 0, end: 13 }],
      context: [],
      images: [],
      text: "帮我做一份小学数学 PPT",
      syntheticText: "办公模式内部上下文",
      messageID: "msg_synthetic",
      sessionID: "ses_synthetic",
      sessionDirectory: "/repo",
    })

    expect(result.requestParts[0]).toMatchObject({ type: "text", text: "办公模式内部上下文", synthetic: true })
    expect(result.requestParts[1]).toMatchObject({ type: "text", text: "帮我做一份小学数学 PPT" })
    expect(result.optimisticParts[0]).toMatchObject({ type: "text", text: "办公模式内部上下文", synthetic: true })
  })
  ...
  test("handles macOS paths correctly", () => {
    ...
    const result = buildRequestParts({
      ...
      messageID: "msg_mac_1",
      sessionID: "ses_mac_1",
      sessionDirectory: "/Users/kelvin/Projects/NovaWay",
    })
    ...
    if (filePart?.type === "file") {
      ...
      expect(() => new URL(filePart.url)).not.toThrow()
      // Should be a normal Unix path
      expect(filePart.url).toBe("file:///Users/kelvin/Projects/NovaWay/README.md")
    }
  })
