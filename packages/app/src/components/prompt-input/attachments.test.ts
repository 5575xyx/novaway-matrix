...
describe("attachmentMime", () => {
  ...
  test("accepts Office documents by browser mime or filename fallback", async () => {
    const docx = new File(["PK"], "proposal.docx", {
      type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    })
    const pptx = new File(["PK"], "deck.pptx", { type: "application/octet-stream" })

    expect(await attachmentMime(docx)).toBe("application/vnd.openxmlformats-officedocument.wordprocessingml.document")
    expect(await attachmentMime(pptx)).toBe("application/vnd.openxmlformats-officedocument.presentationml.presentation")
  })
