...
export function OfficePlatformPanel() {
            ...
            <div class="flex flex-wrap gap-2">
              <For each={connectors() ?? []}>
                {(connector) => {
                  ...
                  return (
                    <label
                      class="flex h-8 cursor-pointer items-center gap-1.5 rounded-[7px] border px-2.5 text-11-medium transition-all duration-150"
                      classList={{
              ...
              >
                {(option) => (
                  <button
                    ...
                    classList={{
                      "border-emerald-300/50 bg-emerald-300/10 text-emerald-100": scheduleDraft.triggerType === option.id,
                      "border-border-weak-base bg-background-base text-text-weak": scheduleDraft.triggerType !== option.id,
                    }}
  ...
  async function sendFeishuMessage() {
    if (!feishuMessage().trim()) return
    await globalSDK.client.office.platform.connector.action({
      id: "feishu",
      action: "send_message",
      arguments: { text: feishuMessage() },
    })
    setFeishuMessage("")
  }

  function openConnectorGuide(id: string) {
    ...
    dialog.show(
      () => (
        <Dialog title={title} class="w-full max-w-[520px] mx-auto">
          <div class="flex flex-col gap-2 px-6 pb-5">
            <div class="rounded-[7px] border border-border-weak-base bg-background-base p-2.5 text-11-regular leading-relaxed text-text-weak">
              {isFeishu
                ? "填写飞书群机器人 Webhook 地址，保存后即可发送消息和接收失败通知。"
                : "填写腾讯文档 MCP Token，保存后会自动重新连接。"}
            </div>
            <Show when={isFeishu}>
              <label class="flex flex-col gap-1">
                <span class="text-11-medium text-text-weak">飞书群 Webhook 地址</span>
                <input
            ...
            <Show when={!isFeishu}>
              <label class="flex flex-col gap-1">
                <span class="text-11-medium text-text-weak">腾讯文档 Token</span>
                <input
                  value={tencentTokenDraft()}
                  onInput={(event) => setTencentTokenDraft(event.currentTarget.value)}
                  placeholder="访问 https://docs.qq.com/open/auth/mcp.html 获取"
                  class="h-9 rounded-[7px] border border-border-weak-base bg-background-base px-3 text-12-regular text-text-strong outline-none"
                />
              </label>
            </Show>
  ...
  async function saveConnectorGuide(id: string) {
    if (id === "feishu") {
      await globalSDK.client.office.platform.connector.config.update({
        feishuWebhookUrl: feishuWebhookDraft().trim() || undefined,
      })
    } else {
      await globalSDK.client.office.platform.connector.config.update({
        tencentDocsToken: tencentTokenDraft().trim() || undefined,
      })
    }
    void refetchConnectors()
    void refetchStatus()
                ...
                <div class="rounded-[8px] border border-border-weaker-base bg-background-base p-3">
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="flex shrink-0 items-center gap-1">
                      ...
                      >
                        <Icon name={schedule.status === "active" ? "circle-check" : "circle-ban-sign"} size="small" />
                      </button>
                      <button
                        type="button"
                        class="grid size-8 place-items-center rounded-[7px] border border-border-weak-base text-text-weak hover:text-rose-300"
            ...
            <For each={connectors() ?? []}>
              {(connector) => (
                <div class="flex items-center justify-between gap-3 rounded-[8px] border border-border-weaker-base bg-background-base p-3">
                  <div class="min-w-0">
                    <div class="truncate text-13-medium text-text-strong">{connector.name}</div>
                  ...
                  <span
                    ...
                    classList={{
                      "border-emerald-300/30 bg-emerald-300/10 text-emerald-200": connector.status === "connected",
                      "border-rose-300/30 bg-rose-300/10 text-rose-200": connector.status === "failed",
                      "border-border-weak-base bg-background-base text-text-muted": connector.status === "disabled",
                    }}
                  >
                    {connector.status === "connected" ? "已连接" : connector.status === "failed" ? "异常" : "未启用"}
                  </span>
                  <Show when={connector.id === "feishu"}>
                    <button
                      type="button"
                      ...
                      title="配置飞书"
                      onClick={() => openConnectorGuide("feishu")}
                    >
                  ...
                  <Show when={connector.id === "tencent-docs" && !connector.configured}>
                    <button
                      ...
                      class="grid size-8 shrink-0 place-items-center rounded-[7px] border border-border-weak-base text-text-weak transition-all duration-150 hover:-translate-y-px hover:border-emerald-300/50 hover:text-emerald-100"
                      title="配置腾讯文档"
                      onClick={() => openConnectorGuide("tencent-docs")}
                    >
                      <Icon name="sliders" size="small" />
                    </button>
                  </Show>
                  <Show when={connector.id !== "feishu" && !(connector.id === "tencent-docs" && !connector.configured)}>
                    <button
                      type="button"
                      class="grid size-8 shrink-0 place-items-center rounded-[7px] border border-border-weak-base text-text-weak hover:text-text-strong"
