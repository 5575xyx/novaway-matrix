...
import { type Session } from "@novaway/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("NovaWay://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("NovaWay://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("NovaWay://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("NovaWay://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("NovaWay://open-project")).toBeUndefined()
    expect(parseDeepLink("NovaWay://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "NovaWay://open-project?directory=/a",
      "NovaWay://other?directory=/b",
      "NovaWay://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("NovaWay://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("NovaWay://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("NovaWay://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("NovaWay://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "NovaWay://new-session?directory=/a",
      "NovaWay://open-project?directory=/b",
      "NovaWay://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __NovaWay__: {
        deepLinks: ["NovaWay://open-project?directory=/a"],
      },
    } as unknown as Window & { __NovaWay__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["NovaWay://open-project?directory=/a"])
