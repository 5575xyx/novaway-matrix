...
import { Session, type Message, type QuestionRequest } from "@novaway/sdk/v2/client"
...
import { showToast, Toast, toaster } from "@novaway/ui/toast"
...
import { retry } from "@novaway/core/util/retry"
...
export default function Layout(props: ParentProps) {
  ...
  const modeStyle = createMemo(() => {
    const mode = layout.mode.all.find((item) => item.id === layout.mode.current())
    if (!mode) return {}
    return {
      "--novaway-mode-color": mode.color,
      "--novaway-mode-tint": `color-mix(in srgb, ${mode.color} 14%, transparent)`,
      "--novaway-mode-soft": `color-mix(in srgb, ${mode.color} 8%, transparent)`,
      "--novaway-mode-border": `color-mix(in srgb, ${mode.color} 38%, transparent)`,
    }
  })
  ...
  const moveProjectToMode = (directory: string, mode: Parameters<typeof layout.projects.assign>[1]) => {
    layout.projects.assign(directory, mode)
    if (mode === layout.mode.current()) return
    if (pathKey(currentProject()?.worktree ?? "") === pathKey(directory)) navigate("/")
  }
  ...
  function createProject() {
    const run = ++dialogRun
    void import("@/components/dialog-create-project").then((x) => {
      if (dialogDead || dialogRun !== run) return
      dialog.show(
        () => (
          <x.DialogCreateProject
            onCreate={(path) => {
              openProject(path)
            }}
          />
  ...
  const SidebarPanel = (panelProps: { mobile?: boolean; merged?: boolean }) => {
    ...
    const expandedDir = createMemo(() => {
      const dir = params.dir
      if (!dir) return ""
      return decode64(dir)
    })

    const expandedProject = createMemo(() => {
      const dir = expandedDir()
      if (!dir) return undefined
      return layout.projects.list().find((p) => p.worktree === dir)
    })

    const expandedWorkspaces = createMemo(() => {
      const p = expandedProject()
      if (!p) return [] as string[]
      return workspaceIds(p)
    })

    const workspacesEnabled = createMemo(() => {
      const p = expandedProject()
      if (!p) return false
      if (p.vcs !== "git") return false
      return layout.sidebar.workspaces(p.worktree)()
    })
      ...
      >
        <div class="shrink-0 py-3 px-1">
          <div class="flex items-center justify-between gap-2">
            <span class="text-14-medium text-text-strong truncate min-w-0">
              工作区
  ...
  return (
    <div
      class="relative tech-radial-light dark:tech-radial-dark flex-1 min-h-0 min-w-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text aurora-overlay"
      style={modeStyle()}
          ...
          <div class="size-full relative overflow-x-hidden">
            ...
            <Show when={showSidebar()}>
              <div class="xl:hidden">
                <div
                  classList={{
                    "fixed inset-x-0 top-[50px] bottom-0 z-40 transition-opacity duration-200": true,
            ...
            <div
              classList={{
                ...
                "z-20": true,
                "transition-[left] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] will-change-[left] motion-reduce:transition-none":
                  !state.sizing,
              }}
              style={{
