import type { Todo } from "@novaway/sdk/v2"
import { AnimatedNumber } from "@novaway/ui/animated-number"
...
const fileIconRegex = /📄\s*([^\s.md]+\.md)/g
  ...
  return (
    ...
    >
      <div ref={contentRef}>
        ...
        >
          <TodoList todos={props.todos} onOpenFile={props.onOpenFile} />
        </div>
      </div>
    </DockTray>
  )
...
function TodoList(props: { todos: Todo[]; onOpenFile?: (fileName: string) => void }) {
  const [store, setStore] = createStore({
    stuck: false,
  })
