...
function init() {
  ...
  const show = (element: DialogElement, owner: Owner, onClose?: () => void, type: "dialog" | "drawer" = "dialog") => {
    ...
    const node = runWithOwner(owner, () =>
      createRoot((d: () => void) => {
        ...
        return (
          ...
          >
            <Kobalte.Portal>
              <Kobalte.Overlay data-component={`${type}-overlay`} onClick={close} />
              {element()}
            </Kobalte.Portal>
          </Kobalte>
        )
    ...
    setStack([...kept, { id, node, dispose, owner, onClose, setClosing, type }])
  }
...
export function useDialog() {
  ...
  return {
    ...
    show(element: DialogElement, onClose?: () => void) {
      ...
      return startTransition(() => ctx.show(element, base, onClose, "dialog"))
    },
    showDrawer(element: DialogElement, onClose?: () => void) {
      const base = ctx.active?.owner ?? owner
      return startTransition(() => ctx.show(element, base, onClose, "drawer"))
    },
    close() {
      ctx.close()
    },
