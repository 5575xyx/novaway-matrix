...
import { getDirectory as _getDirectory, getFilename } from "@novaway/core/util/path"
  ...
  return (
    <Show when={text()}>
      <Collapsible open={open()} onOpenChange={setOpen} variant="ghost" class="reasoning-collapsible">
        <Collapsible.Trigger>
          <div data-component="reasoning-part-trigger" data-timeline-part-id={part().id}>
            <span data-slot="reasoning-part-label" class="text-12-medium text-text-weak">
              {streaming() ? "思考中..." : "思考过程"}
            </span>
            <Collapsible.Arrow />
          </div>
  ...
  render(props) {
    ...
    return (
      <BasicTool
        ...
        trigger={
          <div data-slot="basic-tool-tool-info-structured">
            <span data-slot="tool-badge-icon">
              <Icon name="console" size="small" />
            </span>
            <div data-slot="basic-tool-tool-info-main">
              <span data-slot="basic-tool-tool-title" style={{ color: "inherit" }}>
...
ToolRegistry.register({
  ...
  render(props) {
    ...
    const lineCount = createMemo(() => {
      if (props.metadata.filediff) return null
        ...
        <BasicTool
          ...
          trigger={
            <div data-component="write-trigger">
              ...
              <Show when={props.metadata.filediff || lineCount()}>
                <div data-slot="message-part-actions">
                  <DiffChanges
                    changes={
      ...
      <BasicTool
        ...
        trigger={
          <div data-slot="basic-tool-tool-info-structured">
            ...
            <div data-slot="basic-tool-tool-info-main">
              <span data-slot="basic-tool-tool-title">
                <TextShimmer text={i18n.t("ui.tool.questions")} active={pending()} />
              </span>
              <Show when={!pending()}>
                <span data-slot="basic-tool-tool-subtitle">{subtitle()}</span>
              </Show>
  ...
  render(props) {
    ...
    const trigger = () => (
      <div
        ...
        class="rounded-[10px] border border-cyan-300/25 bg-cyan-300/[0.07] px-3 py-2"
      >
        <div data-slot="basic-tool-tool-info-main" class="flex items-center gap-2">
          <span class="grid size-7 place-items-center rounded-[8px] border border-cyan-300/25 bg-cyan-300/10 text-cyan-100">
            <Icon name="brain" size="small" />
          </span>
          <span class="min-w-0 flex flex-col">
            <span data-slot="basic-tool-tool-title" class="text-13-medium text-cyan-100">
              {titleContent()}
            </span>
            <span class="text-11-regular text-cyan-100/65">{subtitle()}</span>
          </span>
        </div>
      </div>
    )
...
const skillTitleMap: Record<string, string> = {
  "customize-novaway": "自定义 NovaWay 技能",
  ...
  "company-values": "公司价值观技能",
  "find-community": "社群寻找技能",
  "first-customers": "首批客户技能",
  "grow-sustainably": "可持续增长技能",
  "marketing-plan": "营销计划技能",
  "minimalist-review": "极简审查技能",
}

const skillWordMap: Record<string, string> = {
  agent: "智能体",
  agents: "智能体",
  ai: "AI",
  analysis: "分析",
...
function skillToolTitle(name: string) {
...
function skillToolSubtitle(name: unknown, running: boolean) {
  if (typeof name === "string" && name.startsWith("office-")) return running ? "正在加载办公技能" : "办公技能已加载"
  return running ? "正在加载技能" : "技能已加载"
}

// DBHub MCP tools
const DBHUB_TOOLS = new Set(["execute_sql", "list_tables", "describe_table", "search_objects"])

function isDbhubTool(tool: string): boolean {
  return DBHUB_TOOLS.has(tool)
}

const DBHUB_TOOL_LABELS: Record<string, string> = {
  execute_sql: "执行 SQL",
  list_tables: "列出表",
  describe_table: "表结构",
  search_objects: "搜索对象",
...
ToolRegistry.register({
  ...
  render(props) {
    const pending = () => props.status === "pending" || props.status === "running"
    const sql = createMemo(() => props.input.query || "")
    const result = createMemo(() => {
      try {
        return JSON.parse(props.output || "{}")
      } catch {
        return { error: props.output }
      }
    })
    const columns = createMemo(() => result().columns ?? [])
    const rows = createMemo(() => result().rows ?? [])
    const affected = createMemo(() => result().affected_rows)
      ...
      >
        <Show when={sql()}>
          <div class="px-3 py-2 border-b border-border-weaker-base">
            <pre class="text-12-regular text-text-base font-mono whitespace-pre-wrap">{sql()}</pre>
          </div>
        </Show>
        <Show when={!pending() && result()}>
          <div class="px-3 py-2 overflow-auto max-h-64">
            <Show when={result().error}>
              <div class="text-12-regular text-text-error">{result().error}</div>
            </Show>
            ...
            <Show when={affected() !== undefined}>
              <div class="text-12-regular text-text-weak mt-1">{affected()} 行受影响</div>
            </Show>
          </div>
        </Show>
      </BasicTool>
      ...
      <BasicTool
        ...
        trigger={
          <div data-slot="basic-tool-tool-info-structured">
            ...
            <div data-slot="basic-tool-tool-info-main">
              <span data-slot="basic-tool-tool-title" style={{ color: "inherit" }}>
                <TextShimmer text="列出表" active={pending()} />
              </span>
            </div>
          </div>
        }
...
ToolRegistry.register({
  ...
  render(props) {
    const pending = () => props.status === "pending" || props.status === "running"
    const tableName = createMemo(() => props.input.table_name || "")
      ...
      <BasicTool
        ...
        trigger={
          <div data-slot="basic-tool-tool-info-structured">
            ...
            <div data-slot="basic-tool-tool-info-main">
              <span data-slot="basic-tool-tool-title" style={{ color: "inherit" }}>
                <TextShimmer text={`表结构: ${tableName()}`} active={pending()} />
              </span>
            </div>
          </div>
        }
      >
        <Show when={!pending() && Array.isArray(columns()) && columns().length > 0}>
          <div class="px-3 py-2 overflow-auto max-h-64">
            <table class="w-full text-12-regular">
              <thead>
                <tr class="border-b border-border-weaker-base">
                  <th class="px-2 py-1 text-left text-11-medium text-text-weak">字段名</th>
                  <th class="px-2 py-1 text-left text-11-medium text-text-weak">类型</th>
                </tr>
              </thead>
              <tbody>
                <For each={columns()}>
                  {(col: any) => (
                    <tr class="border-b border-border-weaker-base">
                      <td class="px-2 py-1 text-text-base font-mono">{col.name ?? col}</td>
                      <td class="px-2 py-1 text-text-weak">{col.type ?? ""}</td>
                    </tr>
                  )}
                </For>
              </tbody>
