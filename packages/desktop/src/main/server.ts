...
const SIDECAR_SERVICE_NAME = "NovaWay server"
...
export function getWslConfig(): WslConfig {
  const value = getSetting(WSL_ENABLED_KEY)
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...
    NovaWay_EXPERIMENTAL_ICON_DISCOVERY: "true",
    NovaWay_EXPERIMENTAL_FILEWATCHER: "true",
    NovaWay_CLIENT: "desktop",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`NovaWay:${password}`).toString("base64")
