/**
 * Tests for the visual companion's PowersNexus/Prime Radiant branding.
 */
...
function createPackagedServerFixture(version) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "PowersNexus", version }, null, 2),
  )
  return {
    root,
...
function assertBrandedWithLogo(html, version = PACKAGE_VERSION) {
  assert(html.includes(`PowersNexus v${version}`), "branding text should include dynamic package version")
  assert(!html.includes(`PowersNexus v${version} by`), 'branding text should not include "by" when the logo is visible')
  assert(
    /<img class="brand-logo"[^>]*>\s*<span class="brand-copy">PowersNexus v/.test(html),
    "visible logo should appear before the PowersNexus version text",
  )
  assert(
...
function assertBrandedFallbackText(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Prime Radiant PowersNexus v${version}`),
    "disabled telemetry should keep plain text Prime Radiant/PowersNexus branding",
  )
}
  ...
  await test("packaged Codex plugin reads version from .codex-plugin manifest", async () => {
    ...
    try {
      await withServer({ port, dir, serverPath: fixture.serverPath }, async () => {
        ...
        assert(!html.includes("PowersNexus vunknown"), "packaged plugin should not fall back to unknown version")
      })
    } finally {
