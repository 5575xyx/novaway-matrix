...
function isWindowsLikeShell() {
  return process.platform === "win32" || /^msys|^cygwin|^mingw/i.test(process.env.OSTYPE || "") || !!process.env.MSYSTEM
}
...
async function runTests() {
  ...
  await test("server-info reports the configured idle_timeout_ms", async () => {
    ...
    const srv = spawn("node", [SERVER], {
      env: { ...process.env, BRAINSTORM_PORT: 3401, BRAINSTORM_DIR: dir, BRAINSTORM_IDLE_TIMEOUT_MS: 1234567 },
    })
    let out = ""
