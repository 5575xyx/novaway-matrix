...
skill_count=$(find "$PowersNexus_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $PowersNexus_SKILLS_DIR"
    exit 1
fi
...
if node --check "$PowersNexus_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/PowersNexus/' "$PowersNexus_PLUGIN_FILE"; then
