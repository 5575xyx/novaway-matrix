...
# 1. Create in PowersNexus location (lowest priority)
mkdir -p "$PowersNexus_SKILLS_DIR/priority-test"
cat > "$PowersNexus_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: PowersNexus version of priority test skill
---
# Priority Test Skill (PowersNexus Version)

This is the PowersNexus version of the priority test skill.

PRIORITY_MARKER_PowersNexus_VERSION
EOF
...
if [ -f "$PowersNexus_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] PowersNexus version exists"
else
    echo "  [FAIL] PowersNexus version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled PowersNexus priority
echo ""
echo "Test 2: Documenting personal vs PowersNexus priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled PowersNexus version instead of personal version"

# Test 3: Document project vs bundled PowersNexus priority
echo ""
echo "Test 3: Documenting project vs personal/PowersNexus priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled PowersNexus version instead of project version"

# Test 4: Test a non-colliding bundled PowersNexus skill is still available
echo ""
echo "Test 4: Testing non-colliding PowersNexus skill remains available..."

mkdir -p "$PowersNexus_SKILLS_DIR/PowersNexus-only-test"
cat > "$PowersNexus_SKILLS_DIR/PowersNexus-only-test/SKILL.md" <<'EOF'
---
name: PowersNexus-only-test
description: PowersNexus-only priority test skill
---
# PowersNexus Only Test Skill

PRIORITY_MARKER_PowersNexus_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"PowersNexus-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_PowersNexus_ONLY_VERSION" "Non-colliding PowersNexus skill is still registered"
