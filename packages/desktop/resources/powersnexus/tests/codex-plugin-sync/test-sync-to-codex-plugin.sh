...
EOF
    ...
    cat > "$repo/assets/PowersNexus-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/PowersNexus/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/PowersNexus/.fixture-keep"
    cat > "$repo/plugins/PowersNexus/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/PowersNexus/.fixture-keep
    git -C "$repo" add plugins/PowersNexus/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/PowersNexus/skills/example/agents"

    cat > "$repo/plugins/PowersNexus/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/PowersNexus/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/PowersNexus/.codex-plugin" \
        "$repo/plugins/PowersNexus/.private-journal" \
        "$repo/plugins/PowersNexus/assets" \
        "$repo/plugins/PowersNexus/hooks" \
        "$repo/plugins/PowersNexus/skills/example/agents" \
        "$repo/plugins/PowersNexus/skills/example"

    cat > "$repo/plugins/PowersNexus/.codex-plugin/plugin.json" <<EOF
{
  "name": "PowersNexus",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/PowersNexus/assets/PowersNexus-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/PowersNexus/assets/app-icon.png"

    cat > "$repo/plugins/PowersNexus/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/PowersNexus/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/PowersNexus/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/PowersNexus/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
    chmod +x "$repo/plugins/PowersNexus/hooks/session-start" "$repo/plugins/PowersNexus/hooks/session-start-codex" "$repo/plugins/PowersNexus/hooks/run-hook.cmd"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/PowersNexus/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/PowersNexus/.codex-plugin/plugin.json \
        plugins/PowersNexus/assets/app-icon.png \
        plugins/PowersNexus/assets/PowersNexus-small.svg \
        plugins/PowersNexus/hooks/hooks-codex.json \
        plugins/PowersNexus/hooks/run-hook.cmd \
        plugins/PowersNexus/hooks/session-start \
        plugins/PowersNexus/hooks/session-start-codex \
        plugins/PowersNexus/skills/example/agents/openai.yaml \
        plugins/PowersNexus/skills/example/SKILL.md \
        plugins/PowersNexus/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/PowersNexus/.kimi-plugin" \
        "$repo/plugins/PowersNexus/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/PowersNexus/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/PowersNexus/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/PowersNexus/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/PowersNexus/.fixture-keep \
        plugins/PowersNexus/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/PowersNexus/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/PowersNexus/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/PowersNexus-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (PowersNexus-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/PowersNexus-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/PowersNexus/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/PowersNexus" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/PowersNexus-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/PowersNexus'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/PowersNexus-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/PowersNexus-*" "Clean no-op local apply does not create sync branch in destination checkout"
