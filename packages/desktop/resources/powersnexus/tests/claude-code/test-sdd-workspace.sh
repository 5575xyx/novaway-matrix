...
main() {
    ...
    if [[ "$dir" == "$repo/.PowersNexus/sdd" ]]; then
        pass "prints <repo-root>/.PowersNexus/sdd"
    else
        fail "prints <repo-root>/.PowersNexus/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.PowersNexus/sdd/.gitignore" && "$(cat "$repo/.PowersNexus/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.PowersNexus/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.PowersNexus/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.PowersNexus/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.PowersNexus/sdd" && "$wt_dir" != "$dir" ]]; then
    ...
    printf 'y\n' > "$wt/.PowersNexus/sdd/artifact.md"
    local wt_status
