# PowersNexus for Kimi Code

Complete guide for using PowersNexus with [Kimi Code](https://github.com/MoonshotAI/kimi-code).

## Installation

PowersNexus is available in Kimi Code's plugin marketplace.
...
Go to `Marketplace` > `PowersNexus` and install it.
...
```text
/plugins install https://github.com/obra/PowersNexus/tree/dev
```
...
2. Loads `using-PowersNexus` at session start through `sessionStart.skill`.
3. Provides Kimi-specific tool mapping through `skillInstructions`.

Kimi Code reads PowersNexus skills from this repository. There are no copied skills, symlinks, hooks, or extra runtime dependencies.
...
Select PowersNexus and update it from there. Start a fresh session with `/new` after updating.

## Troubleshooting

### Plugin not loading

1. Run `/plugins info PowersNexus` and check diagnostics.
...
Kimi Code installs the latest GitHub release for a bare repository URL when one exists. To test unreleased changes before the next PowersNexus release, install the branch explicitly:
...
1. Confirm `/plugins info PowersNexus` shows the plugin enabled.
