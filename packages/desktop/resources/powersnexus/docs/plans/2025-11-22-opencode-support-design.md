...
Add full PowersNexus support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port PowersNexus to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/PowersNexus-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/PowersNexus.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/PowersNexus/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/PowersNexus-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "PowersNexus:brainstorming")')
  }),
...
1. **Inject using-PowersNexus content**
   - Full content of the using-PowersNexus skill
...
```javascript
// .opencode/plugin/PowersNexus.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const PowersNexusPlugin = async ({ client, directory, $ }) => {
  const PowersNexusDir = path.join(process.env.HOME, '.config/opencode/PowersNexus');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingPowersNexus = await readSkill('using-PowersNexus');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingPowersNexus}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── PowersNexus-codex        # UPDATED: Use skills-core
│   ├── PowersNexus-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── PowersNexus.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/PowersNexus-codex`
...
2. Update `.codex/PowersNexus-codex` to use shared core
...
1. Create `.opencode/plugin/PowersNexus.js`
