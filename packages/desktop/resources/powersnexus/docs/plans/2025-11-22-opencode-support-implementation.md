...
> **For agentic workers:** REQUIRED SUB-SKILL: Use PowersNexus:executing-plans to implement this plan task-by-task.

**Goal:** Add full PowersNexus support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/PowersNexus-codex` (lines 40-74)
...
- Reference: `.codex/PowersNexus-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'PowersNexus' for namespacing
...
- Modify: `lib/skills-core.js`
- Reference: `.codex/PowersNexus-codex` (lines 212-280)
 ...
 * (personal skills override PowersNexus skills).
 *
 * @param {string} skillName - Name like "PowersNexus:brainstorming" or "my-skill"
 * @param {string} PowersNexusDir - Path to PowersNexus skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, PowersNexusDir, personalDir) {
  // Strip PowersNexus: prefix if present
  const forcePowersNexus = skillName.startsWith("PowersNexus:")
  const actualSkillName = forcePowersNexus ? skillName.replace(/^PowersNexus:/, "") : skillName

  // Try personal skills first (unless explicitly PowersNexus:)
  if (!forcePowersNexus && personalDir) {
  ...
  // Try PowersNexus skills
  if (PowersNexusDir) {
    const PowersNexusPath = path.join(PowersNexusDir, actualSkillName)
    const PowersNexusSkillFile = path.join(PowersNexusPath, "SKILL.md")
    if (fs.existsSync(PowersNexusSkillFile)) {
      return {
        skillFile: PowersNexusSkillFile,
        sourceType: "PowersNexus",
        skillPath: actualSkillName,
      }
    }
  }
...
- Modify: `lib/skills-core.js`
- Reference: `.codex/PowersNexus-codex` (lines 16-38)
...
- Modify: `.codex/PowersNexus-codex` (add import at top)
...
Run: `node -c .codex/PowersNexus-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/PowersNexus-codex
...
- Modify: `.codex/PowersNexus-codex` (lines 40-74)
...
Run: `.codex/PowersNexus-codex find-skills | head -20`
...
- Modify: `.codex/PowersNexus-codex` (lines 97-136, approximately)
...
- Modify: `.codex/PowersNexus-codex` (lines 16-38, approximately)
...
Run: `.codex/PowersNexus-codex bootstrap | head -50`
...
- Create: `.opencode/plugin/PowersNexus.js`
...
/**
 * PowersNexus plugin for OpenCode.ai
 *
...
const PowersNexusSkillsDir = path.join(homeDir, ".config/opencode/PowersNexus/skills")
const personalSkillsDir = path.join(homeDir, ".config/opencode/skills")

/**
 * OpenCode plugin entry point
 */
export const PowersNexusPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/PowersNexus.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/PowersNexus.js
...
- Modify: `.opencode/plugin/PowersNexus.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z
            .string()
            .describe('Name of the skill to load (e.g., "PowersNexus:brainstorming" or "my-custom-skill")'),
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > PowersNexus)
          const resolved = skillsCore.resolveSkillPath(skill_name, PowersNexusSkillsDir, personalSkillsDir)
...
Run: `node -c .opencode/plugin/PowersNexus.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the PowersNexus and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const PowersNexusSkills = skillsCore.findSkillsInDir(
            PowersNexusSkillsDir,
            'PowersNexus',
            3
          );
          ...
          const allSkills = [...personalSkills, ...PowersNexusSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install PowersNexus skills to ~/.config/opencode/PowersNexus/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'PowersNexus:';
...
```javascript
    'session.started': async () => {
      // Read using-PowersNexus skill content
      const usingPowersNexusPath = skillsCore.resolveSkillPath(
        'using-PowersNexus',
        PowersNexusSkillsDir,
        personalSkillsDir
      );

      let usingPowersNexusContent = '';
      if (usingPowersNexusPath) {
        const fullContent = fs.readFileSync(usingPowersNexusPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingPowersNexusContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- PowersNexus skills: \`PowersNexus:skill-name\` (from ~/.config/opencode/PowersNexus/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/PowersNexus')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/PowersNexus && git pull` to update PowersNexus.' :
        '';
...
**Below is the full content of your 'PowersNexus:using-PowersNexus' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingPowersNexusContent}
...
````markdown
# Installing PowersNexus for OpenCode
...
### 1. Install PowersNexus Skills

```bash
# Clone PowersNexus skills to OpenCode config directory
mkdir -p ~/.config/opencode/PowersNexus
git clone https://github.com/obra/PowersNexus.git ~/.config/opencode/PowersNexus
```
````

### 2. Install the Plugin

The plugin is included in the PowersNexus repository you just cloned.

OpenCode will automatically discover it from:

- `~/.config/opencode/PowersNexus/.opencode/plugin/PowersNexus.js`
...
ln -s ~/.config/opencode/PowersNexus/.opencode/plugin/PowersNexus.js .opencode/plugin/PowersNexus.js
```
...
```
use use_skill tool with skill_name: "PowersNexus:brainstorming"
```
...
Personal skills override PowersNexus skills with the same name.

## Updating

```bash
cd ~/.config/opencode/PowersNexus
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/PowersNexus/.opencode/plugin/PowersNexus.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/PowersNexus/skills`
...
- Documentation: https://github.com/obra/PowersNexus
...
PowersNexus works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/PowersNexus-codex use-skill PowersNexus:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/PowersNexus-codex bootstrap | head -30`
...
ls -l .opencode/plugin/PowersNexus.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/PowersNexus.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/PowersNexus-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/PowersNexus-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/PowersNexus.js` created with tools and hooks
