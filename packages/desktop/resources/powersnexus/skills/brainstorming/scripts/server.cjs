...
const POWERSNEXUS_VERSION = readPowersNexusVersion();
const POWERSNEXUS_BRAND_IMAGE_URL = 'https://primeradiant.com/brand/powersnexus-visual-brainstorming-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'POWERSNEXUS_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const POWERSNEXUS_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function readPowersNexusVersion() {
...
function brandMarkup() {
  const version = escapeHtmlText(POWERSNEXUS_VERSION);
  const text = POWERSNEXUS_TELEMETRY_DISABLED
    ? 'Prime Radiant PowersNexus v' + version
    : 'PowersNexus v' + version;
  const logo = POWERSNEXUS_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + POWERSNEXUS_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(POWERSNEXUS_VERSION) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a href="https://github.com/obra/powersnexus">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
