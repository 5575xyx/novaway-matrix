...
function brandMarkup() {
  ...
  const text = POWERSNEXUS_TELEMETRY_DISABLED ? "Prime Radiant PowersNexus v" + version : "PowersNexus v" + version
  const logo = POWERSNEXUS_TELEMETRY_DISABLED
    ? ""
    : '<img class="brand-logo" src="' +
      POWERSNEXUS_BRAND_IMAGE_URL +
      "?v=" +
      encodeURIComponent(POWERSNEXUS_VERSION) +
      '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">'

  return (
    '<div class="brand"><a href="https://github.com/obra/powersnexus">' +
    logo +
