import type { TuiPlugin, TuiPluginApi, TuiPluginStatus } from "@opencode/plugin/tui"
          ...
          .then((out) => {
            ...
            return props.api.plugins.add(mod).then((ok) => {
              if (!ok) {
                props.api.ui.toast({
                  variant: "warning",
                  message: "插件已安装，但运行时加载失败。请查看控制台/日志；重启 TUI 以重试。",
                })
                show(props.api)
                return
              }
  ...
  api.keymap.registerLayer({
    commands: [
      ...
      },
      {
        ...
        title: "安装插件",
        slashName: "install-plugin",
        slashAliases: ["安装插件", "install"],
        category: "系统",
        namespace: "palette",
        run() {
          showInstall(api)
