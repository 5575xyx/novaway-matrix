...
import type { TuiPlugin, TuiPluginApi, TuiRouteCurrent } from "@opencode/plugin/tui"
import type { SnapshotFileDiff, VcsFileDiff } from "@novaway/sdk-v2-latest/v2"
import {
  TextAttributes,
  type BorderSides,
  type BoxRenderable,
...
const PLAIN_TEXT_FILETYPE = "NovaWay-plain-text"
...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  useBindings(() => ({
    commands,
    bindings: [
      { key: "j,down", cmd: "diff.down", desc: "向下移动 Diff 查看器" },
      { key: "k,up", cmd: "diff.up", desc: "向上移动 Diff 查看器" },
      { key: "pagedown,ctrl+f", cmd: "diff.page.down", desc: "向下翻页 Diff 查看器" },
      { key: "pageup,ctrl+b", cmd: "diff.page.up", desc: "向上翻页 Diff 查看器" },
      { key: "m", cmd: "diff.mark_reviewed", desc: "标记所选文件为已审查" },
      ...
      <PanelGroup axis="y" width="100%" height="100%">
        <Panel border="none" flexShrink={0} padding={0} paddingLeft={1}>
          ...
          <text fg={theme().textMuted}>
            {files().length} {files().length === 1 ? "文件" : "文件"}
          </text>
        </Panel>
  ...
  const rows = [
    {
      shortcut: () => "q",
      action: "关闭查看器",
      description: "退出 Diff 查看器",
    },
    {
      shortcut: useCommandShortcut("diff.switch_focus"),
      action: "聚焦文件树",
      description: "在文件树和补丁面板之间切换键盘焦点",
    },
    {
    ...
    },
    {
      ...
      action: "切换来源",
      description: "选择工作树、主分支或上一轮变更",
    },
    {
      shortcut: useCommandShortcut("diff.toggle_view"),
      action: "切换视图",
      description: "在分屏和统一差异布局之间切换",
    },
    {
