import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@opencode/plugin/tui"
...
import type { useNovaWayKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useNovaWayKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getNovaWayModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getNovaWayModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
