...
import { registerNovaWaySpinner } from "./component/register-spinner"
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
        ...
        Effect.tryPromise({
          try: () =>
            createCliRenderer({
              ...
              useMouse: !Flag.NOVAWAY_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerNovaWayKeymap(keymap, renderer, input.config)),
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.NovaWay_ROUTE ? JSON.parse(process.env.NovaWay_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.NovaWay_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const kv = useKV()
  // 图标风格优先用 kv 持久化的 /icon 选择,否则回落到 tui 配置;校验后再落定。
  onMount(() => {
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.NOVAWAY_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("NovaWay")
      return
    }
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        run: async () => {
          ...
          await clipboard
            ...
            .then(() => toast.show({ message: "工作目录路径已复制", variant: "info" }))
            .catch(toast.error)
          dialog.clear()
        },
      },
      {
        ...
        title: "管理工作区",
        category: "工作区",
        hidden: !Flag.NOVAWAY_EXPERIMENTAL_WORKSPACES,
