...
import type { ProjectDirectories } from "@novaway/sdk-v2-latest/v2"
...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    if (directories.loading && !data && !current) return [{ title: "加载项目目录中...", value: undefined }]
    const roots = [...(data ?? [])]
    ...
    return list.map((item) => {
      ...
      return {
        ...
        titleView: isRemoving ? (
          ...
          <span style={{ fg: theme.text }}>再次按 {deleteHint()} 以确认</span>
        ) : suffix ? (
          <>
