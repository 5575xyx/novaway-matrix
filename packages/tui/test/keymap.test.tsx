...
import { getNovaWayModeStack, NovaWay_BASE_MODE, NovaWayKeymapProvider, registerNovaWayKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerNovaWayKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when NovaWay mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getNovaWayModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getNovaWayModeStack(keymap).push("autocomplete")
