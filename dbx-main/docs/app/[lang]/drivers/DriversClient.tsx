...
import {
  fetchAgentDownloadCatalog,
  formatSize,
  type AgentDownloadCatalog,
  type JreDisplayEntry,
  type NativeAgentDisplayEntry,
...
const i18n = {
  en: {
    ...
    bundlesDesc:
      "Platform-specific ZIP packages that include the agent registry, database drivers, native agents, and the matching JRE.",
    drivers: "Database Drivers",
    driversDesc: "JDBC driver JAR files for each supported database type.",
    nativeAgents: "Native Agents",
    nativeAgentsDesc:
      "Go-based native agents for Oracle and XuguDB. Download the executable that matches the offline machine.",
    ...
    downloadHint:
      "For air-gapped environments: download the bundle for your platform on an internet-connected machine, then transfer it to the offline machine and import it in DBX from Settings > Driver Manager. Use the driver and JRE tabs only when you need individual artifacts.",
  },
  cn: {
    title: "离线驱动下载",
    ...
    clearSearch: "清空搜索",
    downloadHint:
      "内网环境使用说明：在有网的电脑上下载对应平台的整包，然后传输到内网机器，在 DBX 的“设置 > 驱动管理”中导入。只有需要单个产物时再使用驱动和 JRE 标签页。",
  },
}
  ...
  const filteredBundles = useMemo(
    () =>
      bundles.filter((bundle) =>
        matchesSearch(
          [bundle.platformLabel, bundle.platformKey, bundle.filename, formatSize(bundle.size)],
          normalizedSearch,
        ),
      ),
    [bundles, normalizedSearch],
  )
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-landing-line bg-landing-panel/70 p-3">
                <div className="inline-flex shrink-0 rounded-[8px] border border-landing-line bg-black/10 p-1 max-[760px]:grid max-[760px]:w-full max-[760px]:grid-cols-2">
                  ...
                  <button
                    ...
                    className={`inline-flex h-8 cursor-pointer items-center justify-center gap-2 rounded-[6px] px-3 text-xs font-[650] transition-colors ${activeTab === "native" ? "bg-landing-blue text-white" : "text-landing-muted hover:text-landing-ink"}`}
                  >
                    <Terminal size={14} />
                    {t.nativeAgents}
                  </button>
                  <button
                    ...
                    className={`inline-flex h-8 cursor-pointer items-center justify-center gap-2 rounded-[6px] px-3 text-xs font-[650] transition-colors ${activeTab === "jre" ? "bg-landing-blue text-white" : "text-landing-muted hover:text-landing-ink"}`}
                  >
                    <Cpu size={14} />
                    {t.jre}
                  </button>
                    ...
                    <tbody className="max-[760px]:block">
                      {filteredBundles.map((bundle) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3 font-mono text-[11px] text-landing-muted max-[760px]:hidden">
                            {bundle.filename}
                          </td>
                  ...
                  <table className="w-full table-auto border-collapse text-sm max-[760px]:block">
                    ...
                    <tbody className="max-[760px]:block">
                      {filteredNativeGroups.map((group) => {
                        ...
                        const selectedAgent =
                          group.options.find((option) => option.platformKey === selectedPlatform) ?? group.options[0]
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-5 py-3 max-[760px]:col-span-2 max-[760px]:px-0 max-[760px]:pt-0">
                              ...
                              >
                                {group.options.map((option) => (
                                  <option
                                    key={nativeKey(option)}
                                    value={option.platformKey}
                                    className="bg-[#10151d] text-landing-ink"
                                  >
                                    {option.platformLabel}
                                  </option>
                                ))}
                            ...
                            <td className="px-5 py-3 text-right text-xs text-landing-muted max-[760px]:hidden">
                              {formatSize(selectedAgent.info.size)}
                            </td>
                            <td className="px-5 py-3 text-right max-[760px]:px-0">
                              <a
                                href={selectedAgent.info.url}
                                download
                                className="landing-nav-link inline-flex h-8 items-center gap-1 whitespace-nowrap rounded-[6px] border border-landing-line px-2.5 text-xs font-medium transition-colors hover:border-landing-blue"
                              >
                                <Download size={13} />
