  ...
  return (
    <main className="landing">
      ...
      <RevealSection className="grid grid-cols-4 gap-3 max-w-[1180px] mx-auto px-7 pt-6 pb-11 [animation:landing-rise_0.72s_ease-out_0.1s_both] max-[760px]:grid-cols-1 max-[760px]:px-[18px] max-[760px]:pb-8">
        {metricItems.map((item) => (
          <div
            key={item.label}
            data-stagger
            className="landing-glass-card min-h-[118px] rounded-[10px] p-[22px] max-[760px]:min-h-[96px] max-[760px]:p-[18px]"
          >
      ...
      <RevealSection className="landing-glass-card-green flex items-center justify-between gap-[22px] max-w-[calc(1180px-56px)] mx-auto px-7 py-7 rounded-[10px] max-[760px]:block max-[760px]:px-[18px]">
        ...
        <Link
          href={`/${l}/docs/getting-started`}
          className="landing-inline-link flex shrink-0 items-center gap-[7px] text-sm font-[650] max-[760px]:mt-4"
          target="_blank"
        >
          {t.readDocs}
          <ArrowRight size={15} />
      ...
      <RevealSection className="relative max-w-[1180px] mx-auto px-7 pt-[70px] pb-1 max-[760px]:px-[18px]">
        <div className="grid grid-cols-[minmax(260px,0.28fr)_minmax(0,0.72fr)] gap-9 items-end mb-[30px] max-[760px]:block">
          ...
          <p className="mt-2 max-w-[760px] text-landing-muted text-sm leading-[1.65] justify-self-end text-right max-[760px]:max-w-none max-[760px]:text-left">
            {t.supportDesc}
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className="landing-db-icon grid place-items-center w-12 h-12 mb-[15px]">
                  {isCta ? (
                    <span className="grid place-items-center w-10 h-10 rounded-full border-2 border-dashed text-landing-blue border-landing-blue text-2xl leading-none">
                      +
                    </span>
                  ) : db.icon ? (
                    <img
                      ...
                      width={38}
                      height={38}
                      className="block w-[38px] h-[38px] object-contain"
                    />
                  ) : (
                    <span className="grid place-items-center min-w-[46px] h-8 rounded-lg px-2 text-white text-xs font-[780]">
                      {db.name.slice(0, 2).toUpperCase()}
                    </span>
                  )}
                </div>
                <strong
                  className={`text-sm font-[650] leading-[1.2] text-center ${isCta ? "text-landing-blue" : "text-[color-mix(in_srgb,var(--color-landing-ink)_92%,var(--color-landing-muted))]"}`}
                >
                  {db.name}
                </strong>
              </CardTag>
      ...
      <RevealSection className="max-w-[1180px] mx-auto px-7 pt-[70px] pb-1 max-[760px]:px-[18px]">
        ...
        <div className="grid grid-cols-3 gap-2.5 max-[1040px]:grid-cols-2 max-[760px]:grid-cols-1 max-[760px]:mt-[18px]">
          {capabilityItems.map((item) => (
            <div
              key={item.label}
              className="landing-capability flex items-center gap-2.5 min-h-[72px] rounded-lg px-[15px] py-3.5"
              data-stagger
            >
      ...
      <LandingLatestUpdates
        ...
        initialRelease={initialChangelog.releases[0]}
        initialLatestRelease={initialLatestRelease}
      />
      ...
      <RevealSection className="flex items-center justify-between gap-6 max-w-[1180px] mx-auto px-7 border border-landing-line rounded-[10px] bg-landing-panel mt-[72px] mb-14 py-[30px] max-[760px]:block max-[760px]:px-[18px]">
        ...
        <div className="flex items-center gap-2.5 flex-wrap justify-end max-[760px]:mt-[18px]">
          <Link
            href="https://github.com/t8y2/dbx/releases/latest"
            target="_blank"
            className="landing-final-link inline-flex items-center justify-center min-h-[42px] rounded-[7px] px-[15px] text-sm font-[650]"
          >
            {t.release}
          </Link>
          <Link
            href={`/${l}/docs/getting-started#docker`}
            target="_blank"
            className="landing-final-link inline-flex items-center justify-center min-h-[42px] rounded-[7px] px-[15px] text-sm font-[650]"
          >
            {t.docker}
