...
<Steps>
  ...
  <Step>
    ### Add a New Snippet Click **Add Snippet** and fill in: - **Trigger**: a short key you type in the editor (e.g.,
    `selall`, `joinex`) - **Template**: the SQL text that replaces the trigger
  </Step>
  <Step>
    ### Use the Snippet In the SQL editor, type the trigger key and press `Tab` or select it from the autocomplete list.
    The template expands in place.
  </Step>
</Steps>
