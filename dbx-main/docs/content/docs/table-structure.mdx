...
<Callout type="info">
  The structure editor is enabled for MySQL, PostgreSQL, SQLite, SQL Server, and ClickHouse. Other databases may still
  expose DDL through the schema browser, but DBX does not show the visual editor when it cannot generate safe changes.
</Callout>
...
<Callout type="warn">
  Dropping columns, changing column types, and changing nullable constraints can affect existing data. Back up
  production databases first and confirm application code does not depend on the old structure.
</Callout>
