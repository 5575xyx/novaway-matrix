...
<Callout type="info">
  Field Lineage is enabled for MySQL, PostgreSQL, SQLite, SQL Server, Oracle, Redshift, DM, and GaussDB. Other engines
  may not expose enough relationship metadata for DBX to build useful lineage.
</Callout>
...
<Callout type="info">
  Field Lineage is an analysis aid, not a strongly governed lineage platform. For critical decisions, confirm results
  with database constraints, view definitions, and application code.
</Callout>
