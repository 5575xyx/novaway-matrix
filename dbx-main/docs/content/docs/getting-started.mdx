...
<Callout type="info">
  The core database features are shared, but desktop-only integrations such as opening DBX windows from MCP require the
...
<Callout type="info">
  Connection passwords, SSH passwords, SSH key passphrases, and connection strings are stored separately from the
  regular connection JSON in DBX's local storage. Use [Config Export/Import](/en/docs/config-export) when you need to
  migrate encrypted connection settings.
</Callout>
...
<Tabs groupId="dev-platform" items={["macOS", "Linux", "Windows"]}>
  ...
  <Tab value="Linux">
    ```bash sudo apt-get install -y libwebkit2gtk-4.1-dev libgtk-3-dev libappindicator3-dev librsvg2-dev patchelf
