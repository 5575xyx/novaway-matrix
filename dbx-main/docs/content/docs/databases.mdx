...
<Callout type="info">
  DBX uses native Rust drivers for the main app whenever possible. Some engines are compatibility profiles over MySQL or
  PostgreSQL protocols, and some are handled through Agent drivers or the optional [JDBC Plugin](/en/docs/plugins).
</Callout>
...
<Callout type="info">
  SQLite, DuckDB, Access, and JDBC profiles use port `0` in the saved connection model when a network port is not part
  of the connection.
</Callout>
...
<Callout type="warn">
  If a database can connect but does not appear in a feature row, that feature may be hidden or limited for that engine.
  This prevents DBX from generating SQL it cannot review reliably.
</Callout>
