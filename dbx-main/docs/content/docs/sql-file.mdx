...
<Callout type="warn">
  SQL files may modify large amounts of data or schema. Confirm the target connection, target database, and script
  source before running. Back up production databases first.
</Callout>
...
<Callout type="info">
  If a script must be atomic, include transaction control in the script itself and confirm the target database supports
  the required transaction behavior.
</Callout>
