...
<Callout type="info">
  DBX CLI is a dedicated command line package for terminal, script, and coding-agent workflows. It shares DBX connection
  storage and SQL safety rules with the MCP server.
</Callout>
...
<Callout type="info">
  Before testing system-level `dbx://` launches, install and open DBX Desktop once so the operating system can register
  the URL scheme. On macOS, use `open 'dbx://connection/new?...'` to test it.
</Callout>
