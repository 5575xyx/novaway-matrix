...
test("createUncachedUrl appends cache buster without removing existing query params", () => {
  assert.equal(
    createUncachedUrl("https://dl.dbxio.com/releases/latest/latest.json?lang=cn", 123),
    "https://dl.dbxio.com/releases/latest/latest.json?lang=cn&_ts=123",
  )
})

test("createUncachedUrl leaves server-side build URLs stable without a cache buster", () => {
  assert.equal(
    createUncachedUrl("https://dl.dbxio.com/releases/latest/latest.json"),
