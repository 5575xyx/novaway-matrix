    ...
    >
      <div className="landing-testimonial-track flex w-max min-w-full gap-3.5 px-[7px]">
        {repeatedItems.map((item, index) => (
          <figure
            className="landing-testimonial-card flex-[0_0_340px] min-h-[188px] rounded-[9px] m-0 p-[22px] max-[760px]:flex-[0_0_286px] max-[760px]:min-h-[210px] max-[760px]:p-[18px]"
            key={`${item.name}-${index}`}
          >
            <div className="flex gap-3 items-center">
              <img
                className="w-[42px] h-[42px] shrink-0 border border-[rgba(178,200,230,0.18)] rounded-full bg-[rgba(255,255,255,0.04)] object-cover"
                src={item.avatar}
                alt=""
                width={42}
                height={42}
            ...
            <blockquote className="mt-[18px] text-sm font-[560] leading-[1.58] text-[color-mix(in_srgb,var(--color-landing-ink)_88%,var(--color-landing-muted))]">
              {item.quote}
            </blockquote>
          </figure>
        ))}
