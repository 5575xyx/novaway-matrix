...
export function LandingLatestUpdates({
...
}: LandingLatestUpdatesProps) {
  const [latest, setLatest] = useState(() =>
    buildLandingLatestUpdates(lang, initialRelease, fallbackVersion, initialLatestRelease),
  )
  ...
  useEffect(() => {
    ...
    Promise.all([fetchLatestReleaseInfo(), fetchChangelog(lang)]).then(([releaseInfo, data]) => {
      ...
      setLatest(
        buildLandingLatestUpdates(
          lang,
          data.releases[0] ?? initialRelease,
          fallbackVersion,
  ...
  return (
    <RevealSection className="grid grid-cols-[minmax(86px,0.16fr)_minmax(210px,0.32fr)_minmax(0,0.38fr)_max-content] gap-[22px] items-center max-w-[1180px] mx-auto px-7 border-t border-b border-landing-line mt-[62px] py-6 max-[1040px]:grid-cols-[minmax(0,1fr)_max-content] max-[760px]:block max-[760px]:px-[18px]">
      <div className="landing-update-version w-max rounded-[7px] px-2.5 py-[7px] text-[13px] font-[720]">
        {latest.version}
      </div>
      ...
      <ul className="grid gap-2 m-0 p-0 list-none max-[1040px]:col-span-full max-[760px]:mt-4">
        {latest.items.map((item) => (
          <li
            key={item}
            className="flex gap-2 items-center text-[13px] font-[560] leading-[1.45] text-[color-mix(in_srgb,var(--color-landing-ink)_88%,var(--color-landing-muted))]"
            data-stagger
          >
