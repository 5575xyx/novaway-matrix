...
const displayedSelectedClusters = computed(() => [
  ...normalizedClusterOptions.value.filter((cluster) => selectedAllowedClusters.value.includes(cluster)),
  ...selectedAllowedClusters.value.filter((cluster) => !clusterOptionSet.value.has(cluster)),
])
      ...
      <table>
        ...
        <tbody>
          <tr
            ...
            :key="tenant.name"
            :class="{ selected: selectedTenant === tenant.name }"
    ...
    <div v-if="showCreateDialog" class="dialog-overlay" @click="showCreateDialog = false">
      <div class="dialog" @click.stop>
          ...
          <div class="form-group">
            ...
            <div ref="clusterSelectRef" class="cluster-select-wrap">
              <button
                ...
                :class="{ open: clusterDropdownOpen }"
                :disabled="readOnly"
                ...
                <span v-if="displayedSelectedClusters.length" class="cluster-selected-tags">
                  <span v-for="cluster in displayedSelectedClusters" :key="cluster" class="tag">
                    ...
                    <span
                      ...
                      role="button"
                      tabindex="0"
                      @click.stop="removeCluster(cluster)"
                      @keyup.enter.stop="removeCluster(cluster)"
                      >×</span
                    >
                  </span>
                </span>
                <span v-else class="cluster-placeholder">{{
                  normalizedClusterOptions.length ? "请选择允许集群" : "未探测到集群，可手动添加"
                }}</span>
                <span class="cluster-arrow">{{ clusterDropdownOpen ? "⌃" : "⌄" }}</span>
              </button>
              <div v-if="clusterDropdownOpen" class="cluster-options">
                <button
                  v-for="cluster in normalizedClusterOptions"
                  :key="cluster"
                  type="button"
            ...
            <div class="input-with-button cluster-manual">
              <input
                ...
                placeholder="手动添加集群"
                :disabled="readOnly"
                @keyup.enter="addCluster"
              />
              <button @click="addCluster" :disabled="readOnly" class="btn-sm">添加</button>
            </div>
            <div v-if="!selectedAllowedClusters.length" class="form-hint form-hint-error">
              至少选择或添加一个允许集群
            </div>
          </div>
        ...
        <div class="dialog-footer">
          ...
          <button @click="handleCreate" :disabled="loading || readOnly || !canSubmitTenant" class="btn-primary">
            创建
          </button>
        </div>
      </div>
    ...
    <div v-if="showEditDialog" class="dialog-overlay" @click="showEditDialog = false">
      <div class="dialog" @click.stop>
        ...
        <div class="dialog-footer">
          ...
          <button @click="handleUpdate" :disabled="loading || readOnly || !canSubmitTenant" class="btn-primary">
            保存
          </button>
        </div>
      </div>
