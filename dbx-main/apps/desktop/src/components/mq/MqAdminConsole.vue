...
const effectiveTenant = computed(() =>
  isKafkaCluster.value ? selectedTenant.value || KAFKA_CONTEXT : selectedTenant.value,
)
const effectiveNamespace = computed(() =>
  isKafkaCluster.value ? selectedNamespace.value || KAFKA_CONTEXT : selectedNamespace.value,
)
  ...
  <div class="mq-admin-console">
    ...
    <div class="mq-toolbar">
      <div class="mq-breadcrumb">
        <span v-if="clusterInfo" class="cluster-info">
          {{ clusterInfo.systemKind.toUpperCase() }} {{ clusterInfo.serverVersion || "" }}
        </span>
        <span v-if="selectedTenant" class="breadcrumb-separator">›</span>
        <button v-if="selectedTenant" class="breadcrumb-button" @click="goToTenantLevel" title="查看租户">
          {{ selectedTenant }}
        </button>
        <span v-if="selectedNamespace" class="breadcrumb-separator">›</span>
        <button v-if="selectedNamespace" class="breadcrumb-button" @click="goToNamespaceLevel" title="查看命名空间">
          {{ selectedNamespace }}
        </button>
        <span v-if="selectedTopic" class="breadcrumb-separator">›</span>
        <button v-if="selectedTopic" class="breadcrumb-button" @click="goToTopicLevel" title="查看主题">
    ...
    <!-- Tab Bar -->
    <div class="mq-tabs">
      <button v-if="canManageTenants" :class="{ active: activeTab === 'tenants' }" @click="setActiveTab('tenants')">
        租户
      </button>
      <button
        v-if="canManageNamespaces"
        :class="{ active: activeTab === 'namespaces' }"
        @click="setActiveTab('namespaces')"
      >
        命名空间
      </button>
      ...
      <button v-if="canSendMessage" :class="{ active: activeTab === 'messages' }" @click="setActiveTab('messages')">
        消息
      </button>
      <button :class="{ active: activeTab === 'broker' }" @click="setActiveTab('broker')">Broker</button>
      <button v-if="canManagePolicies" :class="{ active: activeTab === 'policies' }" @click="setActiveTab('policies')">
        策略
      </button>
      <button
    ...
    <div class="mq-content">
      <TenantsPanel
        ...
        :supports-tenants="canManageTenants"
        :read-only="readOnly"
      ...
      />
      <SendMessagePanel
        v-else-if="activeTab === 'messages' && canSendMessage"
        :connection-id="connectionId"
        :tenant="effectiveTenant"
        :namespace="effectiveNamespace"
        :topic="selectedTopic"
      ...
      />
      <PermissionsPanel
        v-else-if="activeTab === 'permissions' && canManagePermissions"
        :connection-id="connectionId"
        :topic="selectedTopic"
        :tenant="effectiveTenant"
        :namespace="effectiveNamespace"
      ...
      />
      <RawApiPanel
        v-else-if="activeTab === 'raw' && canUseRawApi"
        :connection-id="connectionId"
        :tenant="selectedTenant"
