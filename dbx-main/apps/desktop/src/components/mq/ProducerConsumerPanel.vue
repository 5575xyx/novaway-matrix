...
const selectedPartitionSubscription = computed(() =>
  selectedPartition.value?.subscriptions.find((sub) => sub.name === selectedSubscription.value),
)
const aggregateConsumers = computed(
  () => subscriptions.value.find((sub) => sub.name === selectedSubscription.value)?.consumers ?? [],
)
...
async function loadRuntimeClients() {
  ...
  try {
    ...
    subscriptions.value =
      parsedSubscriptions.length || partitionSubscriptions.length
        ? mergeSubscriptionOptions(parsedSubscriptions, extractPartitionClientRows(statsData.raw))
        : await mqListSubscriptions(props.connectionId, current)
    if (!isRuntimeLoadCurrent(loadSeq, currentKey)) return
...
function syncSelectedPartition() {
  ...
  const withActiveConsumers = rows.find((row) =>
    row.subscriptions.some((sub) => sub.name === selectedSubscription.value && sub.consumers.length > 0),
  )
...
function syncSelectedSubscriptionForPartition() {
  ...
  if (!partition) return
  if (selectedSubscription.value && partition.subscriptions.some((sub) => sub.name === selectedSubscription.value))
    return
...
function extractSubscriptionsFromStats(raw: unknown): SubscriptionInfo[] {
  return Object.entries(objectRecord(objectRecord(raw).subscriptions)).map(([name, body]) =>
    parseSubscription(name, body),
  )
}
...
function extractPartitionClientRows(raw: unknown): PartitionClientRow[] {
  return Object.entries(objectRecord(objectRecord(raw).partitions)).map(([name, value]) => {
    ...
    const partitionSubscriptions = Object.entries(objectRecord(body.subscriptions)).map(
      ([subscriptionName, subscriptionBody]) => parseSubscription(subscriptionName, subscriptionBody),
    )
    ...
    <div class="panel-toolbar">
      ...
      <div class="toolbar-actions">
        <button
          v-if="!isKafkaCluster"
          class="btn-sm danger"
          :disabled="readOnly || !topic || unloading"
          @click="unloadTopic"
        >
          {{ unloading ? "卸载中..." : "卸载主题" }}
        </button>
    ...
    <div v-else class="runtime-content">
      ...
      <section v-else-if="partitionRows.length" class="runtime-section">
        ...
        <table class="runtime-table partition-table">
          ...
          <tbody>
            <tr
              v-for="partition in partitionRows"
              :key="partition.name"
              :class="{ selected: selectedPartition?.name === partition.name }"
              @click="selectedPartitionName = partition.name"
            >
      ...
      <section class="runtime-section">
        ...
        <table v-else class="runtime-table">
          ...
          <tbody>
            <tr
              v-for="producer in displayedProducers"
              :key="`${selectedScopeLabel}-${producer.producerId}-${producer.producerName}`"
            >
      ...
      <section class="runtime-section">
        <div class="section-heading">
          ...
          <div class="subscription-selector">
            ...
            <select v-if="!isKafkaStats && partitionRows.length" v-model="selectedPartitionName" :disabled="loading">
              <option v-for="partition in partitionRows" :key="partition.name" :value="partition.name">
                {{ partition.shortName }}
              </option>
            </select>
        ...
        <table v-else class="runtime-table">
          ...
          <tbody>
            <tr
              v-for="consumer in displayedConsumers"
              :key="`${selectedScopeLabel}-${selectedSubscription}-${consumer.consumerName}-${consumer.address}`"
            >
