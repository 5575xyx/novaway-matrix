...
async function loadPermissions() {
  ...
  try {
    ...
    if (msg.includes("SecurityDisabled") || msg.includes("No Authorizer") || msg.includes("authorizer")) {
      error.value = undefined
      notice.value =
        "当前 Kafka 集群未启用权限管理（Authorizer 未配置）。如需 ACL 功能，请在 Broker 配置中添加 authorizer.class.name。"
    } else {
      error.value = msg
    }
  } finally {
        ...
        <div class="grant-row">
          <label>
            ...
            <input
              ...
              v-model="roleName"
              type="text"
              placeholder="例如: app-producer"
              :disabled="readOnly"
              :class="{ invalid: roleNameError }"
            ...
            <div class="token-form">
              ...
              <div class="expiry-control">
                ...
                <label :class="{ muted: tokenExpiresUnlimited }">
                  有效期（天）
                  <input
                    v-model.number="tokenExpiresDays"
                    type="number"
                    min="1"
                    step="1"
                    :disabled="tokenExpiresUnlimited"
                  />
                </label>
              </div>
