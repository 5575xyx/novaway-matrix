...
async function handleSkipMessages() {
  ...
  try {
    const count: SkipCount =
      skipFormData.value.mode === "all" ? { kind: "all" } : { kind: "count", count: skipFormData.value.count }
...
<template>
  <div class="subscriptions-panel">
    <div class="panel-toolbar">
      <h3>订阅管理</h3>
      <button
        v-if="supportsCreateSubscription !== false"
        @click="openCreateDialog"
        :disabled="loading || readOnly || !topic"
        class="btn-primary"
      >
        + 创建订阅
      </button>
      ...
      <table>
        ...
        <tbody>
          <tr
            ...
            :key="sub.name"
            :class="{ selected: selectedSub?.name === sub.name }"
          ...
          >
            ...
            <td class="actions">
              <button
                v-if="supportsResetCursor !== false"
                @click.stop="openResetDialog(sub)"
                :disabled="readOnly"
                class="btn-sm"
              >
                重置游标
              ...
              <button
                v-if="supportsSkipMessages !== false"
                @click.stop="openSkipDialog(sub)"
                :disabled="readOnly"
                class="btn-sm"
              >
                跳过消息
              ...
              <button
                v-if="supportsClearBacklog !== false"
                @click.stop="handleClearBacklog(sub)"
                :disabled="readOnly"
                class="btn-sm"
              >
                清空积压
              ...
              <button
                v-if="supportsExpireMessages !== false"
                @click.stop="openExpireDialog(sub)"
                :disabled="readOnly"
                class="btn-sm"
              >
                过期消息
