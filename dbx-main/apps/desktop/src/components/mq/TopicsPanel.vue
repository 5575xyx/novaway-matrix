...
async function handleCreate() {
  ...
  try {
    ...
    const partitions =
      props.supportsPartitionedTopics !== false && formData.value.partitioned ? formData.value.partitions : undefined
  ...
  <div class="topics-panel">
    <div class="panel-toolbar">
      ...
      <div class="toolbar-actions">
        ...
        <button @click="openCreateDialog" :disabled="loading || readOnly || !tenant || !namespace" class="btn-primary">
          + 创建主题
        </button>
      </div>
    </div>
      ...
      <table>
        ...
        <tbody>
          ...
          >
            ...
            <td class="actions">
              <button
                v-if="topic.partitioned && supportsPartitionedTopics !== false"
                @click.stop="openPartitionsDialog(topic)"
                :disabled="readOnly || !topic.partitions"
                class="btn-sm"
              >
                调整分区
              </button>
      ...
      <div class="dialog" @click.stop>
        ...
        <div class="dialog-body">
          ...
          <div class="form-group">
            <label>新分区数*</label>
            <input
              v-model.number="newPartitions"
              type="number"
              :min="(editingTopic?.partitions || 0) + 1"
              max="256"
              :disabled="readOnly"
            />
