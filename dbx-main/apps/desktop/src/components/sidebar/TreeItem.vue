...
} from "@/lib/connection/connectionFile"
...
import {
  ...
  loadTableMetadata,
  TABLE_METADATA_CACHE_TTL_MS,
  tableMetadataToDataTabMeta,
} from "@/lib/metadata/tableMetadataCache"
import {
...
} from "@/lib/database/createDatabaseSql"
import {
...
} from "@/lib/sidebar/sidebarTreeSelection"
import {
  connectionPasteTargetGroupId,
  selectedConnectionClipboardTargets,
  selectedConnectionDeleteTargets,
  selectedConnectionDuplicateTargets,
  selectedConnectionEditTarget,
} from "@/lib/sidebar/sidebarConnectionSelection"
...
import {
  ...
  fallbackCreateDatabaseCharsetMetadata,
  nextCreateDatabaseCollation,
  normalizeCreateDatabaseCharset,
  parseCreateDatabaseCharsetMetadata,
} from "@/lib/database/createDatabaseCharsetOptions"
...
function currentTableStructureDatabaseType(): DatabaseType | undefined {
  return props.node.connectionId
    ? tableStructureDatabaseTypeForConnection(connectionStore.getConfig(props.node.connectionId))
    : undefined
}
...
function tableStructureDatabaseTypeForNode(node: TreeNode): DatabaseType | undefined {
  return node.connectionId
    ? tableStructureDatabaseTypeForConnection(connectionStore.getConfig(node.connectionId))
    : undefined
}
...
function visibleLabel(node: TreeNode): string {
  if (
    ...
    node.type === "vector-collection" ||
    node.type === "elasticsearch-index"
  ) {
...
function isLocalFileConnection(config: Pick<ConnectionConfig, "db_type" | "port">): boolean {
  return (
    ...
    (config.db_type === "h2" && config.port === 0)
  )
}
...
function redactedConnectionString(value: string): string {
  return value
    .replace(/(:\/\/[^/\s:@?#;]+):([^@\s/?#;]+)@/g, "$1:***@")
    .replace(/([?&;](?:password|pwd|pass|token|secret|key)=)[^&;]*/gi, "$1***")
}
...
const objectCommentTooltip = computed(() => {
  const node = props.node
  const comment =
    node.type === "column" && node.meta && "comment" in node.meta ? (node.meta as ColumnInfo).comment : node.comment
  if (!comment || (node.type !== "schema" && node.type !== "table" && node.type !== "view" && node.type !== "column"))
    return null
  const rows: DetailTooltipRow[] = [
  ...
  try {
    if (node.type === "connection" && node.connectionId) {
      ...
      if (config?.db_type === "redis") {
        ...
        config?.db_type === "qdrant" ||
        config?.db_type === "milvus" ||
        config?.db_type === "weaviate" ||
        config?.db_type === "chromadb"
      ) {
      ...
      await connectionStore.ensureConnected(node.connectionId)
      queryStore.openNacosAdmin(node.connectionId, {
        namespace: node.nacosNamespace || "",
        namespaceName: node.nacosNamespaceName || node.label,
      })
      ...
      await connectionStore.ensureConnected(node.connectionId)
      const collectionRef = node.id.includes("__vector_collection:")
        ? node.id.split("__vector_collection:").pop() || node.label
        : node.label
      ...
      node.type === "linked-server-schema" &&
      node.connectionId &&
...
const isConnectionSelectionChecked = computed(
  () =>
    connectionStore.connectionMultiSelectActive &&
    props.node.type === "connection" &&
    !!props.node.connectionId &&
    selectedConnectionIds.value.includes(props.node.connectionId),
)
...
function toggleConnectionMultiSelection(event: MouseEvent) {
  ...
  connectionStore.selectedTreeNodeId = ids.includes(props.node.connectionId)
    ? props.node.connectionId
    : (ids[0] ?? null)
...
function requestPasteTreeClipboard(): boolean {
  ...
  if (clipboard?.kind === "connection-copy") {
    const targetGroupId = connectionPasteTargetGroupId(props.node, (connectionId) =>
      connectionStore.groupIdForConnection(connectionId),
    )
    void connectionStore
      .pasteConnectionClipboard(targetGroupId)
      .then((count) => {
        if (count > 0)
          toast(count > 1 ? t("connection.duplicatedSelected", { count }) : t("connection.duplicated"), 2000)
      })
...
function onDoubleClick() {
  ...
  if (action === "open-object-browser") {
    ...
    action === "toggle" &&
    (props.node.type === "mongo-gridfs" || isDocumentBrowserTreeNode(props.node.type))
  ) {
...
async function openData() {
  ...
  const isSameDataTableTab = (tab: (typeof queryStore.tabs)[number]) =>
    ...
    tab.database === node.database &&
    (tab.schema || "") === (tableSchema || "") &&
    (tab.tableMeta?.tableName || tab.title) === node.label
  ...
  const existingTableMetaAgeMs = tab?.tableMetaUpdatedAt
    ? Date.now() - tab.tableMetaUpdatedAt
  ...
  const tabCachedTableMeta =
    existingTableMeta?.tableName === node.label &&
    existingTableMeta.schema === tableSchema &&
    existingTableMeta.tableType === tableType &&
    existingTableMeta.columns.length > 0 &&
    existingTableMetaAgeMs < DATA_TAB_METADATA_TTL_MS
      ? existingTableMeta
      : undefined
  const cachedTableMeta = sharedCachedTableMeta
    ? tableMetadataToDataTabMeta(sharedCachedTableMeta.metadata, tableSchema)
    : tabCachedTableMeta
  ...
  const isCurrentDataTab = () => {
    ...
    return (
      current?.mode === "data" &&
      current.connectionId === node.connectionId &&
...
async function generateDdlTemplate() {
  ...
  try {
    ...
    const formatted = await formatSqlForDisplay(
      ddl,
      sqlFormatDialectForDbType(currentDatabaseType()),
...
async function copySelectedNames() {
  ...
  if (connectionTargets.length > 0) {
    const copiedCount = connectionStore.copyConnectionsToTreeClipboard(
      connectionTargets.map((node) => node.connectionId),
    )
...
async function duplicateConnection() {
  ...
  toast(
    duplicatedCount > 1 ? t("connection.duplicatedSelected", { count: duplicatedCount }) : t("connection.duplicated"),
    2000,
  )
}
...
const pasteTableEntries = ref<
  { sourceName: string; targetName: string; connectionId: string; database: string; schema?: string }[]
>([])
...
const ddlDialect = computed(() => {
  ...
  return codeMirrorSqlDialect(
    effectiveDatabaseTypeForConnection(connectionStore.getConfig(ddlTarget.value.connectionId)),
  )
})
const ddlFormatDialect = computed(() => {
  if (!ddlTarget.value?.connectionId) return "generic"
  return sqlFormatDialectForDbType(
    effectiveDatabaseTypeForConnection(connectionStore.getConfig(ddlTarget.value.connectionId)),
  )
})
...
const createDatabaseCollationsByCharset = ref<Record<string, string[]>>(
  fallbackCreateDatabaseCharset.collationsByCharset,
)
...
function dropObjectConfirmMessage(): string {
  ...
  if (props.node.type === "materialized_view")
    return t("contextMenu.confirmDropViewMessage", { name: props.node.label })
...
function viewObjectDdl() {
  ...
  connectionStore
    ...
    .then(async (result) => {
      ...
      const formatted = await formatSqlForDisplay(
        ddl,
        sqlFormatDialectForDbType(effectiveDatabaseTypeForConnection(connection)),
        settingsStore.editorSettings.sqlFormatter,
      )
...
async function refreshBatchDropPreviewSql() {
  ...
  if (mongoIndexTargets.length) {
    batchDropPreviewSql.value = mongoIndexTargets
      .map((target) => mongoIndexDropPreview(target, mongoIndexNameForNode(target)))
      .join("\n")
    return
  }
    ...
    if (mongoIndexTargets.length) {
      ...
      for (const groupTargets of grouped.values()) {
        ...
        const result = await api.mongoDropIndexes(
          ...
          first.database,
          first.tableName,
          JSON.stringify(names.length === 1 ? names[0] : names),
          false,
        )
...
const canCreateTable = computed(() => {
  ...
  return (
    ...
    !isSqlServerLinkedNode(props.node) &&
    !!props.node.database &&
    supportsTableStructureEditing(tableStructureDatabaseTypeForConnection(config))
  )
})
...
const canCreateDatabase = computed(() => {
  ...
  return (
    props.node.type === "connection" &&
    (supportsDatabaseCreation(config?.db_type) ||
      config?.db_type === "duckdb" ||
      (config?.db_type === "mongodb" && config.driver_profile !== "mongodb-legacy"))
  )
})
...
const canDropDatabase = computed(() => {
  const config = props.node.connectionId ? connectionStore.getConfig(props.node.connectionId) : undefined
  return (
    props.node.type === "database" && !isSqlServerLinkedNode(props.node) && supportsDatabaseCreation(config?.db_type)
  )
})
...
function canDropMongoIndexNode(node: TreeNode): boolean {
  ...
  return (
    ...
    mongoIndexNameForNode(node) !== "_id_"
  )
}
...
const canDropAllMongoIndexes = computed(() => {
  const config = props.node.connectionId ? connectionStore.getConfig(props.node.connectionId) : undefined
  return (
    props.node.type === "mongo-collection" &&
    !!props.node.database &&
    config?.db_type === "mongodb" &&
    config.driver_profile !== "mongodb-legacy"
  )
...
const canCreateSchema = computed(() => {
  ...
  return (
    ...
    usesTreeSchemaMode(effectiveDatabaseTypeForConnection(config)) &&
...
const canEditSchemaComment = computed(() => {
  ...
  return (
    ...
    !!props.node.database &&
    !config?.read_only &&
    supportsSchemaComment(effectiveDatabaseTypeForConnection(config))
  )
})
...
function updateCreateDatabaseCharset(value: string) {
  ...
  createDatabaseCollation.value = nextCreateDatabaseCollation(
    value,
    previousCharset,
...
async function loadCreateDatabaseCharsetMetadata() {
  ...
  try {
    ...
    const [charsetResult, collationResult] = await Promise.all([
      api.executeQuery(node.connectionId, "", "SHOW CHARACTER SET", undefined, undefined, { maxRows: 1000 }),
      api.executeQuery(node.connectionId, "", "SHOW COLLATION", undefined, undefined, { maxRows: 10000 }),
    ])
    ...
    if (
      !createDatabaseCharsetOptions.value.includes(createDatabaseCharset.value) &&
      createDatabaseCharsetOptions.value.length
    ) {
      updateCreateDatabaseCharset(createDatabaseCharsetOptions.value[0])
    } else {
      createDatabaseCollation.value = nextCreateDatabaseCollation(
        createDatabaseCharset.value,
        createDatabaseCharset.value,
        createDatabaseCollation.value,
        createDatabaseCollationsByCharset.value,
      )
...
async function confirmDropMongoCollection() {
  const node = props.node
  if (node.type !== "mongo-collection" || !node.connectionId || !node.database || dropMongoCollectionLoading.value)
    return
...
function mongoIndexesGroupNodeId(
  node: Pick<TreeNode, "connectionId" | "database" | "schema" | "tableName" | "label">,
): string | null {
  if (!node.connectionId || !node.database) return null
  const tableName = node.tableName || node.label
  return node.schema
    ? `${node.connectionId}:${node.database}:${node.schema}:${tableName}:__indexes`
    : `${node.connectionId}:${node.database}:${tableName}:__indexes`
}

async function refreshMongoIndexTree(
  node: Pick<TreeNode, "connectionId" | "database" | "schema" | "tableName" | "label">,
) {
...
async function confirmDropAllMongoIndexes() {
  const node = props.node
  if (node.type !== "mongo-collection" || !node.connectionId || !node.database || dropAllMongoIndexesLoading.value)
    return
...
async function confirmPasteTable() {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const databaseType = entry.connectionId
        ? effectiveDatabaseTypeForConnection(connectionStore.getConfig(entry.connectionId))
        : undefined
  ...
  try {
    ...
    for (const target of targets) {
      ...
      const ddl = await api.getTableDdl(
        ...
        target.label,
        tableDdlObjectTypeForNode(target.type),
      )
      parts.push(ddl.trim())
    }
...
async function exportTableData(format: "csv" | "xlsx") {
  ...
  try {
    ...
    const rowLimit = settingsStore.editorSettings.exportRowLimitEnabled
      ? settingsStore.editorSettings.exportRowLimit
      : null
...
function openDatabaseExport() {
  ...
  connectionStore.databaseExportSource = {
    ...
    database: node.database,
    schema:
      node.type === "schema" || node.type === "table" || node.type === "view" || node.type === "materialized_view"
        ? node.schema
        : undefined,
    tableName:
      node.type === "table" || node.type === "view" || node.type === "materialized_view" ? node.label : undefined,
  }
}
...
const canOpenTableImport = computed(() => {
  return (
    ...
    !isSqlServerLinkedNode(props.node) &&
    !!props.node.database &&
    supportsTableImport(currentDatabaseType())
  )
})
const canOpenStructureEditor = computed(() => {
  return (
    ...
    !isSqlServerLinkedNode(props.node) &&
    !!props.node.database &&
    supportsTableStructureEditing(currentTableStructureDatabaseType())
  )
})
...
const tableComment = computed(() =>
  ...
  (props.node.type === "schema" ||
    ...
    props.node.type === "mongo-collection" ||
    props.node.type === "vector-collection" ||
    props.node.type === "elasticsearch-index") &&
  props.node.comment
    ? props.node.comment
    : null,
)
...
const isConnecting = computed(
  () =>
    props.node.type === "connection" &&
    !!props.node.connectionId &&
    connectionStore.connectingIds.has(props.node.connectionId),
)
const isConnectionReadonly = computed(
  () =>
    props.node.type === "connection" &&
    !!props.node.connectionId &&
    (connectionStore.getConfig(props.node.connectionId)?.read_only ?? false),
)
const isOpenedDatabase = computed(() => isSidebarDatabaseOpened(props.node, connectionStore.isTreeNodeChildrenLoaded))
const showsDatabaseOpenIndicator = computed(
  () =>
    props.node.type === "database" &&
    (isOpenedDatabase.value ||
      (!!props.node.connectionId &&
        props.node.database != null &&
        queryStore.isDatabaseOpen(props.node.connectionId, props.node.database))),
)
const canCloseDatabaseConnection = computed(() =>
  canCloseSidebarDatabaseConnection(props.node, connectionStore.isTreeNodeChildrenLoaded),
)
...
watch(
  [
    () => props.node.id,
    () => visibleLabel(props.node),
    () => usesFullWidthLabel.value,
    () => detailTooltip.value?.rows.length ?? 0,
    isRenamingGroup,
  ],
  () => {
...
const currentGroupId = computed(() => {
  ...
  const find = (entries: typeof connectionStore.sidebarLayout.order): string | null => {
    for (const entry of entries) {
      ...
      if (
        (entry.children ?? entry.connectionIds?.map((id) => ({ type: "connection" as const, id })) ?? []).some(
          (child) => child.type === "connection" && child.id === props.node.connectionId,
        )
      ) {
        return entry.id
      }
...
const shortcutEditConnection = computed(() =>
  formatShortcut(settingsStore.editorSettings.shortcuts.editSidebarConnection),
)
...
function treeItemMenuItems(): ContextMenuItem[] {
  ...
  if (node.type === "connection") {
    ...
    items.push({
      ...
      icon: Pencil,
      shortcut: shortcutEditConnection.value,
    })
  ...
  if (node.type === "mongo-collection") {
    ...
    if (canDropAllMongoIndexes.value || canDropMongoCollection.value) {
      ...
      if (canDropAllMongoIndexes.value) {
        items.push({
          label: t("contextMenu.dropAllIndexes"),
          action: dropAllMongoIndexes,
          icon: Trash2,
          variant: "destructive" as const,
        })
      }
      if (canDropMongoCollection.value) {
        items.push({
          label: t("contextMenu.dropCollection"),
          action: dropMongoCollection,
          icon: Trash2,
          shortcut: shortcutDelete,
          variant: "destructive" as const,
        })
...
<template>
  <div
    ...
    class="flex h-7 items-center py-0.5 pr-2"
    :style="{ paddingLeft }"
    @click.stop
    @dblclick.stop
    @mousedown.stop
  ...
  >
    <div class="relative w-full min-w-0">
      ...
      <button
        ...
        type="button"
        class="absolute right-1.5 top-1/2 flex h-4 w-4 -translate-y-1/2 items-center justify-center rounded text-muted-foreground hover:bg-muted hover:text-foreground"
        :aria-label="t('sidebar.clearTableSearch')"
        @click.stop="clearTableSearchQuery"
      >
        <X class="h-3 w-3" />
      </button>
    </div>
    ...
    <div @contextmenu="onTreeItemContextMenu($event, contextMenuSlot.onContextMenu)">
      ...
      >
        ...
        >
          ...
          <template v-if="canExpand">
            <button
              ...
              @mousedown.stop="onToggleMouseDown"
          ...
          />
          <Loader2
            v-else-if="node.type === 'load-more' && node.isLoading"
          ...
          <span
            v-if="columnComment"
            class="sidebar-object-comment ml-auto max-w-[20%] shrink-0 truncate text-right"
          ...
          <Badge v-if="isConnectionReadonly" variant="secondary" class="h-4 px-1.5 text-[10px] gap-0.5"
            ><Lock class="w-2.5 h-2.5" />{{ t("connection.readOnlyBadge") }}</Badge
          >
          ...
          <button
            ...
            :class="[
              { 'opacity-100': isConnectionSelectionChecked || connectionStore.connectionMultiSelectActive },
              isConnecting ? '' : 'ml-auto',
            ]"
            :aria-label="
              isConnectionSelectionChecked
                ? t('connectionGroup.deselectConnection')
                : t('connectionGroup.selectConnection')
            "
            @mousedown.stop
        ...
        <template v-if="detailTooltip" #content>
          <div
            class="w-max min-w-40 max-w-[min(28rem,calc(100vw-24px))] rounded-md border border-border bg-popover p-2 text-popover-foreground shadow-lg"
          >
            <div class="space-y-1">
              <div
                v-for="row in detailTooltip.rows"
                :key="row.label"
                class="grid grid-cols-[max-content_minmax(0,1fr)] gap-2 text-xs leading-5"
              >
                <span class="text-muted-foreground">{{ row.label }}</span>
                <span
                  v-if="row.multiline"
                  class="max-h-20 overflow-hidden whitespace-pre-wrap break-words text-foreground/90"
                >
                  {{ row.value }}
                </span>
  ...
  <SchemaFilterDialog
    v-if="node.type === 'database' && node.connectionId && node.database != null"
  ...
  <SchemaFilterDialog
    v-else-if="node.type === 'connection' && node.connectionId && canConfigureVisibleSchemas"
    v-model:open="showVisibleSchemasDialog"
    :connection-id="node.connectionId"
    :connection-name="node.label"
    :database="connectionStore.getConfig(node.connectionId)?.database || ''"
  />
  ...
  <Dialog v-model:open="showPasteDialog">
    <DialogContent class="sm:max-w-[500px]">
      <DialogHeader>
        <DialogTitle>{{
          pasteTableEntries.length > 1 ? t("contextMenu.batchPasteTitle") : t("contextMenu.pasteTableConfirmTitle")
        }}</DialogTitle>
      </DialogHeader>
      <div class="space-y-4">
        <div class="flex gap-2">
          <label
            ...
            :class="{ 'opacity-50 cursor-not-allowed': !pasteTableDataCopySupported }"
          >
            <input
              v-model="pasteTableMode"
              type="radio"
              value="structure-and-data"
              class="accent-primary"
              :disabled="!pasteTableDataCopySupported"
            />
        ...
        <div class="space-y-2 max-h-64 overflow-y-auto">
          <div v-for="(entry, idx) in pasteTableEntries" :key="idx" class="flex items-center gap-2">
            <span
              class="text-sm text-muted-foreground truncate min-w-0 flex-shrink basis-1/3"
              :title="entry.sourceName"
              >{{ entry.sourceName }}</span
            >
            <span class="text-xs text-muted-foreground flex-shrink-0">&rarr;</span>
            <Input
              v-model="entry.targetName"
      ...
      <DialogFooter>
        ...
        <Button :disabled="pasteTableEntries.every((e) => !e.targetName.trim())" @click="confirmPasteTable">{{
          t("dangerDialog.confirm")
        }}</Button>
      </DialogFooter>
    </DialogContent>
      ...
      <div v-if="canSetCreateDatabaseCharset" class="grid gap-2">
        ...
        <div class="grid gap-1.5">
          ...
          <SearchableSelect
            ...
            :options="
              createDatabaseCollationOptionsForCharset(createDatabaseCharset, createDatabaseCollationsByCharset)
            "
    ...
    <DialogContent class="sm:max-w-[420px]">
      ...
      <div class="grid gap-3">
        <div class="grid gap-1.5">
          ...
          <Input
            v-model="createNacosNamespaceId"
            :placeholder="t('nacos.namespaceIdPlaceholder')"
            @keydown.enter.prevent="confirmCreateNacosNamespace"
          />
        </div>
        <div class="grid gap-1.5">
          <label class="text-xs font-medium text-muted-foreground">{{ t("nacos.namespaceName") }}</label>
          <Input
            v-model="createNacosNamespaceName"
            :placeholder="t('nacos.namespaceNamePlaceholder')"
            @keydown.enter.prevent="confirmCreateNacosNamespace"
          />
        </div>
        <div class="grid gap-1.5">
          ...
          <Input
            v-model="createNacosNamespaceDesc"
            :placeholder="t('nacos.namespaceDescPlaceholder')"
            @keydown.enter.prevent="confirmCreateNacosNamespace"
          />
        </div>
      </div>
      <DialogFooter>
        <Button
          variant="outline"
          :disabled="createNacosNamespaceLoading"
          @click="showCreateNacosNamespaceDialog = false"
          >{{ t("dangerDialog.cancel") }}</Button
        >
        <Button
          :disabled="!createNacosNamespaceName.trim() || createNacosNamespaceLoading"
          @click="confirmCreateNacosNamespace"
        >
    ...
    <DialogContent class="sm:max-w-[420px]">
      ...
      <div class="grid gap-3">
        <div class="grid gap-1.5">
          <label class="text-xs font-medium text-muted-foreground">{{ t("nacos.namespaceName") }}</label>
          <Input
            v-model="editNacosNamespaceName"
            :placeholder="t('nacos.namespaceNamePlaceholder')"
            @keydown.enter.prevent="confirmEditNacosNamespace"
          />
        </div>
        <div class="grid gap-1.5">
          <label class="text-xs font-medium text-muted-foreground">{{ t("nacos.namespaceDesc") }}</label>
          <Input
            v-model="editNacosNamespaceDesc"
            :placeholder="t('nacos.namespaceDescPlaceholder')"
            @keydown.enter.prevent="confirmEditNacosNamespace"
          />
        </div>
      ...
      <DialogFooter>
        <Button variant="outline" :disabled="editNacosNamespaceLoading" @click="showEditNacosNamespaceDialog = false">{{
          t("dangerDialog.cancel")
        }}</Button>
        <Button
          :disabled="!editNacosNamespaceName.trim() || editNacosNamespaceLoading"
          @click="confirmEditNacosNamespace"
        >
  ...
  <DangerConfirmDialog
    ...
    :message="
      t('contextMenu.confirmDropMongoIndexMessage', {
        name: mongoIndexNameForNode(node),
  ...
  <Dialog v-model:open="showEditSchemaCommentDialog">
    <DialogContent class="sm:max-w-[520px]">
      ...
      <div class="grid gap-3">
        ...
        ></textarea>
        <pre
          v-if="schemaCommentPreviewSql"
          class="max-h-32 overflow-auto rounded bg-muted p-3 text-xs whitespace-pre-wrap"
          v-html="highlight(schemaCommentPreviewSql)"
        ></pre>
      </div>
      <DialogFooter>
        <Button variant="outline" :disabled="schemaCommentLoading" @click="showEditSchemaCommentDialog = false">{{
          t("dangerDialog.cancel")
        }}</Button>
