...
const filterMode = computed<FilterMode>(() =>
  connectionUsesVisibleSchemaFilter(connection.value) ? "schema" : "database",
)
...
const descriptionKey = computed(() =>
  isSchemaFilterMode.value ? "visibleSchemas.description" : "visibleDatabases.description",
)
const searchPlaceholderKey = computed(() =>
  isSchemaFilterMode.value ? "visibleSchemas.searchPlaceholder" : "visibleDatabases.searchPlaceholder",
)
const emptySelectionKey = computed(() =>
  isSchemaFilterMode.value ? "visibleSchemas.emptySelection" : "visibleDatabases.emptySelection",
)
const loadFailedKey = computed(() =>
  isSchemaFilterMode.value ? "visibleSchemas.loadFailed" : "visibleDatabases.loadFailed",
)
...
const hasSystemDatabases = computed(
  () =>
    !isSchemaFilterMode.value &&
    objectNames.value.some((database) => isSystemDatabaseName(connection.value?.db_type, database)),
)
...
async function loadDatabases() {
  ...
  try {
    ...
    const configured = isSchemaFilterMode.value
      ? connection.value?.visible_schemas?.[databaseKey.value]
      : connection.value?.visible_databases
    const initialSelection = Array.isArray(configured)
      ? normalizeVisibleDatabaseSelection(configured, names)
      : listedObjectNames.value
    selectedNames.value = new Set(initialSelection)
    showSystemDatabases.value =
      !isSchemaFilterMode.value &&
      initialSelection.some((database) => isSystemDatabaseName(connection.value?.db_type, database))
  } catch (e: any) {
    objectNames.value = []
    selectedNames.value = new Set()
...
async function saveSelection() {
  ...
  if (isSchemaFilterMode.value) {
    await connectionStore.setVisibleSchemas(
      props.connectionId,
      databaseKey.value,
      normalizeVisibleDatabaseSelection([...selectedNames.value], objectNames.value),
    )
  } else {
    ...
    <DialogContent class="sm:max-w-[460px]">
      ...
      <div class="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            class="hover:text-foreground disabled:opacity-50"
            :disabled="isLoading || showAllDisabled"
