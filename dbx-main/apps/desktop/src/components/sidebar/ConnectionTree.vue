...
import {
  ...
  selectedConnectionClipboardNodes,
...
import {
  ...
  type QueryCursorTableCandidate,
...
import {
  insertSidebarTableSearchControls,
  isSidebarTableSearchControlNode,
} from "@/lib/sidebar/sidebarTableSearchControl"
...
const simpleObjectChildTypes = new Set<TreeNodeType>([
  "table",
  "view",
  "materialized_view",
  "procedure",
...
function isSimpleObjectSearchParent(node: TreeNode): boolean {
  return (
    ...
    (!!node.children?.some((child) => simpleObjectChildTypes.has(child.type)) ||
      !!store.sidebarTableSearchQueries[node.id]?.trim())
  )
}
...
function focusTableSearchInput(parentNodeId: string) {
  void nextTick(() => {
    ...
    if (!root) return
    const input = Array.from(root.querySelectorAll<HTMLInputElement>("[data-sidebar-table-search-parent-id]")).find(
      (item) => item.dataset.sidebarTableSearchParentId === parentNodeId,
    )
    if (!input) return
...
const selectableVisibleNodes = computed<TreeNode[]>(() =>
  visibleNodes.value.filter((node) => !isSidebarTableSearchControlNode(node)),
)
...
const stickyHeaderStyle = computed<CSSProperties>(() => {
  ...
  const nextDatabaseIndex = nodes.findIndex(
    (item, index) => index > currentIndex && DATABASE_LEVEL_TYPES.has(item.type),
  )
...
const hasSidebarVerticalOverflow = computed(
  () => sidebarScrollMetrics.value.scrollHeight > sidebarScrollMetrics.value.clientHeight + 1,
)
...
function resolveLoadedLocateTarget(
  target: ActiveTabSidebarTarget,
...
async function ensureTableObjectGroupsLoaded(
  target: Extract<ActiveTabSidebarTarget, { type: "table" }>,
...
function findTableObjectGroupNodes(
  nodes: TreeNode[],
  target: Extract<ActiveTabSidebarTarget, { type: "table" }>,
): TreeNode[] {
...
function findSchemaNode(nodes: TreeNode[], connId: string, database: string, schema: string): TreeNode | null {
  for (const node of nodes) {
    if (
      ...
      sameTreeName(node.database, database) &&
      sameTreeName(node.label, schema)
    ) {
      return node
    }
    if (node.children) {
...
function collapseAllTreeNodes() {
  ...
  if (isSearching.value) {
    searchCollapsedIds.value = new Set(
      flatNodes.value.filter((item) => item.node.children?.length).map((item) => item.id),
    )
  }
}
...
function onWindowKeydown(event: KeyboardEvent) {
  ...
  if (sidebarShortcutTargetIsActive(event.target)) {
    ...
    if (
      sidebarShortcutTargetAllowsAppShortcut(event.target) &&
      isCopySidebarSelectionShortcut(event, settingsStore.editorSettings.shortcuts)
    ) {
    ...
    if (
      sidebarShortcutTargetAllowsAppShortcut(event.target) &&
      isPasteSidebarSelectionShortcut(event, settingsStore.editorSettings.shortcuts)
    ) {
  ...
  if (
    !pointerInsideTree.value ||
    isEditableSidebarTypeSearchTarget(event.target) ||
    isEditableSidebarTypeSearchTarget(document.activeElement)
  )
    return
...
function requestSelectedSidebarPaste(): boolean {
  ...
  if (clipboard?.kind === "connection-copy") {
    ...
    const targetGroupId = connectionPasteTargetGroupId(selectedNode, (connectionId) =>
      store.groupIdForConnection(connectionId),
    )
    void store
...
<template>
  <div
    ref="rootRef"
    class="h-full min-h-0 flex flex-col text-sm select-none"
    @pointerenter="pointerInsideTree = true"
    @pointerleave="pointerInsideTree = false"
  >
    ...
    <div v-if="flatNodes.length > 0 && useVirtualTree" class="connection-tree-scroll-shell relative min-h-0 flex-1">
      ...
      <div
        v-if="stickyNode"
        class="sticky-database-header pointer-events-auto absolute inset-x-0 top-0 z-[5] border-b border-border/60"
      ...
      <div
        ...
        :class="{
          'sidebar-tree-scrollbar--scrolling': isScrollingSidebar,
          'sidebar-tree-scrollbar--dragging': isDraggingSidebarScrollbar,
        }"
        @pointerdown="onSidebarScrollbarTrackPointerDown"
      >
        <div
          class="sidebar-tree-scrollbar__thumb"
          :style="sidebarScrollbarThumbStyle"
          @pointerdown.stop="onSidebarScrollbarThumbPointerDown"
        />
      </div>
    </div>
