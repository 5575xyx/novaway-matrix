...
function defaultRequestText(
  databaseType: DatabaseType | undefined,
  database: string,
  collection: string,
  mode: VectorOperationMode,
): string {
  if (databaseType === "milvus") {
    ...
    const endpoint =
      mode === "delete" ? "delete" : mode === "upsert" ? "upsert" : mode === "search" ? "search" : "query"
    ...
    <div class="flex shrink-0 items-center justify-between gap-3 border-b px-3 py-2">
      <div class="min-w-0">
        ...
        <div class="truncate text-xs text-muted-foreground">
          ...
          <span
            v-if="dimension != null"
            class="ml-1.5 inline-flex items-center rounded border bg-muted/50 px-1.5 py-px text-[11px] font-medium text-foreground/80"
            >{{ dimension }}d</span
          >
        </div>
      </div>
    ...
    <div v-if="operationMode === 'search'" class="flex shrink-0 flex-wrap items-center gap-3 border-b px-3 py-2">
      <div class="flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        <input
          ...
          class="h-7 w-80 rounded border bg-muted/30 px-2 font-mono text-xs outline-none focus:border-primary"
          placeholder="[0.1, 0.2, ...]"
          spellcheck="false"
        />
      </div>
      <div class="flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        <input
          ...
          min="1"
          max="1000"
          class="h-7 w-16 rounded border bg-muted/30 px-2 text-xs outline-none focus:border-primary"
        />
      </div>
    </div>
    <div class="grid min-h-0 flex-1 grid-rows-[minmax(9rem,15rem)_1fr]">
      ...
      <div class="min-h-0">
        ...
        <div
          ...
          class="border-b bg-emerald-50 px-3 py-1.5 text-xs text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300"
        >
          {{ statusMessage }}
        </div>
        <QueryLoadingState
          v-if="loading && result.columns.length === 0"
          class="h-full"
          label-key="editor.fetching"
          :elapsed-seconds="elapsedSeconds"
          show-cancel
          :cancel-disabled="!executionId || cancelling"
