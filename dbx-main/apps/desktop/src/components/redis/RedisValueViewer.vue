...
const deleteDetails = computed(() => {
  ...
  if (pending.kind === "hash")
    return t("dangerDialog.redisHashFieldDetails", { key, field: formatValue(pending.field) })
    ...
    <template v-else-if="data">
        ...
        <div class="flex min-h-7 flex-wrap items-center gap-2 px-4 pb-1">
          ...
          <template v-if="!editingTtl">
            <Badge
              ...
              @click="startEditTtl"
              >TTL: {{ formatTtl(data.ttl, t) }}</Badge
            >
            <Badge
      ...
      <div v-else-if="data.key_type === 'hash'" ref="hashTableRef" class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="grid border-b bg-muted/50 shrink-0" :style="hashGridStyle">
          ...
          <div
            class="px-3 py-1 text-xs font-medium text-muted-foreground cursor-pointer hover:bg-accent/50 flex items-center gap-1 select-none"
            role="columnheader"
            :aria-sort="hashSortBy === 'value' ? (hashSortDir === 'asc' ? 'ascending' : 'descending') : 'none'"
            @click="toggleHashSort('value')"
          >
            Value
      ...
      >
        ...
        <SheetHeader class="border-b px-5 py-4 pr-12">
          <SheetTitle class="flex items-center gap-2">
            <span class="truncate">{{
              selectedMemberTitle ? formatValue(selectedMemberTitle) : t("redis.memberDetail")
            }}</span>
