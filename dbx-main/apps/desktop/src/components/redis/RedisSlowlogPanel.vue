...
async function querySlowlog() {
  ...
  try {
    ...
    if (showNodeSelector.value && selectedEndpoint.value) {
      result = await api.redisSlowlogGet(
        props.connectionId,
        count.value,
        selectedEndpoint.value.host,
        selectedEndpoint.value.port,
      )
    } else {
  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex items-center gap-2 px-3 py-1.5 border-b bg-muted/30 min-h-0 shrink-0">
      ...
      <Input
        ...
        :placeholder="t('redis.slowlogCount')"
      />
    ...
    <div class="flex-1 flex flex-col min-h-0 relative">
      ...
      <div
        ...
        class="flex items-center border-b px-3 shrink-0 bg-muted/20 text-xs font-medium text-muted-foreground select-none"
        style="height: 28px"
      >
        <button
          class="w-16 shrink-0 text-left hover:text-foreground transition-colors text-xs font-medium"
          @click="sortBy('id')"
        >
          {{ t("redis.slowlogColumnId") }}<span v-html="sortIndicator('id')"></span>
        </button>
        <button
          class="w-40 shrink-0 text-left hover:text-foreground transition-colors text-xs font-medium"
          @click="sortBy('timestamp')"
        >
          {{ t("redis.slowlogColumnTimestamp") }}<span v-html="sortIndicator('timestamp')"></span>
        </button>
        <button
          class="w-24 shrink-0 text-left hover:text-foreground transition-colors text-xs font-medium"
          @click="sortBy('duration_micros')"
        >
          {{ t("redis.slowlogColumnDuration") }}<span v-html="sortIndicator('duration_micros')"></span>
        </button>
        <button
          class="flex-1 min-w-0 text-left hover:text-foreground transition-colors text-xs font-medium"
          @click="sortBy('command')"
        >
          {{ t("redis.slowlogColumnCommand") }}<span v-html="sortIndicator('command')"></span>
        </button>
        <button
          v-if="showClientColumns"
          class="w-32 shrink-0 text-left hover:text-foreground transition-colors text-xs font-medium"
          @click="sortBy('client_addr')"
        >
          {{ t("redis.slowlogColumnClientAddr") }}<span v-html="sortIndicator('client_addr')"></span>
        </button>
        <button
          v-if="showClientColumns"
          class="w-32 shrink-0 text-left hover:text-foreground transition-colors text-xs font-medium"
        ...
        >
          {{ t("redis.slowlogColumnClientName") }}<span v-html="sortIndicator('client_name')"></span>
        </button>
      </div>
      ...
      <div v-if="entries.length > 0" class="flex-1 overflow-hidden">
        <RecycleScroller
          ...
          :buffer="400"
          key-field="id"
          v-slot="{ item }: { item: RedisSlowlogEntry }"
        >
          <div
          ...
          >
            ...
            <span
              v-if="showClientColumns"
              class="w-32 shrink-0 text-muted-foreground truncate"
              :title="displayValue(item.client_addr)"
              >{{ displayValue(item.client_addr) }}</span
            >
            <span
              v-if="showClientColumns"
              class="w-32 shrink-0 text-muted-foreground truncate"
              :title="displayValue(item.client_name)"
              >{{ displayValue(item.client_name) }}</span
            >
          </div>
        </RecycleScroller>
      </div>
      ...
      <DialogContent class="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle class="text-sm">{{
            t("redis.slowlogDetailTitle", { id: selectedEntry?.id ?? "" })
          }}</DialogTitle>
        </DialogHeader>
        <div v-if="selectedEntry" class="grid gap-3 text-xs">
          <div class="grid grid-cols-[80px_1fr] gap-x-3 gap-y-2">
            ...
            <pre class="font-mono whitespace-pre-wrap break-words bg-muted rounded p-2 m-0 max-h-48 overflow-auto">{{
