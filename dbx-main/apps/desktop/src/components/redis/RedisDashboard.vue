...
<template>
  <div class="h-full flex flex-col bg-background">
    ...
    <div
      v-if="error"
      class="shrink-0 mx-4 mt-2 flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-xs text-destructive"
    >
    ...
    <div class="flex-1 min-h-0 overflow-auto">
      ...
      <!-- Loading state -->
      <div
        v-if="loading && rawSections.length === 0"
      ...
      <div v-else-if="filteredSections.length > 0" class="px-4 pb-4 space-y-1">
        <div
          ...
          :class="[!isCollapsed(section.name) ? 'bg-muted/60' : '']"
        >
          <!-- Section header (clickable) -->
          <button
            class="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold text-foreground hover:bg-muted/50 transition-colors text-left"
            @click="toggleSection(section.name)"
          >
            <component
              :is="isCollapsed(section.name) ? ChevronRight : ChevronDown"
            ...
            <table class="w-full text-xs border-collapse">
              <tbody>
                <tr v-for="(entry, ei) in section.entries" :key="ei" class="hover:bg-muted/20">
                  <td
                    class="w-1/2 border-r border-b px-3 py-1.5 text-muted-foreground select-text break-all cursor-context-menu"
                    :title="t('common.copy') || 'Copy'"
                    @contextmenu.prevent="copyEntryKey(entry)"
                  >
                    {{ entry.key }}
                  </td>
                  <td
                    class="w-1/2 border-b px-3 py-1.5 dbx-editor-font-family tabular-nums select-text break-all cursor-context-menu"
                    :title="t('common.copy') || 'Copy'"
                    @contextmenu.prevent="copyEntryValue(entry)"
                  >
                    {{ entry.value }}
                  </td>
                </tr>
