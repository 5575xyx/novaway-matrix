...
const loadingEmptyText = computed(() =>
  isValueSearchMode.value && valueQuery.value
    ? t(searchMode.value === "all" ? "redis.searchingAll" : "redis.searchingValues")
    : t("redis.loadingKeys"),
)
const redisKeySeparator = computed(() => connectionStore.getConfig(props.connectionId)?.redis_key_separator ?? ":")
const redisScanPageSize = computed(
  () => connectionStore.getConfig(props.connectionId)?.redis_scan_page_size ?? REDIS_SCAN_PAGE_SIZE_DEFAULT,
)
...
async function fetchScanBatchPage(
  maxIterations: number,
  options: { count?: number; includeTypes?: boolean } = {},
...
function onRedisRowContextMenu(
  event: MouseEvent,
...
async function runRedisCommand(command: string) {
  ...
  try {
    const result = await api.redisExecuteCommand(
      props.connectionId,
      commandDb.value,
      command,
      !props.blockDangerousRedisCommands,
    )
          ...
          <div class="border-b px-2 py-2 shrink-0">
            <div class="flex flex-wrap items-start gap-1.5">
              ...
              <div class="ml-auto flex min-w-0 flex-wrap items-center justify-end gap-1">
                <span class="min-w-0 max-w-full truncate text-xs text-muted-foreground" :title="keyCountText">{{
                  keyCountText
                }}</span>
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  class="h-6 shrink-0 text-xs text-destructive"
              ...
              <CustomContextMenu :items="redisKeyContextMenuItems(row.node)" v-slot="{ onContextMenu }">
                ...
                >
                  ...
                  <div class="flex shrink-0 items-center justify-end gap-1">
                    <Badge
                      v-if="row.node.kind === 'leaf' && row.node.keyType"
                      variant="outline"
