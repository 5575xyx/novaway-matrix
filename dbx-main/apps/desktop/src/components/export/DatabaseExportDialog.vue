...
import {
  ...
  type AllDatabaseExportPlanItem,
...
async function runDatabaseExportUntilTerminal(
  request: api.DatabaseExportRequest,
  ...
  try {
    ...
    for (let index = 0; index < exportPlan.length; index += 1) {
      ...
      const filePath = isTauriRuntime()
        ? joinExportPath(directoryPath, `${sanitizeFileName(item.fileStem)}.sql`)
        : `__web_export_${currentExportId}.sql`
      const request: api.DatabaseExportRequest = {
            ...
            <div class="space-y-2 rounded border border-border/60 p-2">
              <div class="relative">
                ...
                <Input
                  ...
                  :placeholder="t('databaseExport.filterDatabases')"
                />
              </div>
              <div class="flex items-center gap-2">
