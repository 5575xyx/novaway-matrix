...
const triggerTitle = computed(() =>
  failedCount.value > 0 ? t("exportProgress.failedTooltip", { count: failedCount.value }) : t("exportProgress.tooltip"),
)
...
const rowsText = (task: ExportTask) => {
  ...
  if (task.kind === "data-transfer") {
    const tableText = task.totalTables
      ...
      : task.currentTable || ""
    const rowText = task.totalRows
      ? `${task.rowsExported.toLocaleString()} / ${task.totalRows.toLocaleString()} ${t("exportProgress.rowsShort")}`
      : `${task.rowsExported.toLocaleString()} ${t("exportProgress.rowsShort")}`
  ...
  <Popover v-if="tasks.length > 0" v-model:open="open">
    <PopoverTrigger as-child>
      <Button
        ...
        :title="triggerTitle"
        :class="{
          'bg-destructive/10 text-destructive hover:bg-destructive/15': failedCount > 0,
          'bg-accent text-primary': failedCount === 0 && hasActive,
        }"
      >
    ...
    <PopoverContent align="start" class="w-[min(92vw,30rem)] p-0 gap-0 overflow-hidden" :side-offset="8">
      ...
      <div class="max-h-96 overflow-y-auto">
        <div
          ...
          :key="task.exportId"
          class="flex items-start gap-3 border-b px-4 py-3 text-xs last:border-b-0"
        >
          <div class="flex-1 min-w-0 flex flex-col gap-1.5">
            <div class="flex items-center gap-1.5">
              <component
                ...
                :class="[
                  ...
                  task.kind === 'table-export' && isActive(task.status) ? 'animate-spin' : '',
                ]"
            ...
            <div v-if="isActive(task.status)" class="w-full bg-muted rounded-full h-1.5 overflow-hidden">
              <div
                v-if="
                  task.totalRows ||
                  (task.kind === 'database-export' && task.totalObjects) ||
            ...
            <div class="min-w-0 text-muted-foreground">
              ...
              <span
                ...
                class="mt-1 block whitespace-normal break-words text-destructive"
