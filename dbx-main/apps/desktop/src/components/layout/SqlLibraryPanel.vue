...
async function exportFolderContents(folder?: SavedSqlFolder) {
  ...
  try {
    ...
    if (folder) {
      ...
      for (const libraryFolder of savedSqlStore.allFolders.filter(
        (item) => isConnectionVisible(item.connectionId) && !item.parentFolderId,
      )) {
...
function childFolders(parentFolderId?: string) {
  return savedSqlStore.allFolders.filter(
    (folder) => isConnectionVisible(folder.connectionId) && (folder.parentFolderId || "") === (parentFolderId || ""),
  )
}
...
type SqlLibraryRow =
  | { type: "folder"; folder: SavedSqlFolder; depth: number; folderIndex: number }
  | { type: "file"; file: SavedSqlFile; depth: number }
...
const itemsByDate = computed(() => {
  ...
  const allFiles = [
    ...savedSqlStore.allFolders.flatMap((folder) => filesInFolder(folder.id)),
    ...visibleFiles.value,
  ].map((file) => ({ type: "file" as const, item: file, updatedAt: file.updatedAt }))
...
const activeSavedSqlId = computed(
  () => queryStore.tabs.find((tab) => tab.id === queryStore.activeTabId)?.savedSqlId ?? null,
)
...
const allSelectableItems = computed(() => {
  ...
  const treeItems = visibleFolderRows.value.map((row) =>
    row.type === "folder" ? { type: "folder" as const, id: row.folder.id } : { type: "file" as const, id: row.file.id },
  )
...
function folderMoveMenuItems(fileIds: string[]): CtxMenuItem[] {
  const files = [...new Set(fileIds)]
    .map((id) => savedSqlStore.getFile(id))
...
<template>
  <div class="h-full flex flex-col overflow-hidden border-l bg-background select-none">
    <div class="h-9 flex items-center gap-1 px-2 border-b shrink-0 bg-muted/20">
      ...
      <span class="flex-1" />
      <LightTooltip
        :text="sortMode === 'folder' ? t('sqlLibrary.sortByDate') : t('sqlLibrary.sortByFolder')"
        side="bottom"
        :delay="0"
        :close-delay="0"
        nowrap
      >
        <Button
          ...
          size="icon"
          class="h-5 w-5"
          @click="sortMode = sortMode === 'folder' ? 'date' : 'folder'"
        >
            ...
            <div v-if="sortMode === 'date'" class="space-y-0">
              <div v-for="item in itemsByDate" :key="item.type + '-' + item.item.id">
                ...
                >
                  ...
                  <span
                    class="dbx-sql-library-drag-label min-w-0 flex-1 truncate"
                    :title="fileTitleLabel(item.item)"
                    :style="fileTitleStyle(item.item)"
                    >{{ item.item.name }}</span
                  >
                  <span
                    class="min-w-0 max-w-[45%] shrink truncate text-[13px]"
                    :class="fileMetaClass(item.item.id)"
                    :title="getConnectionLabel(item.item.connectionId)"
                    >[{{ getConnectionLabel(item.item.connectionId) }}]</span
                  >
                </div>
              </div>
            </div>
            ...
            <div v-else>
              <div v-for="row in visibleFolderRows" :key="row.type === 'folder' ? row.folder.id : row.file.id">
                <div
                  ...
                  :class="[
                    showDropInside(row.folder.id)
                      ? 'ring-1 ring-primary/50 bg-primary/5'
                      : folderRowClass(row.folder.id),
                    isDraggingItem(row.folder.id) ? 'opacity-50' : '',
                  ]"
                ...
                >
                  <div
                    v-if="showDropBefore(row.folder.id)"
                    class="absolute left-2 right-2 top-0 border-t-2 border-primary"
                  />
                  <div
                    v-if="showDropAfter(row.folder.id)"
                    class="absolute left-2 right-2 bottom-0 border-b-2 border-primary"
                  />
                  <component
                    :is="isFolderExpanded(row.folder.id) ? FolderOpen : FolderClosed"
                ...
                >
                  <div
                    v-if="showDropBefore(row.file.id)"
                    class="absolute left-2 right-2 top-0 border-t-2 border-primary"
                  />
                  <div
                    v-if="showDropAfter(row.file.id)"
                    class="absolute left-2 right-2 bottom-0 border-b-2 border-primary"
                  />
                  <FileText class="h-3.5 w-3.5 text-blue-400 shrink-0" />
                  ...
                  <span
                    v-else
                    class="dbx-sql-library-drag-label min-w-0 flex-1 truncate"
                    :title="fileTitleLabel(row.file)"
                    :style="fileTitleStyle(row.file)"
                    >{{ row.file.name }}</span
                  >
                  <span
                    class="min-w-0 max-w-[45%] shrink truncate text-[13px]"
                    :class="fileMetaClass(row.file.id)"
                    :title="getConnectionLabel(row.file.connectionId)"
                    >[{{ getConnectionLabel(row.file.connectionId) }}]</span
                  >
                </div>
              </div>
              ...
              <div v-if="visibleFiles.length > 0 || dragState.draggedType === 'file'">
                ...
                >
                  ...
                  <span
                    v-else
                    class="dbx-sql-library-drag-label min-w-0 flex-1 truncate"
                    :title="fileTitleLabel(file)"
                    :style="fileTitleStyle(file)"
                    >{{ file.name }}</span
                  >
                  <span
                    class="min-w-0 max-w-[45%] shrink truncate text-[13px]"
                    :class="fileMetaClass(file.id)"
                    :title="getConnectionLabel(file.connectionId)"
                    >[{{ getConnectionLabel(file.connectionId) }}]</span
                  >
                </div>
              </div>
            </div>
    ...
    <Dialog :open="showBatchDeleteConfirm" @update:open="(open) => (showBatchDeleteConfirm = open)">
      <DialogContent>
        ...
        <DialogFooter>
          <Button variant="outline" size="sm" @click="showBatchDeleteConfirm = false">{{
