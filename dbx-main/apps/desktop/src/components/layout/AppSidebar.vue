...
const selectedConnectionIds = computed(() =>
  connectionStore.connectionMultiSelectActive
    ? connectionStore.selectedTreeNodeIds.filter((id) => connectionIdSet.value.has(id))
    : [],
)
const selectedConnectionCount = computed(() => selectedConnectionIds.value.length)
const showConnectionMultiSelectToolbar = computed(
  () => connectionStore.connectionMultiSelectActive && selectedConnectionCount.value > 0,
)
const allConnectionsSelected = computed(
  () => allConnectionIds.value.length > 0 && selectedConnectionCount.value === allConnectionIds.value.length,
)
const selectAllIcon = computed(() =>
  allConnectionsSelected.value ? Check : selectedConnectionCount.value > 0 ? Minus : Square,
)
const selectAllLabel = computed(() =>
  allConnectionsSelected.value
    ? t("connectionGroup.deselectAllConnections")
    : t("connectionGroup.selectAllConnections"),
)
...
<template>
  <div
    class="app-sidebar-panel h-full shrink-0 relative select-none"
    :class="classicLayout ? '' : 'rounded-md border border-border/80 bg-background'"
    :style="{ width: sidebarWidth + 'px' }"
  >
    <div class="h-full flex flex-col overflow-hidden">
      <div
        class="app-sidebar-toolbar flex items-center gap-px px-3 text-xs font-medium text-muted-foreground border-b bg-muted/20"
      ...
      >
        ...
        <template v-if="showConnectionMultiSelectToolbar">
          ...
          <LightTooltip
            :text="t('contextMenu.deleteSelectedConnections', { count: selectedConnectionCount })"
            side="bottom"
            :delay="0"
            :close-delay="0"
            nowrap
    ...
    <Dialog v-model:open="showDeleteSelectedConfirm">
      <DialogContent class="sm:max-w-[400px]">
        ...
        <DialogFooter>
          ...
          <Button variant="destructive" @click="confirmDeleteSelectedConnections">{{
            t("contextMenu.deleteSelectedConnections", { count: selectedConnectionCount })
          }}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    <Dialog v-model:open="showCreateSelectedGroupDialog">
      <DialogContent class="sm:max-w-[360px]">
        ...
        <Input
          ...
          @keydown.enter.prevent="confirmCreateSelectedGroup"
        />
        <DialogFooter>
          <Button variant="outline" @click="showCreateSelectedGroupDialog = false">{{
            t("dangerDialog.cancel")
          }}</Button>
          <Button :disabled="!selectedGroupName.trim()" @click="confirmCreateSelectedGroup">{{
