...
const activeSqlFormatDialect = computed<SqlFormatDialect>(() =>
  sqlFormatDialectForDbType(activeEffectiveDatabaseType.value),
)
...
const visibleResultItems = computed(() =>
  tabularResultItems(props.activeTab.results ?? (props.activeTab.result ? [props.activeTab.result] : undefined)),
)
...
const canExportResultArchive = computed(
  () =>
    props.activeTab.mode === "query" &&
    (!!props.activeTab.result || !!props.activeTab.results?.length || !!props.activeTab.resultRuns?.length),
)
...
const mongoQueryResultSaveHandler = computed<CustomSaveHandler | undefined>(() => {
  ...
  if (
    tab.mode !== "query" ||
    activeEffectiveDatabaseType.value !== "mongodb" ||
    !target ||
    !tab.connectionId ||
    !tab.database ||
    !tab.result
  ...
  const preview: CustomSaveHandler["preview"] = async (changes: MongoQueryGridChanges) => {
    ...
    for (const [rowIdx, dirtyCols] of changes.dirtyRows) {
      ...
      if (Object.keys(updateDoc).length === 0) continue
      stmts.push(
        `${mongoCollectionExpression(target.collection)}.updateOne({_id: ${mongoIdPreview(id)}}, ${formatMongoShellLiteral(updateDoc)})`,
      )
    }
    return stmts
  }
  ...
  return {
    ...
    readonlyColumns: [target.idColumn],
            ...
            <div v-if="hasQueryOutput" class="flex h-10 shrink-0 items-center gap-1 border-b bg-muted/20 px-2">
              ...
              <template v-if="visibleResultItems.length > 0">
                ...
                <div class="relative min-w-0 flex-1 self-stretch">
                  <div
                    v-if="hasResultTabOverflow"
                    class="result-tab-scrollbar"
                    :class="{ 'result-tab-scrollbar--dragging': isResultTabsScrollbarDragging }"
                    @pointerdown="startResultTabsScrollbarDrag"
                  >
                    <div class="result-tab-scrollbar__thumb" :style="resultTabsScrollbarThumbStyle" />
                  </div>
                  <div
                    ref="resultTabsScrollerRef"
                    class="result-tab-scroll flex h-full items-center gap-1 overflow-x-auto overflow-y-hidden px-1"
                    :style="resultTabsScrollerStyle"
                    @scroll="updateResultTabsScrollbar"
                    @wheel="onResultTabsWheel"
                  >
                    <Button
                      ...
                      size="sm"
                      :variant="
                        activeOutputView === 'result' && (activeTab.activeResultIndex ?? 0) === item.index
                          ? 'default'
                          : 'ghost'
                      "
                ...
                <Popover v-if="activeOutputView === 'result' && activeTab.result">
                  ...
                  >
                    ...
                    <div class="flex items-center justify-between gap-3 px-3 py-1.5 text-xs">
                      <div class="min-w-0 flex items-center gap-2 font-medium">
                        <span
                          class="flex h-3.5 w-3.5 shrink-0 items-center justify-center text-[11px] font-semibold text-muted-foreground"
                          >A</span
                        >
                    ...
                    <div
                      ...
                      :class="{ 'opacity-60': !dataGridRef?.canToggleAllNullColumns }"
                    >
                      ...
                      <Switch
                        size="sm"
                        :model-value="!!dataGridRef?.nullColumnsHidden"
                        :disabled="!dataGridRef?.canToggleAllNullColumns"
                        :aria-label="t('grid.hideNullColumns')"
                        @update:model-value="dataGridRef?.toggleAllNullColumns()"
                      />
                    </div>
                  </PopoverContent>
                </Popover>
              ...
              <DataGrid
                ...
                :full-export-result="
                  (onProgress?: (info: { rowsExported: number; totalRows: number | null }) => void) =>
                    queryStore.fetchTabResultForExport(activeTab.id, onProgress)
                "
                :query-result-export-request="
                  (options: { exportId: string; filePath: string; format: 'csv' | 'xlsx' }) =>
                ...
                @sort="
                  (
                    ...
                    whereInput?: string,
                    mode?: DataGridSortMode,
                  ) => emit('sort', column, columnIndex, direction, whereInput, mode)
                "
              >
                <template v-if="activeTab.result?.columns.includes('Error')" #error-actions="{ errorMessage }">
                  <Button
                    v-if="activeTab.connectionId && isQueryTimeoutError(String(errorMessage))"
                    variant="outline"
                    size="sm"
                    class="h-7 gap-1.5 px-2.5 text-xs"
                    @click="emit('openConnectionSettings', activeTab.connectionId, 'advanced')"
                  >
    ...
    <template v-else-if="activeTab.mode === 'data'">
      <div class="flex-1 min-h-0 flex flex-col">
        <div class="h-9 shrink-0 border-b bg-background/80 px-3 flex items-center gap-2 text-xs">
          ...
          <span
            v-if="activeTab.mode === 'data' && activeTab.tableMeta"
            class="inline-flex shrink-0 items-center rounded border border-border bg-muted/30 px-2 py-0.5 font-medium text-muted-foreground tabular-nums"
          >
              ...
              <div class="flex flex-col gap-1 border-t bg-muted/30 px-2 py-1.5">
                ...
                <div class="flex items-center justify-end gap-1">
                  ...
                  <Button
                    ...
                    size="sm"
                    class="h-7 px-2 text-xs"
                    :disabled="!dataGridRef?.hasCustomColumnOrder"
                    @click="dataGridRef?.resetColumnOrder()"
                  >
          ...
          <DropdownMenu v-if="activeTab.tableMeta && activeTab.connectionId">
            <DropdownMenuTrigger as-child>
              <Button variant="ghost" size="sm" class="h-5 text-xs px-1.5 shrink-0" :title="t('tableToolbox.title')"
                ><Toolbox class="h-3.5 w-3.5" />{{ t("tableToolbox.title") }}</Button
              >
            </DropdownMenuTrigger>
            <DropdownMenuContent
              align="end"
              class="w-max min-w-44 gap-0 overflow-hidden rounded-xl border bg-popover p-0 text-popover-foreground shadow-xl"
            >
              <div class="border-b bg-muted/40 px-3 py-2">
        ...
        <DataGrid
          ...
          :editable="
            isTableDataEditable(
              activeEffectiveDatabaseType,
              activeTableMeta?.primaryKeys ?? [],
              activeTableMeta?.tableType,
            )
          "
          context="table-data"
    ...
    <template v-else-if="activeTab.mode === 'redis'">
      <div class="flex-1 min-h-0">
        <RedisKeyBrowser
          ...
          :block-dangerous-redis-commands="props.blockDangerousRedisCommands"
        />
      </div>
    </template>
    ...
    <template v-else-if="activeTab.mode === 'mongo-bucket'">
      <div class="flex-1 min-h-0">
        <MongoBucketBrowser
          ...
          :connection-id="activeTab.connectionId"
          :database="activeTab.database"
          :bucket="activeTab.mongoBucket?.bucketName || activeTab.sql"
        />
      </div>
    </template>
    ...
    <template v-else-if="activeTab.mode === 'mq'">
      <div class="flex-1 min-h-0">
        <MqAdminConsole
          :key="activeTab.id"
          :connection-id="activeTab.connectionId"
          :initial-tenant="activeTab.mqTenant"
          :initial-tab="activeTab.mqInitialTab"
