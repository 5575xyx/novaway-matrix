...
import {
  isSingleDatabase,
  supportsTransaction as supportsTransactionFeature,
...
const keywordCaseToggleTooltip = computed(() =>
  keywordCaseIsLower.value ? t("toolbar.keywordCaseUpper") : t("toolbar.keywordCaseLower"),
)
...
<template>
  <div
    class="app-editor-toolbar h-9 shrink-0 border-b bg-background/80 px-3 flex items-center gap-1 text-xs text-muted-foreground relative z-10"
    :style="toolbarStyle"
  >
      ...
      <Tooltip v-if="supportsTransaction">
        <TooltipTrigger as-child>
          <Button
            ...
            class="h-6 w-6"
            :class="
              isTransactionActive || autoCommit === false
                ? 'text-orange-600 bg-orange-100 dark:text-orange-300 dark:bg-orange-900/30'
                : 'text-muted-foreground/50'
            "
      ...
      <Tooltip>
        <TooltipTrigger as-child>
          <Button
            ...
            :class="
              keywordCaseIsLower
                ? 'bg-amber-500/10 text-amber-700 hover:bg-amber-500/20 hover:text-amber-800 dark:text-amber-300 dark:hover:text-amber-200'
      ...
      <Tooltip>
        <TooltipTrigger as-child>
          <Button
            variant="ghost"
            size="icon"
            class="h-6 w-6 text-cyan-600 hover:bg-cyan-500/10 hover:text-cyan-700 dark:text-cyan-300 dark:hover:text-cyan-200"
            @click="emit('importResultArchive')"
          >
    ...
    <div
      ...
      class="ml-2 inline-flex shrink-0 items-center gap-1 rounded border border-border bg-muted/30 px-2 py-0.5 font-medium text-muted-foreground tabular-nums"
    >
  ...
  <div
    v-if="txnAutoRolledBack"
    class="flex items-center gap-2 px-3 py-1 text-xs bg-amber-500/10 text-amber-700 dark:text-amber-300 border-b border-amber-500/20"
  >
