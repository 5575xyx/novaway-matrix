...
const regularSurfaceCount = computed(
  () => regularTabs.value.length + (props.driverStoreOpen ? 1 : 0) + (props.settingsPageOpen ? 1 : 0),
)
...
const closeConfirmDirtyTabs = computed(() =>
  queryStore.closeConfirmDirtyTabIds
    .map((id) => queryStore.tabs.find((tab) => tab.id === id))
    .filter((tab): tab is QueryTab => !!tab),
)
const closeConfirmCurrentTitle = computed(() => {
  const focusedTab =
    closeConfirmDirtyTabs.value.find((tab) => tab.id === queryStore.pendingCloseTabId) ?? closeConfirmDirtyTabs.value[0]
...
function getTabMenuItems(tab: QueryTab): ContextMenuItem[] {
  ...
  const closeOtherLabel = tab.pinned
    ? t("contextMenu.closeOtherFixedTabs")
    : hasFixedTabs.value
      ? t("contextMenu.closeOtherRegularTabs")
      : t("contextMenu.closeOtherTabs")
  const closeAllLabel = tab.pinned
    ? t("contextMenu.closeAllFixedTabs")
    : hasFixedTabs.value
      ? t("contextMenu.closeAllRegularTabs")
      : t("contextMenu.closeAllTabs")
  const closeOtherDisabled = tab.pinned ? fixedTabs.value.length <= 1 : regularSurfaceCount.value <= 1
  const closeOtherAction = tab.pinned
    ? () => queryStore.closeOtherFixedTabs(tab.id)
    : () => closeOtherRegularTabsFromTab(tab)
      ...
      for (const container of [tabsContainerRef.value, fixedTabsContainerRef.value]) {
        ...
        if (activeEl) {
          activeEl.scrollIntoView({
            behavior: tabScrollBehavior.value,
            block: "nearest",
            inline: activeTabScrollInline(container, queryStore.activeTabId),
          })
          break
        }
      }
...
const tabTailDragRegionClass = computed(() =>
  showRegularTabOverflowControls.value ? "w-0 flex-none self-stretch" : "min-w-8 flex-1 self-stretch",
)
const fixedTabTailDragRegionClass = computed(() =>
  showFixedTabScrollbar.value ? "w-0 flex-none self-stretch" : "min-w-8 flex-1 self-stretch",
)
...
<template>
  <div
    ...
    class="app-tab-bar relative flex w-full min-w-0 shrink-0 overflow-hidden border-b"
    :class="[isClassicLayout ? 'bg-muted' : 'bg-background', hasFixedTabs ? 'flex-col' : '']"
  >
    <div
      class="flex w-full min-w-0 shrink-0 overflow-hidden"
      :class="isClassicLayout ? 'h-9 items-stretch' : 'h-10 items-center px-2'"
    >
      <div class="app-tab-strip relative h-full min-w-0 flex-1 overflow-hidden">
        <div
          v-if="showRegularTabScrollbar"
        ...
        <div
          ...
          :class="isClassicLayout ? 'h-full' : 'h-full gap-1.5 py-1.5'"
        ...
        >
                ...
                <TooltipTrigger as-child>
                    ...
                    :class="
                      isClassicLayout
                        ? [
                            ...
                            tab.id === queryStore.activeTabId && !driverStoreActive && !settingsPageActive
                  ...
                  >
                    <span class="shrink-0" :class="tabIconClass(tab)">
                      <Table2
                        v-if="tab.mode === 'data' || tab.mode === 'mongo' || tab.mode === 'redis'"
                        class="h-3.5 w-3.5"
                      />
          ...
          <CustomContextMenu
            ...
            :items="getSpecialRegularTabMenuItems('settings')"
            v-slot="{ onContextMenu }"
          >
            <div :class="isClassicLayout ? 'h-full' : ''" @contextmenu="onContextMenu">
              <div
                ...
                :class="
                  isClassicLayout
                    ? [
                        'h-full border-r border-border/80 dark:border-border/45 font-medium',
                        settingsPageActive
                          ? 'bg-background text-foreground'
                          : 'text-foreground/70 hover:text-foreground/90',
                      ]
                    : [
                        'h-7 rounded-md border font-medium',
                        settingsPageActive
              ...
              >
                ...
                <button
                  class="rounded hover:bg-muted-foreground/20 p-0.5 shrink-0"
                  @click.stop="emit('close-settings-page')"
                >
                  <X class="h-3 w-3" />
                </button>
              </div>
          ...
          <CustomContextMenu
            ...
            :items="getSpecialRegularTabMenuItems('driverStore')"
            v-slot="{ onContextMenu }"
          >
            <div :class="isClassicLayout ? 'h-full' : ''" @contextmenu="onContextMenu">
              <div
      ...
      <div v-if="showRegularTabOverflowControls" class="relative z-30 flex shrink-0 items-center">
        <Popover v-model:open="regularTabOverflowOpen">
          <PopoverTrigger as-child>
            <button
              type="button"
              :class="['inline-flex shrink-0 items-center justify-center', tabOverflowControlClass].join(' ')"
          ...
          <PopoverContent
            align="end"
            class="w-auto min-w-56 max-w-80 max-h-[min(70vh,28rem)] gap-0 overflow-y-auto rounded-[6px] p-1"
            @click.stop
            @keydown.stop
          >
            <CustomContextMenu
    ...
    <div
      ...
      :class="
        isClassicLayout
          ? 'h-8 items-stretch border-border/80 bg-background/45 dark:border-border/45 dark:bg-background/20'
          : 'h-9 items-center border-border/70 bg-muted/45 px-2 dark:bg-muted/25'
      "
    >
      <div class="app-tab-strip relative h-full min-w-0 flex-1 overflow-hidden">
        <div
          v-if="showFixedTabScrollbar"
          class="app-tab-scrollbar app-tab-scrollbar--bottom"
          :class="{ 'app-tab-scrollbar--dragging': isFixedScrollbarDragging }"
          @pointerdown="startFixedScrollbarDrag"
        >
        ...
        <div
          ...
          class="app-tab-scroll flex w-full min-w-0 flex-1 items-center overflow-x-auto"
          :class="isClassicLayout ? 'h-full' : 'h-full gap-1.5 py-1'"
          :style="tabsContainerStyle"
              ...
              <Tooltip>
                <TooltipTrigger as-child>
                  ...
                  >
                    ...
                    <span
                      v-else
                      class="inline-flex min-w-0 flex-1 items-center gap-0.5 overflow-hidden text-foreground"
                    >
                      <span v-if="isDirtyTab(tab)" aria-hidden="true" class="dirty-tab-marker">*</span>
                      <span class="min-w-0 flex-1 truncate" :style="tabTitleStyle(tab)">{{ tabTitleText(tab) }}</span>
                    </span>
                    ...
                    <button
                      ...
                      :aria-label="t('contextMenu.unfixTab')"
                      :title="t('contextMenu.unfixTab')"
  ...
  >
    <DialogContent class="sm:max-w-md">
      ...
      <div class="space-y-2">
        ...
        <Popover
          v-if="showCloseConfirmBulkActions"
          :open="closeConfirmListOpen"
          @update:open="closeConfirmListOpen = $event"
        >
          <PopoverTrigger as-child>
            <button
              type="button"
          ...
          <PopoverContent
            align="start"
            side="bottom"
            class="w-72 max-w-[calc(100vw-2rem)] gap-1 p-2"
            @mouseenter="openCloseConfirmList"
            @mouseleave="scheduleCloseConfirmListClose"
            @pointerdown.stop
            @click.stop
          ...
          >
            ...
            <div class="max-h-48 overflow-y-auto">
              <div
                v-for="tab in closeConfirmDirtyTabs"
                :key="tab.id"
                class="flex min-w-0 items-center gap-2 rounded-[6px] px-2 py-1.5 text-sm"
                :class="tab.id === queryStore.pendingCloseTabId ? 'bg-muted text-foreground' : 'text-muted-foreground'"
              >
                <span
                  class="h-1.5 w-1.5 shrink-0 rounded-full"
                  :class="tab.id === queryStore.pendingCloseTabId ? 'bg-foreground' : 'bg-muted-foreground/50'"
                />
      ...
      <DialogFooter>
        ...
        <Button v-if="showCloseConfirmBulkActions" variant="secondary" @click="handleDiscardAllAndClose">{{
          t("editor.discardAllChanges")
        }}</Button>
        <Button v-if="showCloseConfirmBulkActions" @click="handleSaveAllAndClose">{{
          t("editor.saveAllChanges")
        }}</Button>
