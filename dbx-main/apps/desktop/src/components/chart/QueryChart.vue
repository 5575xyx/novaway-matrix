...
const allColumnOptions = computed(() =>
  props.result.columns.map((_, index) => ({ index, label: axisColumnLabel(props.result.columns, index) })),
)
const numericColumnOptions = computed(() =>
  numericColumnIndexes.value.map((index) => ({ index, label: axisColumnLabel(props.result.columns, index) })),
)
      ...
      <div class="h-9 shrink-0 border-b bg-muted/20 px-3 flex items-center gap-3 text-xs">
        ...
        <div class="flex items-center gap-1.5">
          ...
          <Select v-model="xColumnValue">
            ...
            <SelectContent>
              <SelectItem v-for="col in allColumnOptions" :key="col.index" :value="String(col.index)">{{
                col.label
              }}</SelectItem>
            </SelectContent>
          </Select>
