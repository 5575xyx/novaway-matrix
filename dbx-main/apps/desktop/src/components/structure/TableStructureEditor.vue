...
async function fetchDdl() {
  if (
    ...
    !props.tableName ||
    ddlFetched.value ||
    !tableMetadataCapabilities.value.ddl
  )
    return
  ddlLoading.value = true
  try {
    const ddl = await api.getTableDdl(props.connectionId, props.database, metadataSchema.value, props.tableName)
    ddlContent.value = await formatSqlForDisplay(
      ddl,
      sqlFormatDialectForDbType(databaseType.value),
...
const secondaryMetadataLoading = computed(
  () => indexesLoading.value || foreignKeysLoading.value || triggersLoading.value,
)
...
function triggerChanged(trigger: EditableStructureTrigger): boolean {
  ...
  return (
    ...
    !sameText(trigger.statement, original.statement)
  )
}
...
function loadStructureIndexColumnWidths(density: StructureEditorDensity): number[] {
  ...
  const stored = normalizeStructureIndexColumnWidths(
    loadStructureWidthsByDensity(STRUCTURE_INDEX_COLUMNS_WIDTHS_STORAGE_KEY, density),
    density,
  )
...
function saveStructureIndexColumnWidths(density: StructureEditorDensity, widths: readonly number[]) {
  ...
  if (!normalized) return
  const fallback = metricsForDensity(density).indexes
  const stored = fallback.map((width, index) =>
    PERSISTED_STRUCTURE_INDEX_COLUMN_WIDTHS.has(index) ? normalized[index] : width,
  )
...
const structureControlClass =
  "h-[var(--structure-control-height)] min-w-0 rounded-[6px] px-[var(--structure-control-px)] py-0 text-[length:var(--structure-font-size)] focus-visible:border-ring/50 focus-visible:ring-1 focus-visible:ring-ring/25"
...
const structureHeaderCellClass =
  "relative min-w-0 overflow-hidden border-b border-r px-[var(--structure-cell-px)] py-[var(--structure-header-py)] text-left"
const structureCellClass =
  "min-w-0 overflow-hidden border-b border-r px-[var(--structure-cell-px)] py-[var(--structure-cell-py)]"
const structureLastCellClass =
  "min-w-0 overflow-hidden border-b px-[var(--structure-cell-px)] py-[var(--structure-cell-py)]"
...
function onColResize(e: MouseEvent, col: number) {
  ...
  const onMove = (ev: MouseEvent) => {
    ...
    colWidths.value[widthIndex] = Math.max(
      structureDensityMetric.value.minColumnWidth,
      colResizing.value.startW + delta,
    )
  }
  const onUp = () => {
...
const dataTypeOptions = computed(() =>
  mergeDataTypeOptions(dynamicDataTypeOptions.value, getDataTypeOptions(databaseType.value)),
)
...
const visibleColWidths = computed(() =>
  colLabels.value.map((column) =>
    column.key === "actions"
      ? columnActionsWidth.value
      : (colWidths.value[column.widthIndex] ?? structureDensityMetric.value.minColumnWidth),
  ),
)
...
const colLabels = computed(() => {
  ...
  if (columnEditorControls.value.length)
    labels.push({ key: "length", label: t("structureEditor.length"), widthIndex: 3 })
  if (columnEditorControls.value.nullable)
    labels.push({ key: "nullable", label: t("structureEditor.nullable"), widthIndex: 4 })
  if (columnEditorControls.value.primaryKey)
    labels.push({ key: "primaryKey", label: t("structureEditor.primaryKey"), widthIndex: 5 })
  if (columnEditorControls.value.defaultValue)
    labels.push({ key: "defaultValue", label: t("structureEditor.defaultValue"), widthIndex: 6 })
  if (columnEditorControls.value.comment)
    labels.push({ key: "comment", label: t("structureEditor.comment"), widthIndex: 7 })
  if (showExtendedProperties.value) {
    labels.push({
      key: "extendedProperties",
      label: t("structureEditor.extendedProperties"),
      widthIndex: extendedPropertiesColumnIndex,
    })
  }
...
function needsColumnDraftMetadataHydration() {
  return (
    !isCreateMode.value &&
    columns.value.some((column) => !column.original && !column.id.startsWith("new:") && !!column.name.trim())
  )
}
...
async function fetchTableCommentValue(
  connectionId: string,
  database: string,
  schema: string,
  tableName: string,
...
async function loadStructure(
  silent = false,
  scope: StructureRefreshScope = FULL_STRUCTURE_REFRESH_SCOPE,
  showErrors = true,
  options: { blockSecondaryMetadata?: boolean; preserveDraft?: boolean } = {},
) {
  const connectionId = props.connectionId
  const database = props.database
  const schema = metadataSchema.value
  ...
  try {
    ...
    const columnsPromise = scope.columns
      ? api.getColumns(connectionId, database, schema, tableName)
      : Promise.resolve(undefined)
    const indexesPromise = scope.indexes
      ? tableMetadataCapabilities.value.indexes
        ? api.listIndexes(connectionId, database, schema, tableName).catch(() => [])
        : Promise.resolve([])
      : Promise.resolve(undefined)
    const foreignKeysPromise = scope.foreignKeys
      ? tableMetadataCapabilities.value.foreignKeys
        ? api.listForeignKeys(connectionId, database, schema, tableName).catch(() => [])
        : Promise.resolve([])
      : Promise.resolve(undefined)
    const triggersPromise = scope.triggers
      ? tableMetadataCapabilities.value.triggers
        ? api.listTriggers(connectionId, database, schema, tableName).catch(() => [])
        : Promise.resolve([])
      : Promise.resolve(undefined)
    const tableCommentPromise =
      scope.tableComment && structureCapabilities.value.comment
        ? fetchTableCommentValue(connectionId, database, schema, tableName)
        : Promise.resolve(undefined)
...
async function addColumn() {
  ...
  activeTab.value = "columns"
  const dataType =
    databaseType.value === "manticoresearch"
      ? combineDataTypeForDatabase(
          databaseType.value,
          dataTypeOptions.value[0] ?? "text",
          getDefaultLengthForType(databaseType.value, dataTypeOptions.value[0] ?? "text"),
        )
      : "varchar(255)"
...
function canDropColumnAt(sourceIndex: number, insertionIndex: number): boolean {
  ...
  const crossedColumns =
    insertionIndex < sourceIndex
      ? columns.value.slice(insertionIndex, sourceIndex)
      : columns.value.slice(sourceIndex + 1, insertionIndex)
...
function updateColumnDataType(column: EditableStructureColumn, baseType: string) {
  column.dataType = combineDataTypeForDatabase(
    databaseType.value,
    baseType,
    getDefaultLengthForType(databaseType.value, baseType),
  )
...
function updateColumnDragInsertion(clientY: number) {
  ...
  state.insertionIndex =
    insertionIndex !== null && canDropColumnAt(state.sourceIndex, insertionIndex) ? insertionIndex : null
}
...
function scrollToColumnSearchMatch(direction: 1 | -1 = 1) {
  ...
  const rows = Array.from(rootRef.value?.querySelectorAll<HTMLElement>("[data-column-row-index]") ?? [])
  const matches = columns.value
    .map((column, index) => ({ column, index }))
    .filter(({ column }) => columnMatchesSearch(column))
  if (!matches.length) return
  const currentIndex = highlightedColumnId.value
    ? matches.findIndex(({ column }) => column.id === highlightedColumnId.value)
    : -1
...
function canDropColumn(column: EditableStructureColumn): boolean {
  return (
    !!column.original &&
    !column.isPrimaryKey &&
    !isProtectedManticoreIdColumn(databaseType.value, column.original.name) &&
    structureCapabilities.value.dropColumn
  )
}
...
function refreshAutoIndexName(index: EditableStructureIndex, previousColumns = index.columns) {
  ...
  const previousUniqueName = generateUniqueIndexName(
    structureIndexTableName(),
    previousColumns,
    existingIndexNamesForDraft(index),
  )
...
function isStructureMetadataTabSupported(tab: TableInfoTab, capabilities = tableMetadataCapabilities.value) {
  return (
    ...
    (tab === "ddl" && capabilities.ddl && !isCreateMode.value)
  )
}

function resolveStructureMetadataTab(
  tab: TableInfoTab | undefined,
  capabilities = tableMetadataCapabilities.value,
): TableInfoTab {
...
watch(tableMetadataCapabilities, (capabilities) => {
  if (!isStructureMetadataTabSupported(activeTab.value, capabilities))
    activeTab.value = firstStructureMetadataTab(capabilities)
})
...
<template>
  <div
    ...
    :data-structure-density="localStructureDensity"
  ...
  >
    ...
    >
      ...
      <Button
        ...
        @click="reloadStructureFromDatabase"
      >
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col gap-2 overflow-hidden">
      <div class="min-h-0 min-w-0 flex-1 overflow-hidden rounded-md border">
        <Tabs v-model="activeTab" class="flex h-full min-h-0 flex-col">
          <div class="flex shrink-0 items-center justify-between gap-2 border-b px-2 py-[var(--structure-header-py)]">
            <TabsList>
              <TabsTrigger v-if="tableMetadataCapabilities.columns" value="columns">{{
                t("structureEditor.columns")
              }}</TabsTrigger>
              <TabsTrigger v-if="tableMetadataCapabilities.indexes" value="indexes">{{
                t("structureEditor.indexes")
              }}</TabsTrigger>
              <TabsTrigger v-if="tableMetadataCapabilities.foreignKeys" value="foreignKeys">{{
                t("structureEditor.foreignKeys")
              }}</TabsTrigger>
              <TabsTrigger v-if="tableMetadataCapabilities.triggers" value="triggers">{{
            ...
            <div class="flex shrink-0 items-center gap-1.5">
              <div class="flex items-center gap-1.5">
                ...
                <div ref="structureDensityMenuRef" class="relative">
                  ...
                  >
                    <span class="min-w-0 text-center truncate">{{
                      structureDensityOptions.find((option) => option.value === localStructureDensity)?.label
                    }}</span>
                  ...
                  <div
                    v-if="structureDensityMenuOpen"
                    class="absolute right-0 top-[calc(100%+4px)] z-50 min-w-full rounded-[6px] bg-popover p-1 text-popover-foreground shadow-md ring-1 ring-foreground/10"
                    role="listbox"
                    :aria-label="t('structureEditor.density')"
                  >
                    <button
              ...
              <div v-if="activeTab === 'columns'" class="relative flex w-40 shrink-0 items-center">
                ...
                <Input
                  ...
                  :class="[
                    structureControlClass,
                    'pl-7 pr-14 text-[length:var(--structure-font-size)] placeholder:text-[length:var(--structure-font-size)]',
                  ]"
              ...
              <Button
                v-if="isCreateMode && activeTab === 'columns'"
                size="sm"
                variant="outline"
                :class="structureToolbarButtonClass"
                :disabled="!canAddColumn"
                @click="applyColumnTemplate(PRESET_FIELDS_TEMPLATE_ID)"
              >
              ...
              <Tooltip v-if="isCreateMode && activeTab === 'columns'">
                <TooltipTrigger as-child>
                  <Button
                    ...
                    :class="structureToolbarButtonClass"
                    :disabled="!canAddColumn"
                    :aria-label="t('structureEditor.configureColumnTemplates')"
                    @click="emit('openSettings', 'data', 'tableColumnTemplates')"
                  >
              ...
              <Button
                ...
                size="sm"
                :class="structureToolbarButtonClass"
                :disabled="!structureCapabilities.createIndex || indexesLoading"
                @click="addIndex"
              >
                <Plus :class="structureIconClass" />
              ...
              <Button
                v-if="activeTab === 'foreignKeys'"
                size="sm"
                :class="structureToolbarButtonClass"
                :disabled="!canEditForeignKeys || foreignKeysLoading"
                @click="addForeignKey"
              >
                <Plus :class="structureIconClass" />
              ...
              <Button
                ...
                size="sm"
                :class="structureToolbarButtonClass"
                :disabled="!canEditMysqlTriggers || triggersLoading"
                @click="addTrigger"
              >
                <Plus :class="structureIconClass" />
          ...
          <TabsContent
            v-if="tableMetadataCapabilities.columns"
            value="columns"
            class="m-0 min-h-0 flex-1 overflow-auto p-0"
          >
            <table
              ...
              <tbody>
                ...
                >
                  ...
                  <td :class="structureCellClass">
                    <Input
                      v-model="column.name"
                      :class="[structureControlClass, columnSearchFieldClass(column, column.name)]"
                  ...
                  <td v-if="columnEditorControls.length" :class="structureCellClass">
                    <Input
                      ...
                      @update:model-value="updateColumnDataTypeLength(column, $event)"
                    />
                  </td>
                  ...
                  <td v-if="columnEditorControls.defaultValue" :class="structureCellClass">
                    <div class="flex min-w-0 items-center gap-1">
                      <Input
                        ...
                        :class="[structureMonoControlClass, 'flex-1']"
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger as-child>
                          <Button
                            ...
                            :disabled="isColumnDefaultDisabled(column)"
                            :aria-label="t('structureEditor.defaultValuePresets')"
                            :title="t('structureEditor.defaultValuePresets')"
                          >
                        ...
                        <DropdownMenuContent align="end" class="max-h-56 min-w-36 overflow-y-auto">
                          <DropdownMenuItem
                            v-for="preset in defaultValuePresets"
                            :key="preset.value"
                            @click="column.defaultValue = preset.value"
                          >
                  ...
                  <td v-if="columnEditorControls.comment" :class="structureCellClass">
                    <div class="flex min-w-0 items-center gap-1">
                      <Input
                        ...
                        :class="[structureControlClass, 'flex-1', columnSearchFieldClass(column, column.comment)]"
                  ...
                  <td v-if="showExtendedProperties" :class="structureCellClass">
                    <div :class="structurePropertyListClass">
                      ...
                      <template v-if="databaseType === 'manticoresearch'">
                        <template v-if="isManticoreTextColumn(column)">
                          <label :class="structurePropertyLabelClass" title="indexed">
                            <input
                              :checked="!!column.extra.manticoreIndexed"
                              type="checkbox"
                              :class="[structureCheckboxClass, 'shrink-0']"
                              :disabled="isManticoreColumnPropertyDisabled(column)"
                              @change="column.extra.manticoreIndexed = ($event.target as HTMLInputElement).checked"
                            />
                            <span class="min-w-0 truncate">indexed</span>
                          </label>
                          <label :class="structurePropertyLabelClass" title="stored">
                            <input
                              :checked="!!column.extra.manticoreStored"
                              type="checkbox"
                              :class="[structureCheckboxClass, 'shrink-0']"
                              :disabled="isManticoreColumnPropertyDisabled(column)"
                              @change="column.extra.manticoreStored = ($event.target as HTMLInputElement).checked"
                            />
                            <span class="min-w-0 truncate">stored</span>
                          </label>
                          <label :class="structurePropertyLabelClass" title="attribute">
                            <input
                              :checked="!!column.extra.manticoreAttribute"
                              type="checkbox"
                              :class="[structureCheckboxClass, 'shrink-0']"
                              :disabled="isManticoreColumnPropertyDisabled(column)"
                              @change="column.extra.manticoreAttribute = ($event.target as HTMLInputElement).checked"
                            />
                        ...
                        <template v-else-if="isManticoreJsonColumn(column)">
                          <label :class="structurePropertyLabelClass" title="secondary_index">
                            <input
                              ...
                              :disabled="isManticoreColumnPropertyDisabled(column)"
                              @change="
                                column.extra.manticoreSecondaryIndex = ($event.target as HTMLInputElement).checked
                              "
                            />
                      ...
                      <template v-else-if="structureDialect === 'mysql'">
                        <label
                          :class="[structurePropertyLabelClass, 'shrink-0 pr-1']"
                          :title="t('structureEditor.autoIncrement')"
                        >
                          <input
                            v-model="column.extra.autoIncrement"
                            type="checkbox"
                            :class="[structureCheckboxClass, 'shrink-0']"
                          />
                        ...
                        <label
                          :class="[structurePropertyLabelClass, 'flex-1 basis-0']"
                          :title="t('structureEditor.onUpdateCurrentTimestamp')"
                        >
                          <input
                      ...
                      <template v-else-if="structureDialect === 'postgres'">
                        ...
                        >
                          <SelectTrigger
                            class="h-[var(--structure-control-height)] w-28 rounded-[6px] px-[var(--structure-control-px)] text-[length:var(--structure-font-size)] focus-visible:border-ring/50 focus-visible:ring-1 focus-visible:ring-ring/25"
                          >
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <template v-else-if="structureDialect === 'sqlserver'">
                        <label
                          :class="structurePropertyLabelClass"
                          :title="
                            canEditSqlServerIdentity(column) || isSqlServerIdentityChecked(column)
                              ? t('structureEditor.identity')
                              : t('structureEditor.sqlServerIdentityTypeHint')
                          "
                        >
                          <input
                            :checked="isSqlServerIdentityChecked(column)"
                            type="checkbox"
                            :class="[structureCheckboxClass, 'shrink-0']"
                            :disabled="!canEditSqlServerIdentity(column)"
                            @change="setSqlServerIdentity(column, ($event.target as HTMLInputElement).checked)"
                          />
                  ...
                  <td :class="structureLastCellClass">
                    <div class="flex min-w-0 items-center justify-start gap-0.5">
                      <Button
                        ...
                        size="icon"
                        :class="[
                          structureActionButtonClass,
                          canDragColumn(index) ? 'cursor-grab active:cursor-grabbing' : 'cursor-not-allowed',
                        ]"
          ...
          <TabsContent
            v-if="tableMetadataCapabilities.indexes"
            value="indexes"
            class="m-0 min-h-0 flex-1 overflow-auto p-0"
          >
              ...
              <tbody>
                ...
                >
                  ...
                  <td :class="structureCellClass">
                    ...
                    >
                      <SelectTrigger
                        class="h-[var(--structure-control-height)] w-full rounded-[6px] px-[var(--structure-control-px)] font-mono text-[length:var(--structure-font-size)] focus-visible:border-ring/50 focus-visible:ring-1 focus-visible:ring-ring/25"
                      >
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
          ...
          <TabsContent
            ...
            value="foreignKeys"
            class="m-0 min-h-0 flex-1 overflow-auto p-[var(--structure-cell-px)]"
          >
            ...
            <div v-else class="space-y-1.5">
              <div
                ...
                :class="fk.markedForDrop ? 'bg-destructive/5 opacity-60' : ''"
              >
                <div
                  class="grid grid-cols-[minmax(110px,1fr)_minmax(110px,1fr)_minmax(110px,1fr)_minmax(90px,0.8fr)_minmax(90px,0.8fr)_auto] gap-1.5"
                >
                  <Input
                    v-model="fk.name"
                    :class="structureControlClass"
                    :placeholder="t('structureEditor.foreignKeyName')"
                    :disabled="!canEditForeignKeyDraft(fk)"
                  />
                  <Input
                    v-model="fk.column"
                    :class="structureControlClass"
                    :placeholder="t('structureEditor.columnName')"
                    :disabled="!canEditForeignKeyDraft(fk)"
                  />
                  <Input
                    v-model="fk.refTable"
                    :class="structureControlClass"
                    :placeholder="t('structureEditor.referencedTable')"
                    :disabled="!canEditForeignKeyDraft(fk)"
                  />
                  <Input
                    v-model="fk.refColumn"
                    :class="structureControlClass"
                    :placeholder="t('structureEditor.referencedColumn')"
                    :disabled="!canEditForeignKeyDraft(fk)"
                  />
                  <Input
                    v-model="fk.refSchema"
                    :class="structureControlClass"
                    :placeholder="t('structureEditor.referencedSchema')"
                    :disabled="!canEditForeignKeyDraft(fk)"
                  />
                ...
                <div class="mt-1.5 grid grid-cols-[minmax(110px,0.5fr)_minmax(110px,0.5fr)_1fr] gap-1.5">
                  <Select
                    :model-value="fk.onDelete || '__default'"
                    :disabled="!canEditForeignKeyDraft(fk)"
                    @update:model-value="(v: any) => (fk.onDelete = String(v === '__default' ? '' : (v ?? '')))"
                  >
                    <SelectTrigger
                      class="h-[var(--structure-control-height)] rounded-[6px] px-[var(--structure-control-px)] text-[length:var(--structure-font-size)] focus-visible:border-ring/50 focus-visible:ring-1 focus-visible:ring-ring/25"
                    >
                      <SelectValue :placeholder="t('structureEditor.onDelete')" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem
                        v-for="action in foreignKeyActionOptions"
                        :key="`delete-${action || 'default'}`"
                        :value="action || '__default'"
                        >{{ action || t("structureEditor.defaultAction") }}</SelectItem
                      >
                    </SelectContent>
                  </Select>
                  <Select
                    :model-value="fk.onUpdate || '__default'"
                    :disabled="!canEditForeignKeyDraft(fk)"
                    @update:model-value="(v: any) => (fk.onUpdate = String(v === '__default' ? '' : (v ?? '')))"
                  >
                    <SelectTrigger
                      class="h-[var(--structure-control-height)] rounded-[6px] px-[var(--structure-control-px)] text-[length:var(--structure-font-size)] focus-visible:border-ring/50 focus-visible:ring-1 focus-visible:ring-ring/25"
                    >
                  ...
                  <div class="truncate font-mono text-muted-foreground">
                    {{ fk.column }} -> {{ fk.refSchema ? `${fk.refSchema}.` : "" }}{{ fk.refTable }}.{{ fk.refColumn }}
                  </div>
                </div>
              </div>
            </div>
          ...
          <TabsContent
            v-if="tableMetadataCapabilities.triggers"
            value="triggers"
            class="m-0 min-h-0 flex-1 overflow-auto p-[var(--structure-cell-px)]"
          >
            ...
            <div v-else class="space-y-1.5">
              <div
                ...
                :key="trigger.id"
                class="rounded-md border px-[var(--structure-cell-px)] py-[var(--structure-header-py)] text-[length:var(--structure-font-size)]"
                :class="trigger.markedForDrop ? 'bg-destructive/5 opacity-60' : ''"
              >
                <div class="grid grid-cols-[minmax(140px,1fr)_110px_110px_auto] gap-1.5">
                  ...
                  <Select v-model="trigger.timing" :disabled="!canEditTriggerDraft(trigger)">
                    ...
                    <SelectContent>
                      <SelectItem v-for="timing in triggerTimingOptions" :key="timing" :value="timing">{{
                        timing
                      }}</SelectItem>
                    </SelectContent>
                  </Select>
                  <Select v-model="trigger.event" :disabled="!canEditTriggerDraft(trigger)">
                    ...
                    <SelectContent>
                      <SelectItem v-for="event in triggerEventOptions" :key="event" :value="event">{{
                        event
                      }}</SelectItem>
                    </SelectContent>
                  </Select>
                  <div class="flex items-center justify-end gap-1">
                    <Button
                      v-if="trigger.original"
                      variant="ghost"
                      size="sm"
                      :class="structureToolbarButtonClass"
                      @click="toggleDropTrigger(trigger)"
                    >
                      <Trash2 :class="structureIconClass" />
                    ...
                    <Button
                      ...
                      size="sm"
                      :class="structureToolbarButtonClass"
                      @click="removeNewTrigger(trigger)"
                    >
                      <X :class="structureIconClass" />
                      {{ t("structureEditor.remove") }}
                    </Button>
      ...
      <div
        :class="[
          'flex min-w-0 shrink-0 flex-col overflow-hidden rounded-md border',
          sqlPreviewCollapsed ? '' : 'h-[28%] min-h-40 max-h-64',
        ]"
      >
        <div
        ...
        >
          ...
          <div class="flex items-center gap-1.5">
            <Button
              ...
              :aria-label="
                sqlPreviewCollapsed ? t('structureEditor.expandSqlPreview') : t('structureEditor.collapseSqlPreview')
              "
              :title="
                sqlPreviewCollapsed ? t('structureEditor.expandSqlPreview') : t('structureEditor.collapseSqlPreview')
              "
