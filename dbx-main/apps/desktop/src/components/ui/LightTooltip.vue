...
<template>
  <span
    ref="triggerRef"
    class="contents"
    @mouseenter="() => scheduleOpen('hover')"
    @mouseleave="scheduleClose"
    @focusin="scheduleFocusOpen"
    @focusout="close"
    @contextmenu.capture="suppressForContextMenu"
  >
    <slot />
  </span>
  <Teleport to="body">
    <div
      ...
      :class="[
        slots.content
          ? ''
          : ['inline-flex w-fit max-w-xs items-center gap-1.5 px-3 py-1.5', nowrap ? 'whitespace-nowrap' : ''],
