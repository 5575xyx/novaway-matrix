  ...
  <Popover v-model:open="open">
    <PopoverTrigger as-child>
      ...
      >
        ...
        <X
          ...
          :class="cn('shrink-0 opacity-60 hover:opacity-100', triggerIconClass)"
          @pointerdown.stop.prevent="emit('update:modelValue', '')"
        />
    ...
    <PopoverContent align="end" :class="cn('w-52 gap-1 p-1.5', contentClass)">
      <div class="relative rounded-sm border bg-background">
        ...
        <span
          v-if="!searchText"
          class="pointer-events-none absolute left-[25px] top-1/2 -translate-y-1/2 text-sm text-muted-foreground"
          >{{ searchPlaceholder }}</span
        >
        <Input
          ref="searchInput"
          :model-value="searchText"
          class="h-6 border-0 pl-6 pr-2 text-sm caret-foreground shadow-none focus-visible:ring-0"
