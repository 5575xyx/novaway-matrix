...
const props = withDefaults(
  ...
  }>(),
  {
    ...
    closeOnSelect: true,
    triggerClass:
      "flex h-6 items-center gap-1 rounded-[6px] border border-border bg-background px-2 text-[11px] text-muted-foreground hover:bg-muted hover:text-foreground disabled:pointer-events-none disabled:opacity-50 aria-expanded:bg-muted aria-expanded:text-foreground",
...
function fitPositionToViewport() {
  ...
  y.value =
    props.side === "top" && fitsAbove
      ? aboveY
      : props.side === "bottom" && fitsBelow
        ? belowY
        : fitsBelow
          ? belowY
...
<template>
  ...
  <Teleport to="body">
    <div
      ...
      role="menu"
    >
      <div v-if="label" :class="cn('text-muted-foreground px-1.5 py-1 text-xs font-medium', labelClass)">
        {{ label }}
      </div>
        ...
        <slot name="item" :item="item" :selected="itemIsSelected(item)" :select="selectItem">
          <button
            ...
            :class="
              cn(
                item.destructive
                  ? 'text-destructive hover:bg-destructive/10 hover:text-destructive focus:bg-destructive/10 focus:text-destructive dark:hover:bg-destructive/20 dark:focus:bg-destructive/20 [&_svg]:text-destructive'
                  : '',
                itemIsSelected(item) ? selectedItemClass : '',
                itemClass,
              )
            "
          ...
          >
            <Check
              ...
              :class="itemIsSelected(item) ? selectedCheckClass : 'opacity-0'"
            />
            <span
            ...
            <component
              :is="item.icon"
              v-if="item.icon"
              :class="cn('shrink-0 text-muted-foreground', itemIconClass, item.iconClass)"
            />
            <span class="min-w-0 flex-1 truncate">{{ item.label }}</span>
            <span v-if="item.shortcut" class="ml-4 shrink-0 text-xs text-muted-foreground">{{ item.shortcut }}</span>
            <Check
              v-if="checkPosition === 'right' && itemIsSelected(item)"
