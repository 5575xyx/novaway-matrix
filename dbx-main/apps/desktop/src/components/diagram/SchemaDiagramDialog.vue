...
function isRelationshipColumn(table: DiagramTable, columnName: string): boolean {
  return visibleRelationships.value.some(
    (relationship) =>
      (relationship.sourceTable === table.name && relationship.sourceColumn === columnName) ||
      (relationship.targetTable === table.name && relationship.targetColumn === columnName),
  )
}
...
function addCustomRelationship() {
  ...
  const relationship = normalizeCustomDiagramRelationship({
    name:
      relationshipDraft.value.name.trim() ||
      defaultRelationshipName({ sourceTable, sourceColumn, targetTable, targetColumn, ...cardinality }),
    sourceTable,
    sourceColumn,
    targetTable,
    targetColumn,
...
async function loadDiagram() {
  ...
  try {
    ...
    const baseTables = tableInfos
      .filter((table) => table.table_type !== "VIEW" && table.table_type !== "MATERIALIZED_VIEW")
    ...
    >
      ...
      <div class="flex items-center gap-2 border-b px-3 py-2 shrink-0 overflow-x-auto">
        ...
        <Button
          ...
          :title="t('diagram.modelRelationships')"
        ...
        <Button
          ...
          size="sm"
          class="h-8 px-2 text-xs"
          :disabled="!generatedJoinSql.trim()"
          :title="t('diagram.copyJoinSql')"
          @click="copyJoinSql"
        >
        ...
        <div v-if="showRelationshipPanel && tables.length > 0" class="shrink-0 border-b bg-background/95 px-3 py-2">
          <div class="flex flex-wrap items-end gap-2">
            <div class="w-44">
              ...
              <Input
                v-model="relationshipDraft.name"
                class="h-8 text-xs"
                :placeholder="t('diagram.relationshipNamePlaceholder')"
              />
            </div>
            ...
            <Button
              ...
              @click="showRelationshipPanel = false"
            >
              <X class="h-4 w-4" />
            </Button>
          </div>
          ...
          <div v-if="customRelationships.length > 0" class="mt-2 flex flex-wrap gap-1.5">
            <Badge
              v-for="relationship in customRelationships"
              :key="relationship.id"
              variant="secondary"
              class="gap-1 pr-1"
            >
              <span class="max-w-80 truncate"
                >{{ relationship.sourceTable }}.{{ relationship.sourceColumn }} {{ relationship.sourceCardinality }}:{{
                  relationship.targetCardinality
                }}
                {{ relationship.targetTable }}.{{ relationship.targetColumn }}</span
              >
              <button
                type="button"
                class="rounded-sm p-0.5 hover:bg-background/80"
                :title="t('diagram.removeRelationship')"
                @click="removeCustomRelationship(relationship.id)"
              >
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        ...
                        />
                        <Link2
                          v-else-if="isRelationshipColumn(table, column.name)"
