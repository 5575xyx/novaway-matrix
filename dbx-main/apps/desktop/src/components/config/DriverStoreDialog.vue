...
} from "@/lib/connection/driverRuntimePresentation"
import {
...
import {
  PRESTOSQL_DRIVER_DB_TYPE,
  prestoSqlBuiltinDriverRow,
  prestoSqlMavenBundle,
} from "@/lib/database/prestoSqlBuiltinDriver"
...
const progressText = computed(() => {
  ...
  const label =
    p.step === "jre"
      ? t("driverStore.progressDownloadJre")
      : p.step === "jdbc-plugin"
        ? t("driverStore.progressDownloadJdbcPlugin")
        : t("driverStore.progressDownloadDriver")
...
const builtinDriverRows = computed<AgentDriverInfo[]>(() => [
  ...drivers.value,
  prestoSqlBuiltinDriverRow(jdbcMavenBundles.value),
])
...
const filteredAgentDrivers = computed(() => {
  ...
  return builtinDriverRows.value.filter((driver) =>
    [
      driver.label,
      driver.db_type,
      driver.version,
      driver.installed_version,
      driverRequiresJavaRuntime(driver) ? driver.jre : "",
    ]
      .filter(Boolean)
      .join(" ")
      .toLowerCase()
...
const filteredJdbcDrivers = computed(() => {
  ...
  return jdbcDriverListItems.value.filter((item) =>
...
async function installJdbcMavenDriver() {
  ...
  const repository =
    jdbcMavenRepository.value === "custom" ? customJdbcMavenRepository.value.trim() : jdbcMavenRepository.value
          ...
          <div class="flex items-center justify-between">
            ...
            <div v-if="driverStoreTab !== 'storage'" class="flex items-center gap-2">
              <Button
                variant="ghost"
                size="sm"
                class="h-7 rounded-[6px] text-xs gap-1 text-muted-foreground"
            ...
            <div class="rounded-xl border bg-muted/20 p-4 space-y-3">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <Button
                  v-if="javaRuntimeConfig.mode === 'custom'"
                  variant="outline"
                  class="h-8 shrink-0 rounded-[6px] text-xs"
                ...
                >
                  ...
                  <div class="flex shrink-0 items-center gap-3">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      class="h-8 rounded-[6px] text-xs text-muted-foreground hover:text-destructive"
                      :disabled="reinstallingJre !== null || installing !== null"
            ...
            <div v-else class="rounded-md border divide-y">
              ...
              >
                ...
                <div class="flex shrink-0 items-center gap-1.5">
                  <span
                    v-if="driverRequiresJavaRuntime(driver) && driver.jre"
                ...
                <div class="flex shrink-0 items-center gap-2">
                  ...
                  <Button
                    v-if="
                      !driver.installed &&
                      !isPrestoSqlBuiltinDriver(driver.db_type) &&
                      !isDriverProgressActive(driver.db_type) &&
                      !isDriverQueued(driver.db_type)
                    "
                    size="sm"
                    variant="ghost"
                  ...
                  <template v-if="driver.installed">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      class="h-7 rounded-[6px] text-xs text-muted-foreground hover:text-destructive"
          ...
          <TabsContent value="jdbc" class="mt-5 space-y-5">
            ...
            <div class="rounded-xl border bg-muted/20 p-4">
              <div class="flex min-h-12 items-center justify-between gap-3">
                ...
                <div class="flex shrink-0 items-center gap-3">
                  <DriverInstallProgressCircle
                    v-if="isInstallingJdbcPlugin"
                    :percent="progressNumber"
                    :title="progressTitle(t('driverStore.progressDownloadJdbcPlugin'))"
                  />
                  <span
            ...
            <div class="space-y-3">
              ...
              <div class="grid gap-2 md:grid-cols-[minmax(0,1fr)_180px_auto]">
                <Input
                  v-model="jdbcMavenCoordinateInput"
                  class="h-8 text-xs"
                  :placeholder="t('driverStore.jdbcMavenCoordinatePlaceholder')"
                  @keydown.enter.prevent="installJdbcMavenDriver"
                />
                ...
                <Button
                  class="h-8 shrink-0 rounded-[6px]"
                  :disabled="
                    !jdbcMavenCoordinateInput.trim() ||
                    isInstallingJdbcMavenDriver ||
                    (jdbcMavenRepository === 'custom' && !customJdbcMavenRepository.trim())
                  "
                  @click="installJdbcMavenDriver"
                >
              ...
              <Input
                v-if="jdbcMavenRepository === 'custom'"
                v-model="customJdbcMavenRepository"
                class="h-8 text-xs"
                placeholder="https://repo.example.com/repository/maven-public"
                @keydown.enter.prevent="installJdbcMavenDriver"
              />
            </div>
              ...
              <div v-else class="divide-y">
                <div v-for="item in filteredJdbcDrivers" :key="item.id" class="flex items-center gap-3 p-3">
                  ...
                  <Button
                    ...
                    @click="
                      item.kind === 'maven' ? deleteJdbcMavenBundle(item.bundle.id) : deleteJdbcDriver(item.driver.path)
                    "
                  >
              ...
              <div class="space-y-2.5">
                <div v-for="row in driverStorePathRows" :key="row.kind" class="rounded-lg border bg-background/50 p-3">
                  <div class="mb-2 flex items-start justify-between gap-3">
                    ...
                    <Loader2
                      v-if="driverStoreDirMigrating === row.kind"
                      class="mt-0.5 h-3.5 w-3.5 shrink-0 animate-spin text-muted-foreground"
                    />
                  </div>
                  <div class="flex items-center gap-2">
                    <Tooltip>
                      <TooltipTrigger as-child>
                        <div
                          class="min-w-0 flex-1 rounded-md border bg-background px-3 py-2 text-xs font-mono truncate"
                        >
                          {{ row.display }}
                        </div>
                      </TooltipTrigger>
                    ...
                    <Button
                      ...
                      :disabled="Boolean(driverStoreDirMigrating)"
                      @click="chooseDriverStoreDir(row.kind)"
                    >
                    ...
                    <Button
                      ...
                      :disabled="Boolean(driverStoreDirMigrating)"
                      @click="resetDriverStoreDir(row.kind)"
                    >
            ...
            <div class="overflow-hidden rounded-md border bg-background">
              <div class="flex flex-col gap-3 border-b px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  class="h-8 w-8 shrink-0 rounded-[6px] text-muted-foreground"
                  ...
                  >
                    ...
                    <div class="flex min-w-0 items-center gap-1.5 lg:justify-end">
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        class="h-7 w-7 rounded-[6px] text-muted-foreground hover:text-destructive"
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        class="h-7 w-7 rounded-[6px] text-muted-foreground"
