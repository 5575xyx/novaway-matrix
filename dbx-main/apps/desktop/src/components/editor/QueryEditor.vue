...
import {
  ...
  type SqlExecutionSnapshot,
  type SqlExecutionOverride,
  type SqlExecutionCandidate,
} from "@/lib/sql/sqlExecutionTarget"
import {
  buildExecutionCandidates,
  hasMultipleExecutionTargets,
  supportsExecutionTargetPicker,
  type SqlTextRange,
...
import {
  usesLocalOnlyEditorCompletionMetadata,
  usesOnDemandOnlyEditorColumnMetadata,
} from "@/lib/metadata/completionMetadataPolicy"
...
} from "@/lib/redis/redisCompletion"
import type {
  SqlCompletionColumn,
...
let setPreviewRangeEffect: import("@codemirror/state").StateEffectType<{ from: number; to: number } | null> | null =
  null
...
function requestExecuteFromView(currentView: EditorViewType, cursorPos: number, options: RequestExecuteOptions = {}) {
  ...
  if (
    !settingsStore.editorSettings.showExecutionTargetPicker ||
    !hasMultipleExecutionTargets(doc, props.databaseType)
  ) {
...
function handleSqlSingleQuote(view: EditorViewType): boolean {
  ...
  if (
    state.selection.ranges.some(
      (range) => !range.empty || range.from === 0 || state.doc.sliceString(range.from - 1, range.from) !== "'",
    )
  )
    return false
...
function executionPickerAnchor(
...
function isEditorScrollbarPointerEvent(currentView: EditorViewType, event: MouseEvent) {
  ...
  const inVerticalScrollbar =
    hasVerticalScrollbar && event.clientX >= rect.right - verticalGutter && event.clientX <= rect.right
  const inHorizontalScrollbar =
    hasHorizontalScrollbar && event.clientY >= rect.bottom - horizontalGutter && event.clientY <= rect.bottom
...
function executableStatementRangeStartingAt(currentView: EditorViewType, lineFrom: number) {
  executableStatementRangeCache = executableStatementRangeCacheForDoc(
...
function executeSqlStatementFromGutter(
  currentView: EditorViewType,
...
function runKeymapExtension(codeMirrorKeymap: (typeof import("@codemirror/view"))["keymap"]) {
  ...
  const binding = (shortcut: string, run: (view: EditorViewType) => boolean) =>
...
function completionMetadataTarget(table: {
  name: string
  schema?: string | null
...
function completionQualifiedTableTarget(
...
function completionTablesMatch(
  left: { name: string; schema?: string | null },
  right: { name: string; schema?: string | null },
) {
...
async function findExactSemanticDiagnosticTable(
  table: SqlTableReference,
): Promise<{ name: string; schema?: string; type?: "table" | "view" } | null> {
  if (!props.connectionId || props.database == null) return null
  const target = completionMetadataTarget(table)
  if (!target) return null
  const localMatches = connectionStore.lookupLocalCompletionTables(
    props.connectionId,
    target.database,
    table.name,
  ...
  const remoteMatches = await connectionStore.listCompletionTables(
    props.connectionId,
    target.database,
    table.name,
    MAX_COMPLETION_TABLES,
...
async function ensureForeignKeysForTable(table: { name: string; schema?: string | null }) {
  ...
  if (!target) return
  try {
    const foreignKeys = await connectionStore.listCompletionForeignKeys(
      props.connectionId,
      target.database,
      table.name,
      target.schema,
...
async function ensureForeignKeysForTables(tables: Array<{ name: string; schema?: string | null }>) {
  ...
  for (let index = 0; index < uniqueTables.length; index += COMPLETION_METADATA_CONCURRENCY) {
    await Promise.all(
      uniqueTables
        .slice(index, index + COMPLETION_METADATA_CONCURRENCY)
        .map((table) => ensureForeignKeysForTable(table)),
    )
  }
}
...
async function resolveSqlHoverTooltip(currentView: EditorViewType, pos: number) {
  ...
  const semanticModel = SEMANTIC_SQL_COMPLETION_ENABLED
    ? buildSqlSemanticModel(sql, pos, { databaseType: props.databaseType, dialect: props.dialect })
    : null
  const semanticTarget = semanticModel ? resolveSqlSemanticNavigationTarget(semanticModel, parts) : null
  const semanticQualifierIsRowSource =
    !!qualifier &&
    !!semanticTarget &&
    (semanticTarget.alias?.toLowerCase() === qualifier.toLowerCase() ||
      semanticTarget.source.name.toLowerCase() === qualifier.toLowerCase())
  ...
  try {
    ...
    let table =
      matchTable(qualifiedTableLookup, cachedTables) ??
      matchTable(tableLookupName, cachedTables) ??
      matchTable(identifier, cachedTables) ??
      matchTable(name, cachedTables)
    if (!table && !usesLocalOnlyCompletionMetadata()) {
      const hoverTables = await connectionStore.listCompletionTables(
        ...
        tableLookupName,
        MAX_COMPLETION_TABLES,
        semanticTarget?.schema ?? props.schema,
      )
      cachedTables = [...cachedTables, ...hoverTables]
      table =
        matchTable(qualifiedTableLookup, hoverTables) ??
        matchTable(tableLookupName, hoverTables) ??
        matchTable(identifier, hoverTables) ??
        matchTable(name, hoverTables)
    }
    if (
      table &&
      !semanticQualifierIsRowSource &&
      (!qualifier || table.schema?.toLowerCase() === qualifier.toLowerCase() || table.name === name)
    ) {
      return {
        pos: range.from,
        end: range.to,
...
function offsetSqlSemanticDiagnostics(
...
function replaceSemanticDiagnosticsInRanges(
  next: SqlSemanticDiagnostic[],
  ranges: readonly SqlTextRange[],
  fullSql: string,
) {
...
async function enrichSemanticDiagnosticTables(
  tables: SqlTableReference[],
): Promise<{ tables: SqlTableReference[]; missingTables: Set<string> }> {
...
async function refreshSemanticDiagnostics(options: { preserveOutsideRanges?: boolean } = {}) {
  ...
  if (
    codeMirrorCompletionStatus?.(currentView.state) &&
    isSqlSemanticDiagnosticInputContext(sql, currentView.state.selection.main.head, {
  ...
  for (const range of diagnosticRanges) {
    try {
      ...
      const semanticModel = SEMANTIC_SQL_COMPLETION_ENABLED
        ? buildSqlSemanticModel(range.sql, semanticCursor, { databaseType: props.databaseType, dialect: props.dialect })
        : null
  ...
  try {
    const formatted =
      props.databaseType === "mongodb"
        ? formatMongoShellText(source, settingsStore.editorSettings.sqlFormatter)
        : await formatSqlText(
            source,
...
async function provideRedisCompletions(
  currentState: import("@codemirror/state").EditorState,
  position: number,
  explicit: boolean,
) {
...
async function provideMongoCompletions(
  currentState: import("@codemirror/state").EditorState,
  position: number,
  explicit: boolean,
) {
  ...
  if (props.database && completionContext.mode === "field" && completionContext.collection) {
    try {
      fields = await connectionStore.listMongoCompletionFields(
        props.connectionId,
        props.database,
        completionContext.collection,
      )
    } catch {
      fields = []
    }
...
async function provideSqlCompletions(context: CompletionContext) {
  ...
  try {
    ...
    const completionContext = SEMANTIC_SQL_COMPLETION_ENABLED
      ? sqlCompletionContextFromSemantic(
          buildSqlSemanticModel(fullDoc, position, { databaseType: props.databaseType, dialect: props.dialect }),
          legacyCompletionContext,
        )
      : legacyCompletionContext
    ...
    const shouldResolveAsyncColumnCompletion =
      completionContext.suggestColumns &&
      completionContext.referencedTables.length > 0 &&
...
function buildLocalSqlCompletionResult(
  ...
  if (!props.connectionId || props.database == null) return null
  const databaseNames =
    supportsDatabaseQualifierCompletion() && completionContext.suggestTables && !completionContext.insertTable
      ? connectionStore.lookupLocalCompletionDatabases(
          props.connectionId,
          completionContext.qualifier || completionContext.prefix,
          MAX_COMPLETION_TABLES,
        )
      : []
  const qualifierDatabase = completionContext.qualifier
    ? findExactName(databaseNames, completionContext.qualifier)
    : undefined
  ...
  const tableLookupSchema = qualifierDatabase
    ? undefined
  ...
  if (qualifiedColumnTarget) {
    ...
    if (cached) {
      ...
      const localColumns = target
        ? connectionStore.lookupLocalCompletionColumns(
            props.connectionId,
            target.database,
            qualifiedColumnTarget.name,
            target.schema,
          )
        : []
  ...
  for (const refTable of completionContext.referencedTables) {
    ...
    const localColumns = target
      ? connectionStore.lookupLocalCompletionColumns(props.connectionId, target.database, refTable.name, target.schema)
      : []
    if (localColumns.length > 0) {
      columnsByTable.set(cacheKey, localColumns)
    }
    const localForeignKeys = target
      ? connectionStore.lookupLocalCompletionForeignKeys(
          props.connectionId,
          target.database,
          refTable.name,
          target.schema,
...
function scheduleCompletionMetadataRefresh(completionContext: ReturnType<typeof getSqlCompletionContext>) {
    ...
    void connectionStore
      ...
      .then((tables) => {
        ...
        if (completionContext.suggestTables && completionContext.referencedTables.length > 0) {
          void ensureForeignKeysForTables([
            ...completionContext.referencedTables,
            ...tables.slice(0, MAX_JOIN_FK_PREFETCH_TABLES),
          ])
        }
      })
      .catch(() => {})
  ...
  if (
    !onDemandOnlyColumns &&
    qualifiedColumnTarget &&
    qualifiedColumnCacheKey &&
    !cachedColumnsByTable.has(qualifiedColumnCacheKey)
  ) {
    const target = completionMetadataTarget(qualifiedColumnTarget)
    if (target) {
      void connectionStore
  ...
  if (
    (completionContext.suggestTables || completionContext.suggestJoinConditions) &&
    completionContext.referencedTables.length > 0
  ) {
...
function withCompletionLatencyBudget<T>(remote: Promise<T>, local: T): Promise<T> {
  return Promise.race([
    remote,
    new Promise<T>((resolve) => setTimeout(() => resolve(local), COMPLETION_REMOTE_LATENCY_BUDGET_MS)),
  ])
}

function listCompletionTablesWithLatencyBudget(
  connectionId: string,
  database: string,
  filter: string,
  limit: number,
...
async function performAsyncCompletionWithResult(
  ...
  const tableLookupFilter =
    completionContext.qualifier && !qualifierDatabase ? completionContext.qualifier : completionContext.prefix
  let tables = shouldLoadTables
    ? localOnlyMetadata
      ...
      : await listCompletionTablesWithLatencyBudget(
          props.connectionId!,
          tableLookupDatabase,
          tableLookupFilter,
          MAX_COMPLETION_TABLES,
  ...
  if (completionContext.suggestTables && !completionContext.qualifier && !completionContext.insertTable) {
    if (localOnlyMetadata) {
      schemaNames = mergeCompletionQualifierNames(
        connectionStore.lookupLocalCompletionSchemas(
          props.connectionId!,
          props.database!,
          completionContext.prefix,
  ...
  if (
    ...
    let schemaTables = connectionStore.lookupLocalCompletionTables(
      props.connectionId!,
      props.database!,
      completionContext.prefix,
      MAX_COMPLETION_TABLES,
    ...
    if (!localOnlyMetadata) {
      schemaTables = await listCompletionTablesWithLatencyBudget(
        props.connectionId!,
        props.database!,
        completionContext.prefix,
        MAX_COMPLETION_TABLES,
      ...
      schemaTables = await listCompletionTablesWithLatencyBudget(
        ...
        props.database!,
        completionContext.prefix,
        PRESTO_ON_DEMAND_TABLE_COMPLETION_LIMIT,
        completionContext.qualifier,
      )
    }
  ...
  const shouldFetchColumnsForCompletion =
    !onDemandOnlyColumns ||
    ((completionContext.suggestColumns || completionContext.exclusiveColumnSuggestions) &&
      !isTableNameCompletionContext) ||
    !!completionContext.insertTable
  ...
  if ((completionContext.suggestTables || completionContext.suggestJoinConditions) && refs.length > 0) {
    const fkPrefetchTables = completionContext.suggestTables
      ? [...refs, ...tables.slice(0, MAX_JOIN_FK_PREFETCH_TABLES)]
      : refs
    await ensureForeignKeysForTables(
      fkPrefetchTables.filter((table) => !("columns" in table) || !table.columns || table.columns.length === 0),
    )
    if (epoch !== completionEpoch) return null
  }
...
function isReferencedTableQualifier(completionContext: ReturnType<typeof getSqlCompletionContext>): boolean {
  ...
  return completionContext.referencedTables.some(
    (table) =>
      table.alias?.toLowerCase() === qualifier ||
      table.name.toLowerCase() === qualifier ||
      (!!qualifiedColumnTarget && completionTablesMatch(table, qualifiedColumnTarget)),
  )
}
...
watch(
  ...
  async ([ss]) => {
    ...
    const [themeExt] = await Promise.all([
      loadEditorTheme(ss.theme, editorThemeAppearance(), themeColors, themePalette.value),
      ss.vimModeEnabled ? ensureCodeMirrorVim() : Promise.resolve(false),
    ])
...
<template>
  ...
  >
    ...
    <SqlExecutionTargetPicker
      v-if="pickerVisible"
      :candidates="pickerCandidates"
      :active-index="pickerActiveIndex"
      :anchor="pickerAnchor"
      @update:active-index="onPickerActiveIndexChange"
