...
<template>
  <div
    ...
    role="listbox"
    tabindex="-1"
    class="sql-execution-picker absolute z-[9998] w-[520px] max-w-[calc(100vw-32px)] flex flex-col rounded-md border bg-popover shadow-lg outline-none"
    :style="pickerStyle"
    @keydown="onKeydown"
    @blur="emit('cancel')"
  >
    ...
    <div class="flex flex-col py-1">
      <div
        ...
        :class="
          index === activeIndex ? 'bg-primary text-primary-foreground' : 'hover:bg-accent hover:text-accent-foreground'
        "
        @mousemove="setActiveIndex(index)"
        @click="emit('confirm', candidate)"
      >
        <span
          class="shrink-0 text-xs font-medium px-1.5 py-0.5 rounded border"
          :class="
            index === activeIndex
