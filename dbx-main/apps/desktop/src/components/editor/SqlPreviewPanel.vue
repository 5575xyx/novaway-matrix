  ...
  <div class="h-full flex flex-col bg-background border-t">
      ...
      <Tooltip>
        <TooltipTrigger as-child>
          <Button
            ...
            :aria-label="t('toolbar.formatSql')"
            @click="toggleFormat"
          >
      ...
      <Tooltip>
        <TooltipTrigger as-child>
          <Button
            variant="ghost"
            size="icon"
            class="h-6 w-6 text-muted-foreground/60 hover:text-foreground hover:bg-accent"
            :disabled="!canUndo"
            :aria-label="t('grid.undoChange')"
            @click="emit('undo')"
          >
      ...
      <Tooltip>
        <TooltipTrigger as-child>
          <Button
            ...
            size="icon"
            class="h-6 w-6 text-muted-foreground/60 hover:text-foreground hover:bg-accent"
            :aria-label="t('toolbar.hidePreviewSql')"
            @click="emit('close')"
          >
    ...
    <div class="flex-1 min-h-0 overflow-auto">
      ...
      <div
        ...
        class="p-3 text-xs leading-relaxed [&_pre]:!bg-transparent [&_pre]:!p-0 [&_code]:!font-mono [&_code]:text-xs"
        v-html="highlightedHtml"
      />

      <!-- Plain text fallback -->
      <pre v-else data-native-clipboard class="p-3 text-xs font-mono whitespace-pre-wrap select-text">{{
        displaySql
      }}</pre>
    </div>
  </div>
