...
import {
  clearDebugLogs as clearStoredDebugLogs,
  downloadDebugLogs,
  getDebugLogBundleText,
} from "@/lib/backend/debugLog"
import {
  aiListModels,
  aiTestConnection,
...
import {
  EMPTY_TABLE_COLUMN_TEMPLATE_DATA_TYPE,
  parseTableColumnTemplateFields,
  TABLE_COLUMN_TEMPLATE_DATABASE_TYPES,
} from "@/lib/table/tableColumnTemplates"
import {
  buildMcpCodexConfig,
  buildMcpJsonConfig,
...
const selectedThemePaletteOption = computed(
  () =>
    appThemePaletteOptions.value.find((option) => option.value === themePalette.value) ??
    appThemePaletteOptions.value[0],
)
const selectedLocaleOption = computed(
  () => LOCALE_OPTIONS.find((locale) => locale.value === currentLocale()) ?? LOCALE_OPTIONS[0],
)
...
const settingsContentClass = computed(() =>
  isSettingsPage.value
    ...
    : "h-[min(660px,calc(100dvh-80px))] !max-w-[min(920px,calc(100vw-32px))] grid-rows-[auto_minmax(0,1fr)] gap-3 p-4 sm:!max-w-[min(920px,calc(100vw-48px))]",
)
...
const editSqlSemanticDiagnosticsMode = ref<SqlSemanticDiagnosticsMode>(
  settingsStore.editorSettings.sqlSemanticDiagnosticsMode,
)
...
const editSidebarTablePageSize = ref(
  settingsStore.desktopSettings.sidebar_table_page_size ?? DEFAULT_SIDEBAR_TABLE_PAGE_SIZE,
)
...
const editTableColumnTemplateRows = ref<TableColumnTemplateGridRow[]>(
  tableColumnTemplateRowsFromSettings(settingsStore.editorSettings.tableColumnTemplateFields),
)
...
const normalizedEditTableColumnTemplateFields = computed(() =>
  tableColumnTemplateRowsToSettings(editTableColumnTemplateRows.value),
)
...
function fontOptionStyle(value: string, selectedValue = editFontFamily.value) {
  return presetFontValues.has(value) || uiFontPreviewValues.has(value) || value === selectedValue
    ? { fontFamily: value }
    : undefined
}
...
watch(
  ...
  (open) => {
    if (open) {
      ...
      editSidebarTablePageSize.value =
        settingsStore.desktopSettings.sidebar_table_page_size ?? DEFAULT_SIDEBAR_TABLE_PAGE_SIZE
      ...
      editTableColumnTemplateRows.value = tableColumnTemplateRowsFromSettings(
        settingsStore.editorSettings.tableColumnTemplateFields,
      )
...
const formatterEditorShortcutDefinitions = computed(() =>
  formatterEditorShortcutIds
    .map((id) => SHORTCUT_DEFINITIONS.find((definition) => definition.id === id))
    .filter((definition): definition is (typeof SHORTCUT_DEFINITIONS)[number] => !!definition),
)
...
const duckDbWorkerProcessIsolationRequiresRestart = computed(
  () => editDuckDbWorkerProcessIsolation.value !== startupDuckDbWorkerProcessIsolation.value,
)
...
function hasChanges(): boolean {
  return (
    ...
    editInfiniteScroll.value !== settingsStore.editorSettings.infiniteScroll ||
    editInfiniteScrollMaxRows.value !== settingsStore.editorSettings.infiniteScrollMaxRows ||
    JSON.stringify(normalizedEditTableColumnTemplateFields.value) !==
      JSON.stringify(settingsStore.editorSettings.tableColumnTemplateFields) ||
    JSON.stringify(editShortcuts.value) !== JSON.stringify(settingsStore.editorSettings.shortcuts) ||
    ...
    editExportRowLimitEnabled.value !== settingsStore.editorSettings.exportRowLimitEnabled ||
    editExportRowLimit.value !== settingsStore.editorSettings.exportRowLimit ||
    editQueryExportKeysetOptimizationEnabled.value !==
      settingsStore.editorSettings.queryExportKeysetOptimizationEnabled ||
    editUpdateDownloadSource.value !== settingsStore.editorSettings.updateDownloadSource ||
...
async function persistSettings() {
  ...
  const sidebarObjectDisplayChanged =
    editSidebarObjectDisplay.value !== settingsStore.editorSettings.sidebarObjectDisplay
  const sidebarTablePageSizeChanged =
    editSidebarTablePageSize.value !==
    (settingsStore.desktopSettings.sidebar_table_page_size ?? DEFAULT_SIDEBAR_TABLE_PAGE_SIZE)
...
function resetDefaultsForTab(tab: SettingsCategory) {
  if (tab === "editor") {
    ...
    editTableColumnTemplateRows.value = tableColumnTemplateRowsFromSettings(
      DEFAULT_EDITOR_SETTINGS.tableColumnTemplateFields,
    )
...
function removeTableColumnTemplateRow(id: string) {
  ...
  if (row.overrides.some((override) => override.databaseType === editTableColumnTemplateDatabaseType.value)) {
    row.overrides = row.overrides.filter(
      (override) => override.databaseType !== editTableColumnTemplateDatabaseType.value,
    )
...
function startTableColumnTemplateRowDrag(id: string, event: PointerEvent) {
  ...
  const onPointerMove = (moveEvent: PointerEvent) => {
    ...
    const targetRow = document
      ...
      ?.closest<HTMLElement>("[data-table-column-template-row-id]")
...
function tableColumnTemplateDataTypeForSelectedDatabase(row: TableColumnTemplateGridRow): string {
  return (
    row.overrides.find((override) => override.databaseType === editTableColumnTemplateDatabaseType.value)?.dataType ??
    ""
  )
}
...
function tableColumnTemplateLengthForSelectedDatabase(row: TableColumnTemplateGridRow): string {
  return dataTypeLengthInputValue(
    editTableColumnTemplateDatabaseType.value,
    tableColumnTemplateDataTypeForSelectedDatabase(row),
  )
}
...
function setTableColumnTemplateBaseTypeForSelectedDatabase(row: TableColumnTemplateGridRow, value: string) {
  ...
  const databaseType = editTableColumnTemplateDatabaseType.value
  setTableColumnTemplateDataTypeForSelectedDatabase(
    row,
    combineDataTypeForDatabase(databaseType, baseType, getDefaultLengthForType(databaseType, baseType)),
  )
}
...
function settingsCategoryButton(value: SettingsCategory): string {
  return [
    "w-auto shrink-0 whitespace-nowrap rounded-md px-3 py-2 text-left text-sm transition-colors lg:w-full",
...
const webdavAutoUploadIntervalMinutes = ref(
  Number(
    localStorage.getItem("dbx-webdav-auto-upload-interval-minutes") ||
      String(DEFAULT_WEB_DAV_AUTO_UPLOAD_INTERVAL_MINUTES),
  ),
)
...
const webdavReady = computed(
  () =>
    ...
    !webdavBusy.value &&
    (!webdavSyncSecrets.value || !!webdavSecretsPassphrase.value.trim() || webdavHasSavedSecretsPassphrase.value),
)
...
const aiProviderOptions = computed(() =>
  Object.values(AI_PROVIDER_PRESETS).filter((provider) => !isWeb || provider.provider !== "codex-cli"),
)
...
const aiEditAuthMethod = ref<AiAuthMethod>(
  settingsStore.aiConfig.authMethod || AI_PROVIDER_PRESETS[settingsStore.aiConfig.provider].authMethod,
)
...
const aiSupportsAuthMethod = computed(
  () => aiEditProvider.value === "claude" || (aiEditProvider.value === "custom" && aiAnthropicMessagesMode.value),
)
const aiCredentialLabel = computed(() =>
  aiSupportsAuthMethod.value && aiEditAuthMethod.value === "bearer" ? "Auth Token" : "API Key",
)
...
const aiCanListModels = computed(
  () =>
    ...
    (aiIsCodexCli.value || !!aiEditEndpoint.value.trim()) &&
...
const aiCodexValidationError = computed(() =>
  aiIsCodexCli.value ? aiCodexPathError.value || aiCodexEnvError.value : "",
)
...
function savedCodexEnvSignature(): string {
  return JSON.stringify(
    Object.entries(normalizeAiEnv(settingsStore.aiConfig.codexCliEnv)).sort(([left], [right]) =>
...
function syncAiEditState() {
  ...
  aiEditEndpoint.value =
    provider === settingsStore.aiConfig.provider
      ? settingsStore.aiConfig.endpoint
      : AI_PROVIDER_PRESETS[provider].endpoint
  aiEditModel.value =
    provider === settingsStore.aiConfig.provider ? settingsStore.aiConfig.model : AI_PROVIDER_PRESETS[provider].model
  aiEditApiStyle.value =
    provider === settingsStore.aiConfig.provider
      ? settingsStore.aiConfig.apiStyle || "completions"
      : AI_PROVIDER_PRESETS[provider].apiStyle
...
async function aiTestConn() {
  if (
    (aiRequiresApiKey.value && !aiEditApiKey.value.trim()) ||
    (!aiIsCodexCli.value && !aiEditEndpoint.value.trim()) ||
    (!aiIsCodexCli.value && !aiEditModel.value.trim())
  )
    return
  if (aiCodexValidationError.value) {
    aiTestResult.value = "error"
...
async function ensureCodexMcpStatus() {
  if (isWeb || activeSettingsTab.value !== "ai" || !aiIsCodexCli.value || mcpStatus.value || mcpStatusLoading.value)
    return
  await refreshMcpStatus()
}
...
<template>
  <component
    :is="settingsRootComponent"
    v-bind="settingsRootProps"
    :class="settingsRootClass"
    @update:open="onSettingsRootOpenChange"
  >
    <component :is="settingsContentComponent" :class="settingsContentClass">
      <DialogHeader>
        <component
          :is="settingsTitleComponent"
          class="flex items-center gap-2 text-base leading-none font-medium cn-font-heading"
        >
      ...
      <div class="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden lg:flex-row">
        <nav
          class="settingsCategoryNav flex min-h-0 shrink-0 gap-1 overflow-x-auto border-b pb-3 lg:w-40 lg:flex-col lg:overflow-x-hidden lg:overflow-y-auto lg:border-b-0 lg:border-r lg:pb-0 lg:pr-3"
        >
          <button
        ...
        <div class="min-w-0 flex-1 overflow-hidden px-1 flex flex-col">
          <div class="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-1 pr-2">
            <section v-if="activeSettingsTab === 'editor'" class="flex flex-col gap-5 py-2">
              ...
              <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                ...
                <div class="flex items-center justify-between gap-4 rounded-md border bg-muted/20 px-3 py-2">
                  <div class="space-y-1">
                    <Label for="editor-show-execution-target-picker">{{
                      t("settings.showExecutionTargetPicker")
                    }}</Label>
                  ...
                  <Switch
                    id="editor-show-execution-target-picker"
                    v-model="editShowExecutionTargetPicker"
                    class="mt-0.5"
                  />
                </div>
              ...
              <div class="grid gap-3 md:grid-cols-2">
                <div class="flex items-center justify-between gap-4 rounded-md border bg-muted/20 px-3 py-2">
                  <div class="space-y-1">
                    <Label for="editor-sql-semantic-diagnostics">{{
                  ...
                  <Switch
                    id="editor-sql-semantic-diagnostics"
                    :model-value="editSqlSemanticDiagnosticsEnabled"
                    class="mt-0.5"
                    @update:model-value="onSqlSemanticDiagnosticsEnabledChange"
                  />
                </div>
                  ...
                  >
                    ...
                    <div class="min-w-0 space-y-1">
                      <div class="flex items-center justify-end gap-1.5">
                        ...
                        <span
                          v-else-if="editingShortcutId !== definition.id"
            ...
            <section v-else-if="activeSettingsTab === 'appearance'" class="flex flex-col gap-5 py-2">
              <div
                class="grid gap-x-1.5 gap-y-4 sm:grid-cols-[minmax(0,127fr)_minmax(0,127fr)_minmax(0,191fr)_minmax(0,130fr)]"
              >
                <div class="space-y-2 min-w-0">
                    ...
                    <SelectTrigger class="h-8 w-full gap-0.5 px-0.5">
                      <SelectValue>
                        <span v-if="selectedLocaleOption" class="flex min-w-0 items-center gap-0.5">
                          <span
                            class="inline-flex h-5 shrink-0 items-center justify-center text-sm font-medium leading-none"
                          >
                ...
                <div class="space-y-2 min-w-0">
                  ...
                  <Select
                    ...
                    @update:model-value="(value) => setThemePalette(value as AppThemePalette)"
                  >
                      ...
                      <SelectValue :placeholder="t('settings.selectColorTheme')">
                        <span v-if="selectedThemePaletteOption" class="flex min-w-0 items-center gap-1">
                          <span
                            ...
                            :style="{ background: selectedThemePaletteOption.previewColor }"
                          />
                    ...
                    <SelectContent>
                      <SelectItem v-for="option in appThemePaletteOptions" :key="option.value" :value="option.value">
                        <div class="flex items-center gap-2">
                          <span
                            class="h-3 w-3 rounded-full border border-border shadow-xs"
                            :style="{ background: option.previewColor }"
                          />
                          {{ option.label }}
                        </div>
                      </SelectItem>
                ...
                <div class="space-y-2 min-w-0">
                  <div class="flex h-9 items-end gap-1">
                    ...
                    <HelpTooltip
                      ...
                      trigger-class="[&_svg]:h-3 [&_svg]:w-3"
                ...
                <div class="space-y-2 min-w-0">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem v-for="scale in uiScaleOptions" :key="scale" :value="String(scale)" class="pl-2.5">
                      ...
                      <Tooltip>
                        <TooltipTrigger as-child>
                          <div class="w-full min-w-0 text-left">
                            ...
                            <div
                              :ref="(el) => setLayoutDescRef('separated', el)"
                        ...
                        <TooltipContent
                          v-if="layoutDescTruncated.separated.value"
                      ...
                      <Tooltip>
                        <TooltipTrigger as-child>
                          <div class="w-full min-w-0 text-left">
                            ...
                            <div
                              :ref="(el) => setLayoutDescRef('classic', el)"
                              class="text-xs text-muted-foreground truncate"
                            >
                        ...
                        <TooltipContent
                          v-if="layoutDescTruncated.classic.value"
                          class="max-w-[320px] text-xs leading-relaxed"
                        >
                          {{ t("settings.appLayoutClassicDescription") }}
                        </TooltipContent>
                        ...
                        <Tooltip>
                          <TooltipTrigger as-child>
                            <div class="w-full min-w-0 text-left">
                              ...
                              <div
                                :ref="(el) => setIconThemeDescRef('default', el)"
                                class="text-xs text-muted-foreground truncate"
                              >
                          ...
                          <TooltipContent
                            v-if="iconThemeDescTruncated.default.value"
                            class="max-w-[320px] text-xs leading-relaxed"
                          >
                            {{ t("settings.iconThemeDefaultDescription") }}
                          </TooltipContent>
                        ...
                        <Tooltip>
                          <TooltipTrigger as-child>
                            <div class="w-full min-w-0 text-left">
                              ...
                              <div
                                :ref="(el) => setIconThemeDescRef('black', el)"
                                class="text-xs text-muted-foreground truncate"
                              >
                          ...
                          <TooltipContent
                            v-if="iconThemeDescTruncated.black.value"
                            class="max-w-[320px] text-xs leading-relaxed"
                          >
                            {{ t("settings.iconThemeBlackDescription") }}
                          </TooltipContent>
              ...
              <div class="space-y-2">
                ...
                <div class="grid grid-cols-3 gap-2 mt-2">
                  ...
                  >
                    <Switch
                      :id="`toolbar-${item.key}`"
                      :model-value="(editToolbarItems as any)[item.key]"
                      @update:model-value="(v: boolean) => ((editToolbarItems as any)[item.key] = v)"
                    />
            ...
            <section v-else-if="activeSettingsTab === 'navigation'" class="flex flex-col gap-5 py-2">
              ...
              <div class="space-y-2 rounded-md border bg-muted/20 px-3 py-2">
                ...
                <Select
                  :model-value="editOpenTabsRestoreMode"
                  @update:model-value="(value) => (editOpenTabsRestoreMode = value as OpenTabsRestoreMode)"
                >
            ...
            <section v-else-if="activeSettingsTab === 'data'" class="flex flex-col gap-5 py-2">
                ...
                <div class="space-y-3">
                  ...
                  <div class="flex items-start justify-between gap-4 rounded-md border bg-muted/20 px-3 py-2">
                    <div class="space-y-1">
                      ...
                      <div
                        v-if="duckDbWorkerProcessIsolationRequiresRestart"
                      ...
                      >
                        ...
                        <Button
                          ...
                          :disabled="duckDbRestarting || hasApplyBlocker"
                          @click="restartDbxForDuckDbIsolation"
                        >
                    ...
                    <Switch
                      id="duckdb-worker-process-isolation"
                      v-model="editDuckDbWorkerProcessIsolation"
                      class="mt-0.5"
                    />
                  </div>
                </div>
              ...
              <div class="space-y-3">
                ...
                <div class="space-y-2">
                  ...
                  <div class="flex items-center gap-3">
                    <Input
                      ...
                      max="2147483647"
                      step="100"
                      v-model.number="editExportRowLimit"
                      :disabled="!editExportRowLimitEnabled"
                      class="h-9 w-32 [&::-webkit-inner-spin-button]:appearance-none"
                    />
                    <span class="text-xs text-muted-foreground">
                      {{
                        editExportRowLimitEnabled
                          ? t("settings.exportRowLimitDescription")
                          : t("settings.exportRowLimitUnlimited")
                      }}
                    </span>
                  </div>
                </div>
                <div class="flex items-start justify-between gap-3">
                  <div class="space-y-0.5">
                    <Label for="query-export-keyset-enabled">{{
                      t("settings.queryExportKeysetOptimizationEnabled")
                    }}</Label>
                    <p class="text-xs text-muted-foreground">
                  ...
                  <Switch
                    id="query-export-keyset-enabled"
                    v-model="editQueryExportKeysetOptimizationEnabled"
                    class="mt-0.5"
                  />
                </div>
              </div>
                ...
                <div ref="tableColumnTemplateSectionRef" class="space-y-2 rounded-md border bg-muted/20 px-3 py-2">
                    ...
                    <div class="flex items-center gap-2">
                      <Select v-model="editTableColumnTemplateDatabaseType">
                        ...
                        <SelectContent class="max-h-72">
                          <SelectItem
                            v-for="dbType in TABLE_COLUMN_TEMPLATE_DATABASE_TYPES"
                            :key="dbType"
                            :value="dbType"
                          >
                            {{ dbType }}
                  ...
                  <div class="overflow-x-auto rounded-md border bg-background">
                    <table class="w-full min-w-[900px] border-separate border-spacing-0 text-xs">
                      ...
                      <tbody>
                        <tr
                          v-for="row in visibleTableColumnTemplateRows"
                          :key="row.id"
                          :data-table-column-template-row-id="row.id"
                          :class="draggedTableColumnTemplateRowId === row.id ? 'opacity-60' : ''"
                        >
                          ...
                          <td class="border-b px-2 py-1.5">
                            <Input
                              :model-value="tableColumnTemplateLengthForSelectedDatabase(row)"
                              class="h-7 w-28 px-2 font-mono text-xs"
                              :disabled="isTableColumnTemplateLengthDisabled(row)"
                              @update:model-value="setTableColumnTemplateLengthForSelectedDatabase(row, String($event))"
                            />
                          </td>
                          <td class="border-b px-2 py-1.5">
                          ...
                          <td class="border-b px-2 py-1.5 text-right">
                            <Button
                              ...
                              size="icon"
                              class="h-7 w-7"
                              @click="removeTableColumnTemplateRow(row.id)"
                            >
                              <X class="h-3.5 w-3.5" />
                            </Button>
                          </td>
            ...
            <section v-else-if="activeSettingsTab === 'sync'" class="flex flex-col gap-5 py-2">
              ...
              <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    <PasswordInput
                      ...
                      :show-toggle="!webdavHasSavedPassword"
                ...
                <div class="space-y-2 md:col-span-2 rounded-md border bg-muted/20 px-3 py-3">
                  ...
                  <div class="flex items-center gap-2">
                    <Label for="webdav-auto-upload-interval" class="text-xs text-muted-foreground">{{
                      t("settings.syncAutoUploadInterval")
                    }}</Label>
                    <Input
                      id="webdav-auto-upload-interval"
                      v-model.number="webdavAutoUploadIntervalMinutes"
                      type="number"
                      min="1"
                      max="1440"
                      step="1"
                      class="h-7 w-24 text-xs"
                      :disabled="!webdavAutoUploadEnabled"
                    />
                ...
                <div v-if="webdavSyncSecrets" class="space-y-2">
                  ...
                  <div class="flex items-center gap-2">
                    <PasswordInput
                      ...
                      :placeholder="webdavHasSavedSecretsPassphrase ? '••••••••' : ''"
                      :show-toggle="!webdavHasSavedSecretsPassphrase || !!webdavSecretsPassphrase"
            ...
            <section v-else-if="activeSettingsTab === 'ai'" class="flex flex-col gap-5 py-2">
              <div class="space-y-3">
                      ...
                      >
                        <span class="flex w-full items-center justify-between gap-4">
                          ...
                          <span class="flex shrink-0 items-center gap-1">
                            <span
                              v-if="aiEditProvider === provider.provider"
                              class="rounded px-1 py-0.5 text-[10px] font-medium leading-none"
                              :class="
                                settingsStore.isAiProviderConfigured(provider.provider)
                                  ? 'bg-primary/15 text-primary'
                                  : 'bg-muted text-muted-foreground'
                              "
                              >{{ t("ai.providerStatusActive") }}</span
                            >
                            <span
                              v-else-if="settingsStore.isAiProviderConfigured(provider.provider)"
                              class="rounded bg-green-500/15 px-1 py-0.5 text-[10px] font-medium leading-none text-green-700 dark:text-green-400"
                              >{{ t("ai.providerStatusConfigured") }}</span
                            >
                          </span>
                        </span>
                      </SelectItem>
                ...
                <div
                  v-if="aiIsCodexCli && !isWeb"
                  class="rounded-md border px-3 py-2.5 text-xs"
                  :class="
                    aiCodexMcpNeedsInstall
                  ...
                  <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div class="min-w-0 space-y-1">
                      <div class="flex min-w-0 items-center gap-2 font-medium">
                        ...
                        <AlertTriangle
                          v-else-if="aiCodexMcpNeedsInstall || mcpStatus?.error || mcpStatusError"
                        ...
                        <Badge
                          variant="outline"
                          class="h-5 shrink-0 rounded-md border-current/30 px-1.5 text-[11px] font-normal"
                        >
                          {{ mcpStatusLabel }}
                        </Badge>
                      </div>
                    ...
                    <div class="flex shrink-0 items-center gap-2">
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        class="h-7 bg-background/80 px-2 text-xs"
                      ...
                      <Button
                        v-if="aiCodexMcpNeedsInstall || mcpStatus?.update_available"
                        type="button"
                        size="sm"
                        class="h-7 px-2 text-xs"
                        :disabled="!aiCodexMcpCanInstall"
                        @click="installMcp"
                      >
                ...
                <div v-if="!aiIsCodexCli" class="grid grid-cols-3 items-center gap-3">
                  ...
                  <PasswordInput
                    ...
                    :placeholder="aiCredentialPlaceholder"
                  />
                </div>
                ...
                <div v-if="aiIsCodexCli" class="grid grid-cols-3 items-start gap-3">
                  ...
                  <div class="col-span-2 space-y-2">
                    <div class="space-y-1.5">
                      <div
                        v-for="row in aiEditCodexCliEnvRows"
                        :key="row.id"
                        class="grid grid-cols-[minmax(0,0.9fr)_minmax(0,1.3fr)_2rem] gap-2"
                      >
                        <Input
                          v-model="row.key"
                          autocomplete="off"
                          class="h-8 font-mono text-xs"
                          :placeholder="t('ai.codexCliEnvKeyPlaceholder')"
                        />
                        <Input
                          v-model="row.value"
                          autocomplete="off"
                          class="h-8 font-mono text-xs"
                          :placeholder="t('ai.codexCliEnvValuePlaceholder')"
                        />
                        <Button
                          type="button"
                          variant="ghost"
                          ...
                          @click="removeCodexEnvRow(row.id)"
                        >
                          <X class="h-3.5 w-3.5" />
                        </Button>
                      </div>
                      ...
                      >
                        ...
                        <template #option-label="{ option, label }">
                          <span class="flex min-w-0 flex-col leading-tight">
                            ...
                            <span
                              v-if="aiModelOptionSecondary(option, label)"
                              class="mt-0.5 truncate text-[11px] text-muted-foreground"
                              >{{ aiModelOptionSecondary(option, label) }}</span
                            >
                          </span>
                        </template>
                      </SearchableSelect>
                ...
                <div v-if="aiSupportsApiStyle" class="grid grid-cols-3 items-center gap-3">
                  ...
                  <div class="col-span-2 flex gap-2">
                    ...
                    <Button
                      ...
                      class="h-8 flex-1 text-xs"
                      :class="{
                        'border-blue-300 border-2 ring-2 ring-blue-300/50': aiEditApiStyle === 'anthropic-messages',
                      }"
                ...
                <div v-if="!aiIsCodexCli" class="grid grid-cols-3 items-start gap-3">
                  ...
                  <div class="col-span-2">
                    <Input
                      v-model.number="aiEditContextWindow"
                      type="number"
                      min="1000"
                      step="1000"
                      class="h-8 text-xs"
                      :placeholder="t('ai.contextWindowAuto')"
                    />
                ...
                <div class="flex min-w-0 items-center gap-2">
                  ...
                  <Button
                    ...
                    :disabled="
                      ...
                      (mcpStatus?.installed && !mcpStatus?.update_available)
                    "
                    @click="installMcp"
                  >
                    <Loader2 v-if="mcpInstalling" class="mr-2 h-4 w-4 animate-spin" />
                    <CheckCircle2
                      v-if="!mcpInstalling && mcpStatus?.installed && !mcpStatus?.update_available"
                      class="mr-2 h-4 w-4"
                    />
                    {{
                      mcpInstalling
                        ? t("settings.mcpInstalling")
                        : !mcpStatus?.installed
                          ? t("settings.mcpInstallButton")
                          : mcpStatus?.update_available
                            ? t("settings.mcpUpdateButton")
                            : t("settings.mcpUpToDate")
                    }}
                  </Button>
                </div>
                <div
                ...
                <Tabs v-model="mcpConfigTab" class="space-y-3">
                  ...
                  <TabsContent value="claude" class="m-0">
                    <div class="relative rounded-md border bg-background p-3">
                      ...
                      ><code>{{ mcpJsonRecommendedConfig }}</code></pre>
                      <Button
                        type="button"
                        variant="outline"
                        size="icon"
                        ...
                        :title="t('common.copy')"
                        @click="copyMcpText('claude-config', mcpJsonRecommendedConfig)"
                      >
                    ...
                    <div class="space-y-2">
                      ...
                      <div class="relative rounded-md border bg-background p-3">
                        ...
                        <Button
                          ...
                          class="absolute right-2 top-2 h-7 w-7"
                          :title="t('common.copy')"
                          @click="copyMcpText('cursor-config', mcpJsonRecommendedConfig)"
                        >
                    ...
                    <div class="space-y-2">
                      ...
                      <div class="relative rounded-md border bg-background p-3">
                        ...
                        <Button
                          ...
                          class="absolute right-2 top-2 h-7 w-7"
                          :title="t('common.copy')"
                          @click="copyMcpText('trae-config', mcpJsonRecommendedConfig)"
                        >
                  ...
                  <TabsContent value="vscode" class="m-0">
                    <div class="space-y-2">
                      ...
                      <div class="relative rounded-md border bg-background p-3">
                        <pre
                          class="overflow-x-auto whitespace-pre text-xs leading-relaxed"
                        ><code>{{ mcpVsCodeRecommendedConfig }}</code></pre>
                        <Button
                          type="button"
                          variant="outline"
                          size="icon"
                          class="absolute right-2 top-2 h-7 w-7"
                          :title="t('common.copy')"
                          @click="copyMcpText('vscode-config', mcpVsCodeRecommendedConfig)"
                        >
                    ...
                    <div class="space-y-2">
                      ...
                      <div class="relative rounded-md border bg-background p-3">
                        ...
                        <Button
                          ...
                          class="absolute right-2 top-2 h-7 w-7"
                          :title="t('common.copy')"
                          @click="copyMcpText('windsurf-config', mcpJsonRecommendedConfig)"
                        >
                  ...
                  <TabsContent value="opencode" class="m-0">
                    <div class="space-y-2">
                      ...
                      <div class="relative rounded-md border bg-background p-3">
                        <pre
                          class="overflow-x-auto whitespace-pre text-xs leading-relaxed"
                        ><code>{{ mcpOpenCodeRecommendedConfig }}</code></pre>
                        <Button
                          type="button"
                          variant="outline"
                          size="icon"
                          class="absolute right-2 top-2 h-7 w-7"
                          :title="t('common.copy')"
                          @click="copyMcpText('opencode-config', mcpOpenCodeRecommendedConfig)"
                        >
          ...
          <DialogFooter
            ...
            class="mx-0 mb-0 flex-row flex-wrap items-center justify-end gap-2 rounded-none border-t border-border/60 bg-transparent px-0 pb-0 pt-3 sm:flex-row sm:gap-2 [&>button]:w-auto [&>button]:shrink-0"
          >
          ...
          >
            <div class="flex flex-1 items-center gap-2">
              <Button
                ...
                :disabled="
                  aiTesting ||
                  !!aiCodexValidationError ||
                  (aiRequiresApiKey && !aiEditApiKey?.trim()) ||
                  (!aiIsCodexCli && !aiEditEndpoint?.trim()) ||
                  (!aiIsCodexCli && !aiEditModel?.trim())
                "
                @click="aiTestConn"
              >
              ...
              <span
                ...
                class="min-w-0 max-w-[360px] flex items-center gap-1.5 text-xs text-destructive"
              >
            ...
            <Button :disabled="!aiHasChanges() || !!aiCodexValidationError" @click="aiApplySettings">{{
              t("settings.apply")
            }}</Button>
          </DialogFooter>
