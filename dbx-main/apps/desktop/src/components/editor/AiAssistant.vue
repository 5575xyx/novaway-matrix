...
import {
  buildAiAgentStepItems,
  toolCallStepKey,
  upsertAgentStep,
  type AiAgentStepItem,
  type AiAgentStepTone,
} from "@/lib/ai/aiAgentStepPresentation"
...
} from "@/lib/database/defaultDatabase"
...
import {
  ...
  AI_TABLE_MENTION_SCHEMA_LIMIT,
  filterAiTableMentionCandidates,
  formatAiTableMention,
  parseAiTableMentions,
  type AiTableMention,
} from "@/lib/ai/aiTableMentions"
...
const configuredProviders = computed(() =>
  (Object.keys(AI_PROVIDER_PRESETS) as AiProvider[]).filter(
    (p) => p !== settings.aiConfig.provider && settings.isAiProviderConfigured(p),
  ),
)
...
function sendProposalReply(positive: boolean) {
  ...
  const replyEn = positive
    ? "Execute the action you just proposed above; do not ask for confirmation again."
    : "Do not execute the action mentioned above; continue the current conversation."
...
function parseExplainFromData(explainData: unknown, dbType: string): ParsedExplainPlan | undefined {
  ...
  try {
    return parseExplainResult(
      dbType as (typeof supportedTypes)[number],
      explainData as import("@/types/database").QueryResult,
    )
  } catch {
    return undefined
  }
...
function agentEventToStep(event: AgentEvent, index: number): AiAgentStepItem | undefined {
  ...
  const labelKey = isExecuteQuery
    ? event.is_error
      ? "ai.agentSteps.executeBlocked"
      : "ai.agentSteps.executeSafe"
    : event.is_error
      ? "ai.agentSteps.toolError"
      : "ai.agentSteps.toolDone"
...
function maxTextareaHeight() {
  ...
  return Math.max(
    AI_TEXTAREA_MIN_HEIGHT_PX,
    Math.floor(panelHeight * AI_TEXTAREA_MAX_PANEL_RATIO - promptPanelChromeHeight),
  )
}
  ...
  <div ref="assistantRootRef" class="flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <ScrollArea v-else ref="scrollRef" class="min-h-0 flex-1 overflow-hidden">
      <div class="flex flex-col gap-3 p-3">
            ...
            <div class="max-w-[85%]">
              <template v-if="editingMessageIndex === i">
                ...
                <div class="mt-1.5 flex justify-end gap-1.5">
                  <Button size="sm" variant="ghost" class="h-6 px-2 text-[11px]" @click="cancelEdit">{{
                    t("ai.editCancel")
                  }}</Button>
                  <Button size="sm" class="h-6 px-2 text-[11px]" @click="submitEdit(i)">{{
                    t("ai.editResend")
                  }}</Button>
                </div>
              </template>
              <template v-else>
                <div class="flex items-start gap-1">
                  <button
                    v-if="!isGenerating"
                    class="mt-1 hidden h-5 w-5 shrink-0 items-center justify-center rounded text-muted-foreground hover:bg-muted hover:text-foreground group-hover:flex"
          ...
          <div v-else-if="msg.content || msg.reasoning || msg.isThinking" class="flex">
            <div class="max-w-[95%] min-w-0 rounded-lg bg-muted px-3 py-2 text-xs leading-relaxed">
              ...
              <div v-if="msg.agentSteps?.length" class="mb-2 space-y-1">
                ...
                >
                  <button
                    class="flex w-full items-center gap-1 px-2 py-1.5 text-left"
                    @click="step.toolResult || step.toolArgs?.sql ? toggleStep(step.key) : undefined"
                  >
                    ...
                    <ChevronRight
                      v-if="step.toolResult || step.toolArgs?.sql"
                      class="ml-auto h-3 w-3 shrink-0 transition-transform duration-150"
                      :class="{ 'rotate-90': expandedSteps.has(step.key) }"
                    />
                  </button>
                  <div v-if="expandedSteps.has(step.key)" class="border-t border-current/10 px-2 pb-2 pt-1">
                    <div
                      v-if="step.toolArgs?.sql"
                      class="mb-1 rounded bg-background/50 px-2 py-1 font-mono text-[10px] text-foreground/80 whitespace-pre-wrap"
                    >
                      {{ step.toolArgs.sql }}
                    </div>
                    <Button
                      v-if="step.toolName === 'explain_query' && step.toolArgs?.sql"
                      size="sm"
                      variant="outline"
                      class="mb-1 h-6 gap-1 text-[10px]"
                      @click="emit('openExplainPlan', step.toolArgs.sql as string)"
                    >
                      <GitBranch class="h-3 w-3" />
                      {{ t("explain.title") }}
                    </Button>
                    <div
                      v-if="step.toolName === 'explain_query' && step.explainData && connection?.db_type"
                      class="mb-1"
                    >
                      <ExplainPlanViewer
                    ...
                    <div v-else-if="step.isError && step.toolResult" class="text-[10px] text-red-600 dark:text-red-400">
                      {{ step.toolResult }}
                    </div>
                    <div
                      v-else-if="step.toolResult"
                      class="max-h-48 overflow-auto text-[10px] text-muted-foreground whitespace-pre-wrap"
                    >
                      {{ step.toolResult }}
                    </div>
                  </div>
        ...
        <div
          v-if="agentTokens && !isGenerating"
          class="flex items-center gap-1 text-[10px] text-muted-foreground px-2 pb-1"
        >
          <span
            >&#8593;{{ agentTokens.input.toLocaleString() }} &#8595;{{
              agentTokens.output.toLocaleString()
            }}
            tokens</span
          >
        </div>
      ...
      <div ref="promptPanelRef" class="relative rounded-[6px] border bg-background">
        ...
        <div class="px-2 pb-2 pt-1">
          <div
            ...
            class="flex items-center gap-1 mb-1 text-xs text-foreground/80"
          >
            ...
            <template v-if="connection">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem
                    v-if="!dbSelectOptions.length && connection && tab"
                    :value="selectedDatabaseSelectValue"
                    >{{ selectedDatabaseLabel }}</SelectItem
                  >
                </SelectContent>
              </Select>
            </template>
            ...
            <template v-if="settings.isConfigured()">
              ...
              <Popover v-model:open="providerSelectorOpen">
                <PopoverTrigger as-child>
                  <button
                    type="button"
                    class="min-w-0 flex shrink items-center gap-1.5 max-w-[220px] rounded-[6px] border px-2 py-0.5 text-[11px] text-muted-foreground hover:bg-muted hover:text-foreground"
                  >
                    <AiProviderLogo
                      :provider="settings.aiConfig.provider"
                      :label="AI_PROVIDER_PRESETS[settings.aiConfig.provider]?.label ?? settings.aiConfig.provider"
                      :icon-slug="AI_PROVIDER_PRESETS[settings.aiConfig.provider]?.iconSlug"
                      class="h-3 w-3 shrink-0"
                    />
                    <span class="min-w-0 truncate">{{
                      modelLoading ? t("ai.loadingModels") : settings.aiConfig.model
                    }}</span>
                    <svg
                ...
                <PopoverContent align="end" class="w-72 gap-0 p-1.5" @open-auto-focus.prevent>
                  ...
                  <template v-if="configuredProviders.length">
                    ...
                    <button
                      ...
                      @click="handleProviderSwitch(p)"
                    >
                      <AiProviderLogo
                        :provider="p"
                        :label="AI_PROVIDER_PRESETS[p]?.label ?? p"
                        :icon-slug="AI_PROVIDER_PRESETS[p]?.iconSlug"
                        class="h-3.5 w-3.5 shrink-0"
                      />
                      <span class="font-medium">{{ AI_PROVIDER_PRESETS[p]?.label ?? p }}</span>
                      <span class="ml-auto min-w-0 truncate text-[11px] text-muted-foreground">{{
                        settings.aiProviderConfigs[p]?.model
                      }}</span>
                    </button>
                  ...
                  >
                    ...
                    <template #option-label="{ option, label }">
                      <span class="flex min-w-0 flex-col leading-tight">
                        ...
                        <span
                          v-if="modelOptionSecondary(option, label)"
                          class="mt-0.5 truncate text-[11px] text-muted-foreground"
                          >{{ modelOptionSecondary(option, label) }}</span
                        >
                      </span>
                    </template>
                  </SearchableSelect>
