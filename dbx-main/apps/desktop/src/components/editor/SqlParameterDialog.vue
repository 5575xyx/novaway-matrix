...
function updateValue(name: string, value: string) {
  ...
  values.value[name] = {
    ...(values.value[name] ?? { kind: "string" }),
    ...(matchedHistory ? { kind: matchedHistory.kind } : {}),
    value,
  }
}
...
<template>
  <Dialog v-model:open="open">
    <DialogContent
      class="max-h-[86vh] border border-border !bg-background text-foreground shadow-2xl !backdrop-blur-none sm:max-w-[720px]"
    >
      <DialogHeader>
      ...
      <div class="grid max-h-[calc(86vh-8rem)] gap-4 overflow-y-auto pr-1">
        ...
        <div class="relative z-20 max-h-[302px] overflow-auto rounded-md border bg-background">
          <div class="min-w-[580px]">
            <div
              class="sticky top-0 z-10 grid grid-cols-[minmax(140px,1fr)_132px_minmax(180px,1.5fr)] border-b bg-muted px-3 py-2 text-xs font-medium text-muted-foreground"
            >
            ...
            <div
              v-for="name in parameters"
              :key="name"
              class="grid grid-cols-[minmax(140px,1fr)_132px_minmax(180px,1.5fr)] items-center gap-2 border-b px-3 py-2 text-sm last:border-b-0"
            >
              <div class="min-w-0 truncate font-mono text-xs">{{ name }}</div>
              <Select
                :model-value="values[name]?.kind || 'string'"
                @update:model-value="(value) => updateKind(name, value as SqlParameterValueKind)"
              >
              ...
              <div class="relative min-w-0">
                <Popover :open="activeHistoryName === name && filteredSqlParameterHistory(name).length > 0">
                  ...
                  <PopoverContent
                    align="start"
                    side="bottom"
                    class="z-[80] w-[var(--reka-popover-trigger-width)] max-h-40 gap-0 overflow-auto p-1"
                  ...
                  >
                    ...
                    >
                      ...
                      <span class="shrink-0 text-[10px] uppercase text-muted-foreground">{{
                        t(`sqlParameters.kind.${entry.kind}`)
                      }}</span>
                    </button>
                  </PopoverContent>
                </Popover>
        ...
        <div class="relative z-10 grid gap-2">
          ...
          <pre
            class="max-h-48 min-w-0 overflow-auto rounded-md bg-muted px-3 py-3 text-xs font-mono whitespace-pre"
