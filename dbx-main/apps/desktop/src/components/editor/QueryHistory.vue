...
import {
  hasHistoryDateRange,
  historyDateRangeIsValid,
  historyEntryMatchesDateRange,
  type HistoryDateRange,
} from "@/lib/history/historyTimeRange"
  ...
  <div class="h-full flex flex-col overflow-hidden border-l">
    ...
    <div class="border-b shrink-0">
            ...
            <div class="flex flex-wrap items-end gap-2">
              <label class="grid min-w-0 gap-1 text-[11px] text-muted-foreground">
                ...
                <input
                  ...
                  v-model="dateRangeDraft.startDate"
                  type="date"
                  class="sr-only"
                  tabindex="-1"
                  aria-hidden="true"
                ...
                <button
                  type="button"
                  class="flex h-8 w-28 min-w-0 items-center gap-1.5 rounded-md border border-input bg-background px-2 text-left text-xs text-foreground outline-none hover:bg-muted/50 focus:border-ring focus:ring-2 focus:ring-ring/30"
                  @click="openDatePicker(startDateInputRef)"
                >
                  <span
                    class="min-w-0 flex-1 truncate tabular-nums"
                    :class="{ 'text-muted-foreground': !dateRangeDraft.startDate }"
                  >
              ...
              <label class="grid min-w-0 gap-1 text-[11px] text-muted-foreground">
                ...
                <input
                  ref="endDateInputRef"
                  v-model="dateRangeDraft.endDate"
                  type="date"
                  class="sr-only"
                  tabindex="-1"
                  aria-hidden="true"
                ...
                <button
                  type="button"
                  class="flex h-8 w-28 min-w-0 items-center gap-1.5 rounded-md border border-input bg-background px-2 text-left text-xs text-foreground outline-none hover:bg-muted/50 focus:border-ring focus:ring-2 focus:ring-ring/30"
                  @click="openDatePicker(endDateInputRef)"
                >
                  <span
                    class="min-w-0 flex-1 truncate tabular-nums"
                    :class="{ 'text-muted-foreground': !dateRangeDraft.endDate }"
                  >
      ...
      <div v-if="hasDateFilter" class="flex items-center gap-1 px-2 pb-2">
        <button
          type="button"
          class="flex min-w-0 items-center gap-1 rounded border border-primary/30 bg-primary/10 px-1.5 py-0.5 text-[11px] font-medium text-primary hover:bg-primary/15"
          :title="t('history.dateRange.title')"
          @click="setDateRangeOpen(true)"
        >
        ...
        <button
          type="button"
          class="flex h-5 w-5 shrink-0 items-center justify-center rounded text-muted-foreground hover:bg-muted hover:text-foreground"
          :title="t('history.dateRange.clear')"
