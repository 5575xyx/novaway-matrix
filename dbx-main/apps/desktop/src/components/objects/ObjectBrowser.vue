...
const tableStructureDatabaseType = computed(
  () => tableStructureDatabaseTypeForConnection(props.connection) ?? props.connection.db_type,
)
...
const objectGridMinWidth = computed(() => {
  return (
    objectBrowserColumns.value.reduce((total, key) => total + objectColumnWidths.value[key], 0) +
    Math.max(0, objectBrowserColumns.value.length - 1) * 12 +
    24
  )
})
...
async function openSource(row: ObjectBrowserRow) {
  ...
  try {
    ...
    sourceContent.value = await formatSqlForDisplay(
      editable,
      sourceFormatDialect.value,
...
async function fetchSortedTableRowsForDrop(): Promise<ObjectBrowserRow[]> {
  ...
  const fkResults = await Promise.all(
    rows.map((row) =>
      api
        .listForeignKeys(props.connection.id, props.database, row.schema || selectedSchema.value || "", row.name)
...
function getTableMenuItems(item: ObjectBrowserRow): ContextMenuItem[] {
  return [
    ...
    ...(canOpenTableImport.value
      ? [{ label: t("contextMenu.importData"), action: () => openTableImport(item), icon: Upload }]
      : []),
...
<template>
  <div
    ref="rootRef"
    data-object-browser-root
    class="flex h-full min-h-0 min-w-0 flex-col bg-background outline-none"
    tabindex="0"
    @keydown="onObjectBrowserKeydown"
  >
    <div class="flex h-10 shrink-0 items-center gap-2 border-b px-3">
      <div class="flex min-w-0 items-center gap-2">
        <span
          class="inline-flex max-w-[14rem] min-w-0 items-center rounded border border-border bg-muted/50 px-2 py-0.5 text-xs font-medium truncate"
          :title="selectedSchema || props.database"
        >
          {{ selectedSchema || props.database }}
        </span>
        <span
          v-if="selectedSchema"
          class="inline-flex max-w-[14rem] min-w-0 items-center rounded border border-border bg-muted/30 px-2 py-0.5 text-xs text-muted-foreground truncate"
      ...
      <Button
        v-if="canPasteTableClipboard()"
        variant="ghost"
        size="sm"
        class="h-7 px-2 text-xs"
        @click="openPasteTableDialog"
      >
    ...
    <div v-else class="flex min-h-0 min-w-0 flex-1 flex-col">
      <div class="object-browser-table flex min-h-0 min-w-0 flex-1 flex-col overflow-x-auto overflow-y-hidden">
        <div
          class="grid h-7 shrink-0 items-center gap-3 border-b bg-muted/40 px-3 text-xs font-medium text-muted-foreground"
          :style="{ gridTemplateColumns, minWidth: `${objectGridMinWidth}px` }"
        >
          <div class="relative flex min-w-0 items-center">
            ...
            <div
              ...
              @mousedown="onObjectColumnResizeStart('select', $event)"
              @dblclick="resetObjectColumnWidth('select', 34, $event)"
            >
          ...
          <div class="relative flex min-w-0 items-center">
            <button
              class="flex min-w-0 items-center gap-1 truncate pr-4 text-left"
              type="button"
            ...
            <div
              class="absolute -right-2 top-0 bottom-0 z-10 flex w-3 cursor-col-resize items-center justify-center text-muted-foreground/70 hover:bg-primary/30 hover:text-primary"
              @mousedown="onObjectColumnResizeStart('name', $event)"
              @dblclick="resetObjectColumnWidth('name', 260, $event)"
            >
              <GripVertical class="h-3 w-3" />
            </div>
          </div>
          <div class="relative flex min-w-0 items-center">
            ...
            <div
              class="absolute -right-2 top-0 bottom-0 z-10 flex w-3 cursor-col-resize items-center justify-center text-muted-foreground/70 hover:bg-primary/30 hover:text-primary"
              @mousedown="onObjectColumnResizeStart('type', $event)"
              @dblclick="resetObjectColumnWidth('type', 110, $event)"
            >
              <GripVertical class="h-3 w-3" />
            </div>
          </div>
          <div class="relative flex min-w-0 items-center">
            <button
              class="flex min-w-0 items-center gap-1 truncate pr-4 text-left"
              type="button"
              :title="t('objects.statisticsHint')"
              @click="toggleSort('estimatedRows')"
            >
              <span class="truncate">{{ t("objects.rows") }}</span>
              <component
                :is="sortIconFor('estimatedRows')"
                v-if="sortIconFor('estimatedRows')"
        ...
        <RecycleScroller
          ...
          :style="{ minWidth: `${objectGridMinWidth}px` }"
          :items="filteredRows"
          :item-size="34"
          :buffer="600"
          :skip-hover="true"
            ...
            <CustomContextMenu :items="getObjectBrowserMenuItems(item)" v-slot="{ onContextMenu }">
              ...
              >
                ...
                <div class="flex min-w-0 items-center gap-2">
                  ...
                  <span class="truncate text-[13px] font-medium text-foreground" :title="item.displayName">{{
                    item.displayName
                  }}</span>
                  <span
                    v-if="item.partitionCount"
                ...
                <div
                  ...
                  :title="item.estimatedRows == null ? '' : formatObjectBrowserCount(item.estimatedRows)"
                >
                  {{ formatObjectBrowserCount(item.estimatedRows) }}
                </div>
                <div
                  class="truncate text-xs tabular-nums text-muted-foreground"
                  :title="item.totalBytes == null ? '' : formatObjectBrowserCount(item.totalBytes)"
                >
