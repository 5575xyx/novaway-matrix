...
const selectedMetadata = computed(() =>
  selectedFile.value?.metadata ? JSON.stringify(selectedFile.value.metadata, null, 2) : "",
)
...
async function uploadFile() {
  ...
  try {
    ...
    const fileId = await api.documentUploadGridFsFile(
      props.connectionId,
      props.database,
      props.bucket,
      selected.name,
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="min-w-0">
          ...
          <div class="text-xs text-muted-foreground">
            {{ files.length }} {{ t("gridfsBrowser.fileCount") }} / {{ formatBytes(totalBytes) }}
          </div>
        </div>
        <div class="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            class="h-8 gap-1.5"
            :disabled="isReadonly || !selectedFile || deleting"
        ...
        <table class="min-w-full border-collapse text-sm">
          ...
          <tbody>
            <tr
              ...
              :class="{ 'bg-accent/45': selectedFileId === file.id }"
              @click="selectedFileId = file.id"
              @dblclick="downloadFile(file)"
            >
              <td class="px-4 py-2 align-top">
                <div class="font-medium">{{ displayName(file) }}</div>
                <div v-if="file.aliases?.length" class="mt-1 text-xs text-muted-foreground">
                  {{ file.aliases.join(", ") }}
                </div>
              </td>
        ...
        <template v-if="selectedFile">
          ...
          <div class="mt-5 space-y-4 text-sm">
            ...
            <div v-if="selectedMetadata">
              ...
              <pre class="mt-1 max-h-44 overflow-auto rounded bg-muted px-3 py-2 text-xs whitespace-pre-wrap">{{
                selectedMetadata
              }}</pre>
            </div>
          </div>
