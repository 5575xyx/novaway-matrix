...
<template>
  <div class="flex h-full min-h-0 flex-col">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="min-w-0">
          ...
          <div class="text-xs text-muted-foreground">
            {{ buckets.length }} {{ t("gridfsBrowser.bucketCount") }} / {{ totalFiles }}
            {{ t("gridfsBrowser.fileCount") }} / {{ formatBytes(totalBytes) }}
          </div>
        </div>
        <div class="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            class="h-8 gap-1.5"
            :disabled="isReadonly || !selectedBucket || deleting"
    ...
    <div
      v-else-if="loading && buckets.length === 0"
        ...
        <table class="min-w-full border-collapse text-sm">
          ...
          <tbody>
            <tr
              ...
              :class="{ 'bg-accent/45': selectedBucketName === bucket.name }"
              @click="selectedBucketName = bucket.name"
              @dblclick="openBucket(bucket.name)"
            >
      ...
      <DialogContent class="sm:max-w-[420px]">
        ...
        <div class="space-y-2 py-2">
          ...
          <Input
            ...
            :placeholder="t('gridfsBrowser.bucketNamePlaceholder')"
            @keydown.enter="createBucket"
          />
        </div>
        <DialogFooter>
          <Button variant="outline" :disabled="creating" @click="showCreateDialog = false">{{
            t("common.cancel")
          }}</Button>
    ...
    <DangerConfirmDialog
      ...
      :loading="deleting"
      :title="t('gridfsBrowser.deleteTitle')"
      :message="t('gridfsBrowser.deleteMessage')"
      :details="selectedBucketName"
      :confirm-label="t('gridfsBrowser.deleteBucket')"
