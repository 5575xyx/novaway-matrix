...
const deleteDetails = computed(() => {
  ...
  if (pending.kind === "document") {
    ...
    if (props.databaseType === "elasticsearch")
      return `Elasticsearch index: ${props.collection}\nDocument _id: ${String(id)}`
...
async function gridSave(changes: DocumentGridChanges) {
  ...
  for (const [rowIdx, dirtyCols] of changes.dirtyRows) {
    ...
    if (isEs) {
      ...
      await api.documentUpdateDocument(
        props.connectionId,
        props.database,
        props.collection,
        String(id),
    ...
    if (isEs) {
      ...
      if (id) {
        await api.documentUpdateDocument(
          ...
          id,
          JSON.stringify(doc),
          documentRoutingFromGridRow(newRow, cols),
        )
      } else {
...
function buildElasticsearchPartialUpdateDocument(
  changes: Map<number, MongoInputValue>,
  columns: string[],
...
async function previewDocumentChanges(changes: DocumentGridChanges): Promise<string[]> {
  ...
  for (const [rowIdx, dirtyCols] of dirtyRows) {
    ...
    if (isEs) {
      ...
      stmts.push(
        `POST /${coll}/_update/${elasticsearchPathIdPreview(String(id))}${elasticsearchRoutingPreview(routing)}\n${JSON.stringify({ doc: updateDoc.$set ?? updateDoc }, null, 2)}`,
      )
    } else {
  ...
  for (const rowIdx of deletedRows) {
    ...
    if (isEs) {
      const routing = documentRoutingFromGridRow(row, columns)
      stmts.push(
        `DELETE /${coll}/_doc/${elasticsearchPathIdPreview(String(id))}${elasticsearchRoutingPreview(routing)}`,
      )
    } else {
...
async function load() {
  ...
  const previousSelectedId =
    previousSelectedIdx === null ? null : documentIdentity(documents.value[previousSelectedIdx])
  try {
    const filter = currentDocumentFilter()
    const sort = currentDocumentSortJson(sortInput.value)
    const result = await api.documentFindDocuments(
      props.connectionId,
      props.database,
      props.collection,
...
function syncSelectedDocumentAfterLoad(previousSelectedIdx: number | null, previousSelectedId: string | null) {
  ...
  const nextIdx = previousSelectedId
    ? documents.value.findIndex((doc) => documentIdentity(doc) === previousSelectedId)
    : previousSelectedIdx < documents.value.length
      ? previousSelectedIdx
      : -1
  if (nextIdx < 0) {
    selectedIdx.value = null
...
function startEdit() {
  ...
  editFields.value = Object.entries(doc).map(([name, value]) => {
    const readonlyMetadata =
      name === "_id" || (documentStoreProvider.value.kind === "elasticsearch" && name === "_routing")
...
function buildObjectFromNodes(nodes: EditNode[], path: string): JsonRecord {
  ...
  for (const field of nodes) {
    ...
    if (
      !name ||
      (!path && (name === "_id" || (documentStoreProvider.value.kind === "elasticsearch" && name === "_routing")))
    )
      continue
  ...
  try {
    ...
    if (isNew.value) {
      ...
      await api.documentUpdateDocument(
        ...
        String(id),
        JSON.stringify(doc),
        documentRoutingFromDocument(current),
      )
    }
    isEditing.value = false
    isNew.value = false
...
async function applyDeleteDoc(idx: number) {
  ...
  error.value = ""
  try {
    await api.documentDeleteDocument(
      props.connectionId,
      props.database,
      props.collection,
      String(id),
      documentRoutingFromDocument(doc),
    )
      ...
      >
        <div ref="tableSearchSplitContainerRef" class="flex flex-1 min-w-0">
          <div class="flex flex-1 items-center gap-1 px-2 py-0.5 min-w-0" :style="tableFindPaneStyle">
            <Popover v-model:open="documentFilterBuilderOpen">
              <PopoverTrigger as-child>
                <button
                  ...
                  :class="
                    hasLocalColumnFilters || appliedDocumentFilter
                ...
                >
                  ...
                  <span
                    v-if="localFilterCount + documentStructuredFilterCount"
              ...
              <PopoverContent align="start" class="w-[360px] max-w-[calc(100vw-24px)] gap-3 p-3">
                  ...
                  <template v-for="(rule, index) in documentFilterRules" :key="rule.id">
                    ...
                    >
                      <Select
                        ...
                        @update:model-value="
                          (value: any) => updateDocumentFilterRule(rule.id, { fieldName: String(value) })
                        "
                      >
                        <SelectTrigger
                        ...
                        <SelectContent position="popper">
                          <SelectItem
                            v-for="fieldName in documentFilterFieldOptions"
                            :key="fieldName"
                            :value="fieldName"
                          >
                            {{ fieldName }}
                      ...
                      <Select
                        ...
                        @update:model-value="
                          (value: any) => updateDocumentFilterRule(rule.id, { mode: value as DocumentFilterMode })
                        "
                      >
                        <SelectTrigger
                          class="h-8 w-full min-w-0 overflow-hidden text-xs [&_[data-slot=select-value]]:min-w-0 [&_[data-slot=select-value]]:truncate"
                        ...
                        <SelectContent position="popper">
                          <SelectItem
                            v-for="option in documentFilterModeOptions"
                            :key="option.value"
                            :value="option.value"
                          >
                            {{ t(option.labelKey) }}
                      ...
                      <Input
                        ...
                        @update:model-value="
                          (value) => updateDocumentFilterRule(rule.id, { rawValue: String(value ?? '') })
                        "
                      ...
                      <Button
                        ...
                        :disabled="documentFilterRules.length === 1"
                        @click="removeDocumentFilterRule(rule.id)"
                      >
                ...
                <div class="flex items-center justify-between gap-2 pt-1">
                  <Button
                    ...
                    @click="clearDocumentFilters(clearLocalFilter)"
                  >
                    {{ t("grid.clearFilter") }}
                  </Button>
            ...
            <span class="text-blue-600 dark:text-blue-400 text-xs font-medium select-none shrink-0">{{
              documentStoreProvider.filterInputLabel
            }}</span>
            <input
              v-model="filterInput"
              autocapitalize="off"
          ...
          <div class="flex flex-1 items-center gap-1 px-2 py-0.5 min-w-0">
            <span class="text-orange-600 dark:text-orange-400 text-xs font-medium select-none shrink-0">{{
              documentStoreProvider.sortInputLabel
            }}</span>
            <input
              v-model="sortInput"
              autocapitalize="off"
