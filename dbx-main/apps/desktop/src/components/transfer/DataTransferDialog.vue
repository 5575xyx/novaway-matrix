...
async function loadTables() {
  ...
  try {
    if (isMongoConnection(sourceConnectionId.value)) {
      sourceTables.value = (await api.mongoListCollections(sourceConnectionId.value, sourceDatabase.value)).map(
        (c) => c.name,
      )
                ...
                >
                  <template #option-label="{ option, label }">
                    <div class="flex items-center gap-1.5">
                      <DatabaseIcon
                        :db-type="sqlConnections.find((c) => c.id === option)?.db_type ?? 'mysql'"
                        class="w-3.5 h-3.5"
                      />
                      {{ label }}
                    </div>
