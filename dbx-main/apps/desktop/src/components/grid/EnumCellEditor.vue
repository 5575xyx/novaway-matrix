...
<template>
  ...
  >
    ...
    <PopoverContent
      align="start"
      side="bottom"
      class="w-auto min-w-[120px] rounded-md p-1"
      @click.stop
    ...
    >
      <div class="flex flex-col gap-0.5">
        <button
          v-if="nullable"
          type="button"
          class="flex items-center rounded-sm px-2 py-1 text-xs text-muted-foreground hover:bg-accent hover:text-accent-foreground"
          :class="{ 'bg-accent text-accent-foreground': modelValue === '' }"
          @click="selectValue('')"
        >
          (NULL)
        </button>
        <button
          ...
          :key="val"
          type="button"
          class="flex items-center rounded-sm px-2 py-1 text-xs hover:bg-accent hover:text-accent-foreground"
          :class="{ 'bg-accent text-accent-foreground': modelValue === val }"
