...
import {
  buildDataGridColumnDistinctValuesSql,
...
import {
  canApplyGridSelectionValue,
  canDeleteGridRowItem,
...
import {
  ...
  type CellDetailTab,
} from "@/lib/dataGrid/cellDetailPresentation"
...
} from "@/lib/dataGrid/paginationPageSize"
import {
...
} from "@/lib/dataGrid/dataGridColumnVisibility"
import {
  columnOrderKeysForIndexes,
  isDefaultColumnOrder,
  moveVisibleColumnIndex,
  orderedColumnIndexes,
  uniqueDataGridColumnOrderKeys,
} from "@/lib/dataGrid/dataGridColumnOrder"
import {
...
import {
  forgetDataGridConditionHistory,
  loadDataGridConditionHistory,
  rememberDataGridConditionHistory,
} from "@/lib/dataGrid/dataGridConditionHistory"
...
const autoRefreshLabel = computed(() =>
  autoRefreshEnabled.value
    ? t("tabs.autoRefreshEvery", { seconds: autoRefreshIntervalSeconds.value })
    : t("tabs.autoRefresh"),
)
...
const resolvedDatabaseType = computed(
  () => props.databaseType ?? effectiveDatabaseTypeForConnection(connectionStore.getConfig(props.connectionId ?? "")),
)
...
const compactDataGridToolbar = computed(
  () => dataGridTopbarWidth.value > 0 && dataGridTopbarWidth.value < DATA_GRID_COMPACT_TOPBAR_WIDTH,
)
...
const conditionHistoryPreview = ref<{
  value: string
  left: number
  top: number
  maxWidth: number
...
const whereHistoryEmptyText = computed(() =>
  whereFilterInput.value.trim() ? t("grid.conditionHistoryNoMatches") : t("grid.conditionHistoryEmpty"),
)
...
const showOrderBySuggestionDropdown = computed(
  () => orderBySuggestions.value.length > 0 || orderByHistoryMenuOpen.value,
)
const orderByHistoryEmptyText = computed(() =>
  orderByInput.value.trim() ? t("grid.conditionHistoryNoMatches") : t("grid.conditionHistoryEmpty"),
)
...
function conditionExpandedHeight(input: HTMLTextAreaElement): number {
  ...
  const verticalInset = paneRect
    ? Math.max(0, inputRect.top - paneRect.top) + Math.max(0, paneRect.bottom - inputRect.bottom)
    : 0
  ...
  return Math.min(
    Math.max(minimumHeight, contentHeight + verticalInset),
    Math.min(CONDITION_EXPANDED_MAX_HEIGHT, availableHeight),
  )
}
...
function resetConditionOverlayScrollIfContentFits(kind: ConditionInputKind) {
  requestAnimationFrame(() => {
    ...
    if (
      ...
      pane.getBoundingClientRect().height >= CONDITION_EXPANDED_MAX_HEIGHT - 1
    )
      return
    overlay.scrollTop = 0
...
function resizeConditionInput(
  kind: ConditionInputKind,
  options: { focusOverlay?: boolean; expandFromFocus?: boolean; immediateHeight?: boolean } = {},
) {
  nextTick(() => {
    const input = conditionInputRef(kind)
    if (!input) return
    ...
    const expanded =
      focused &&
      conditionInputShouldExpand(input) &&
      (options.expandFromFocus || document.activeElement === conditionOverlayRef(kind))
    ...
    if (expanded)
      setConditionRect(kind, (conditionPaneRef(kind) ?? conditionControlRef(kind) ?? input).getBoundingClientRect())
...
const structuredFilterCacheKey = computed(
  () =>
    props.cacheKey ||
    [
      props.connectionId ?? "",
...
const structuredFilterScopeKey = computed(() =>
  [
    props.connectionId ?? "",
    props.database ?? "",
    props.schema ?? "",
    ...
    props.tableMeta?.schema ?? "",
    props.tableMeta?.tableName ?? "",
    filterBuilderColumnOptions.value.join("\0"),
  ].join("\u0001"),
)
...
const structuredFilterCount = computed(
  () =>
    structuredFilterRules.value.filter(
      (rule) =>
        !rule.disabled && !!rule.columnName && (!filterModeNeedsValue(rule.mode) || rule.rawValue.trim().length > 0),
    ).length,
)
...
function columnFilterPanelTitle(columnName: string): string {
  return localFilterDraft.value?.mode === "server"
    ? t("grid.databaseValueFilterFor", { column: columnName })
    : t("grid.localFilterFor", { column: columnName })
}
...
function updateWhereSuggestionPosition() {
  nextTick(() => {
    ...
    if (!input) return
    const inputRect = input.getBoundingClientRect()
    whereSuggestionPosition.value = getDataGridConditionSuggestionPosition(inputRect, {
...
function onConditionSuggestionDocumentPointerDown(event: PointerEvent) {
  ...
  if (
    !eventTargetInside(
      event,
      whereFilterInputRef.value,
      whereFilterOverlayRef.value,
      whereConditionPaneRef.value,
      whereExpandedPane,
      whereSuggestionDropdownRef.value,
    )
  ) {
    dismissWhereSuggestions()
  }
  if (
    !eventTargetInside(
      event,
      orderByInputRef.value,
      orderByOverlayRef.value,
      orderByConditionPaneRef.value,
      orderByExpandedPane,
      orderBySuggestionDropdownRef.value,
    )
  ) {
    dismissOrderBySuggestions()
  }
...
function deleteWhereHistorySuggestion(value: string) {
  ...
  whereSuggestionIndex.value = whereSuggestions.value.length
    ? Math.min(whereSuggestionIndex.value, whereSuggestions.value.length - 1)
    : -1
  whereHistoryMenuOpen.value = true
  updateWhereSuggestionPosition()
}
...
function updateOrderBySuggestionPosition() {
  nextTick(() => {
    ...
    if (!input) return
    const inputRect = input.getBoundingClientRect()
    orderBySuggestionPosition.value = getDataGridConditionSuggestionPosition(inputRect, {
      viewportWidth: window.innerWidth,
    })
  })
}
...
function deleteOrderByHistorySuggestion(value: string) {
  ...
  orderBySuggestionIndex.value = orderBySuggestions.value.length
    ? Math.min(orderBySuggestionIndex.value, orderBySuggestions.value.length - 1)
    : -1
  orderByHistoryMenuOpen.value = true
  updateOrderBySuggestionPosition()
}
...
const displayableColumnIndexes = computed(() =>
  props.result.columns
    ...
    .filter(
      ({ column }) =>
        !isHiddenGridColumn(props.databaseType, column, props.tableMeta?.primaryKeys ?? [], props.tableMeta?.tableType),
    )
    .map(({ index }) => index),
)
...
const filteredGoToColumns = computed(() => {
  ...
  return goToColumnItems.value.filter(
    (column) =>
      column.name.toLocaleLowerCase().includes(query) || column.sourceName?.toLocaleLowerCase().includes(query),
  )
})
...
const visibleColumnIndexes = computed(() =>
  visibleColumnIndexesForFilter(orderedDisplayableColumnIndexes.value, hiddenColumnIndexes.value),
)
...
const visibleColumnTypes = computed(() =>
  visibleColumnIndexes.value.map((index) => {
    ...
    return (
      ...
      props.result.column_types?.[index]
    )
  }),
)
...
const hasCustomColumnOrder = computed(
  () => !isDefaultColumnOrder(displayableColumnIndexes.value, orderedDisplayableColumnIndexes.value),
)
...
function updateGridHorizontalScrollbar(element: HTMLElement | null = gridScrollerElement()) {
  ...
  gridHorizontalScrollbarThumbLeftPercent.value =
    maxScrollLeft > 0 ? (element.scrollLeft / maxScrollLeft) * thumbTravel : 0
}
...
function columnHeaderInteractiveTarget(target: EventTarget | null): boolean {
  return (
    ...
    !!target.closest(
      "button, input, textarea, select, [contenteditable='true'], [role='button'], [data-column-resize-handle]",
    )
  )
}
...
function columnHeaderDragClass(visibleColIdx: number) {
  const state = columnHeaderDragState.value
  return {
    "z-30 shadow-lg ring-1 ring-primary/40 bg-background dark:bg-muted pointer-events-none":
      state?.dragging && state.sourceVisibleIndex === visibleColIdx,
  }
}
...
function columnHeaderStyle(visibleColIdx: number) {
  ...
  return {
    ...
    transition:
      columnHeaderDragState.value?.sourceVisibleIndex === visibleColIdx ? undefined : "transform 120ms ease-out",
  }
}
...
const showQueryEditReadyBadge = computed(
  () => isResultsContext.value && hasData.value && !!props.editable && (!!props.tableMeta || !!props.customSaveHandler),
)
const queryEditReadyTargetLabel = computed(
  () => props.tableMeta?.tableName ?? props.customSaveHandler?.targetLabel ?? "",
)
const showKeylessEditWarning = computed(
  () =>
    !!props.editable &&
    !!props.tableMeta &&
    canUseKeylessRowPredicate(props.databaseType, props.tableMeta.primaryKeys ?? []),
)
...
const customReadonlyColumns = computed(
  () => new Set((props.customSaveHandler?.readonlyColumns ?? []).map((column) => column.toLowerCase())),
)
...
function cellUsesExpandedEditor(rowId: number | undefined, columnIndex: number): boolean {
  return (
    ...
    expandedCellEditor.value.rowId === rowId &&
    expandedCellEditor.value.col === columnIndex
  )
}
...
function startDomCellEdit(rowId: number, columnIndex: number, displayText: string, event: MouseEvent) {
  ...
  startCellEdit(
    rowId,
    columnIndex,
    cellEditContentNeedsExpandedEditor({ displayText, editText, target: event.currentTarget }),
  )
}

function cellEditContentNeedsExpandedEditor(options: {
  displayText: string
  editText: string
...
function isIntegerColumnType(dataType: string): boolean {
  return /^(tinyint|smallint|mediumint|int|integer|bigint|serial|smallserial|bigserial|int2|int4|int8|uint|uint8|uint16|uint32|uint64)\b/.test(
    dataType,
  )
}
...
const hasActiveFilter = computed(
  () =>
    (dataGridSearchMode.value === "filter" && !!deferredClientSearchText.value) ||
...
const rowDetail = computed(() => {
  ...
  return buildDataGridRowDetail({
    ...
    isEditableColumn: (columnIndex) =>
      canEditGridCellDetail({ canEditCell: canEditCellItem(item, columnIndex), isDraft: !!item.isDraft }),
  })
})
...
const activeBinaryHexRows = computed(() =>
  activeBinaryHexBytes.value ? buildBinaryHexViewRows(activeBinaryHexBytes.value) : [],
)
...
const sideDetailPrioritizesValue = computed(
  () => !cellDetailPanelIsBottom.value && !isEditingDetail.value && !!activeCellDetail.value?.formattedJson,
)
const sideDetailValueFillsHeight = computed(
  () =>
    cellDetailPanelIsBottom.value ||
    isEditingDetail.value ||
    (!cellDetailPanelIsBottom.value && !activeCellDetail.value?.imagePreviewUrl),
)
...
const sideDetailEditorStyle = computed(() => {
  ...
  const lines = detailEditValue.value
    .split(/\r\n|\r|\n/)
    .reduce((total, line) => total + Math.max(1, Math.ceil(line.length / SIDE_DETAIL_EDITOR_SOFT_WRAP_CHARS)), 0)
  const height = Math.min(
    SIDE_DETAIL_EDITOR_MAX_HEIGHT,
    Math.max(SIDE_DETAIL_EDITOR_MIN_HEIGHT, lines * SIDE_DETAIL_EDITOR_LINE_HEIGHT + 28),
  )
...
function applyColumnSort(
  ...
  columnIndex: number,
  direction: "asc" | "desc" | null,
  mode: DataGridSortMode = "database",
) {
...
function rowNumberText(item: RowItem | undefined): string {
  ...
  return String(
    infiniteScrollEnabled.value
      ? item.displayIndex + 1
      : item.displayIndex + 1 + (currentPage.value - 1) * pageSize.value,
  )
}
...
const canvasRenderStyleKey = computed(
  () =>
    `${settingsStore.editorSettings.theme}:${settingsStore.editorSettings.uiScale}:${canvasBackingPixelRatio.value}:${isDark.value}:${themePalette.value}:${settingsStore.editorSettings.fontFamily}:${tableFontSize.value}`,
)
...
const canvasEditingCellStyle = computed(() => {
  ...
  const clippedRight =
    viewportWidth > 0 ? Math.min(viewportWidth, cell.rect.left + cell.rect.width) : cell.rect.left + cell.rect.width
  return {
    left: `${clippedLeft}px`,
...
const exportMenuItems = computed(() => {
  ...
  const allResultItems =
    (props.allExportResults?.length ?? 0) > 1
      ? [{ value: "all-results-xlsx", label: t("grid.exportAllResultsXlsx"), separatorBefore: true }]
      : []
...
function applyVisibleSelectedCellValue(
  item: RowItem,
  visibleCol: number,
  value: string | null,
  allowDraft = selectedRangeTargetsOnlyDraftRow(),
): boolean {
...
function prepareDataCellMouseDown(item: RowItem, actualColIdx: number) {
  const editing = editingCell.value
  if (
    editing?.rowId === quickEntryDraftRowId &&
    item.isDraft &&
    item.id === quickEntryDraftRowId &&
    editing.col !== actualColIdx
  ) {
...
const CELL_DETAIL_TABLE_MIN_VISIBLE_HEIGHT = Math.ceil(
  CELL_DETAIL_TABLE_HEADER_HEIGHT +
    CANVAS_DATA_GRID_ROW_HEIGHT * CELL_DETAIL_TABLE_MIN_VISIBLE_ROWS +
    CELL_DETAIL_TABLE_HORIZONTAL_SCROLLBAR_HEIGHT,
)
...
watch(
  [showCellDetail, activeCellDetail, cellDetailPanelIsBottom, detailPanelHeight],
  scheduleActiveCellEditTextareaResize,
)
...
const canOpenTableStructureEditor = computed(
  () =>
    !!props.connectionId &&
    !!props.database &&
    !!props.tableMeta?.tableName &&
    supportsTableStructureEditing(resolvedDatabaseType.value),
)
const mongoConnectionConfig = computed(() => connectionStore.getConfig(props.connectionId ?? ""))
const canManageMongoIndexes = computed(
  () =>
    resolvedDatabaseType.value === "mongodb" &&
    !!props.connectionId &&
    !!props.database &&
    !!props.tableMeta?.tableName &&
    mongoConnectionConfig.value?.db_type === "mongodb" &&
    mongoConnectionConfig.value?.driver_profile !== "mongodb-legacy",
)
...
function openTableStructureEditor() {
  if (!props.connectionId || !props.database || !props.tableMeta?.tableName || !canOpenTableStructureEditor.value)
    return
  queryStore.openTableStructure(
    props.connectionId,
    props.database,
    props.tableMeta.schema,
    props.tableMeta.tableName,
    activeTableInfoTab.value,
  )
}
...
const dropAllMongoIndexesConfirmMessage = computed(() =>
  t("contextMenu.confirmDropMongoAllIndexesMessage", { name: props.tableMeta?.tableName || "" }),
)
...
const dropMongoIndexPreview = computed(() =>
  pendingDropMongoIndex.value
    ? `db.getCollection(${JSON.stringify(props.tableMeta?.tableName || "")}).dropIndex(${JSON.stringify(pendingDropMongoIndex.value.name)})`
    : "",
)
const dropAllMongoIndexesPreview = computed(
  () => `db.getCollection(${JSON.stringify(props.tableMeta?.tableName || "")}).dropIndexes()`,
)
...
async function confirmDropMongoIndex() {
  if (
    ...
    !props.tableMeta?.tableName ||
    !pendingDropMongoIndex.value ||
    dropMongoIndexLoading.value
  )
    return
  dropMongoIndexLoading.value = true
  try {
    await connectionStore.ensureConnected(props.connectionId)
    await api.mongoDropIndexes(
      props.connectionId,
      props.database,
      props.tableMeta.tableName,
      JSON.stringify(pendingDropMongoIndex.value.name),
      true,
    )
...
async function confirmDropAllMongoIndexes() {
  ...
  try {
    await connectionStore.ensureConnected(props.connectionId)
    const result = await api.mongoDropIndexes(
      props.connectionId,
      props.database,
      props.tableMeta.tableName,
      undefined,
    ...
    toast(
      t("contextMenu.dropAllIndexesSuccess", { count: result.dropped_names.length, name: props.tableMeta.tableName }),
      3000,
    )
...
const gridContextMenuItems = computed<ContextMenuItem[]>(() => {
  ...
  if (contextColumn.value) {
    ...
    if (sortCol.value) {
      items.push({
        label: t("grid.clearSort"),
        action: () => applyContextSort(null, sortMode.value),
        icon: ArrowUpDown,
      })
    }
  ...
  if (props.editable && contextRowItem.value) {
    ...
    if (contextRowItem.value.isDeleted) {
      ...
      items.push({
        ...
        action: () =>
          isMultiRow.value
            ? requestDeleteRows(deletableRowIds(affectedRowIds()))
            : requestDeleteRow(contextRowItem.value!.id),
        icon: Trash2,
...
<template>
  <div
    ...
    :class="{ 'data-grid--editing-cell': !!editingCell }"
    :style="gridStyle"
    tabindex="0"
  ...
  >
    <CustomContextMenu :items="gridContextMenuItems" v-slot="{ onContextMenu }">
      ...
      >
        ...
        <div
          ...
          class="data-grid-topbar-shell shrink-0 flex min-w-0 border-b bg-muted/20"
        >
          <div
          ...
          >
            <div
              class="data-grid-topbar flex items-stretch relative"
              :class="{ 'data-grid-topbar--compact': compactDataGridToolbar }"
            >
              <div
                v-if="useTransaction && editable && hasDataGridSaveTarget"
              ...
              <template v-if="canShowWhereSearch">
                <div ref="searchSplitContainerRef" class="flex flex-1 min-w-0">
                  <div
                    ref="whereConditionPaneRef"
                    class="flex flex-1 items-center gap-1 px-2 py-0.5 min-w-0 relative"
                    :class="{ 'border-l': useTransaction && editable && hasDataGridSaveTarget }"
                  ...
                  >
                            ...
                            >
                              ...
                              <div class="flex items-center gap-1">
                                <Button
                                  ...
                                  @click="updateStructuredFilterRule(rule.id, { disabled: !rule.disabled })"
                                >
                    ...
                    <div ref="whereConditionControlRef" class="flex min-w-0 flex-1 items-center gap-1">
                      <span
                        class="data-grid-topbar-condition-label data-grid-topbar-condition-label--where"
                        :class="{ 'data-grid-topbar-condition-label--compact': compactDataGridToolbar }"
                        >WHERE</span
                      >
                      ...
                      <button
                        ...
                        type="button"
                        @mousedown.prevent="openWhereHistoryMenu"
                      >
                    ...
                    <Teleport to="body">
                      <div
                        v-if="whereConditionRendered"
                        class="data-grid-topbar-condition-pane--expanded flex min-w-0 items-start gap-1"
                        :class="{ 'border-l': useTransaction && editable && hasDataGridSaveTarget }"
                        :style="conditionPaneStyle('where')"
                      >
                        <textarea
                        ...
                        />
                        <div
                          class="data-grid-topbar-condition-floating-controls pointer-events-none absolute inset-x-2 z-[1] flex h-6 min-w-0 items-center gap-1"
                          :style="conditionTopbarOverlayStyle('where')"
                        >
                          <button
                            type="button"
                    ...
                    <Teleport to="body">
                      <div
                        v-if="showWhereSuggestionDropdown"
                        ref="whereSuggestionDropdownRef"
                        class="fixed z-[90] min-w-[180px] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md"
                      ...
                      >
                        <div
                          ...
                          @mouseenter="
                            whereSuggestionIndex = idx
                            sug.kind === 'history'
                              ? showConditionHistoryPreview(sug.value, $event)
                              : hideConditionHistoryPreview()
                          "
                          @mouseleave="hideConditionHistoryPreview"
                        >
                          <Search class="w-3 h-3 mr-2 text-muted-foreground shrink-0" />
                          <span data-condition-history-text class="min-w-0 flex-1 truncate font-mono">{{
                            sug.value
                          }}</span>
                          <button
                            v-if="sug.kind === 'history'"
                            class="shrink-0 rounded p-0.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
                            type="button"
                            @mousedown.stop.prevent="deleteWhereHistorySuggestion(sug.value)"
                          >
                            <X class="h-3 w-3" />
                          </button>
                        </div>
                  ...
                  <div
                    ref="orderByConditionPaneRef"
                    class="flex flex-1 items-center gap-1 px-2 py-0.5 border-r min-w-0 relative"
                  >
                    <div ref="orderByConditionControlRef" class="flex min-w-0 flex-1 items-center gap-1">
                      <span
                        class="data-grid-topbar-condition-label data-grid-topbar-condition-label--order"
                        :class="{ 'data-grid-topbar-condition-label--compact': compactDataGridToolbar }"
                        >ORDER BY</span
                      >
                      <div class="relative h-6 min-w-0 flex-1 overflow-hidden">
                      ...
                      <button
                        class="text-muted-foreground hover:text-foreground shrink-0"
                        type="button"
                        @mousedown.prevent="openOrderByHistoryMenu"
                      >
                        <ChevronDown class="w-3 h-3" />
                      </button>
                      <button
                    ...
                    <Teleport to="body">
                      <div
                        v-if="orderByConditionRendered"
                        class="data-grid-topbar-condition-pane--expanded flex min-w-0 items-start gap-1 border-r"
                        :style="conditionPaneStyle('orderBy')"
                      >
                        <textarea
                        ...
                        <div
                          class="data-grid-topbar-condition-floating-controls pointer-events-none absolute inset-x-2 z-[1] flex h-6 min-w-0 items-center gap-1"
                          :style="conditionTopbarOverlayStyle('orderBy')"
                        >
                          <span
                            class="data-grid-topbar-condition-label data-grid-topbar-condition-label--order"
                            :class="{ 'data-grid-topbar-condition-label--compact': compactDataGridToolbar }"
                    ...
                    <Teleport to="body">
                      <div
                        v-if="showOrderBySuggestionDropdown"
                        ref="orderBySuggestionDropdownRef"
                        class="fixed z-[90] min-w-[180px] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md"
                      ...
                      >
                        ...
                        >
                          ...
                          <button
                            ...
                            class="shrink-0 rounded p-0.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
                            type="button"
                            @mousedown.stop.prevent="deleteOrderByHistorySuggestion(sug.value)"
                          >
                            <X class="h-3 w-3" />
                          </button>
                        </div>
                    ...
                    <Teleport to="body">
                      <div
                        v-if="conditionHistoryPreview"
                        class="pointer-events-none fixed z-[140] rounded-md bg-foreground shadow-xl"
                        :style="conditionHistoryPreviewStyle"
                      >
                        <span
                          class="absolute h-3 w-3 rotate-45 bg-foreground"
                          :class="conditionHistoryPreview.side === 'left' ? '-left-1.5' : '-right-1.5'"
                          :style="conditionHistoryPreviewArrowStyle"
                        />
                        <div
                          class="max-h-[min(320px,calc(100vh-16px))] overflow-auto rounded-md px-3 py-2 font-mono text-xs leading-relaxed whitespace-pre-wrap break-words text-background"
                        >
          ...
          <div class="flex shrink-0 items-center gap-1 px-1 ml-auto" @wheel="onDataGridTopbarFixedActionWheel">
            ...
            <Tooltip>
              <TooltipTrigger as-child>
                <Button
                  ...
                  size="sm"
                  :class="[
                    'data-grid-topbar-action-button h-5 shrink-0 text-xs px-1.5',
                    compactDataGridToolbar ? 'data-grid-topbar-action-button--compact' : '',
                    isSaving ? '' : '',
                  ]"
                  :disabled="isSaving"
                ...
                >
                  ...
                  <span
                    ...
                    :class="{ 'data-grid-topbar-action-label--compact': compactDataGridToolbar }"
                    >{{ t("grid.refresh") }}</span
                  >
                </Button>
              </TooltipTrigger>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger as-child>
                ...
                >
                  ...
                  <span
                    class="data-grid-topbar-action-label"
                    :class="{ 'data-grid-topbar-action-label--compact': compactDataGridToolbar }"
                    >{{ autoRefreshEnabled ? `${autoRefreshIntervalSeconds}s` : t("tabs.autoRefreshShort") }}</span
                  >
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" class="w-40">
                ...
                <DropdownMenuItem
                  v-for="seconds in AUTO_REFRESH_INTERVAL_OPTIONS"
                  :key="seconds"
                  class="gap-2"
                  @select="setAutoRefreshInterval(seconds)"
                >
                ...
                <Popover v-model:open="goToColumnOpen">
                  <PopoverTrigger as-child>
                    <Button
                      ...
                      :class="[
                        'data-grid-topbar-action-button h-5 shrink-0 text-xs px-1.5',
                        compactDataGridToolbar ? 'data-grid-topbar-action-button--compact' : '',
                        goToColumnOpen ? 'text-primary bg-primary/10' : '',
                      ]"
                    >
                  ...
                  <PopoverContent align="end" class="w-56 p-2" @keydown="onGoToColumnKeydown">
                    <div class="relative mb-1">
                      ...
                      />
                      <input
                        v-model="goToColumnSearch"
                        :placeholder="t('grid.searchColumn')"
                        class="h-8 w-full rounded-md border bg-transparent pl-7 pr-6 text-xs outline-none focus-visible:border-ring/50 focus-visible:ring-1 focus-visible:ring-ring/25"
                      />
                      <button
                        v-if="goToColumnSearch"
                        type="button"
                        class="absolute right-1.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"
                        @click="goToColumnSearch = ''"
                      >
                        <X class="h-3.5 w-3.5" />
                      </button>
                    </div>
                    <div class="max-h-56 overflow-auto rounded border">
                      ...
                      <div
                        v-if="!filteredGoToColumns.length"
              ...
              <Tooltip v-if="pendingChangeCount > 0">
                <TooltipTrigger as-child>
                  <Button
                    ...
                    size="sm"
                    :class="[
                      'data-grid-topbar-action-button h-5 shrink-0 text-xs px-1.5 text-sky-600 hover:bg-sky-500/10 hover:text-sky-700',
                      compactDataGridToolbar ? 'data-grid-topbar-action-button--compact' : '',
                    ]"
              ...
              <Tooltip>
                <TooltipTrigger as-child>
                  <Button
                    ...
                    size="sm"
                    :class="[
                      'data-grid-topbar-action-button data-grid-topbar-action-button--commit relative h-5 shrink-0 text-xs px-1.5',
                      compactDataGridToolbar ? 'data-grid-topbar-action-button--compact' : '',
                    ]"
                  ...
                  >
                    ...
                    <Save
                      v-else-if="compactDataGridToolbar || pendingChangeCount === 0"
                      class="data-grid-topbar-action-icon w-3 h-3"
                    />
                    <span
                    ...
                    <span
                      class="data-grid-topbar-action-label"
                      :class="{ 'data-grid-topbar-action-label--compact': compactDataGridToolbar }"
                      >{{ t(saveActionMode.labelKey, { count: pendingChangeCount }) }}</span
                    >
                  </Button>
                </TooltipTrigger>
                <TooltipContent side="bottom" class="max-w-sm">
                  {{ t(saveActionMode.tooltipKey, { count: pendingChangeCount }) }} ({{ saveShortcutLabel }})
                </TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger as-child>
                  ...
                  >
                    ...
                    <span
                      class="data-grid-topbar-action-label"
                      :class="{ 'data-grid-topbar-action-label--compact': compactDataGridToolbar }"
                      >{{ t(saveActionMode.secondaryActionKey) }}</span
                    >
                  </Button>
                </TooltipTrigger>
          ...
          <div class="col-start-1 row-start-1 flex flex-col min-w-0 overflow-hidden relative">
            ...
            <div v-else-if="isTransposeMode" class="flex-1 flex flex-col min-h-0 overflow-hidden">
              ...
              >
                <template #before>
                  <div
                    class="data-grid-transpose-header sticky top-0 z-20 flex h-7 border-b border-border bg-[rgb(239_239_239)] font-semibold text-muted-foreground dark:bg-muted"
                ...
                <template #default="{ item }">
                  ...
                  >
                    ...
                    <div
                      ...
                      @dblclick.stop="
                        canEditCellItem(displayItems[cell.recordIndex], cell.valueIndex) &&
                        startDomCellEdit(displayItems[cell.recordIndex].id, cell.valueIndex, cell.display, $event)
                      "
                    >
                      <template
                        v-if="
                      ...
                      <template v-else>
                        <template v-if="draftCellPlaceholder(displayItems[cell.recordIndex], cell.valueIndex)">
                          <span class="text-muted-foreground/70 italic">{{
                            draftCellPlaceholder(displayItems[cell.recordIndex], cell.valueIndex)
                          }}</span>
                        </template>
                        <template v-else>{{ cell.display }}</template>
                        <div
                          v-if="cellDetailButtonVisible(cell.recordIndex, cell.valueIndex)"
                          class="absolute right-2 top-0.5 flex items-center gap-1"
                        >
                          <LightDropdownMenu
                            ...
                            @select="
                              (mode: string) =>
                                downloadCellBinaryValue(
                                  cell.recordIndex,
                                  cell.valueIndex,
                                  mode as BinaryCellDownloadMode,
                                )
                            "
                          >
            ...
            <template v-else>
              ...
              <div
                ref="headerRef"
                class="shrink-0 bg-[rgb(239_239_239)] dark:bg-muted/60 z-10 w-full border-y border-border overflow-hidden"
              >
                <div class="data-grid-header-row flex w-(--header-total-w) font-semibold text-foreground">
                  <div
                    ...
                    :class="{
                      '!bg-gray-300 dark:!bg-gray-900 outline outline-primary -outline-offset-1': isSelectingAll,
                    }"
                  ...
                  >
                    <div
                      ...
                      :class="{
                        '!bg-gray-300 dark:!bg-gray-900 outline outline-primary -outline-offset-1':
                        ...
                        'bg-amber-500/20 ring-1 ring-inset ring-amber-500/40':
                          currentSearchMatch?.kind === 'column' && currentSearchMatch.col === col.actualColIdx,
                    ...
                    >
                      <span class="flex min-w-0 items-center gap-1 overflow-hidden">
                        ...
                        <LightDropdownMenu
                          ...
                          :match-trigger-width="false"
                          @update:open="
                            (value: boolean) => (headerSortMenuOpenColumn = value ? col.actualColIdx : null)
                          "
                        ...
                        >
                          <template #trigger="{ open, toggle }">
                            <button
                              ...
                              :class="
                                columnIsSorted(col.name, col.actualColIdx) ? 'text-primary opacity-100' : 'opacity-80'
                              "
                              :title="t('grid.sort')"
                              :aria-expanded="open"
                              @mousedown.stop
                            ...
                            >
                              <ArrowUp
                                v-if="columnIsSorted(col.name, col.actualColIdx) && sortDir === 'asc'"
                                class="h-3 w-3 shrink-0"
                              />
                              <ArrowDown
                                v-else-if="columnIsSorted(col.name, col.actualColIdx) && sortDir === 'desc'"
                                class="h-3 w-3 shrink-0"
                              />
                        ...
                        >
                          ...
                          <PopoverContent
                            align="start"
                            side="bottom"
                            class="w-[420px] max-w-[calc(100vw-2rem)] gap-0 overflow-hidden rounded-xl border bg-popover p-0 text-popover-foreground shadow-xl"
                            @click.stop
                            @keydown.stop
                          >
                        ...
                        >
                          ...
                          >
                            ...
                            <div
                              v-if="
                                localFilterDraft?.mode === 'server' &&
                                (serverFilterLoading || serverFilterError || serverFilterLimited)
                              "
                              class="flex items-center gap-1.5 border-b px-2 py-1 text-[11px] text-muted-foreground"
                            >
                              ...
                              <span class="min-w-0 truncate">
                                ...
                                <template v-else>{{
                                  t("grid.serverValuesLimited", { count: SERVER_COLUMN_FILTER_LIMIT })
                                }}</template>
                              </span>
                            </div>
                            <div class="max-h-72 overflow-auto py-0.5">
                              ...
                              <div
                                v-if="
                                  localFilterDraft?.mode === 'local' &&
                                  localFilterAllOptions.length > localFilterOptions.length
                                "
                        ...
                        <button
                          ...
                          class="flex h-4 w-4 shrink-0 items-center justify-center rounded text-muted-foreground hover:bg-gray-200 dark:hover:bg-gray-800 hover:text-foreground"
                          :class="
                            localFilterOpenColumn === col.actualColIdx && localFilterDraft?.mode === 'server'
                  ...
                  <div
                    ...
                    class="shrink-0 border-l border-border w-(--grid-scrollbar-gutter)"
                  />
                </div>
              </div>
              ...
              >
                <div class="relative" :style="{ width: `${totalWidth}px`, height: `${canvasContentHeight}px` }">
                  ...
                  <div
                    ...
                    class="canvas-grid-overlay dbx-data-grid-font-family sticky left-0 top-0 z-10 overflow-visible"
                    :style="canvasOverlayStyle"
                  >
                    <div
                      v-if="canvasEditingCell"
                      class="absolute pointer-events-auto z-20 tabular-nums"
                ...
                <div
                  v-if="infiniteScrollEnabled && infiniteScrollLoading"
                  class="absolute bottom-0 left-0 right-0 flex items-center justify-center py-2 text-xs text-muted-foreground bg-background/80 backdrop-blur-sm z-10"
                >
                  ...
                  >
                    ...
                    <div
                      ...
                      :class="{
                        ...
                        'overflow-visible z-20 border-r-transparent':
                          editingCell?.rowId === item.id && editingCell?.col === col.actualColIdx,
                      ...
                      @dblclick="
                        canEditCellItem(item, col.actualColIdx) &&
                        startDomCellEdit(
                          item.id,
                          col.actualColIdx,
                      ...
                      :data-visible-col-index="col.visibleColIdx"
                      @contextmenu="
                        onCellContext(item.id, item.displayIndex, col.actualColIdx, col.visibleColIdx, $event)
                      "
                    >
              ...
              <div
                v-if="infiniteScrollEnabled && infiniteScrollLoading && !loading"
              ...
              <div
                v-if="hasGridHorizontalOverflow"
                ref="gridHorizontalScrollbarTrackRef"
                class="data-grid-horizontal-scrollbar"
                :class="{ 'data-grid-horizontal-scrollbar--dragging': gridHorizontalScrollbarDragging }"
                @pointerdown="startGridHorizontalScrollbarDrag"
              >
                <div class="data-grid-horizontal-scrollbar__thumb" :style="gridHorizontalScrollbarThumbStyle" />
              </div>
              <div
                v-if="hasGridVerticalOverflow"
                ref="gridVerticalScrollbarTrackRef"
                class="data-grid-vertical-scrollbar"
                :class="{ 'data-grid-vertical-scrollbar--dragging': gridVerticalScrollbarDragging }"
                @pointerdown="startGridVerticalScrollbarDrag"
              >
          ...
          >
            ...
            <div class="flex items-center gap-2 px-3 py-1.5 border-b shrink-0 bg-muted/20 h-9">
              ...
              <div
                ...
                class="table-info-actions flex min-w-0 shrink-0 items-center gap-1"
              >
                <Button
                  variant="ghost"
                  size="sm"
                  class="table-info-action-button h-6 px-2 text-xs"
              ...
              <div
                v-else-if="activeTableInfoTab === 'indexes' && canManageMongoIndexes"
                class="table-info-actions flex min-w-0 shrink-0 items-center gap-1"
              >
                <Button
                  variant="ghost"
                  size="sm"
                  class="table-info-action-button h-6 px-2 text-xs text-destructive hover:text-destructive"
                  :disabled="indexesLoading || dropAllMongoIndexesLoading || droppableMongoIndexes.length === 0"
                  @click="requestDropAllMongoIndexes"
                >
              ...
              <Button
                v-if="canOpenTableStructureEditor"
                variant="ghost"
                size="sm"
                class="table-info-action-button h-6 px-2 text-xs"
                :title="t('contextMenu.editStructure')"
            ...
            <div class="grid border-b bg-background shrink-0" :style="tableInfoTabListStyle">
              <button
                ...
                :class="
                  activeTableInfoTab === tab.id
                    ? 'border-primary bg-gray-300/80 text-foreground dark:bg-gray-700/80'
                    : 'border-transparent text-muted-foreground hover:bg-gray-200 hover:text-foreground dark:hover:bg-gray-800/50'
                "
                :title="tab.label"
              ...
              <div v-else class="divide-y">
                <div v-for="index in filteredIndexes" :key="index.name" class="p-3 text-xs">
                  <div class="flex items-start gap-2">
                    ...
                    <Button
                      v-if="canManageMongoIndexes && !index.is_primary"
                      variant="ghost"
                      size="sm"
                      class="h-7 shrink-0 px-2 text-[11px] text-destructive hover:text-destructive"
                      @click="requestDropMongoIndex(index)"
                    >
          ...
          >
            ...
            <Tabs v-model="activeCellDetailTab" class="flex-1 min-h-0 gap-0">
              ...
              <TabsContent value="details" class="m-0 min-h-0 flex-1 flex flex-col">
                <div
                  data-native-clipboard
                  class="flex-1 min-h-0 overflow-auto px-3 pt-3 text-xs"
                  :class="[
                    sideDetailValueFillsHeight ? 'flex flex-col gap-3' : 'space-y-3',
                    isEditingDetail && !cellDetailPanelIsBottom ? 'pb-1' : 'pb-3',
                  ]"
                >
                  <div
                    v-if="cellDetailPanelIsBottom"
                  ...
                  <div
                    class="space-y-1"
                    :class="[
                      { 'min-h-0 flex flex-col': sideDetailValueFillsHeight },
                      sideDetailValueFillsHeight && !(activeCellDetail.imagePreviewUrl && !isEditingDetail)
                        ? 'flex-1'
                        : '',
                      activeCellDetail.imagePreviewUrl && !isEditingDetail ? 'shrink-0' : '',
                    ]"
                  >
                    <div class="flex min-h-5 items-center justify-between gap-2">
                      ...
                      <div class="flex items-center gap-1">
                        <Button
                          v-if="showCompactDetailJson"
                          variant="ghost"
                          size="sm"
                          class="h-5 gap-1 px-1.5 text-xs"
                          :disabled="!canCompactDetailJson"
                          :title="t('grid.compactJson')"
                          @click="compactDetailJson"
                        >
                          <Code2 class="h-3 w-3" />
                          {{ t("grid.compactJson") }}
                        </Button>
                        <Button
                          v-if="!isEditingDetail && activeCellDetail.formattedJson"
                          :variant="sideDetailJsonView ? 'secondary' : 'ghost'"
                          size="sm"
                          class="h-5 gap-1 px-1.5 text-xs"
                          :title="t('grid.formattedJson')"
                          @click="toggleCellDetailJsonFormatted"
                        >
                          <Code2 class="h-3 w-3" />
                          {{ t("grid.formattedJson") }}
                        </Button>
                        <Button
                          v-if="!isEditingDetail && activeCellDetail.isEditable"
                          variant="ghost"
                          size="icon"
                          class="h-5 w-5"
                    ...
                    <div v-if="activeCellDetail.imagePreviewUrl && !isEditingDetail" class="shrink-0 space-y-1.5">
                      ...
                      <a
                        ...
                        role="button"
                        class="flex overflow-hidden rounded border bg-muted/20"
                        :class="cellDetailPanelIsBottom ? 'max-h-28' : 'max-h-40'"
                    ...
                    <template v-if="isEditingDetail">
                      <div class="min-h-0 flex-1" :style="sideDetailEditorStyle">
                        ...
                        <div
                          ...
                          class="min-h-0 h-full w-full rounded border overflow-hidden"
                        />
                      </div>
                    ...
                    <div
                      ...
                      :class="[
                        ...
                        sideDetailValueFillsHeight ? 'min-h-0 flex-1' : 'h-72 max-h-[42vh]',
                      ]"
                    ...
                    <pre
                      ...
                      :class="[
                        { 'cursor-text': activeCellDetail.isEditable },
                        cellDetailPanelIsBottom && activeCellDetail.imagePreviewUrl ? 'min-h-24 max-h-32 shrink-0' : '',
                        sideDetailValueFillsHeight && !activeCellDetail.imagePreviewUrl ? 'min-h-0 flex-1' : '',
                      ]"
                ...
                <div
                  class="border-t flex gap-1 overflow-hidden bg-background p-1.5"
                  :class="cellDetailPanelIsBottom ? 'shrink-0 items-center' : 'shrink-0 flex-col'"
                >
                  ...
                  <div
                    class="flex gap-1"
                    :class="cellDetailPanelIsBottom ? 'ml-auto shrink-0 justify-end' : 'flex-col'"
                  >
                    <Button
                      v-if="activeCellDetail.isEditable && activeCellDetail.value !== null"
                      variant="ghost"
                      size="sm"
                      class="h-6 justify-start text-xs"
                      @click="setDetailNull"
                    >
                      <X class="w-3 h-3 mr-2" /> {{ t("grid.setNull") }}
                    </Button>
                    <Button variant="ghost" size="sm" class="h-6 justify-start text-xs" @click="copyDetailColumnName">
              ...
              <TabsContent
                v-if="activeCellDetailTabs.includes('hexViewer')"
                value="hexViewer"
              ...
              >
                <div class="mb-2 min-w-0 shrink-0">
                  ...
                  <div class="text-[11px] text-muted-foreground">
                    {{ t("grid.hexViewerByteCount", { count: activeBinaryHexByteCount }) }}
                  </div>
                </div>
                <div class="min-h-0 flex-1 overflow-auto rounded border bg-muted/20 font-mono text-[11px]">
                  <div
                    class="sticky top-0 grid grid-cols-[5.5rem_minmax(24rem,1fr)_8rem] gap-3 border-b bg-muted px-2 py-1 font-semibold text-muted-foreground"
                  >
                  ...
                  <div
                    v-for="row in activeBinaryHexRows"
                    :key="row.offset"
                    class="grid grid-cols-[5.5rem_minmax(24rem,1fr)_8rem] gap-3 border-b border-border/50 px-2 py-1 last:border-b-0"
                  >
                  ...
                  <div
                    v-if="activeBinaryHexRows.length === 0"
                    class="px-2 py-6 text-center font-sans text-muted-foreground"
                  >
    ...
    <div
      v-if="!isErrorResult"
      class="grid grid-cols-[max-content_minmax(0,1fr)_max-content] items-center gap-2 px-3 py-1 border-t text-xs text-muted-foreground bg-muted/30 shrink-0"
    >
      ...
      <DialogContent v-if="dialogCellDetail" class="sm:max-w-[840px] max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div class="min-h-0 flex-1 overflow-auto pr-1 text-xs space-y-4">
          ...
          <div class="space-y-2">
            ...
            <div
              v-if="cellDetailJsonView && dialogCellDetail.formattedJson"
              ref="dialogJsonPreviewContainer"
              data-cell-detail-editor-root
              class="h-[44vh] min-h-60 overflow-hidden rounded border bg-muted/20 p-3"
            />
            <pre
              v-else
    ...
    <div v-if="showSqlPreview" class="h-52 shrink-0 border-t">
      <SqlPreviewPanel
        ...
        :can-undo="canUndoPendingChange"
        :can-redo="canRedoPendingChange"
        @undo="undoGridChange"
        @redo="redoGridChange"
        @close="closeSqlPreview"
      />
    </div>
...
.data-grid-topbar {
  ...
  --data-grid-condition-font-family:
