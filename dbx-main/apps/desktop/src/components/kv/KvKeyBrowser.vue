...
import {
  buildKvKeyTree,
  flattenVisibleKvKeyTree,
  preserveKvExpandedGroupIds,
  type KvKeyTreeNode,
} from "@/lib/kv/kvKeyTree"
...
interface KvKeyBrowserApi {
  ...
  ) => Promise<{ keys: KvKeySummary[]; continuation?: string | null }>
...
const visibleRows = computed<BrowserTreeRow[]>(() => {
  ...
  return flattenVisibleKvKeyTree(tree.value, expandedGroupIds.value).map((row) => ({
    type: "node",
    node: row.node,
    depth: row.depth,
  }))
...
const selectedMetadata = computed(
  () =>
    selectedValue.value?.metadata ??
    (props.lazyHierarchy && selectedKey.value
      ? lazyTreeState.nodeByKey.get(selectedKey.value)
      : keys.value.find((key) => key.key === selectedKey.value)),
)
...
const showCreateModeSelect = computed(
  () => props.supportsCreateModes && isCreating.value && props.createModeOptions.length > 0,
)
...
const selectedValueCanPrettyJson = computed(
  () => selectedValue.value?.value?.encoding === "utf8" && prettyPrintJsonText(selectedTextValue.value).ok,
)
...
async function loadKeys(reset = true, options: LoadKeysOptions = {}) {
  ...
  try {
    const result = await props.api.listPrefix(
      props.connectionId,
      prefix.value.trim(),
      pageSize,
...
async function loadLazyRootSummary(
  rootPath: string,
  children: KvKeySummary[],
  continuation?: string | null,
): Promise<KvKeySummary | null> {
  try {
...
async function loadLazyChildren(parentKey: string, reset = true) {
  ...
  try {
    ...
    const result = await props.api.listPrefix(props.connectionId, parentKey, pageSize, continuationToUse, {
      recursive: false,
    })
...
async function loadMoreLazyChildren(parentKey: string | null) {
  ...
  try {
    const result = await props.api.listPrefix(
      props.connectionId,
      lazyTreeState.rootPath,
      pageSize,
      lazyTreeState.rootContinuation,
      { recursive: false },
    )
...
async function refreshLazyParent(parentPath: string) {
  ...
  if (normalizedParent === lazyTreeState.rootPath) {
    if (lazyTreeState.rootPath === "/") {
      const result = await props.api.listPrefix(props.connectionId, lazyTreeState.rootPath, pageSize, null, {
        recursive: false,
      })
      replaceLazyKvChildren(lazyTreeState, null, result.keys, result.continuation)
    } else {
...
function createKeyPrefix(parentPath?: string): string {
  const path = props.lazyHierarchy
    ? normalizeZooKeeperPath(parentPath ?? prefix.value)
    : (parentPath ?? prefix.value.trim())
  if (!path) return ""
  if (path === "/") return "/"
  ...
  <div class="flex h-full min-h-0 flex-col bg-background">
    <div class="flex shrink-0 items-center gap-2 border-b px-3 py-2">
      ...
      <Button
        ...
        class="h-8 gap-1.5"
        :disabled="loading"
        @click="loadKeys(true, { preserveSelection: true })"
      >
      ...
      <div class="min-h-0 border-r">
        ...
        <div v-else class="h-full overflow-auto py-1 text-sm">
          <template v-for="row in visibleRows" :key="row.type === 'node' ? row.node.id : row.id">
            <CustomContextMenu
              v-if="row.type === 'node'"
              :items="nodeContextMenuItems(row.node)"
            ...
            <div v-else class="px-2 py-1" :style="{ paddingLeft: `${8 + row.depth * 18}px` }">
              <Button
                ...
                :disabled="row.loading || loadingMore"
                @click="loadMoreLazyChildren(row.parentKey)"
              >
                <Loader2
                  v-if="row.loading || (row.parentKey === null && loadingMore)"
      ...
      <div class="min-h-0 overflow-auto">
        ...
        <div v-else class="flex min-h-full flex-col">
          <div class="flex shrink-0 items-start justify-between gap-3 border-b px-4 py-3">
            <div class="min-w-0">
              <div
                ...
                :class="{ 'text-blue-600 dark:text-blue-400': metadataStyle === 'zookeeper' }"
              >
                {{ selectedKey }}
              </div>
              <div class="mt-1 flex flex-wrap gap-1.5 text-xs text-muted-foreground">
                <template v-if="metadataStyle === 'zookeeper'">
                  <Badge
                    v-for="badge in zookeeperSummaryBadges"
                    :key="badge.label"
                    variant="outline"
                    class="rounded-full"
                  >
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col gap-4 p-4">
            <div class="min-h-0">
              <div v-if="metadataStyle === 'zookeeper'" class="mb-2 text-xs font-medium text-muted-foreground">
                {{ labels.value || "Value" }}
              </div>
              <pre
                class="dbx-editor-font-family m-0 max-h-[40vh] min-h-32 overflow-auto rounded-md border bg-muted/20 whitespace-pre-wrap break-words p-3 text-sm"
                >{{ displayedSelectedTextValue }}</pre
              >
            ...
            <div v-if="metadataStyle === 'zookeeper'" class="grid gap-3 border-t pt-4">
              ...
              <div class="grid gap-x-10 gap-y-4 sm:grid-cols-2">
                <div
                  v-for="row in zookeeperMetadataRows"
                  :key="row.label"
                  class="grid grid-cols-[minmax(96px,auto)_1fr] items-baseline gap-5 text-sm"
                >
                  <div class="text-foreground">{{ row.label }}</div>
                  <div class="dbx-editor-font-family min-w-0 break-all text-blue-600 dark:text-blue-400">
                    {{ row.value }}
                  </div>
                </div>
              </div>
