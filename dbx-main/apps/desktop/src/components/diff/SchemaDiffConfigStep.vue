...
const sqlConnections = computed(() =>
  store.connections.filter(
    (c: any) => !["mongodb", "redis", "elasticsearch", "etcd", "mq", "nacos"].includes(c.db_type),
  ),
)
          ...
          >
            <template #option-label="{ option, label }">
              <div class="flex items-center gap-2">
                <DatabaseIcon
                  :db-type="
                    sqlConnections.find((c) => c.id === option)?.driver_profile ||
                    sqlConnections.find((c) => c.id === option)?.db_type ||
                    'mysql'
                  "
                  class="w-3.5 h-3.5"
                />
