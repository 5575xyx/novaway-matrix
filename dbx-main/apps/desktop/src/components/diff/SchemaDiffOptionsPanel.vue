...
import type {
  BooleanSchemaDiffCompareOptionKey,
  SchemaDiffCompareOptions,
  SchemaDiffOptionItem,
  ...
  <div class="flex flex-col h-full">
    <div class="grid flex-1 grid-cols-[minmax(0,1fr)_minmax(300px,0.95fr)] gap-4 overflow-auto pr-1">
      ...
      <div class="grid min-w-0 grid-cols-[112px_minmax(0,1fr)] content-start gap-x-3 gap-y-2 border-l pl-4">
        ...
        <label class="pt-2 text-xs font-medium text-muted-foreground" for="schema-diff-table-include">{{
          t("schemaDiff.tableIncludePattern")
        }}</label>
        <input
          id="schema-diff-table-include"
          v-model="localOptions.tableIncludePattern"
          class="h-8 w-full rounded-md border border-input bg-background px-2 text-xs outline-none focus:ring-1 focus:ring-ring"
          :placeholder="t('schemaDiff.tableIncludePatternPlaceholder')"
        />
        <label class="pt-2 text-xs font-medium text-muted-foreground" for="schema-diff-table-exclude">{{
          t("schemaDiff.tableExcludePattern")
        }}</label>
        <input
          id="schema-diff-table-exclude"
          v-model="localOptions.tableExcludePattern"
          class="h-8 w-full rounded-md border border-input bg-background px-2 text-xs outline-none focus:ring-1 focus:ring-ring"
          :placeholder="t('schemaDiff.tableExcludePatternPlaceholder')"
        />
        <label class="pt-2 text-xs font-medium text-muted-foreground" for="schema-diff-table-priority">{{
          t("schemaDiff.tableFilterPriority")
        }}</label>
        <div>
