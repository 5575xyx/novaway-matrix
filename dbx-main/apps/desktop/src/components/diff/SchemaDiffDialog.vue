...
} from "@/lib/schema/schemaDiffMetadataLoad"
...
const schemaDiffPanelOptions = computed(() =>
  normalizeSchemaDiffCompareOptions(activeConfig.value?.options, getDbType()),
)
...
async function loadSchemaDetails(tables: TableInfo[], context: SchemaDetailLoadContext): Promise<TableSchemaDetail[]> {
  ...
  return mapWithConcurrency(tables, concurrency, async (table) => {
    ...
    const [columns, indexes, foreignKeys, triggers, ddl] = await Promise.all([
      ...
      shouldLoadIndexes(context.options)
        ? runMetadataQuery(() => api.listIndexes(context.connectionId, context.database, context.schema, table.name))
        : Promise.resolve([]),
      context.options.foreignKeys
        ? runMetadataQuery(() =>
            api.listForeignKeys(context.connectionId, context.database, context.schema, table.name),
          )
        : Promise.resolve([]),
      context.options.triggers
        ? runMetadataQuery(() => api.listTriggers(context.connectionId, context.database, context.schema, table.name))
        : Promise.resolve([]),
      runMetadataQuery(() =>
        api.getTableDdl(context.connectionId, context.database, context.schema, table.name, objectType),
      ),
    ])
...
<template>
  <Dialog v-model:open="open">
    ...
    >
      ...
      >
        <div
          class="bg-card border rounded-lg shadow-lg w-[760px] max-w-[calc(100vw-2rem)] max-h-[80vh] overflow-auto p-4"
        >
