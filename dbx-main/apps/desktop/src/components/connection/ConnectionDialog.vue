...
import {
  REDIS_SCAN_PAGE_SIZE_DEFAULT,
  REDIS_SCAN_PAGE_SIZE_MIN,
  REDIS_SCAN_PAGE_SIZE_MAX,
  REDIS_SCAN_PAGE_SIZE_OPTIONS,
} from "@/lib/redis/redisKeyPattern"
...
import {
  applyParsedConnectionUrl,
  normalizeMongoConnectionString,
  parseConnectionUrl,
} from "@/lib/connection/connectionUrl"
...
import {
  MQ_PINNED_VERSION_OPTIONS,
  pinnedVersionToSelection,
  selectionToPinnedVersion,
} from "@/lib/mq/mqPinnedVersionOptions"
import {
  mongodbAuthFailureHint,
  mongoUrlParam,
  mongoUrlParamIsTrue,
  normalizeMongoTlsFormState,
  setMongoUrlParam,
  setMongoUrlParamBoolean,
} from "@/lib/mongo/mongoConnectionOptions"
import {
  mysqlCleartextPasswordAuthEnabled,
  setMysqlCleartextPasswordAuthEnabled,
} from "@/lib/database/mysqlConnectionOptions"
import { copyToClipboard } from "@/lib/common/clipboard"
import {
  agentDriverInstallKey,
  appendAgentDriverUpdateHint,
  hasAgentDriverUpdate,
  showAgentDriverInstallHint,
  type AgentDriverInstallState,
} from "@/lib/connection/agentDriverInstallHint"
...
} from "@/lib/connection/connectionVisibleDatabases"
import {
  canSaveVisibleDatabaseSelection,
  connectionUsesVisibleSchemaFilter,
  filterDatabaseNamesForVisiblePicker,
  isSystemDatabaseName,
  normalizeVisibleDatabaseSelection,
  buildDraftVisibleSchemasConnectionId,
  normalizeVisibleSchemaSelection,
} from "@/lib/database/visibleDatabases"
...
function profileForConfig(config: ConnectionConfig) {
  ...
  if (config.db_type === "mq" && (config.external_config as MqAdminConfig | undefined)?.systemKind === "kafka")
    return "kafka"
...
function resetMqFields(config?: Partial<MqAdminConfig>) {
  ...
  mqTokenSigningMode.value =
    tokenSigning?.algorithm === "hs256" || tokenSigning?.algorithm === "rs256" ? tokenSigning.algorithm : "none"
...
function buildMqAdminConfig(): MqAdminConfig {
  ...
  if (systemKind === "kafka") {
    ...
    const securityProtocol =
      mqKafkaSecurityProtocol.value === MQ_KAFKA_SECURITY_PROTOCOL_AUTO ? "" : mqKafkaSecurityProtocol.value.trim()
...
async function tryNacosDockerConsoleFallback(
...
function installedAgentDriver(
  drivers: readonly AgentDriverInstallState[],
  key: string,
): AgentDriverInstallState | undefined {
...
function isSqlServerTlsHandshakeFailure(message: string): boolean {
  ...
  return (
    ...
    text.includes("tls") &&
    (text.includes("handshake") || text.includes("eof") || text.includes("performing i/o"))
  )
}
...
async function testConnectionWithTimeout(config: ConnectionConfig, runId: number): Promise<string> {
  ...
  void promise.catch((error) => {
    ...
    testResult.value = {
      ok: false,
      message: connectionErrorWithDriverUpdateHint(
        config,
        connectionAttemptOriginalErrorMessage(timeoutMessage, errorMessage(error)),
      ),
    }
  })
  try {
...
function applyMqKafkaBootstrapServers(
  config: LegacyConnectionConfig,
...
function isDremioGeneratedDefaultDriverClass(value: string | undefined) {
  ...
  return (
    !driverClass ||
    driverClass === DREMIO_ARROW_FLIGHT_SQL_JDBC_DRIVER_CLASS ||
    driverClass === DREMIO_LEGACY_JDBC_DRIVER_CLASS
  )
}
...
function dremioConnectionModeForConfig(
  config: Pick<ConnectionConfig, "connection_string" | "jdbc_driver_class">,
): DremioConnectionMode {
  const haystack = `${config.connection_string || ""}\n${config.jdbc_driver_class || ""}`.toLowerCase()
  return haystack.includes("jdbc:dremio:") || haystack.includes("com.dremio.jdbc.driver")
    ? "legacy"
    : "arrow-flight-sql"
}
  ...
  (config) => {
    if (config) {
      ...
      form.value = {
        ...
        driver_label:
          config.driver_label || oceanbasePatch?.driver_label || driverProfiles[profile]?.label || config.db_type,
        url_params: config.url_params || "",
        host:
          config.db_type === "h2" && h2FilePathFromJdbcUrl(config.connection_string)
            ? h2FilePathFromJdbcUrl(config.connection_string)
            : config.host,
...
const selectedHttpTunnelLayer = computed(() =>
  selectedTransportLayer.value?.type === "http_tunnel" ? selectedTransportLayer.value : null,
)
...
const usesLocalFilePathInput = computed(
  () => isLocalFileTypeDb(form.value.db_type) && (form.value.db_type !== "h2" || isH2FileMode.value),
)
...
const supportsMysqlTlsOptions = computed(
  () =>
    form.value.db_type === "starrocks" ||
    (form.value.db_type === "mysql" && !bareMysqlProfiles.has(selectedType.value)),
)
const supportsMysqlCleartextPasswordAuth = computed(
  () => form.value.db_type === "mysql" && !bareMysqlProfiles.has(selectedType.value),
)
...
const h2DriverMissing = computed(
  () =>
    form.value.db_type === "h2" &&
    isH2FileMode.value &&
    agentDrivers.value.find((d) => d.db_type === "h2")?.installed !== true,
)
...
const hasVisibleObjectFilter = computed(() =>
  visibleFilterUsesSchemas.value ? Array.isArray(visibleSchemaObjectSelection.value) : hasVisibleDatabaseFilter.value,
)
...
const visibleObjectTitleKey = computed(() =>
  visibleFilterUsesSchemas.value ? "visibleSchemas.title" : "visibleDatabases.title",
)
const visibleObjectDescriptionKey = computed(() =>
  visibleFilterUsesSchemas.value ? "visibleSchemas.description" : "visibleDatabases.description",
)
const visibleObjectSearchPlaceholderKey = computed(() =>
  visibleFilterUsesSchemas.value ? "visibleSchemas.searchPlaceholder" : "visibleDatabases.searchPlaceholder",
)
const visibleObjectSelectedCountKey = computed(() =>
  visibleFilterUsesSchemas.value ? "visibleSchemas.selectedCount" : "visibleDatabases.selectedCount",
)
const visibleObjectEmptySelectionKey = computed(() =>
  visibleFilterUsesSchemas.value ? "visibleSchemas.emptySelection" : "visibleDatabases.emptySelection",
)
const visibleObjectLoadFailedKey = computed(() =>
  visibleFilterUsesSchemas.value ? "visibleSchemas.loadFailed" : "visibleDatabases.loadFailed",
)
const visibleObjectSaveKey = computed(() =>
  visibleFilterUsesSchemas.value ? "visibleSchemas.save" : "visibleDatabases.save",
)
...
const agentInstallProgressLabel = computed(() => {
  ...
  const label =
    progress.step === "jre" ? "下载 JRE" : progress.step === "driver" ? "下载驱动" : progress.step || "安装驱动"
...
const shouldUseWideConnectionDialog = computed(
  () =>
    dialogStep.value === "config" &&
    (canChooseVisibleDatabases.value || (canChooseVisibleSchemas.value && !visibleFilterUsesSchemas.value)),
)
...
const hasRequiredConnectionTarget = computed(() => {
  ...
  if (form.value.db_type === "zookeeper")
    return !!(form.value.host || form.value.connection_string || connectionUrlInput.value.trim())
...
async function testConnection() {
  ...
  try {
    ...
    if (runId !== testRunId) return
    const shouldShowSqlServerLegacyMode =
      !fallbackMessage &&
      config.db_type === "sqlserver" &&
      !isSqlServerLegacyUnencryptedMode(config.url_params) &&
      isSqlServerTlsHandshakeFailure(message)
...
function applyDremioJdbcMetadata(config: LegacyConnectionConfig) {
  ...
  try {
    ...
    config.connection_string = dremioConnectionStringForSubmit(
...
function dremioConnectionStringForSubmit(
...
function dremioSubmitUrlParams(
...
function connectionConfigForSubmit(id: string): ConnectionConfig {
  ...
  if (
    selectedType.value === "oceanbase" &&
    (config.driver_profile === "oceanbase" || config.driver_profile === "oceanbase-oracle")
  ) {
  ...
  if (config.db_type === "mq") {
    ...
    if (mqConfig.systemKind === "kafka") {
      ...
      applyMqKafkaBootstrapServers(
        config,
        mqExtraString(extra, "bootstrapServers"),
        mqExtraString(extra, "securityProtocol"),
      )
    } else {
  ...
  if (config.db_type === "redis") {
    ...
    config.redis_scan_page_size =
      Number.isFinite(scanSize) && scanSize >= REDIS_SCAN_PAGE_SIZE_MIN && scanSize <= REDIS_SCAN_PAGE_SIZE_MAX
        ? Math.round(scanSize)
        : REDIS_SCAN_PAGE_SIZE_DEFAULT
  }
  if (config.db_type === "zookeeper") {
    ...
    const firstEndpoint = firstZooKeeperEndpoint(
      normalizedConnectString || (config.host ? `${config.host}:${config.port || 2181}` : ""),
    )
    if (firstEndpoint) {
  ...
  if (config.db_type === "h2") {
    ...
    if (h2Mode === "file") {
      ...
      config.port = 0
      config.connection_string = isH2SplitJdbcUrl(config.connection_string)
        ? h2FileJdbcUrlWithPath(config.connection_string, filePath)
        : h2FileJdbcUrlWithPath(undefined, filePath)
...
async function openVisibleDatabasesPicker() {
  ...
  try {
    ...
    const initialSelection = visibleFilterUsesSchemas.value
      ? Array.isArray(configuredSchemas)
        ? normalizeVisibleSchemaSelection(configuredSchemas, names)
        : names
      : initialVisibleDatabaseSelection(names, form.value.visible_databases, draftConfig)
    visibleDatabaseSelection.value = new Set(initialSelection)
    visibleDatabaseShowSystem.value =
      !visibleFilterUsesSchemas.value &&
      initialSelection.some((database) => isSystemDatabaseName(draftConfig.db_type, database))
...
function applyConnectionDraftToConfig(
  config: Omit<ConnectionConfig, "id">,
  draft: ConnectionDeepLinkDraft,
...
function applyPrestoSqlBuiltinDriverPathsIfAvailable() {
  ...
  selectedJdbcDriverPath.value =
    jdbcDriverSelectItems.value.find((item) => paths.every((path) => item.paths.includes(path)))?.id ?? ""
      ...
      <template v-else>
              ...
              <div class="connection-form-body grid gap-4 py-4 pr-2 max-h-[65vh] overflow-y-auto">
                ...
                <div v-if="selectedType === 'gbase'" class="grid grid-cols-4 items-center gap-4">
                  ...
                  <div class="col-span-3 flex gap-2">
                    <Button
                      ...
                      @click="switchGbaseProfile('gbase8a')"
                    >
                      GBase 8a
                    </Button>
                    <Button
                ...
                <template v-if="isJdbcConnection">
                  <div v-if="form.driver_profile === 'dremio'" class="grid grid-cols-4 items-center gap-4">
                    ...
                    <div class="col-span-3 flex gap-2">
                      <Button
                        size="sm"
                        :variant="dremioConnectionMode === 'arrow-flight-sql' ? 'default' : 'outline'"
                        @click="applyDremioConnectionMode('arrow-flight-sql')"
                      >
                        {{ t("connection.dremioArrowFlightSqlMode") }}
                      </Button>
                      <Button
                        size="sm"
                        :variant="dremioConnectionMode === 'legacy' ? 'default' : 'outline'"
                        @click="applyDremioConnectionMode('legacy')"
                      >
                  ...
                  <div class="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input
                      ...
                      @blur="syncDremioConnectionModeFromUrl"
                    />
                  </div>
                  <div class="grid grid-cols-4 items-center gap-4">
                  ...
                  <div class="grid grid-cols-4 items-start gap-4">
                    ...
                    <div class="col-span-3 space-y-2">
                      <Select
                        v-if="jdbcDriverSelectItems.length > 0"
                ...
                <template v-else-if="usesLocalFilePathInput">
                  ...
                  <template v-if="form.db_type === 'h2' || form.db_type === 'access'">
                    <div class="grid grid-cols-4 items-center gap-4">
                      <Label :class="connectionLabelClass"
                        >{{ t("connection.user") }}{{ form.db_type === "access" ? "（可选）" : "" }}</Label
                      >
                      <Input
                    ...
                    <div class="grid grid-cols-4 items-center gap-4">
                      <Label :class="connectionLabelClass"
                        >{{ t("connection.password") }}{{ form.db_type === "access" ? "（可选）" : "" }}</Label
                      >
                      <PasswordInput v-model="form.password" class="col-span-3" />
                    </div>
                  </template>
                ...
                <template v-else-if="form.db_type === 'mq'">
                  ...
                  <template v-if="mqSystemKind === 'kafka'">
                    ...
                    <div class="grid grid-cols-4 items-center gap-4">
                      ...
                      <Select v-model="mqKafkaSecurityProtocol">
                        ...
                        <SelectContent>
                          <SelectItem
                            v-for="option in mqKafkaSecurityProtocolOptions"
                            :key="option.value"
                            :value="option.value"
                          >
                            {{ option.label }}
                  ...
                  <div class="grid grid-cols-4 items-center gap-4">
                    ...
                    <div class="col-span-3 flex flex-wrap gap-2">
                      <Button
                        size="sm"
                        :variant="mqAuthKind === 'none' ? 'default' : 'outline'"
                        @click="mqAuthKind = 'none'"
                        >None</Button
                      >
                      <Button
                        v-if="mqSystemKind !== 'kafka'"
                        size="sm"
                        :variant="mqAuthKind === 'token' ? 'default' : 'outline'"
                        @click="mqAuthKind = 'token'"
                        >Token</Button
                      >
                      <Button
                        size="sm"
                        :variant="mqAuthKind === 'basic' ? 'default' : 'outline'"
                        @click="mqAuthKind = 'basic'"
                        >Basic</Button
                      >
                      <Button
                        v-if="mqSystemKind !== 'kafka'"
                        size="sm"
                        :variant="mqAuthKind === 'apiKey' ? 'default' : 'outline'"
                        @click="mqAuthKind = 'apiKey'"
                        >API Key</Button
                      >
                      <Button
                        v-if="mqSystemKind !== 'kafka'"
                        size="sm"
                        :variant="mqAuthKind === 'oauth2' ? 'default' : 'outline'"
                        @click="mqAuthKind = 'oauth2'"
                        >OAuth2</Button
                      >
                    </div>
                  </div>
                  ...
                  <template v-else-if="mqAuthKind === 'basic'">
                    ...
                    <div v-if="mqSystemKind === 'kafka'" class="grid grid-cols-4 items-center gap-4">
                      ...
                      <Select v-model="mqKafkaSaslMechanism">
                        ...
                        <SelectContent>
                          <SelectItem
                            v-for="option in mqKafkaSaslMechanismOptions"
                            :key="option.value"
                            :value="option.value"
                          >
                            {{ option.label }}
                  ...
                  <div v-if="mqSystemKind !== 'kafka'" class="grid grid-cols-4 items-center gap-4">
                    ...
                    <Select v-model="mqPinnedVersion">
                      ...
                      <SelectContent>
                        <SelectItem
                          v-for="option in MQ_PINNED_VERSION_OPTIONS"
                          :key="option.value"
                          :value="option.value"
                        >
                  ...
                  <div
                    v-if="mqSystemKind !== 'kafka' && mqTokenSigningMode !== 'none'"
                  ...
                  >
                    <span />
                    <p class="col-span-3 m-0 text-xs leading-5 text-muted-foreground">
                      按 Broker 的 jwt.broker.token.mode 选择：SECRET 使用 HS256，PRIVATE 使用 RS256。密钥会走连接
                      secret 存储。
                    </p>
                  </div>
                </template>
                ...
                <template v-else-if="form.db_type === 'nacos'">
                  ...
                  <div class="grid grid-cols-4 items-start gap-4">
                    <span />
                    <p class="col-span-3 m-0 text-xs leading-5 text-muted-foreground">
                      {{ t("connection.nacosConsoleUrlHint") }}
                    </p>
                  </div>
                  <div class="grid grid-cols-4 items-center gap-4">
                  ...
                  <div class="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input
                      v-model="nacosContextPath"
                      class="col-span-3"
                      :placeholder="t('connection.nacosContextPathPlaceholder')"
                    />
                  </div>
                  <div class="grid grid-cols-4 items-center gap-4">
                    ...
                    <div class="col-span-3 flex flex-wrap gap-2">
                      <Button
                        size="sm"
                        :variant="nacosAuthKind === 'none' ? 'default' : 'outline'"
                        @click="nacosAuthKind = 'none'"
                        >{{ t("connection.nacosAuthNone") }}</Button
                      >
                      <Button
                        size="sm"
                        :variant="nacosAuthKind === 'usernamePassword' ? 'default' : 'outline'"
                        @click="nacosAuthKind = 'usernamePassword'"
                        >{{ t("connection.nacosAuthUserPassword") }}</Button
                      >
                    </div>
                  </div>
                ...
                <template v-else-if="form.db_type === 'mongodb'">
                  <div class="grid grid-cols-4 items-center gap-4">
                    ...
                    <div class="col-span-3 flex items-center gap-2">
                      <Button
                        size="sm"
                        :variant="mongoDriverMode === 'legacy' ? 'outline' : 'default'"
                        @click="mongoDriverMode = 'auto'"
                        >{{ t("connection.mongoDriverAuto") }}</Button
                      >
                      <Button
                        size="sm"
                        :variant="mongoDriverMode === 'legacy' ? 'default' : 'outline'"
                        @click="mongoDriverMode = 'legacy'"
                        >{{ t("connection.mongoDriverLegacy") }}</Button
                      >
                      <Tooltip>
                        <TooltipTrigger as-child>
                  ...
                  <template v-else>
                    ...
                    <template v-if="form.ssl">
                      <div class="grid grid-cols-4 items-start gap-4">
                        <Label :class="connectionLabelClass">{{
                          t("connection.mongoTlsAllowInvalidCertificates")
                        }}</Label>
                ...
                <template v-else-if="form.db_type === 'turso'">
                  <div class="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input
                      v-model="form.host"
                      class="col-span-3"
                      placeholder="your-database.turso.io 或 libsql://your-database.turso.io"
                    />
                  </div>
                  ...
                  <div class="grid grid-cols-4 items-start gap-4">
                    <span />
                    <p class="col-span-3 text-xs text-muted-foreground">
                      支持 libsql:// 或 https:// 协议，也可以只填主机名（自动使用 HTTPS）
                    </p>
                  </div>
                  ...
                  <div class="grid grid-cols-4 items-start gap-4">
                    ...
                    <p class="col-span-3 text-xs text-muted-foreground">
                      ...
                      <code class="px-1 py-0.5 rounded bg-muted text-xs"
                        >turso db tokens create &lt;database-name&gt;</code
                      >
                      创建 token
                    </p>
                  </div>
                  ...
                  <div class="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input
                      v-model="form.url_params"
                      class="col-span-3"
                      placeholder="authToken=xxx（可选，优先使用上面的 Token 字段）"
                    />
                  </div>
                </template>
              ...
              <div class="connection-form-body grid gap-4 py-4 pr-2 max-h-[65vh] overflow-y-auto overflow-x-hidden">
                ...
                <template v-if="supportsMysqlTlsOptions">
                  <div v-if="supportsMysqlCleartextPasswordAuth" class="grid grid-cols-4 items-start gap-4">
                    <Label :class="[connectionLabelSmallPaddedClass, 'min-w-0 break-words']">{{
                      t("connection.mysqlCleartextPasswordAuth")
                    }}</Label>
              ...
              <div class="connection-form-body grid gap-4 py-4 pr-2 max-h-[65vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-4 items-center gap-4">
                  ...
                  <div class="col-span-3 flex items-center gap-2">
                    ...
                    <Input
                      v-model.number="form.keepalive_interval_secs"
                      type="number"
                      min="1"
                      max="3600"
                      step="1"
                  ...
                  <div class="col-span-3 flex flex-col gap-1">
                    <label class="flex h-5 cursor-pointer items-center gap-2">
                      ...
                      <span class="text-xs text-foreground">{{
                        t("connection.sqlServerLegacyUnencryptedModeEnable")
                      }}</span>
                    </label>
                ...
                <div v-show="form.db_type === 'redis'" class="grid grid-cols-4 items-center gap-4">
                  ...
                  <div class="col-span-3 flex flex-col gap-1">
                    <Select
                      :model-value="String(form.redis_scan_page_size ?? REDIS_SCAN_PAGE_SIZE_DEFAULT)"
                      @update:model-value="form.redis_scan_page_size = Number($event)"
                    >
                      <SelectTrigger>
                  ...
                  <div class="col-span-3 grid min-w-0 gap-3">
                    <div class="flex min-w-0 flex-wrap items-center gap-1 text-[11px] text-muted-foreground">
                      <template v-for="(segment, index) in transportPathSegments" :key="`${segment}-${index}`">
                        <span class="inline-block max-w-full truncate rounded border bg-muted/40 px-2 py-1">{{
                          segment
                        }}</span>
                ...
                <template v-if="selectedTransportLayer">
                  ...
                  <template v-if="selectedSshLayer">
                    ...
                    <div class="grid grid-cols-4 items-center gap-4">
                      ...
                      <label class="col-span-3 flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          v-model="selectedSshLayer.use_ssh_agent"
                          class="mr-0"
                          :disabled="selectedSshLayer.enabled === false"
                        />
                    ...
                    <div v-if="selectedSshLayer.use_ssh_agent" class="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input
                        v-model="selectedSshLayer.ssh_agent_sock_path"
                        class="col-span-3"
                        :placeholder="t('connection.sshAgentSockPathPlaceholder')"
                        :disabled="selectedSshLayer.enabled === false"
                      />
                    </div>
                    <div class="grid grid-cols-4 items-center gap-4">
                  ...
                  <template v-else-if="selectedHttpTunnelLayer">
                    <div class="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input
                        v-model="selectedHttpTunnelLayer.url"
                        class="col-span-3"
                        placeholder="https://dbx.example.com/dbx_tunnel.php"
                        :disabled="selectedHttpTunnelLayer.enabled === false"
                      />
                    </div>
                    <div class="grid grid-cols-4 items-center gap-4">
                      <Label :class="connectionLabelSmallClass">{{ t("connection.httpTunnelToken") }}</Label>
                      <PasswordInput
                        v-model="selectedHttpTunnelLayer.token"
                        class="col-span-3"
                        :placeholder="t('connection.httpTunnelTokenPlaceholder')"
                        :disabled="selectedHttpTunnelLayer.enabled === false"
                      />
                    </div>
                    <div class="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input
                        v-model.number="selectedHttpTunnelLayer.connect_timeout_secs"
                        type="number"
                        min="1"
                        max="300"
                        step="1"
        ...
        <DialogFooter class="flex min-w-0 items-center gap-2 sm:flex-nowrap">
          ...
          >
            ...
            {{
              hasVisibleObjectFilter
                ? visibleObjectSummary
                : visibleFilterUsesSchemas
                  ? t("contextMenu.configureVisibleObjects")
                  : t("contextMenu.selectVisibleDatabases")
            }}
          </Button>
          <Button
            v-if="canChooseVisibleSchemas && !visibleFilterUsesSchemas && hasVisibleSchemaFilter"
            variant="outline"
            class="shrink-0"
            :disabled="isTesting || isSaving || isLoadingVisibleSchemas || !hasRequiredConnectionTarget"
            @click="openVisibleSchemasPicker"
          >
      ...
      <div class="space-y-4">
        <div class="rounded-lg border bg-muted/20 p-4">
          <div class="flex items-center justify-between gap-3">
            ...
            <Loader2
              v-if="agentInstallRunning && !agentInstallError"
          ...
          <div v-if="!agentInstallError" class="mt-4 h-2 overflow-hidden rounded-full bg-muted">
            <div
              ...
              :class="{ 'animate-pulse': agentInstallPercent === null }"
              :style="{ width: `${agentInstallPercent ?? 35}%` }"
            />
          </div>
        </div>
        ...
        <div v-if="agentInstallError" class="space-y-2">
          ...
          <pre
            class="max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-md border bg-muted/30 p-3 text-xs leading-5 text-destructive"
            >{{ agentInstallError }}</pre
          >
        </div>
      </div>
  ...
  <Dialog v-model:open="showConnectionErrorDialog">
    <DialogContent class="sm:max-w-[560px]">
      ...
      <div class="space-y-2">
        ...
        <pre
          class="max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-md border bg-muted/30 p-3 text-xs leading-5 text-destructive"
          >{{ connectionErrorDetail }}</pre
        >
      </div>
    ...
    <DialogContent class="sm:max-w-[460px]">
      ...
      <div class="flex items-center gap-2 rounded-md border bg-background px-2">
        ...
        <Input
          ...
          :placeholder="t(visibleObjectSearchPlaceholderKey)"
