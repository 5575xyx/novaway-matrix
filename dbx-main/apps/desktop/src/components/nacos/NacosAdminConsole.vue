...
const serviceTotalPages = computed(() =>
  Math.max(1, Math.ceil(serviceTotal.value / Math.max(1, servicePageSize.value))),
)
...
const isConfigDirty = computed(
  () =>
    ...
    (configContent.value !== originalConfigContent.value ||
      configFormatValue(selectedConfig.value) !== configType.value),
)
const pendingDeleteDetails = computed(() =>
  pendingDeleteConfig.value ? buildNacosConfigDeleteConfirm(pendingDeleteConfig.value, namespace.value) : "",
)
const pendingHistoryRollbackDetails = computed(() =>
  pendingHistoryRollback.value
    ? buildNacosConfigHistoryRollbackConfirm(pendingHistoryRollback.value, namespace.value)
    : "",
)
const pendingInstanceDetails = computed(() =>
  pendingInstanceUpdate.value && selectedService.value
    ? buildNacosInstanceConfirm(
        selectedService.value,
        pendingInstanceUpdate.value.instance,
        pendingInstanceUpdate.value.patch,
        serviceGroup.value,
        namespace.value,
      )
    : "",
...
function currentCustomThemeColors() {
  ...
  const activeTheme =
    settings.customThemes?.find((theme) => theme.id === settings.activeCustomThemeId) || settings.customThemes?.[0]
  ...
  const view = new EditorView({
    ...
    state: EditorState.create({
      ...
      extensions: [
        ...
        configEditorFontTheme.of(
          editorFontTheme(EditorView, editorSettings.fontSize, editorSettings.fontFamily, {
            fixedHeight: true,
            scrollable: true,
          }),
        ),
...
function isSameConfigKey(item: NacosConfigItem, key: NacosConfigKey): boolean {
  return (
    (item.namespace || namespace.value || "") === (key.namespace || namespace.value || "") &&
    item.dataId === key.dataId &&
...
function saveConfigAsCopy() {
  ...
  const copy = createNacosSaveAsCopy({
    ...selectedConfig.value,
  ...
  try {
    if (isTauriRuntime()) {
      ...
      const path = await save({
        ...
        filters: [
          {
            name: configFormatDisplayLabel(configType.value || item.configType || "text"),
...
async function compareConfigHistory(item: NacosConfigHistoryItem) {
  ...
  try {
    const [current, history] = await Promise.all([
      api.nacosGetConfig(props.connectionId, selectedConfigOriginalKey.value),
      api.nacosGetConfigHistory(props.connectionId, historyKeyFor(item)),
    ])
...
async function loadServicesWithRetry(page = servicePageNo.value) {
  for (let attempt = 0; ; attempt += 1) {
    ...
    if (!isConnectionNotFoundError(servicesError.value) || attempt >= CONNECTION_NOT_FOUND_RETRY_DELAYS_MS.length)
      return
    await delay(CONNECTION_NOT_FOUND_RETRY_DELAYS_MS[attempt])
  }
}
...
watch(
  ...
  async ([settings]) => {
    const view = configEditorView.value
    if (!view) return
    const [{ EditorView }, theme] = await Promise.all([
      import("@codemirror/view"),
      loadEditorTheme(settings.theme, editorThemeAppearance(), currentCustomThemeColors(), themePalette.value),
    ])
    if (configEditorView.value !== view) return
    view.dispatch({
      effects: [
        configEditorTheme.reconfigure(theme),
        configEditorFontTheme.reconfigure(
  ...
  <div class="flex h-full min-h-0 flex-col bg-background">
    <div class="flex shrink-0 items-center justify-between gap-3 border-b px-3 py-2">
      <div class="flex min-w-0 items-center gap-2 text-sm">
        ...
        <span class="truncate font-medium">{{
          connectionInfo?.displayServerAddr || connectionInfo?.serverAddr || "Nacos"
        }}</span>
        ...
        <Badge v-if="namespaceIdLabel" variant="outline" class="max-w-72 truncate font-mono">{{
          namespaceIdLabel
        }}</Badge>
    ...
    <div class="flex shrink-0 items-center gap-1 border-b px-3 py-1.5">
      <button
        ...
        :class="activeTab === 'configs' ? 'bg-accent font-medium' : 'text-muted-foreground hover:bg-accent/60'"
      ...
      <button
        class="rounded px-3 py-1.5 text-sm"
        :class="activeTab === 'services' ? 'bg-accent font-medium' : 'text-muted-foreground hover:bg-accent/60'"
    ...
    <Splitpanes
      ...
      class="nacos-admin-splitpanes min-h-0 flex-1"
      @resized="handleNacosSplitResized"
    >
      <Pane :size="nacosSplitSize" min-size="24">
        <div class="flex h-full min-h-0 flex-col">
          <div class="grid shrink-0 grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto_auto] gap-2 border-b p-2">
            <Input
              v-model="configDataId"
              class="h-8 min-w-0"
              placeholder="dataId"
              @keyup.enter="loadConfigsWithRetry(1)"
            />
            <Input
              v-model="configGroup"
              class="h-8 min-w-0"
              :placeholder="t('nacos.allGroups')"
              @keyup.enter="loadConfigsWithRetry(1)"
            />
            <Input
              v-model="configAppName"
              class="h-8 min-w-0"
              :placeholder="t('nacos.application')"
              @keyup.enter="loadConfigsWithRetry(1)"
            />
            <Button
              size="sm"
          ...
          <div class="min-h-0 flex-1 overflow-auto">
            <div
              class="sticky top-0 z-20 grid grid-cols-4 border-b bg-muted px-3 py-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground shadow-sm"
            >
            ...
            >
              ...
              <span class="truncate pr-3 text-xs text-muted-foreground" :title="item.group || 'DEFAULT_GROUP'">{{
                item.group || "DEFAULT_GROUP"
              }}</span>
              <span class="truncate pr-3 text-xs text-muted-foreground" :title="item.appName || '-'">{{
                item.appName || "-"
              }}</span>
              <span class="truncate text-xs text-muted-foreground" :title="configFormatLabel(item)">{{
                configFormatLabel(item)
              }}</span>
            </button>
            <div
              v-if="!configLoading && configs.length === 0"
          ...
          <div class="flex shrink-0 items-center justify-between border-t px-3 py-2 text-xs text-muted-foreground">
            ...
            <div class="flex items-center gap-2">
              <Button
                ...
                variant="outline"
                class="h-7"
                :disabled="configPageNo <= 1 || configLoading"
                @click="loadConfigs(configPageNo - 1)"
                >{{ t("nacos.prev") }}</Button
              >
              ...
              <Button
                ...
                variant="outline"
                class="h-7"
                :disabled="configPageNo >= configTotalPages || configLoading"
                @click="loadConfigs(configPageNo + 1)"
                >{{ t("nacos.next") }}</Button
              >
            </div>
        ...
        <div class="flex h-full min-h-0 flex-col">
          <div v-if="selectedConfig" class="shrink-0 border-b p-3">
            <div class="grid grid-cols-[96px_minmax(0,1fr)] items-center gap-x-3 gap-y-2">
              ...
              <Input
                v-model="selectedConfig.dataId"
                class="h-8"
                :readonly="!isCreatingConfig"
                :class="{ 'bg-muted text-muted-foreground': !isCreatingConfig }"
                @input="configSaveNotice = ''"
              />
            ...
            <div class="mt-3 flex items-center justify-between gap-3">
              ...
              <div class="flex shrink-0 gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  class="h-8 gap-1.5"
                  :disabled="!selectedConfigOriginalKey"
                  @click="openConfigHistory"
                >
                ...
                <Button
                  size="sm"
                  class="h-8 gap-1.5"
                  :disabled="readOnly || savingConfig || !isConfigDirty"
                  @click="requestSaveConfig"
                >
                ...
                <Button
                  ...
                  variant="outline"
                  class="h-8"
                  :disabled="readOnly || deletingConfig"
                  @click="requestDeleteConfig"
                >
        ...
        <div class="flex h-full min-h-0 flex-col">
          <div class="grid shrink-0 grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto] gap-2 border-b p-2">
            <Input
              v-model="serviceName"
              class="h-8 min-w-0"
              :placeholder="t('nacos.service')"
              @keyup.enter="loadServicesWithRetry(1)"
            />
            <Input
            ...
            />
            <Input
              v-model="serviceCluster"
              class="h-8 min-w-0"
              :placeholder="t('nacos.cluster')"
            ...
            <button
              ...
              :class="{
                'bg-accent':
                  ...
                  selectedService?.groupName === service.groupName,
              }"
          ...
          <div class="flex shrink-0 items-center justify-between border-t px-3 py-2 text-xs text-muted-foreground">
            ...
            <div class="flex items-center gap-2">
              <Button
                ...
                variant="outline"
                class="h-7"
                :disabled="servicePageNo <= 1 || servicesLoading"
                @click="loadServices(servicePageNo - 1)"
                >{{ t("nacos.prev") }}</Button
              >
              ...
              <Button
                ...
                variant="outline"
                class="h-7"
                :disabled="servicePageNo >= serviceTotalPages || servicesLoading"
                @click="loadServices(servicePageNo + 1)"
                >{{ t("nacos.next") }}</Button
              >
            </div>
        ...
        <div class="flex h-full min-h-0 flex-col">
          <div class="flex shrink-0 items-center justify-between border-b px-3 py-2">
            ...
            <Button
              ...
              variant="outline"
              class="h-8 gap-1.5"
              :disabled="!selectedService || instancesLoading"
              @click="loadInstances"
            >
                ...
                <tr v-for="instance in instances" :key="`${instance.ip}:${instance.port}`" class="border-b">
                  ...
                  <td class="px-3 py-2">
                    <Input
                      ...
                      @change="
                        (event: Event) =>
                          requestUpdateInstance(instance, { weight: Number((event.target as HTMLInputElement).value) })
                      "
                    />
                  </td>
                  <td
                    class="max-w-56 truncate px-3 py-2 font-mono text-xs"
                    :title="JSON.stringify(instance.metadata ?? null)"
                  >
                    {{ JSON.stringify(instance.metadata ?? null) }}
                  </td>
                  <td class="px-3 py-2">
                    <div class="flex flex-wrap gap-1">
                      <Badge :variant="instance.healthy === false ? 'outline' : 'secondary'">{{
                        instance.healthy === false ? t("nacos.unhealthy") : t("nacos.healthy")
                      }}</Badge>
                      <Badge :variant="instance.enabled === false ? 'outline' : 'secondary'">{{
                        instance.enabled === false ? t("nacos.offline") : t("nacos.enabled")
                      }}</Badge>
                      <Badge v-if="instance.ephemeral != null" variant="outline">{{
                        instance.ephemeral ? t("nacos.ephemeral") : t("nacos.persistent")
                      }}</Badge>
                    </div>
                  </td>
                  <td class="px-3 py-2 text-right">
                    <div class="inline-flex gap-2">
                      ...
                      >
                        <Loader2
                          v-if="updatingInstanceKey === `${instance.ip}:${instance.port}`"
                      ...
                      <Button
                        ...
                        class="h-7"
                        :disabled="readOnly || updatingInstanceKey === `${instance.ip}:${instance.port}`"
                        @click="requestUpdateInstance(instance, { healthy: !instance.healthy })"
                      >
    ...
    <NacosConfigDiffDialog
      v-model:open="pendingConfigSave"
      :before="originalConfigContent"
      :after="configContent"
