...
<template>
  <Dialog v-model:open="open">
    <DialogContent
      ...
      class="nacos-config-history-dialog flex h-[min(82vh,760px)] flex-col gap-0 overflow-hidden rounded-lg p-0 shadow-2xl"
    >
      <DialogHeader class="shrink-0 border-b bg-muted/20 px-5 py-4">
        <div class="flex items-center justify-between gap-4">
          <div class="flex min-w-0 items-start gap-3">
            <div
              class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-md border bg-background text-primary"
            >
      ...
      <div class="min-h-0 flex-1 overflow-auto">
        <table class="w-full text-left text-sm">
          ...
          <tbody>
            <tr v-for="item in items" :key="`${item.historyId}:${item.nid ?? ''}`" class="border-b">
              ...
              <td class="max-w-44 truncate px-4 py-2 text-xs text-muted-foreground" :title="item.group">
                {{ item.group || "DEFAULT_GROUP" }}
              </td>
              <td class="whitespace-nowrap px-4 py-2 text-xs text-muted-foreground">
                {{ display(item.lastModifiedTime) }}
              </td>
              <td class="max-w-40 truncate px-4 py-2 text-xs text-muted-foreground" :title="item.appName || ''">
                {{ display(item.appName) }}
              </td>
              <td class="px-4 py-2">
                <Badge variant="outline">{{ operationLabel(item.operation) }}</Badge>
              </td>
              <td class="max-w-40 truncate px-4 py-2 text-xs text-muted-foreground" :title="item.operator || ''">
        ...
        <div
          ...
          class="flex h-52 flex-col items-center justify-center gap-2 text-sm text-muted-foreground"
        >
  ...
  <Dialog v-model:open="detailOpen">
    <DialogContent
      class="nacos-config-history-detail-dialog flex h-[min(88vh,900px)] flex-col gap-0 overflow-hidden p-0"
    >
      <DialogHeader class="shrink-0 border-b px-5 py-4">
        ...
        <div v-if="viewingItem" class="mt-2 grid gap-1.5 text-xs text-muted-foreground sm:grid-cols-2">
          <div class="min-w-0 truncate font-mono" :title="viewingItem.namespace || 'public'">
            namespace={{ viewingItem.namespace || "public" }}
          </div>
          <div class="min-w-0 truncate font-mono" :title="viewingItem.dataId">dataId={{ viewingItem.dataId }}</div>
          <div class="min-w-0 truncate font-mono" :title="viewingItem.group || 'DEFAULT_GROUP'">
            group={{ viewingItem.group || "DEFAULT_GROUP" }}
          </div>
          <div class="min-w-0 truncate" :title="display(viewingItem.lastModifiedTime)">
            {{ t("nacos.updatedAt") }}={{ display(viewingItem.lastModifiedTime) }}
          </div>
        </div>
      </DialogHeader>
      <div class="min-h-0 flex-1 overflow-auto bg-muted/20 p-4">
        ...
        <pre v-else class="min-h-full rounded-md border bg-background p-3 font-mono text-xs leading-5">{{
          viewingContent || ""
        }}</pre>
      </div>
