...
import {
  ...
  type NacosDiffLineType,
  type NacosInlineSegment,
...
function rightInlineClass(type: NacosDiffLineType, changed: boolean) {
  ...
  if (type === "modify" || type === "insert")
    return "nacos-inline-change rounded-[2px] bg-emerald-500/75 text-emerald-50"
...
<template>
  <Dialog v-model:open="dialogOpen">
    <DialogContent
      ...
      class="nacos-config-diff-dialog flex h-[min(88vh,900px)] flex-col gap-0 overflow-hidden rounded-lg p-0 shadow-2xl"
    >
      <DialogHeader class="shrink-0 border-b px-5 py-4">
        ...
        <div class="mt-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="text-xs text-muted-foreground">
            {{ t("nacos.confirmSaveMessage", { added: summary.addedLines, removed: summary.removedLines }) }}
          </div>
        </div>
      </DialogHeader>
      ...
      <div v-if="!inlineCompare" class="grid min-h-0 flex-1 grid-cols-1 gap-4 bg-background px-5 py-4 lg:grid-cols-2">
        <section class="flex min-w-0 min-h-0 flex-col">
          <div class="mb-2 text-sm font-medium text-foreground">
            {{ beforeLabel || t("nacos.currentVersionContent") }}
          </div>
          <div
            class="min-h-0 flex-1 overflow-auto rounded-sm border border-zinc-700 bg-[#1f1f1f] font-mono text-[13px] leading-6 text-zinc-200"
          >
            <div
              v-for="row in rows"
              :key="`${row.id}-left`"
              class="grid min-w-max grid-cols-[52px_22px_minmax(48rem,1fr)]"
              :class="lineClass(row.leftType, 'left')"
            >
              <span
                class="select-none border-r border-white/8 pr-2 text-right"
                :class="gutterClass(row.leftType, 'left')"
                >{{ row.leftLineNumber ?? "" }}</span
              >
              <span class="select-none pl-2" :class="gutterClass(row.leftType, 'left')">{{
                prefix(row.leftType, "left")
              }}</span>
              <pre
                class="whitespace-pre px-2"
              ><template v-for="(segment, index) in inlineSegments(row.leftContent, row.leftInline)" :key="index"><span :class="inlineClass(row.leftType, segment.changed)">{{ segment.value }}</span></template></pre>
            </div>
          </div>
        </section>
        ...
        <section class="flex min-w-0 min-h-0 flex-col">
          <div class="mb-2 text-sm font-medium text-foreground">
            {{ afterLabel || t("nacos.publishVersionContent") }}
          </div>
          <div
            class="min-h-0 flex-1 overflow-auto rounded-sm border border-zinc-700 bg-[#1f1f1f] font-mono text-[13px] leading-6 text-zinc-200"
          >
            <div
              ...
              :key="`${row.id}-right`"
              class="grid min-w-max grid-cols-[52px_22px_minmax(48rem,1fr)]"
              :class="lineClass(row.rightType, 'right')"
            >
              <span
                class="select-none border-r border-white/8 pr-2 text-right"
                :class="gutterClass(row.rightType, 'right')"
                >{{ row.rightLineNumber ?? "" }}</span
              >
              <span class="select-none pl-2" :class="gutterClass(row.rightType, 'right')">{{
                prefix(row.rightType, "right")
              }}</span>
              <pre
                class="whitespace-pre px-2"
              ><template v-for="(segment, index) in inlineSegments(row.rightContent, row.rightInline)" :key="index"><span :class="rightInlineClass(row.rightType, segment.changed)">{{ segment.value }}</span></template></pre>
            </div>
          </div>
        </section>
      </div>
        ...
        <section class="flex h-full min-h-0 flex-col">
          ...
          >
            <div
              ...
              :key="row.id"
              class="grid min-w-max grid-cols-[52px_22px_minmax(96rem,1fr)]"
              :class="inlineRowClass(row.type)"
            >
              <span class="select-none border-r border-white/8 pr-2 text-right" :class="inlineGutterClass(row.type)">{{
                row.lineNumber ?? ""
              }}</span>
              <span class="select-none pl-2" :class="inlineGutterClass(row.type)">{{ inlinePrefix(row.type) }}</span>
              <pre
                class="whitespace-pre px-2"
              ><template v-for="(segment, index) in row.segments" :key="index"><span :class="inlineRowSegmentClass(row.type, segment.changed)">{{ segment.value }}</span></template></pre>
            </div>
          </div>
        </section>
      </div>
      ...
      <DialogFooter class="shrink-0 gap-3 border-t bg-muted/20 px-5 pb-6 pt-4">
        ...
        <Button variant="outline" class="min-w-24 px-5" :disabled="loading" @click="open = false">{{
