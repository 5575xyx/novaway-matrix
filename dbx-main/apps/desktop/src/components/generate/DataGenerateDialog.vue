...
const connectionName = computed(() =>
  props.prefillConnectionId ? store.getConfig(props.prefillConnectionId)?.name : "",
)
...
async function loadSchemas() {
  ...
  try {
    ...
    const schemaCandidates =
      props.prefillSchema && schemaList.includes(props.prefillSchema) ? [props.prefillSchema] : [schemaList[0]]
          ...
          if (tables.some((t: { name: string }) => t.name === props.prefillTable)) {
            ...
            configs[key] = {
              ...
              columns: cols.map((c: ColumnInfo) => {
                const isAI =
                  c.extra === "auto_increment" || (c.column_default?.toLowerCase().includes("nextval") ?? false)
...
const currentPreview = computed(
  () => generatedResults.value[previewTableIndex.value] ?? { tableName: "", columns: [], rows: [], sql: "" },
)
...
async function doGenerate() {
  ...
  for (const key of order) {
    ...
    let columns = cfg.columns.filter(
      (col) => checkedColumns[colKey(cfg.schema, cfg.tableName, col.columnName)] !== false,
    )
...
async function regenerate() {
  ...
  const key = Object.keys(configs).find(
    (k) => configs[k].tableName === generatedResults.value[previewTableIndex.value]?.tableName,
  )
  if (!key) return
  const cfg = configs[key]
          ...
          <div class="w-64 shrink-0 rounded-md border">
            ...
            <ScrollArea class="h-[380px] p-1">
              ...
              <div v-for="sc in schemas" :key="sc">
                <div
                  class="group flex items-center gap-1.5 py-1 px-2 cursor-pointer hover:bg-accent"
                  @click="toggleSchema(sc)"
                >
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <ChevronRight
                      ...
                      :class="{ 'rotate-90': expandedSchemas[sc] }"
                    />
                  </button>
                  ...
                  <div v-for="tbl in schemaTables[sc]" :key="tbl.name">
                    ...
                    <div class="group flex items-center gap-1.5 py-1 px-2 cursor-pointer hover:bg-accent">
                      ...
                      <input
                        ...
                        @change="selectTable(sc, tbl.name, ($event.target as HTMLInputElement).checked)"
                      />
                    ...
                    <!-- column children -->
                    <div
                      v-if="expandedTables[tableKey(sc, tbl.name)] && tableColumnsExt[tableKey(sc, tbl.name)]"
                      class="ml-[36px]"
                    >
                      <div
                      ...
                      >
                        <input
                          ...
                          :checked="!!checkedColumns[colKey(sc, tbl.name, col.name)]"
                          @click.stop
                          @change="toggleColumn(sc, tbl.name, col.name)"
                        />
                ...
                <div
                  v-else-if="expandedSchemas[sc] && schemaLoading[sc]"
                  class="ml-[18px] px-2 py-1 text-xs text-muted-foreground"
                >
                  Loading...
                </div>
                <div
                  v-else-if="expandedSchemas[sc] && schemaError[sc]"
                  class="ml-[18px] px-2 py-1 text-xs text-destructive"
                >
                  {{ schemaError[sc] }}
                </div>
              </div>
              ...
              <template v-else-if="activeCol">
                <div class="p-3 space-y-3">
                  ...
                  <GeneratorParamsPanel
                    :config="activeCol"
                    :connection-id="props.prefillConnectionId"
                    :database="props.prefillDatabase"
                  />
                  <div
                    class="rounded border border-dashed border-amber-300 bg-amber-50 dark:bg-amber-950/20 px-2 py-1 text-[10px] font-mono text-amber-700 dark:text-amber-400 leading-relaxed break-all"
                  >
      ...
      <template v-else-if="currentStep === 'preview'">
          ...
          <div class="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <span>{{
              Object.keys(checkedTables).length > 0
                ? Object.keys(checkedTables)[0].split(".")[0]
                : props.prefillSchema || "main"
            }}</span>
          </div>
        ...
        <div class="rounded-md border w-full overflow-hidden">
          <div
            ...
            class="flex h-[420px] items-center justify-center text-xs text-muted-foreground"
          >
          ...
          <template v-else>
            <div class="flex items-center justify-between px-3 py-2 border-b bg-muted/10">
              <div class="flex items-center gap-2">
                ...
                <select
                  v-if="generatedResults.length > 1"
                  v-model="previewTableIndex"
              ...
              <Button variant="outline" size="sm" class="h-7 text-xs" @click="regenerate">{{
                t("dataGenerate.regenerate")
              }}</Button>
            </div>
            <div class="flex flex-col h-[380px]">
              <div class="flex-1 overflow-auto overscroll-none bg-background">
                <table class="w-full text-xs border-collapse" style="table-layout: auto">
                  <thead>
                    <tr class="sticky top-0 z-10 bg-[rgb(239_239_239)] dark:bg-muted/60 border-y border-border">
                      <th
                        class="px-2 py-1.5 border-r border-border text-center text-muted-foreground select-none w-10 shrink-0"
                      >
                        #
                      </th>
                      <th
                        ...
                        :style="{ minWidth: '60px', width: previewColWidths[ci] + 'px' }"
                      >
                        {{ col }}
                        <div
                          class="absolute right-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-primary/30"
                          @mousedown.stop="onPreviewColResizeStart(ci, $event)"
                          @dblclick.stop="delete previewColWidths[ci]"
                        />
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr
                      ...
                      :class="{ 'bg-muted/30': ri % 2 === 1 }"
                    >
                      <td
                        class="data-grid-row-number px-2 py-1 border-r border-border text-center text-muted-foreground select-none text-xs"
                      >
                        {{ ri + 1 }}
                      </td>
                      <td
                ...
                <div
                  v-if="currentPreview.rows.length > 50"
                  class="sticky left-0 text-xs text-muted-foreground px-3 py-1.5 border-t border-border bg-background"
                >
                  {{ t("dataGenerate.first50Rows", { count: currentPreview.rows.length }) }}
                </div>
              </div>
            </div>
          </template>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="flex items-center gap-3 shrink-0">
                  <span class="text-green-600 dark:text-green-400">{{
                    t("dataGenerate.successLabel", { count: r.ok })
                  }}</span>
                  <span v-if="r.err > 0" class="text-destructive">{{
                    t("dataGenerate.failLabel", { count: r.err })
                  }}</span>
      ...
      <DialogFooter class="flex items-center justify-between border-t pt-3 sm:justify-between">
        <div class="flex items-center gap-2">
          ...
          <template v-else-if="currentStep === 'preview' && generatedResults.length > 0">
            <Button variant="outline" size="sm" class="h-7 text-xs" @click="copyAllSql">{{
              t("dataGenerate.copyAllSql")
            }}</Button>
          </template>
        </div>
        <div class="flex items-center gap-2">
          ...
          <template v-else-if="currentStep === 'preview'">
            <Button variant="outline" size="sm" class="h-7 text-xs" @click="currentStep = 'config'">{{
              t("dataGenerate.prevStep")
            }}</Button>
          ...
          <template v-else-if="currentStep === 'result'">
            <Button variant="outline" size="sm" class="h-7 text-xs" @click="currentStep = 'preview'">{{
              t("dataGenerate.back")
            }}</Button>
            <Button variant="default" size="sm" class="h-7 text-xs" @click="open = false">{{
              t("dataGenerate.confirm")
            }}</Button>
          </template>
        </div>
      </DialogFooter>
    ...
    <Dialog v-model:open="orderDialogOpen">
      <DialogContent class="max-w-md">
        ...
        <div class="space-y-1 max-h-80 overflow-y-auto">
          <div
            v-for="(key, i) in tableOrder"
            :key="key"
            class="flex items-center gap-2 rounded border bg-background px-3 py-2 text-xs"
          >
            <span
              class="flex h-5 w-5 items-center justify-center rounded bg-muted text-muted-foreground text-[10px] font-medium"
              >{{ i + 1 }}</span
            >
            ...
            <button
              type="button"
              class="flex h-5 w-5 items-center justify-center rounded text-muted-foreground hover:bg-accent hover:text-foreground disabled:opacity-20"
              :disabled="i === 0"
              @click="moveOrderItem(i, -1)"
            >
            ...
            <button
              type="button"
              class="flex h-5 w-5 items-center justify-center rounded text-muted-foreground hover:bg-accent hover:text-foreground disabled:opacity-20"
              :disabled="i === tableOrder.length - 1"
              @click="moveOrderItem(i, 1)"
            >
