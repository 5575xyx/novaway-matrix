  ...
  <Popover v-model:open="open">
    ...
    <PopoverContent class="w-64 p-0" align="start">
      <div v-if="!selectedCategory" class="max-h-[300px] overflow-auto p-1">
        <div
          v-for="cat in GeneratorHierarchy"
          :key="cat.key"
          class="flex cursor-pointer items-center justify-between rounded-md px-2.5 py-1.5 text-xs hover:bg-accent"
      ...
      <div v-else class="max-h-[300px] overflow-auto p-1">
        <div class="border-b px-2.5 py-1.5">
          <button class="text-xs text-muted-foreground hover:text-foreground" @click="backToCategories">
            ← {{ tr(selectedCategory.key, selectedCategory.label) }}
          </button>
        </div>
        <div
          v-for="gen in selectedCategory.children"
          :key="gen.key"
          class="flex cursor-pointer items-center justify-between rounded-md px-2.5 py-1.5 text-xs hover:bg-accent"
          @click="selectGenerator(selectedCategory, gen)"
        >
