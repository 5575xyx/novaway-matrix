...
function generateLinuxPath(includeName: boolean, ext: string): string {
  ...
  const nameBases = ["report", "doc", "img", "photo", "backup", "config", "data", "file"]
  const base =
    baseDirs[0] === "home" || pick(baseDirs) === "home"
      ? `/home/${pick(users)}/${pick(folders)}`
      : `/${pick(baseDirs)}/${pick(folders)}`
  if (!includeName) return base
  return `${base}/${pick(nameBases)}_${Math.floor(Math.random() * 999999)}${ext}`
}
  ...
  <div class="space-y-3">
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex flex-wrap gap-1.5">
        <button
          ...
          :class="
            (params.pathTypes || []).includes(pt.value)
    ...
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="mt-1 flex flex-wrap gap-1">
        <span
          v-for="ext in extensionCategoryMap[params.extensionCategory || 'image'] || []"
          :key="ext"
          class="px-1.5 py-0.5 rounded bg-background border text-xs font-mono"
          >{{ ext }}</span
        >
      </div>
    </div>
