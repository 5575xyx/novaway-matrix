  ...
  <div class="space-y-3">
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex items-center gap-2 text-xs">
        <button
          ...
          :class="isGenerate ? 'bg-primary text-primary-foreground border-primary' : 'bg-background'"
          @click="params.imageMode = 'generate'"
        >
          {{ t("dataGenerate.generate") }}
        </button>
        <button
          type="button"
          class="px-2 py-1 rounded border text-xs"
          :class="!isGenerate ? 'bg-primary text-primary-foreground border-primary' : 'bg-background'"
          @click="params.imageMode = 'folder'"
        >
          {{ t("dataGenerate.fromFolder") }}
        </button>
      </div>
    </div>
        ...
        <div class="grid grid-cols-[80px_1fr] items-center gap-2 text-xs">
          ...
          <div class="flex items-center gap-2">
            <button
              ...
              class="px-2 py-1 rounded border text-xs"
              :class="
                (params.imageFormat || 'JPEG') === 'JPEG'
                  ? 'bg-primary text-primary-foreground border-primary'
                  : 'bg-background'
              "
              @click="params.imageFormat = 'JPEG'"
            >
              JPEG
            </button>
            <button
              ...
              class="px-2 py-1 rounded border text-xs"
              :class="
                params.imageFormat === 'PNG' ? 'bg-primary text-primary-foreground border-primary' : 'bg-background'
              "
              @click="params.imageFormat = 'PNG'"
            >
              PNG
            </button>
          </div>
    ...
    <template v-else>
      <div class="rounded-md border bg-muted/10 p-3 space-y-2">
        ...
        <textarea
          v-model="params.fileExtensions"
          rows="3"
          class="w-full rounded border bg-background px-2 py-1 text-xs font-mono resize-y"
          placeholder="每行一个扩展名&#10;png&#10;jpg&#10;txt"
        />
      </div>
    </template>
