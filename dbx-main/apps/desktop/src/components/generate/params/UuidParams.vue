...
const regex = computed(() =>
  props.params.uuidHyphens !== false
    ...
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex items-center gap-2 text-xs">
        <button
          ...
          :class="params.uuidHyphens !== false ? 'bg-primary text-primary-foreground border-primary' : 'bg-background'"
          @click="params.uuidHyphens = true"
        >
          包含连字符
        </button>
        <button
          ...
          class="px-2 py-1 rounded border text-xs"
          :class="params.uuidHyphens === false ? 'bg-primary text-primary-foreground border-primary' : 'bg-background'"
          @click="params.uuidHyphens = false"
        >
          无格式
        </button>
      </div>
