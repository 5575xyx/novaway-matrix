    ...
    <div class="rounded-md border bg-muted/10 p-3 space-y-2">
      <div class="flex items-center gap-2 text-xs">
        <input
          ...
          @change="params.allDay = !params.allDay"
        />
    ...
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex items-center gap-2 text-xs">
        <button
          ...
          :class="
            !params.weekdayMode || params.weekdayMode === 'all'
              ? 'bg-primary text-primary-foreground border-primary'
              : 'bg-background'
          "
          @click="params.weekdayMode = 'all'"
        >
          {{ t("dataGenerate.all") }}
        </button>
        <button
          ...
          class="px-2 py-1 rounded border text-xs"
          :class="
            params.weekdayMode === 'weekday' ? 'bg-primary text-primary-foreground border-primary' : 'bg-background'
          "
          @click="params.weekdayMode = 'weekday'"
        >
          {{ t("dataGenerate.weekdayOnly") }}
        </button>
        <button
          type="button"
          class="px-2 py-1 rounded border text-xs"
          :class="
            params.weekdayMode === 'custom' ? 'bg-primary text-primary-foreground border-primary' : 'bg-background'
          "
          @click="params.weekdayMode = 'custom'"
        >
          {{ t("dataGenerate.custom") }}
        </button>
      </div>
      <div v-if="params.weekdayMode === 'custom'" class="flex gap-1 mt-2">
        <button
          ...
          :key="i"
          type="button"
          class="w-7 h-7 rounded text-xs border"
          :class="
            (params.weekdays ?? []).includes(i) ? 'bg-primary text-primary-foreground border-primary' : 'bg-background'
          "
