...
<template>
  <div class="space-y-3">
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex items-center gap-2 text-xs">
        <button
          ...
          :class="
            (params.regionFormat || 'name') === f.value
              ? 'bg-primary text-primary-foreground border-primary'
              : 'bg-background'
          "
          @click="params.regionFormat = f.value"
        >
          {{ f.label }}
        </button>
      </div>
    ...
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex items-center gap-2 text-xs">
        <button
          v-for="t in textTransforms"
          :key="t.value"
          type="button"
          class="px-2 py-1 rounded border text-xs"
          :class="
            (params.textTransform || 'none') === t.value
              ? 'bg-primary text-primary-foreground border-primary'
              : 'bg-background'
          "
          @click="params.textTransform = t.value"
        >
          {{ t.label }}
        </button>
      </div>
