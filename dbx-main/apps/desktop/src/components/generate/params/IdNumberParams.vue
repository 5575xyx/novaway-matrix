...
function generateUSCC(): string {
  const randChars = (n: number) =>
    Array.from({ length: n }, () =>
      "ABCDEFGHJKLMNPQRSTUVWXY23456789".charAt(Math.floor(Math.random() * "ABCDEFGHJKLMNPQRSTUVWXY23456789".length)),
    ).join("")
  ...
  <div class="space-y-3">
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex flex-wrap gap-1.5">
        <button
          v-for="k in idTypeKeys"
          :key="k"
          type="button"
          class="px-2 py-1 rounded border text-xs"
          :class="
            (params.idTypes ?? []).includes(k) ? 'bg-primary text-primary-foreground border-primary' : 'bg-background'
          "
          @click="toggleType(k)"
        >
          {{ tr(k, k) }}
    ...
    <div v-if="hasCustom" class="rounded-md border bg-muted/10 p-3">
      ...
      <input
        v-model="params.idCustomPattern"
        class="w-full rounded border bg-background px-2 py-1 text-xs font-mono"
        :placeholder="tr('customPatternPlaceholder', '如 ([A-Z]{2})-([0-9]{8})')"
      />
      <div class="text-[10px] text-muted-foreground mt-1">
        {{ tr("customPatternHint", "支持 [A-Z] 字母、[0-9] 数字、{n} 重复、连字符等简单模式") }}
      </div>
    </div>
    <div class="rounded-md border bg-muted/10 p-3">
