  ...
  <div class="space-y-3">
    <div class="rounded-md border bg-muted/10 p-3 space-y-2">
      ...
      <div class="grid grid-cols-[60px_1fr] items-center gap-2 text-xs">
        ...
        <select
          v-model="props.params.fkTable"
          class="h-7 rounded border bg-background px-2 text-xs"
          @change="onTableChange(($event.target as HTMLSelectElement).value)"
        >
    ...
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex items-center gap-2 text-xs flex-wrap">
        <button
          ...
          :class="
            (params.fkMode || 'random') === m.value
              ? 'bg-primary text-primary-foreground border-primary'
              : 'bg-background'
          "
          @click="params.fkMode = m.value as 'random' | 'unique' | 'repeat'"
        >
          {{ m.label }}
        </button>
      </div>
