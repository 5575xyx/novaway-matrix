    ...
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex items-center gap-2 text-xs">
        <button
          ...
          :class="
            (params.phoneFormat || 'domestic') === 'domestic'
              ? 'bg-primary text-primary-foreground border-primary'
              : 'bg-background'
          "
          @click="params.phoneFormat = 'domestic'"
        >
          国内
        </button>
        <button
          ...
          class="px-2 py-1 rounded border text-xs"
          :class="
            params.phoneFormat === 'international'
              ? 'bg-primary text-primary-foreground border-primary'
              : 'bg-background'
          "
          @click="params.phoneFormat = 'international'"
        >
          国际
        </button>
      </div>
    ...
    <div class="rounded-md border bg-muted/10 p-3">
      <div class="flex items-center gap-2 text-xs">
        <input
          ...
          :checked="!!params.phoneSeparator"
          @change="params.phoneSeparator = !params.phoneSeparator"
        />
    ...
    <div class="rounded-md border bg-muted/10 p-3">
      ...
      <div class="flex flex-wrap gap-1.5">
        <button
          ...
          class="px-2 py-1 rounded border text-xs"
          :class="
            (params.phoneRegions ?? []).includes(r.value)
              ? 'bg-primary text-primary-foreground border-primary'
              : 'bg-background'
          "
