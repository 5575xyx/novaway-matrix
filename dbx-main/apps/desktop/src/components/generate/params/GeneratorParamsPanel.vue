...
<template>
  <div class="space-y-3">
    ...
    <component
      :is="componentMap[key!] ?? DefaultParams"
      v-if="key"
      :params="initParams()"
      :config="config"
      ...
      :database="database"
    />
    <div v-else class="text-xs text-muted-foreground px-1 py-3 text-center border rounded-md bg-muted/5">
      {{ t("dataGenerate.selectGeneratorHint") }}
    </div>
  </div>
</template>
