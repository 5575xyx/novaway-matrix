...
export default withEnglishFallback({
  ...
  connection: {
    ...
    nacosConsoleUrlHint:
      "填寫 Nacos 控制台/admin API 位址。Nacos 3 Docker 通常是 8085；如果舊版控制台和服務共用 8848，也可以填 8848。",
    ...
    sqlServerLegacyUnencryptedModeHint:
      "適用於非加密或僅登入階段加密的場景。若伺服器強制 TLS 1.0 加密，此模式仍會失敗\n請僅在可信內網、VPN 或 SSH 隧道中使用",
    ...
    agentDriverUpdateConnectionHint:
      "目前連線使用的內建驅動有可用更新，連線失敗可能與本機驅動版本過舊有關。請在「驅動管理」中更新對應驅動後重試。",
    ...
    dmDownload: "前往達夢官網下載",
    mongoLegacyHint:
      "舊版 MongoDB（尤其是 4.2 以下版本）請選擇舊版相容。若帳號建立在 admin 且認證失敗，請將認證庫設為 admin。",
    mongoTlsAllowInvalidCertificates: "停用 TLS 憑證校驗",
    mongoTlsAllowInvalidCertificatesHint:
      "對應 tlsAllowInvalidCertificates=true，會停用全部憑證校驗（不只是主機名），存在中間人攻擊風險。經 SSH 隧道存取 AWS DocumentDB 時通常需要開啟。",
  ...
  ai: {
    ...
    codexMcpRequiredDescription:
      "Codex CLI 會透過 DBX MCP Server 存取資料庫結構與查詢工具。請先安裝後再於 AI 助理中使用 Codex。",
  ...
  contextMenu: {
    ...
    confirmDropMongoAllIndexesDetails:
      "MongoDB 會保留「_id_」索引。若服務端因 shard key 規則保留了額外索引，最終結果只會列出實際刪除的索引。",
  ...
  structureEditor: {
    ...
    sqlServerIdentityTypeHint:
      "SQL Server 自動遞增僅支援 tinyint、smallint、int、bigint、decimal/numeric（小數位為 0）",
  },
  diagram: {
    title: "關係圖",
    open: "檢視關係圖",
  ...
  settings: {
    ...
    duckDbWorkerProcessIsolationDescription:
      "將 DuckDB 查詢放到獨立 worker 進程中執行。停止長查詢時，如果 DuckDB 未及時回應取消請求，DBX 會結束 worker 進程以釋放資料庫檔案鎖。",
    ...
    dataGridQuickEntryDescription:
      "開啟後，編輯儲存格或填寫底部空白列並離焦會立即儲存到資料庫。適合高頻輸入，但有誤操作風險。",
    ...
    sqlSemanticDiagnosticsEnabledDescription:
      "開啟後，編輯器會提示未知資料表、欄位等語意問題；關閉可減少 SQL 解析和中繼資料檢查的效能負擔。",
    ...
    mcpAllowDangerousDescription:
      "開啟後會為示例配置附加 DBX_MCP_ALLOW_DANGEROUS_SQL=1，允許 DROP、TRUNCATE、ALTER 等語句。",
    ...
    updateDownloadSourceDescription:
      "選擇應用程式內更新安裝程式的下載來源。建議使用官方來源；CNB 或 AtomGit 在中國大陸網路環境下可能更快。",
  ...
  driverStore: {
    ...
    driverStoreDirConfirm:
      "更改 {target} 將遷移對應驅動資料到新位置，遷移期間可能會斷開相關驅動連線。遷移完成後應用將自動重啟。是否繼續？",
