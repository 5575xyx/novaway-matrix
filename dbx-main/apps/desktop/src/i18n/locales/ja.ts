...
export default withEnglishFallback({
  ...
  updates: {
    ...
    portableManualUpdate:
      "ポータブル版はアプリ内インストーラーを使用できません。リリースページからポータブルZIPをダウンロードし、現在のDBXフォルダに上書き展開してください。portable.dbxとデータは保持されます。",
  ...
  connection: {
    ...
    sqliteExtensionsHint:
      "1行に1つの.dylib/.so/.dllファイルを指定します。エントリポイントが必要な場合は「パス|エントリポイント」形式で指定します。接続時に読み込まれ、その後load_extensionは無効になります。",
    ...
    mysqlCleartextPasswordAuthHint:
      "MySQLサーバーがcleartext password認証プラグインを要求する場合のみ有効にしてください。",
    ...
    postgresRootCertHint:
      "サーバー証明書がプライベートCAで署名されている場合、verify-caまたはverify-fullに使用します。",
    ...
    postgresClientCertHint:
      "PostgreSQLがmTLSを要求する場合、クライアント証明書と秘密鍵の両方を指定する必要があります。",
    ...
    nacosConsoleUrlHint:
      "Nacosのコンソール/admin APIアドレスを指定します。Nacos 3 Dockerでは通常8085、古い構成ではサービス用ポート8848と共用される場合があります。",
    ...
    sqlServerLegacyUnencryptedModeHint:
      "非暗号化またはログイン時のみ暗号化する場合に適用されます。サーバーが TLS 1.0 暗号化を要求する場合は失敗します。\n信頼できるネットワーク、VPN、SSH トンネルでのみ使用してください",
    ...
    driverNotInstalled:
      "{driver} ドライバーがインストールされていません。ドライバーマネージャーからインストールしてください。",
    jreNotInstalled:
      "JRE {jre} ランタイムがインストールされていません。ドライバーマネージャーからインストールしてください。",
    systemJavaNotFound:
      "システムJavaランタイムがPATHに見つかりませんでした。Javaをインストールするか、カスタムJava実行ファイルを選択してください。",
    customJavaPathEmpty: "カスタムJavaランタイムのパスが空です。Java実行ファイルを選択してください。",
    agentJavaTooOld:
      "このドライバーにはJava 21が必要です。ドライバーマネージャーでDBX管理JRE 21を使用するか、Java 21実行ファイルを選択してください。",
    agentDriverUpdateConnectionHint:
      "この接続で使用中の内蔵ドライバに更新が利用可能です。接続失敗はローカルドライバの旧版が原因の可能性があります。ドライバマネージャーで該当ドライバを更新してから再試行してください。",
    jdbcPluginNotInstalled:
      "JDBCプラグインがインストールされていません。この接続を使用するにはオプションのJDBCプラグインをインストールしてください。",
    lastError: "接続エラー",
    ...
    jdbcPluginHint:
      "最初にDBX JDBCプラグインをインストールし、次にデータベースベンダーのJDBCドライバーJARをインポートしてください。",
    dmCompatHint: "システムにDM8 ODBCドライバーがインストールされている必要があります。",
    dmDownload: "達夢からダウンロード",
    mongoLegacyHint:
      "古いMongoDBサーバー（特に4.2未満）ではレガシーを使用してください。認証に失敗しユーザーがadminで作成された場合は、認証DBをadminに設定してください。",
    mongoTlsAllowInvalidCertificates: "TLS 証明書検証を無効化",
    mongoTlsAllowInvalidCertificatesHint:
      "tlsAllowInvalidCertificates=true に対応し、ホスト名だけでなくすべての証明書検証を無効化します。MITM 攻撃のリスクが高まります。SSH トンネル経由の AWS DocumentDB では通常必要です。",
    mongoRetryWrites: "書き込み再試行",
    mongoRetryWritesHint:
      "retryWrites に対応します。AWS DocumentDB は再試行可能な書き込みをサポートしないため、無効にしてください。",
    compatible: "互換",
    mainstream: "人気",
    color: "色",
    colorNone: "色なし",
    ...
    zookeeperConnectStringHint:
      "カンマまたは改行区切りの host:port エントリ。空欄の場合は上のホストとポートを使用します。",
  ...
  editor: {
    ...
    unsavedChangesBatchCloseMessage:
      "閉じようとしているタブに未保存の変更があり、閉じると失われます。保存してから続行しますか？",
    unsavedChangesBatchCloseMultipleMessage:
      "閉じようとしている {count} 個の SQL タブに未保存の変更があります。現在「{title}」を表示中です。保存してから続行しますか？",
    unsavedChangesAppCloseMessage:
      "「{title}」に未保存の SQL 変更があり、DBX の終了後に失われます。保存してから終了しますか？",
    unsavedChangesAppCloseMultipleMessage:
      "{count} 個の SQL タブが未保存です。現在「{title}」を表示中です。DBX の終了後に失われます。保存してから終了しますか？",
  ...
  grid: {
    ...
    formatterCustomTemplateHint:
      "利用可能な変数: ${value}, ${upper}, ${lower}, ${length}。名前を付けるとリストに保存されます。",
    ...
    formattedJsonEditWarning:
      "フォーマット済みJSONを編集中です。保存するとJSONとして書き込まれるため、元の書式が変わる場合があります。",
    setNull: "NULLに設定",
    ...
    nonTransactionalSaveHint:
      "{count}件の保留中の変更を1件ずつ保存します。途中で失敗した場合、それまでの成功した変更はロールバックされません。",
    keylessEditWarning: "主キーなし",
    keylessEditWarningHint:
      "このテーブルには主キーがありません。更新と削除はWHERE句にすべての元の行値を使用します。完全に重複する行はまとめて影響を受ける可能性があります。",
    queryEditReady: "編集可能",
    queryEditReadyHint:
      "この単一テーブル結果は「{table}」から取得されています。行の編集、削除、追加が可能で、保留中の変更アクションから保存できます。",
    ...
    queryEditUnsupported: {
      ...
      "external-source":
        "外部ファイルとテーブル関数の結果は直接書き戻せません。編集前にデータベーステーブルにインポートしてください。",
    ...
    truncatedHint:
      "結果は{count}行に切り詰められました。フッターのページネーションを使用するか、1ページあたりの行数を調整してください。",
  ...
  welcome: {
    ...
    mcpTitle: "AIエージェント連携",
    mcpDescription:
      "内蔵AIアシスタントに加えて、Claude Code、Cursor、その他のコーディングエージェントをMCP経由でデータベースクエリに使用できます。",
    mcpLearnMore: "詳細を見る",
  },
  common: {
    language: "言語",
  ...
  lineage: {
    ...
    description: {
      ...
      foreignKeyOutgoing:
        "現在のフィールドは外部キーを通じて {target} を参照しています。これは検証済みの依存関係です。",
      viewLikely: "ビュー定義が対象テーブルとフィールドの両方に言及しています。通常はクエリ依存関係を示します。",
      viewPossible: "ビュー定義が同名フィールドに言及していますが対象テーブルには言及していないため、確認が必要です。",
      historyLikely:
        "履歴SQL文が対象テーブルとフィールドの両方に言及しています。影響分析のコンテキストとして使用してください。",
      historyPossible:
        "履歴SQL文が同名フィールドに言及しています。関連している可能性がありますがコンテキストが必要です。",
      sameName:
        "別のテーブルに同名フィールドがあります。ビジネス上の意味を共有している可能性がありますが、検証済みのデータベース依存関係ではありません。",
    },
    cancel: "キャンセル",
    refresh: "再分析",
  },
  ...
  configExport: {
    ...
    passphraseExportHint:
      "エクスポートに接続パスワードを含めるには、暗号化用のパスフレーズを設定してください。インポート時に同じパスフレーズが必要です。",
    passphraseImportHint:
      "このファイルは暗号化されています。復号してインポートするためにパスフレーズを入力してください。",
    passphrase: "パスフレーズ",
    ...
    cryptoUnavailable:
      "暗号化設定のインポートはこのブラウザ環境では利用できません。HTTPS、localhost、またはDBX Desktopを使用してください。",
    exportSuccess: "接続を正常にエクスポートしました",
    importSuccess: "{count}件の接続をインポートしました",
    importNavicatSuccess:
      "{count}件のNavicat接続をインポートしました。テスト前にパスワードが空の接続を入力してください。",
    importDatagripSuccess:
      "{count}件のDataGrip接続をインポートし、macOSキーチェーンから{filled}件のパスワードを復元しました。",
    importDbeaverSuccess:
      "{count}件のDBeaver接続をインポートしました。テスト前にパスワードが空の接続を入力してください。",
  ...
  ai: {
    ...
    codexMcpRequiredDescription:
      "Codex CLIはDBX MCP Server経由でデータベーススキーマとクエリツールにアクセスします。AIアシスタントでCodexを使う前にインストールしてください。",
    codexCliPathEnvError:
      "Codex CLI Pathには実行ファイルのパスのみを入力してください。環境変数は下に追加してください。",
    codexCliEnv: "環境変数",
    ...
    autoSqlBlocked:
      "AIが生成したSQLは自動実行するにはリスクが高すぎると判断されました。実行前に手動で確認してください。",
    ...
    enableThinkingHint:
      "このオプションは/chat/completions APIとサポートされているモデルでのみ有効です。無効にするとトークン使用量を大幅に削減できますが、生成結果の品質が若干低下する可能性があります。",
  ...
  contextMenu: {
    ...
    structureDocCopyFallbackHint:
      "自動コピーがブロックされました。再度コピーをクリックするか、下の内容を選択してください。",
    ...
    confirmDropTableMessage:
      "本当にテーブル「{name}」をドロップ（削除）しますか？テーブルとすべてのデータが永久に削除されます。",
    ...
    confirmExecuteProcedureMessage:
      "プロシージャ「{name}」を実行しますか？実行前に入力パラメータの値を設定・調整できます。",
    loadingProcedureParameters: "パラメータを読み込み中...",
    procedureParametersUnavailable:
      "パラメータメタデータを読み込めませんでした。以下のSQLを編集して実行することはできます。",
    ...
    parameterDefaultHint:
      "DEFAULTがチェックされている場合、DBXはその引数を省略し、データベースがプロシージャのデフォルト値を使用します。",
    ...
    confirmDropMongoAllIndexesDetails:
      "MongoDB は「_id_」インデックスを保持します。shard key の制約で追加のインデックスが残る場合があり、結果には実際に削除されたインデックスのみ表示されます。",
    ...
    confirmDropBatchIndexesMessage:
      "本当に「{table}」から選択した {count} 個のインデックスを削除しますか？この操作は取り消せません。",
    ...
    confirmDropDatabaseMessage:
      "本当にデータベース「{name}」をドロップ（削除）しますか？データベースとすべてのデータが永久に削除されます。",
    ...
    confirmDropCollectionMessage:
      "本当にコレクション「{name}」を削除しますか？コレクションとすべてのドキュメントが永久に削除されます。",
    ...
    confirmDropSchemaMessage:
      "本当にスキーマ「{name}」を削除しますか？スキーマとすべてのオブジェクトが永久に削除されます。",
  ...
  userAdmin: {
    title: "ユーザーと権限",
    unsupported:
      "MySQL互換およびPostgreSQL互換の接続がサポートされています。SQL Server、Oracle、その他の権限モデルは今後追加予定です。",
    newUser: "新しいユーザー",
    anonymous: "匿名",
    roleName: "ロール名",
    allowLogin: "ログインを許可",
    ...
    privilegeHint:
      "権限、データベース、テーブルを選択し、適用前にSQLをプレビューしてください。* はグローバルまたは全テーブルを意味します。",
    database: "データベース",
    schema: "スキーマ",
    table: "テーブル",
    scope: "権限スコープ",
  ...
  structureEditor: {
    ...
    sqlServerIdentityTypeHint:
      "SQL Server の ID 列は tinyint、smallint、int、bigint、scale 0 の decimal/numeric のみ対応します",
  },
  diagram: {
    title: "リレーション図",
    open: "図を表示",
  ...
  redis: {
    ...
    binaryStringReadonlyHint:
      "バイナリ文字列値は読み取り専用モードでエスケープテキストとして表示されます。生バイトの編集はサポートされていません。",
    selectedKeys: "選択中のキー",
    deleteGroup: "グループを削除",
    ...
    blockedCommand:
      "コマンド {command} は安全のためブロックされています。許可するにはツールバーのシールドアイコンを無効にしてください。",
    pubsub: "Pub/Sub",
  ...
  dangerDialog: {
    ...
    deleteRowsMessage:
      "これら{count}行は削除対象としてマークされ、保存後にデータベースから削除されます。続行しますか？",
  ...
  dataCompare: {
    ...
    batchSummary:
      "{tables}テーブルを比較: {different}不一致、{same}一致、{failed}失敗; 追加 {added}、削除 {removed}、変更 {modified}",
  ...
  settings: {
    ...
    sqlFormatterConfigErrorUnsupportedParams:
      "パラメータ置換オプションはサポートされていません。SQLフォーマットはプレースホルダーを置換しません。",
    ...
    uiScale: "UIスケール",
    uiScaleDescription:
      "高DPIディスプレイ向けにデスクトップUI全体をスケーリングします。変更は即座に適用され、次回起動時に復元されます。",
    theme: "テーマ",
    selectTheme: "テーマを選択...",
    colorTheme: "配色テーマ",
    ...
    showTrayIconDescription:
      "無効時はアイコンが表示されませんが、ウィンドウを閉じると従来通りDBXはバックグラウンドに隠れます。",
    ...
    updateNotificationsEnabledDescription:
      "無効時、DBXはアプリやドライバーのアップデートを自動確認せず、アップデートバッジも表示しません。手動確認は引き続き利用可能です。",
    debugLoggingEnabled: "デバッグログを有効にする",
    debugLoggingEnabledDescription:
      "トラブルシューティング時に、より詳細なローカル診断ログを記録します。デフォルトでは無効です。",
    duckDbWorkerProcessIsolation: "DuckDB プロセス分離モード（実験的）",
    duckDbWorkerProcessIsolationDescription:
      "DuckDB クエリを独立した worker プロセスで実行します。長時間クエリがキャンセルに応答しない場合、DBX は worker プロセスを停止してデータベースファイルのロックを解放できます。",
    ...
    compactColumnHeaderActionsDescription:
      "フォーマッターとローカルフィルターツールをもっと見るメニューに移動し、列名を優先表示します。",
    dataGridQuickEntry: "グリッド高速入力",
    dataGridQuickEntryDescription:
      "有効にすると、セル編集や下部の空行入力はフォーカスが外れた時点でデータベースに保存されます。頻繁な入力に便利ですが、誤操作による書き込みのリスクがあります。",
    tableColumnTemplateFields: "新規テーブルのプリセット列",
    tableColumnTemplateFieldsDescription:
      "データベース種別を選択し、新規テーブル作成時に使うプリセット列の型を設定します。",
    ...
    syncWebDavDescription:
      "保存済みSQLライブラリを含むDBXスナップショットをWebDAV互換ストレージへアップロードまたは復元します。",
    ...
    syncRememberWebDavPasswordDescription:
      "パスワードは暗号化され、このデバイスにのみ保存されます。WebDAVには同期されず、同期パスワードを置き換えるものでもありません。",
    syncRemotePath: "リモートスナップショットパス",
    syncRemotePathDescription: "アップロード時に不足している親フォルダをDBXが作成します。",
    syncSecretNotice:
      "デフォルトでは、DBXは接続詳細と設定のみを同期します。データベースパスワード、SSHパスワード、プロキシパスワード、接続文字列、AI APIキーはローカルに保持されます。",
    syncSecrets: "暗号化されたシークレットを同期",
    syncSecretsDescription:
      "有効時、DBXはデータベースパスワード、SSHパスワード、AI APIキーを暗号化してからWebDAVにアップロードします。",
    syncSecretsPassphrase: "同期パスワード",
    syncSecretsPassphraseDescription:
      "このパスワードは機密データの暗号化と復元にのみ使用されます。DBXは保存せず、各デバイスで再度入力する必要があります。",
    syncTest: "テスト",
    syncUpload: "アップロード",
    syncDownload: "ダウンロード",
    ...
    syncAutoUploadDescription:
      "DBXを開いている間だけ実行されます。ローカルの保存済みSQLを誤って置き換えないよう、ダウンロードは手動確認が必要です。",
    ...
    syncDownloadConfirm:
      "リモートDBXスナップショットをダウンロードして適用しますか？ローカルのメタデータと保存済みSQLが置き換えられます。",
    apply: "適用",
    ...
    showExecutionTargetPickerDescription:
      "有効にすると、選択なしで実行するときに現在の文とすべてのSQLを一時的に選べます。",
    wordWrap: "折り返し",
    ...
    sqlSemanticDiagnosticsEnabledDescription:
      "有効時、エディタは不明なテーブルや列などの意味的な問題を表示します。大きなSQLの解析とメタデータ確認の負荷を減らすには無効にします。",
    confirmDangerousSqlExecution: "危険なSQLの前に確認",
    confirmDangerousSqlExecutionDescription:
      "無効時、ALTER、DROP、DELETE、TRUNCATEなどの危険なSQLが警告ダイアログなしで実行されます。",
    confirmUnsavedSqlClose: "未保存のSQLを閉じる前に確認",
    confirmUnsavedSqlCloseDescription:
      "無効時、未保存の編集があるSQLタブは保存確認ダイアログなしで閉じるか終了します。",
    autoAliasTables: "テーブル別名を自動追加",
    autoAliasTablesDescription:
      "FROM または JOIN でテーブル補完を選択すると、order_items AS oi のような生成済み別名を挿入します。",
    ...
    mcpTitle: "MCPサーバー",
    mcpDescription:
      "Claude Code、Cursor、その他のエージェントが使用するDBX MCPサーバーのインストールとバージョンを確認します。",
    mcpChecking: "確認中",
    ...
    mcpVsCodeConfigPath:
      "VS Code/Copilotはワークスペースの .vscode/mcp.json またはユーザープロファイルの mcp.json を使用できます。",
    ...
    mcpAllowDangerousDescription:
      "サンプル設定にDBX_MCP_ALLOW_DANGEROUS_SQL=1を追加し、DROP、TRUNCATE、ALTERなどの文を許可します。",
    mcpDetectionTiming: "このページを開くとDBXが自動的に確認します。更新するには再確認を使用してください。",
    mcpNpmBoundary:
      "DBXはMCPステータスの確認と説明のみを行います。インストールとアップグレードはnpmを通じて実行されます。",
    mcpRefresh: "再確認",
    mcpGuide: "MCPガイド",
    updateDownloadSource: "アップデートのダウンロード元",
    updateDownloadSourceDescription:
      "アプリ内アップデートのインストーラーのダウンロード元を選択します。公式ソース推奨。中国本土ではCNBまたはAtomGitの方が高速な場合があります。",
    ...
    queryExportKeysetOptimizationEnabledDescription:
      "安全に認識された単一テーブルクエリにのみ適用。複雑なクエリは自動的にフォールバックします。",
  ...
  driverStore: {
    ...
    offlineDownloadHint:
      "エアギャップ環境の場合は、インターネット接続されたマシンでオフラインドライバーパッケージをダウンロードし、ここでインポートしてください。",
    ...
    driverStoreDirDescription:
      "JDBCプラグインとエージェント/JREデータの保存場所を個別に設定します。移行後にアプリは自動的に再起動します。",
    ...
    driverStoreDirConfirm:
      "{target}を変更すると関連ドライバーデータが新しい場所に移行されます。移行中に関連ドライバー接続が切断される可能性があります。移行後にアプリは自動的に再起動します。続行しますか？",
