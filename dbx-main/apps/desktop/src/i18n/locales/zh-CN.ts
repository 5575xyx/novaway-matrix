...
export default withEnglishFallback({
  ...
  connection: {
    ...
    nacosConsoleUrlHint:
      "填写 Nacos 控制台/admin API 地址。Nacos 3 Docker 通常是 8085；如果旧版控制台和服务共用 8848，也可以填 8848。",
    ...
    sqlServerLegacyUnencryptedModeHint:
      "适用于非加密或仅登录阶段加密的场景。若服务端强制 TLS 1.0 加密，此模式仍会失败\n请仅在可信内网、VPN 或 SSH 隧道中使用",
    ...
    agentDriverUpdateConnectionHint:
      "当前连接使用的内置驱动有可用更新，连接失败可能与本地驱动版本过旧有关。请在「驱动管理」中更新对应驱动后重试。",
    ...
    dmDownload: "前往达梦官网下载",
    mongoLegacyHint:
      "旧版 MongoDB（尤其是 4.2 以下版本）请选择旧版兼容。若账号创建在 admin 且认证失败，请将认证库设为 admin。",
    mongoTlsAllowInvalidCertificates: "禁用 TLS 证书校验",
    mongoTlsAllowInvalidCertificatesHint:
      "对应 tlsAllowInvalidCertificates=true，会禁用全部证书校验（不仅是主机名），存在中间人攻击风险。经 SSH 隧道访问 AWS DocumentDB 时通常需要开启。",
  ...
  ai: {
    ...
    codexMcpRequiredDescription:
      "Codex CLI 通过 DBX MCP Server 访问数据库结构和查询工具。请先安装后再在 AI 助手中使用 Codex。",
    reasoningLevel: "推理级别",
  ...
  contextMenu: {
    ...
    confirmDropMongoAllIndexesDetails:
      "MongoDB 会保留 “_id_” 索引。若服务端因 shard key 规则保留了额外索引，最终结果将仅列出实际删除的索引。",
  ...
  settings: {
    ...
    updateDownloadSourceDescription:
      "选择应用内更新安装包的下载来源。官方源为推荐选项，CNB 或 AtomGit 适合国内网络环境。",
    ...
    duckDbWorkerProcessIsolationDescription:
      "将 DuckDB 查询放到独立 worker 进程中运行。停止长查询时，如果 DuckDB 未及时响应取消请求，DBX 会结束 worker 进程以释放数据库文件锁。",
    ...
    dataGridQuickEntryDescription:
      "开启后，编辑单元格或填写底部空行并离焦会立即保存到数据库。便于高频录入，但存在误操作风险。",
    ...
    syncSecretNotice:
      "默认只同步连接信息和设置，不会同步数据库密码、SSH 密码、代理密码、HTTP 隧道 Token、连接串或 AI API Key。",
    syncSecrets: "同步加密后的敏感信息",
    syncSecretsDescription:
      "开启后，DBX 会先加密数据库密码、SSH 密码、代理密码、HTTP 隧道 Token 和 AI API Key，再上传到 WebDAV。",
    ...
    sqlSemanticDiagnosticsEnabledDescription:
      "开启后，编辑器会提示未知表、字段等语义问题；关闭可减少 SQL 解析和元数据检查的性能开销。",
  ...
  driverStore: {
    ...
    driverStoreDirConfirm:
      "更改 {target} 将迁移对应驱动数据到新位置，迁移期间可能会断开相关驱动连接。迁移完成后应用将自动重启。是否继续？",
