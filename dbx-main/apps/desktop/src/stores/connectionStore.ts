...
} from "@/lib/database/databaseTree"
...
} from "@/lib/sidebar/treeNodeContext"
...
import {
  createMetadataLoadTrace,
  logMetadataLoadTrace,
  MetadataLoadCoordinator,
  type MetadataLoadTraceLogger,
} from "@/lib/metadata/metadataLoadCoordinator"
...
function ensureSqlServerLinkedRootNode(
  connectionId: string,
  children: TreeNode[],
  ...
  if (
    children.some((child) => child.type === "linked-server-root" || child.id === sqlServerLinkedRootId(connectionId))
  ) {
    return children
  }
...
export const useConnectionStore = defineStore("connection", () => {
  ...
  function runTreeMetadataLoad<T>(
    scope: MetadataScopeInput,
    task: () => Promise<T>,
  ...
  async function loadCachedMetadataListPage<T extends MetadataListPageResult>(
    scope: MetadataScopeInput,
    load: () => Promise<T>,
    ...
    logMetadataLoadTrace(metadataTraceLogger, trace, "cache-miss", {
  ...
  async function ensureLocalConnectionAttemptActiveAfterConnectResult(
    connectionId: string,
    attempt: number,
    cleanupConnectionId: string,
  ) {
    if (isCancelledLocalConnectionAttempt(connectionId, attempt)) {
      ...
      return await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          timer = setTimeout(() => {
            reject(
              new Error(
                `Connection health check timed out after ${Math.ceil(CONNECTION_HEALTH_CHECK_TIMEOUT_MS / 1000)}s.`,
              ),
            )
      ...
      return await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          timer = setTimeout(() => {
            reject(
              new Error(
                `Connection timed out while loading ${label} after ${Math.ceil(timeoutMs / 1000)}s. Please check the network or VPN and try again.`,
              ),
            )
          }, timeoutMs)
        }),
    ...
    try {
      await Promise.race([
        ...
        new Promise<void>((resolve) => {
          timer = setTimeout(() => {
            timedOut = true
            console.warn("[DBX][connection:disconnect-timeout]", {
              connectionId,
              timeoutMs: DISCONNECT_REQUEST_TIMEOUT_MS,
            })
            resolve()
  ...
  async function withConnectionAttemptTimeout<T>(promise: Promise<T>, config: ConnectionConfig): Promise<T> {
    ...
    void promise.then(
      ...
      (error) => {
        ...
        setConnectionError(
          config.id,
          connectionAttemptOriginalErrorMessage(timeoutMessage, connectionErrorMessage(error)),
        )
      },
    )
    try {
  ...
  function objectGroupChildrenFromObjects(options: {
    node: TreeNode
    parentNodeId: string
    ...
    const grouped = buildGroupedObjectTreeNodes({
      ...
      objects: options.objects.filter((object) =>
        options.objectTypes.includes(normalizedObjectTreeKind(object.object_type)),
      ),
    })
  ...
  function mergeLocatedTreeChildren(
    parent: TreeNode,
    ...
    let merged = tableChildren.length
      ? mergeTableTreePageChildren(currentChildren, tableChildren, connectionId, database)
      : [...currentChildren]
  ...
  async function loadPagedTableGroupChildren(options: {
    ...
    const tables = await loadCachedMetadataListPage<TableInfo[]>(
      ...
      () =>
        api.listTables(
          options.node.connectionId!,
          options.node.database!,
          options.querySchema,
          searchFilter,
          fetchLimit,
          fetchOffset,
    ...
    indexCompletionTables(
      options.node.connectionId,
      options.node.database,
      options.effectiveSchema,
      tableInfosToCompletionTables(pageTables, options.effectiveSchema),
    )
  ...
  async function loadSimpleSupplementalObjectChildren(options: {
    ...
    try {
      ...
      const mergedChildren = mergeLocatedTreeChildren(
        options.node,
        currentChildren,
  ...
  async function loadPersistedTreeChildren(node: TreeNode, cacheKey: string): Promise<PersistedTreeChildrenLoadResult> {
    ...
    const childrenWithLinkedServers =
      node.type === "connection" && node.connectionId
        ? ensureSqlServerLinkedRootNode(node.connectionId, cachedChildren, config)
        : cachedChildren
  ...
  function isSidebarSearchQueryChanged(options?: LoadTreeOptions) {
    return (
      options?.expectedSidebarSearchQuery !== undefined &&
      (sidebarSearchQuery.value || "") !== options.expectedSidebarSearchQuery
    )
  }
  ...
  function isSidebarTableSearchQueryChanged(options?: LoadTreeOptions) {
    ...
    return (
      (sidebarTableSearchQueries.value[options.sidebarTableSearchParentId]?.trim() || "") !==
      options.expectedSidebarTableSearchQuery
    )
  }
  ...
  async function syncMongoLegacyDriverFallback(connectionId: string, previousConfig: ConnectionConfig) {
    if (
      !isDesktop ||
      previousConfig.db_type !== "mongodb" ||
      previousConfig.driver_profile === MONGO_LEGACY_DRIVER_PROFILE
    ) {
      return
    }

    const savedConnections = await api.loadConnections().catch(() => null)
    const savedConfig = savedConnections
      ?.map((connection) => normalizeConnection(connection))
      .find((connection) => connection.id === connectionId && connection.driver_profile === MONGO_LEGACY_DRIVER_PROFILE)
        ...
        try {
          ...
          if (config?.db_type === "duckdb") {
            ...
            const [databases, schemas] = await Promise.all([
              withMetadataLoadTimeout(connectionId, api.listDatabases(connectionId), "databases"),
              withMetadataLoadTimeout(connectionId, api.listSchemas(connectionId, "main"), "schemas"),
            ])
            ...
            const schemas = await withMetadataLoadTimeout(
              connectionId,
              api.listSchemas(connectionId, effectiveDb, true),
              "schemas",
            )
            ...
            const databaseNodes = buildDatabaseTreeNodes(connectionId, visibleDatabases, {
              includeDefaultWhenEmpty:
                usesTreeSchemaMode(effectiveDbType) || shouldIncludeDefaultDatabaseNode(config, visibleDatabases),
            })
            if (config?.db_type === "sqlserver") {
              const linkedServers = await withMetadataLoadTimeout(
                connectionId,
                api.listSqlServerLinkedServers(connectionId),
                "linked servers",
              ).catch(() => [])
  ...
  async function loadMqTenants(connectionId: string, options?: LoadTreeOptions) {
    ...
    try {
      ...
      if (isKafkaMqConnection(config)) {
        ...
        const tenants = await withMetadataLoadTimeout(
          connectionId,
          api.mqListTenants(connectionId),
          "message queue tenants",
        )
  ...
  async function loadElasticsearchIndices(connectionId: string) {
    ...
    try {
      await ensureConnected(connectionId)
      const indices = await withMetadataLoadTimeout(
        connectionId,
        api.elasticsearchListIndices(connectionId),
  ...
  async function loadVectorCollections(connectionId: string) {
    ...
    try {
      await ensureConnected(connectionId)
      const collections = await withMetadataLoadTimeout(
        connectionId,
        api.vectorListCollections(connectionId, database),
        "vector collections",
      )
  ...
  async function loadMongoCollections(connectionId: string, database: string) {
    ...
    try {
      ...
      const bucketNames = new Set(
        collections.filter((c) => c.kind === "bucket" && c.bucketName).map((c) => c.bucketName as string),
      )
      const hiddenCollectionNames = new Set(
        [...bucketNames].flatMap((bucketName) => [`${bucketName}.files`, `${bucketName}.chunks`]),
      )
  ...
  async function loadSqlServerDatabaseObjects(connectionId: string, database: string, options?: LoadTreeOptions) {
    ...
    try {
      ...
      const cacheKey = schemaCacheKey(
        connectionId,
        database,
        simpleObjectDisplay ? "sqlserver-schemas-simple-v4" : "sqlserver-schemas-grouped-v4",
      )
      if (!options?.force) {
        const cached = await loadPersistedTreeChildren(node, cacheKey)
        if (cached.hit) {
  ...
  async function loadSqlServerLinkedServerSchemas(node: TreeNode, options?: LoadTreeOptions) {
    ...
    try {
      ...
      if (useCachedChildren(node, options)) return
      const schemas = await api.listSqlServerLinkedServerSchemas(
        node.connectionId,
        node.linkedServer,
        node.linkedCatalog,
      )
    ...
    return runTreeMetadataLoad(
      {
        ...
        driverProfile: metadataDriverProfile(configForScope),
        extra: options?.sidebarTableSearchParentId
          ? { sidebarTableSearchParentId: options.sidebarTableSearchParentId }
          : undefined,
      },
      async () => {
        ...
        try {
          ...
          const cacheKey = schemaCacheKey(
            ...
            database,
            schema || "",
            simpleObjectDisplay ? "objects-simple-v3" : "objects-grouped-v3",
          )
          ...
          const nonTableObjectTypes = simpleObjectDisplay
            ? supportedSidebarObjectTypes(config).filter((objectType) => objectType !== "TABLE")
            : []
          ...
          if (simpleObjectDisplay) {
            ...
            children =
              page.hasMore && !searchFilter
                ? [...page.children, buildLoadMoreNode(node, page.nextOffset, pageSize)]
                : page.children
    ...
    return runTreeMetadataLoad(
      ...
      async () => {
        ...
        try {
          ...
          const wantsOnlyTablesOrViews = objectTypes.every(
            (objectType) => objectType === "TABLE" || objectType === "VIEW" || objectType === "MATERIALIZED_VIEW",
          )
          let children: TreeNode[]
          ...
          if (wantsOnlyTablesOrViews) {
            ...
            children =
              page.hasMore && !searchFilter
                ? [...page.children, buildLoadMoreNode(node, page.nextOffset, sidebarObjectGroupPageSize())]
                : page.children
            nextObjectCount = page.objectCount
          } else {
            const objects = await loadCachedMetadataListPage<ObjectInfo[]>(
      ...
      async () => {
        ...
        try {
          ...
          if (parent.type === "database" || parent.type === "schema" || parent.type === "linked-server-schema") {
            ...
            const page = await loadPagedSimpleTableChildren({
              nodeId: parent.schema
                ? `${parentConnectionId}:${parentDatabase}:${parent.schema}`
                : `${parentConnectionId}:${parentDatabase}`,
              connectionId: parentConnectionId,
            ...
            const mergedChildren = mergeTableTreePageChildren(
              currentChildren,
              page.children,
              parentConnectionId,
              parentDatabase,
            )
            const nextChildren = page.hasMore
              ? [...mergedChildren, buildLoadMoreNode(parent, page.nextOffset, loadMore.pageSize)]
              : mergedChildren
            ...
            await savePersistedTreeChildren(
              schemaCacheKey(parentConnectionId, parentDatabase, parent.schema || "", "objects-simple-v3"),
              nextChildren,
            )
  ...
  async function loadTableForLocate(target: LocateTableTarget): Promise<boolean> {
    ...
    return runTreeMetadataLoad(
      ...
      async () => {
        ...
        if (simpleObjectDisplay) {
          const parentId = target.schema
            ? `${target.connectionId}:${target.database}:${target.schema}`
            : `${target.connectionId}:${target.database}`
  ...
  async function loadIndexes(connectionId: string, database: string, table: string, schema?: string, nodeId?: string) {
    ...
    node.isLoading = true
    try {
      const metadataCapabilities = getTableMetadataCapabilities(
        effectiveDatabaseTypeForConnection(getConfig(connectionId)),
      )
      if (!metadataCapabilities.indexes) {
        setChildren(node, [])
        node.isExpanded = true
  ...
  function withCompletionInFlight<T>(
    key: string,
    load: () => Promise<T>,
    limit?: { scope: string; kind: string },
  ): Promise<T> {
  ...
  function completionAssistantColumns(
  ...
  async function listCompletionAssistantColumns(
    connectionId: string,
    database: string,
    table: string,
    schema?: string,
  ...
  function tableMatchScore(table: SqlCompletionTable, filter: string, preferredSchema?: string): number {
    ...
    if (
      normalized.length <= segments.length &&
      segments.every((segment, index) => segment.startsWith(normalized[index] ?? ""))
    )
      return score + 6_700 - text.length
  ...
  function indexCompletionForeignKeys(
    connectionId: string,
    database: string,
    table: string,
    schema: string | undefined,
  ...
  function lookupLocalCompletionSchemas(connectionId: string, database: string, filter = "", limit = 50): string[] {
    const schemas = dedupeCompletionQualifierNames([
      ...(schemaListCache.value[`${connectionId}:${database}`] ?? []),
      ...completionSchemasFromTree(treeNodes.value, connectionId, database),
    ])
    const normalized = filter.trim().toLowerCase()
    return schemas
  ...
  function lookupLocalCompletionForeignKeys(
    connectionId: string,
    database: string,
    table: string,
    schema?: string,
    ...
    return (
      completionForeignKeyIndex.get(completionForeignKeysKey(connectionId, database, table, schema))?.foreignKeys ?? []
    )
  }
  ...
  async function listMongoCompletionFields(
    connectionId: string,
    database: string,
    collection: string,
  ): Promise<MongoCompletionField[]> {
      ...
      async () => {
        ...
        if (isSchemaAwareDatabase(connectionId)) {
          if (normalizedFilter || limit) {
            ...
            if (results.length === 0 && relaxedFilter) {
              if (schema) {
                ...
                results = lookupLocalCompletionTables(
                  connectionId,
                  database,
                  relaxedFilter,
                  expandedCompletionLimit(limit),
          ...
          if (schema) {
            ...
            completionTablesCache.value[cacheKey] = lookupLocalCompletionTables(
              connectionId,
              database,
              normalizedFilter,
              limit,
  ...
  function fuzzyCompletionObjectMatch(object: SqlCompletionObject, filter: string): boolean {
    return (
      fuzzyTextMatch(object.name, filter) ||
      (!!object.schema && fuzzyTextMatch(object.schema, filter)) ||
      (!!object.parentName && fuzzyTextMatch(object.parentName, filter)) ||
      (!!object.parentSchema && fuzzyTextMatch(`${object.parentSchema}.${object.parentName ?? ""}`, filter))
    )
  }
  ...
  async function listCompletionForeignKeys(
    connectionId: string,
    database: string,
    table: string,
    schema?: string,
  ): Promise<SqlCompletionForeignKey[]> {
    if (
      isSchemaAwareDatabase(connectionId) &&
      !connectionUsesDatabaseObjectTreeMode(getConfig(connectionId)) &&
      !schema
    ...
    if (!completionForeignKeysCache.value[cacheKey]) {
      await withCompletionInFlight(
        ...
        async () => {
          await ensureConnected(connectionId)
          const querySchema = metadataQuerySchema(connectionId, database, schema)
          completionForeignKeysCache.value[cacheKey] = await api.listForeignKeys(
            connectionId,
            database,
            querySchema,
            table,
  ...
  function refreshCompletionForeignKeys(
    connectionId: string,
    database: string,
    table: string,
    schema?: string,
  ...
  async function refreshSidebarObjectPagination() {
    ...
    const isDirectObjectParent = (node: TreeNode) => {
      ...
      return node.children.some(
        (child) =>
          child.type === "table" ||
          child.type === "view" ||
          child.type === "materialized_view" ||
          child.type === "procedure" ||
          child.type === "function" ||
          child.type === "sequence" ||
          child.type === "package" ||
          child.type === "package-body" ||
          child.type === "load-more",
      )
    }
    const refreshNodes = async (nodes: TreeNode[]) => {
      for (const node of nodes) {
        ...
        if (
          simpleObjectDisplay &&
          (node.type === "database" || node.type === "schema" || node.type === "linked-server-schema")
        ) {
