...
export const useSavedSqlStore = defineStore("savedSql", () => {
  ...
  function listChildFolders(connectionId: string, parentFolderId?: string) {
    return sortFoldersByOrder(
      folders.value.filter(
        (folder) => folder.connectionId === connectionId && (folder.parentFolderId || "") === (parentFolderId || ""),
      ),
    )
  }
    ...
    const createPromise = (async () => {
      ...
      const folder: SavedSqlFolder = {
        ...
        orderIndex:
          maxOrderIndex(
            folders.value.filter(
              (item) => item.connectionId === connectionId && (item.parentFolderId || "") === (parentFolderId || ""),
            ),
          ) + 1,
        createdAt: timestamp,
        updatedAt: timestamp,
  ...
  async function renameFile(id: string, name: string) {
    ...
    files.value = files.value.map((file) =>
      file.id === id ? { ...saved, sql: file.sql, sqlLoaded: file.sqlLoaded } : file,
    )
    bumpVersion()
    await syncToLocalDirectory()
  }
  ...
  async function syncToLocalDirectory() {
    ...
    const syncPromise =
      pendingSync?.catch(() => {}).then(() => api.syncSavedSqlDirectory({ targetDir, entries })) ??
      api.syncSavedSqlDirectory({ targetDir, entries })
  ...
  async function reorderFolders(draggedId: string, targetId: string, position: "before" | "after") {
    ...
    const ordered = sortFoldersByOrder(
      folders.value.filter((folder) => (folder.parentFolderId || "") === (targetParentFolderId || "")),
    )
    ...
    const updatedSourceGroup =
      previousParentFolderId === targetParentFolderId
        ...
        : reindexFolders(
            sortFoldersByOrder(
              folders.value.filter(
                (folder) => folder.id !== draggedId && (folder.parentFolderId || "") === (previousParentFolderId || ""),
              ),
            ),
          ).map((folder) => ({
            ...folder,
    ...
    const untouched = folders.value.filter(
      (folder) =>
        folder.id !== draggedId &&
        (folder.parentFolderId || "") !== (targetParentFolderId || "") &&
        (folder.parentFolderId || "") !== (previousParentFolderId || ""),
    )
  ...
  async function moveFolderToFolder(folderId: string, parentFolderId?: string) {
    ...
    const sourceGroup = reindexFolders(
      sortFoldersByOrder(
        folders.value.filter(
          (folder) => folder.id !== folderId && (folder.parentFolderId || "") === (previousParentFolderId || ""),
        ),
      ),
    ).map((folder) => ({ ...folder, updatedAt: timestamp }))
    const destinationGroup = reindexFolders([
      ...sortFoldersByOrder(
        folders.value.filter(
          (folder) => folder.id !== folderId && (folder.parentFolderId || "") === (nextParentFolderId || ""),
        ),
      ),
      { ...target, parentFolderId: nextParentFolderId, updatedAt: timestamp },
    ]).map((folder) => ({
      ...folder,
  ...
  async function moveFilesToFolder(fileIds: string[], folderId?: string) {
    ...
    const nextAffectedFiles = Array.from(affectedFolderIds).flatMap((groupId) => {
      ...
      const remaining = sortFilesByOrder(
        files.value.filter((file) => (file.folderId || "") === groupId && !moveIdSet.has(file.id)),
      )
    ...
    const untouched = files.value.filter(
      (file) => !affectedFolderIds.has(file.folderId || "") && !moveIdSet.has(file.id),
    )
