...
describe("connectionStore MQ sidebar tree", () => {
  ...
  it("loads Pulsar tenants under a message queue connection", async () => {
    ...
    expect(
      node.children?.map((child) => ({
        ...
        label: child.label,
        type: child.type,
        tenant: (child as TreeNode & { mqTenant?: string }).mqTenant,
      })),
    ).toEqual([
  ...
  it("adds a Kafka topics child with a topics initial tab", async () => {
    ...
    expect(
      node.children?.map((child) => ({ label: child.label, tenant: child.mqTenant, initialTab: child.mqInitialTab })),
    ).toEqual([{ label: "Topics", tenant: "_kafka", initialTab: "topics" }])
  })
