...
import {
  ...
  restoreOpenTabsPayload,
  restoreOpenTabsState,
  serializeOpenTabs,
} from "@/lib/app/openTabsPersistence"
import {
...
import {
  ...
  type DecodedQueryResultArchive,
} from "@/lib/query/queryResultArchive"
...
import {
  ...
  initSavedSqlEditorPositions,
  restoreSavedSqlEditorPosition,
  saveSavedSqlEditorPosition,
} from "@/lib/app/savedSqlEditorPosition"
...
function tabClientSessionId(
  tab: Pick<QueryTab, "id">,
  suffix?: (typeof BACKGROUND_CLIENT_SESSION_SUFFIXES)[number],
): string {
...
function markQueryResultRunsRowsRaw(
  resultRuns: NonNullable<QueryTab["resultRuns"]>,
): NonNullable<QueryTab["resultRuns"]> {
...
function restoreSavedTabsFromPayload(payload: { tabs?: unknown; activeTabId?: unknown } | null | undefined): {
  tabs: QueryTab[]
...
export const useQueryStore = defineStore("query", () => {
  ...
  async function closeClientSessionId(
    connectionId: string,
    database: string,
  ...
  async function closeClientConnectionSession(tab: QueryTab | undefined) {
    ...
    const clientSessionIds = [
      tabClientSessionId(tab),
      ...BACKGROUND_CLIENT_SESSION_SUFFIXES.map((suffix) => tabClientSessionId(tab, suffix)),
    ]
  ...
  function sortTabResultLocally(
    ...
    const rows = direction
      ? sortDataGridRows(tab.resultLocalSortOriginalRows, columnIndex, direction)
      : tab.resultLocalSortOriginalRows
  ...
  function openMongoBucket(connectionId: string, database: string, bucketName: string) {
    ...
    const existing = tabs.value.find(
      (tab) =>
        tab.mode === "mongo-bucket" &&
        tab.connectionId === connectionId &&
        tab.database === database &&
        tab.mongoBucket?.bucketName === bucketName,
    )
    if (existing) {
      activeTabId.value = existing.id
      return existing.id
  ...
  function openMongoGridFs(connectionId: string, database: string) {
    const existing = tabs.value.find(
      (tab) => tab.mode === "mongo-gridfs" && tab.connectionId === connectionId && tab.database === database,
    )
    if (existing) {
      activeTabId.value = existing.id
      return existing.id
  ...
  function openNacosAdmin(connectionId: string, target?: { namespace?: string; namespaceName?: string }) {
    ...
    const existing = tabs.value.find(
      (tab) => tab.mode === "nacos" && tab.connectionId === connectionId && (tab.nacosNamespace || "") === namespace,
    )
    if (existing) {
      existing.nacosNamespaceName = namespaceName
      if (!existing.customTitle)
        existing.title = `${useConnectionStore().getConfig(connectionId)?.name || "Nacos"}:${namespaceName}`
      activeTabId.value = existing.id
      return existing.id
    }
  ...
  const closeConfirmDirtyTabIds = computed(() => {
    ...
    const pendingTab = pendingCloseTabId.value
      ? tabs.value.find((tab) => tab.id === pendingCloseTabId.value)
      : undefined
  ...
  function finishPendingBatchClose() {
    ...
    if (finalActiveTabId !== undefined) {
      activeTabId.value =
        finalActiveTabId && tabs.value.some((tab) => tab.id === finalActiveTabId) ? finalActiveTabId : null
    }
  }
  ...
  function continuePendingBatchClose() {
    ...
    const dirtyTab = shouldConfirmUnsavedSqlClose.value
      ? remainingIds
          .map((id) => tabs.value.find((tab) => tab.id === id))
          .find((tab): tab is QueryTab => !!tab && isTabDirty(tab))
      : undefined
    if (dirtyTab) {
  ...
  function duplicateTab(id: string) {
    ...
    const newTab: QueryTab = {
      ...
      queryAnalysis: original.queryAnalysis
        ? { ...original.queryAnalysis, columns: original.queryAnalysis.columns.map((c) => ({ ...c })) }
        : undefined,
  ...
  function rollbackDatabaseTransactions(connectionId: string, database: string) {
    rollbackTabsWhere((tab) => tab.connectionId === connectionId && tab.database === database, {
  ...
  async function buildQueryMetadataPatch(
    ...
    try {
      ...
      const loadedMetadata = await loadTableMetadata({
        ...
        traceLogger: (event) =>
          console.debug("[DBX][executeTabSql:metadata:table-trace]", { sourceTraceId: traceId, ...event }),
      })
  ...
  async function executeTabSql(
    ...
    try {
      ...
      if (options?.skipEnsureConnected) {
        ...
        console.info("[DBX][executeTabSql:ensure-connected:skip]", {
          traceId,
          elapsed: elapsed(),
      ...
      if (conn?.db_type === "redis") {
        ...
        for (const command of commands) {
          try {
            ...
            allResults.push(
              markQueryResultRowsRaw(
                redisCommandResultToQueryResult(result.value, performance.now() - startedAt, result.command),
              ),
            )
      ...
      if (mongoCommands.length > 0) {
        console.info("[DBX][executeTabSql:mongo:start]", {
          traceId,
          database: tab.database,
        ...
        for (const parsedCommand of mongoCommands) {
          ...
          try {
            switch (mongoCommand.kind) {
              case "find": {
                console.info("[DBX][executeTabSql:mongo-find:start]", {
                  traceId,
                ...
                const queryResult = markQueryResultRowsRaw(
                  mongoDocumentsToQueryResult(result.documents, performance.now() - commandStartedAt, result.total),
                )
                allResults.push(queryResult)
                mongoEditTarget =
                  mongoCommands.length === 1 && queryResult.columns.includes("_id")
                    ? { collection: mongoCommand.collection, idColumn: "_id" }
                    : undefined
              ...
              case "version": {
                ...
                allResults.push(
                  markQueryResultRowsRaw(mongoVersionToQueryResult(version, performance.now() - commandStartedAt)),
                )
              ...
              case "countDocuments": {
                console.info("[DBX][executeTabSql:mongo-count:start]", {
                  traceId,
                  collection: mongoCommand.collection,
                  database: currentDatabase,
                })
                ...
                allResults.push(
                  markQueryResultRowsRaw(mongoCountToQueryResult(result.total, performance.now() - commandStartedAt)),
                )
                mongoEditTarget = undefined
              ...
              case "aggregate": {
                ...
                console.info("[DBX][executeTabSql:mongo-aggregate:start]", {
                  traceId,
                  collection: mongoCommand.collection,
                  database: currentDatabase,
                })
                const aggregateMaxRows = normalizeResultPageSize(
              ...
              case "getIndexes": {
                console.info("[DBX][executeTabSql:mongo-indexes:start]", {
                  traceId,
                  collection: mongoCommand.collection,
                  database: currentDatabase,
                })
                ...
                allResults.push(
                  markQueryResultRowsRaw(mongoIndexesToQueryResult(indexes, performance.now() - commandStartedAt)),
                )
                mongoEditTarget = undefined
              ...
              case "collectionStats": {
                ...
                const stats = await api.mongoCollectionStats(
                  tab.connectionId,
                  currentDatabase,
                  mongoCommand.collection,
                  mongoCommand.scale,
              ...
              case "dropIndexes": {
                ...
                if (mongoCommand.kind === "insert") {
                  const result = await api.mongoInsertDocuments(
                    ...
                    currentDatabase,
                    mongoCommand.collection,
                    mongoCommand.docsJson,
                  )
                  allResults.push(
                    markQueryResultRowsRaw(
                      mongoWriteToQueryResult(result.affected_rows, performance.now() - commandStartedAt),
                    ),
                  )
                  ...
                  const result = await api.mongoCreateIndex(
                    tab.connectionId,
                    currentDatabase,
                    mongoCommand.collection,
                    mongoCommand.keys,
                    mongoCommand.options,
                  )
                  allResults.push(
                    markQueryResultRowsRaw(
                      mongoCreateIndexToQueryResult(result.name, performance.now() - commandStartedAt),
                    ),
                  )
                  ...
                  const result = await api.mongoDropIndexes(
                    ...
                    currentDatabase,
                    mongoCommand.collection,
                    mongoCommand.kind === "dropIndex" ? mongoCommand.index : mongoCommand.indexes,
                    mongoCommand.kind === "dropIndex",
                  )
                  allResults.push(
                    markQueryResultRowsRaw(
                      mongoDroppedIndexesToQueryResult(result.dropped_names, performance.now() - commandStartedAt),
                    ),
                  )
                } else {
              ...
              case "use": {
                ...
                allResults.push(
                  markQueryResultRowsRaw(mongoUseToQueryResult(currentDatabase, performance.now() - commandStartedAt)),
                )
                mongoEditTarget = undefined
      ...
      if (tab.autoCommit === false) {
        if (!tab.txnSessionId) {
          ...
          console.info("[DBX][executeTabSql:begin-manual-txn:done]", {
            traceId,
            txnSessionId: tab.txnSessionId,
            elapsed: elapsed(),
          })
        }
        console.info("[DBX][executeTabSql:execute-in-txn:invoke]", {
          traceId,
          txnSessionId: tab.txnSessionId,
          elapsed: elapsed(),
        })
        executionPromise = api.executeInManualTransaction(
          tab.txnSessionId,
          sqlToExecute,
          tab.database,
          executionSchema,
        ...
        executionPromise = api.executeMulti(
          tab.connectionId,
          tab.database,
          sqlToExecute,
          executionSchema,
      ...
      const results = annotateQueryResultSources(
  ...
  function fallbackActiveTabAfterClose(closedId: string, closedIndex: number): string | null {
    ...
    return (
      [...history].reverse().find((tabId) => remainingIds.has(tabId)) ??
      tabs.value[Math.min(closedIndex, tabs.value.length - 1)]?.id ??
      null
    )
  }
  ...
  async function fetchTabResultForExport(
    id: string,
    ...
    const exportRowLimit = exportSettings.exportRowLimitEnabled
      ? exportSettings.exportRowLimit
      : Number.POSITIVE_INFINITY
    const agentExportMaxRows = exportSettings.exportRowLimitEnabled ? exportSettings.exportRowLimit : 2_147_483_647
    // Use the already-computed total row count as a progress estimate so the
    // export dialog shows a moving bar instead of a stuck 0 while paginating.
    const totalRows =
      typeof tab.resultTotalRowCount === "number" ? Math.min(tab.resultTotalRowCount, exportRowLimit) : null
  ...
  async function buildQueryResultExportRequest(id: string, options: BuildQueryResultExportRequestOptions) {
    ...
    const totalRows =
      typeof tab.resultTotalRowCount === "number"
        ? rowLimit === null
          ? tab.resultTotalRowCount
          : Math.min(tab.resultTotalRowCount, rowLimit)
        : null
    const clientSessionId = tabClientSessionId(tab, "export")
