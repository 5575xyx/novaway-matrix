...
import {
  useCloseActionPrompt,
  type AppCloseAction,
  type AppCloseRequestOptions,
} from "@/composables/useCloseActionPrompt"
...
import {
  ...
  resolveExecutableSqlWithBackend,
...
import {
  ...
  type AgentDriverUpdateBadgeState,
} from "@/lib/connection/agentDriverUpdateBadge"
...
const queryEditorDdlTarget = ref<{ connectionId: string; database: string; schema?: string; tableName: string } | null>(
  null,
)
...
} = useCloseActionPrompt({ requestClose: requestAppClose })
useVisibilityChange()
useWebDavAutoUpload()
...
const queryEditorDdlDialect = computed(() => {
  ...
  return codeMirrorSqlDialect(
    effectiveDatabaseTypeForConnection(connectionStore.getConfig(queryEditorDdlTarget.value.connectionId)),
  )
})
  ...
  if (item.type === "connection") {
    ...
    try {
      ...
      const tabId = queryStore.createTab(
        item.connectionId,
        item.database,
        `Source - ${item.objectName || item.tableName}`,
      )
    ...
    <TooltipProvider :delay-duration="300">
      ...
      >
        ...
        >
          ...
          >
            <div class="h-full flex flex-col min-w-0">
              ...
              <div
                v-if="activeTab"
                v-show="!driverStoreActive && !settingsPageActive"
        ...
        <CloseActionPromptDialog
          v-if="isDesktop && showCloseActionPrompt"
          :open="showCloseActionPrompt"
          @update:open="handleCloseActionPromptOpenChange"
          @quit="chooseQuit"
      ...
      >
        <DialogContent class="sm:max-w-[420px]">
          ...
          <DialogFooter>
            <Button v-if="isDesktop" variant="secondary" @click="saveActiveSqlAsLocalFile">{{
              t("savedSql.saveToFile")
            }}</Button>
