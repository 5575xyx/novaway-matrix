...
export function useDataGridActions(activeTab: ComputedRef<QueryTab | undefined>) {
  ...
  async function refreshDataTabTableMeta(
    tab: QueryTab,
    trace?: { traceId: string; elapsed: () => string },
  ): Promise<void> {
    ...
    console.info("[DBX][reloadData:metadata:ensure-connected:start]", {
      traceId: trace?.traceId,
      elapsed: trace?.elapsed(),
    })
    await connectionStore.ensureConnected(target.connectionId)
    console.info("[DBX][reloadData:metadata:ensure-connected:done]", {
      traceId: trace?.traceId,
      elapsed: trace?.elapsed(),
    })
    const config = connectionStore.getConfig(target.connectionId)
    const querySchema = metadataSchemaForConnection(config, target.database, target.schema)
    console.info("[DBX][reloadData:metadata:get-columns:start]", {
      traceId: trace?.traceId,
      elapsed: trace?.elapsed(),
      schema: querySchema,
    ...
    const indexes = await api
      .listIndexes(target.connectionId, target.database, querySchema, target.tableName)
      .catch(() => [])
    console.info("[DBX][reloadData:metadata:get-columns:done]", {
      traceId: trace?.traceId,
      elapsed: trace?.elapsed(),
    ...
    if (
      ...
      console.info("[DBX][reloadData:metadata:stale-tab]", {
        traceId: trace?.traceId,
        elapsed: trace?.elapsed(),
        table: target.tableName,
      })
    ...
    const primaryKeys = editableRowIdentifierColumns(
  ...
  async function onReloadData(
    ...
    if (tab.mode === "data" && tableMetaForDataTab(tab)) {
      ...
      const shouldRefreshMetadata =
        !tab.tableMeta || !tableMeta?.columns.length || metadataAgeMs > DATA_TAB_METADATA_TTL_MS
      if (shouldRefreshMetadata) {
        console.info("[DBX][reloadData:metadata:background:start]", {
          traceId,
          elapsed: elapsed(),
          reason: tableMeta?.columns.length ? "stale" : "missing",
          metadataAgeMs,
        })
        ...
        console.info("[DBX][reloadData:metadata:skip]", {
          traceId,
          elapsed: elapsed(),
