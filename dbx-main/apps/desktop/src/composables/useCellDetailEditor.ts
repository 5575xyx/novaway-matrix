...
export function useCellDetailEditor(options: UseCellDetailEditorOptions): UseCellDetailEditorReturn {
  ...
  watch(
    ...
    async ([fontSize, fontFamily, editorTheme, appearance, palette]) => {
      const editor = view.value
      ...
      view.value.dispatch({
        effects: [
          ...
          fontThemeComp.reconfigure(
            editorFontTheme(EditorView, liveFontSize, fontFamily, { fixedHeight: true, scrollable: true }),
          ),
        ],
      })
    },
  ...
  async function create(parent: HTMLElement, initialValue: string, columnType?: string): Promise<void> {
    ...
    const fontTheme = editorFontTheme(EditorView, liveFontSize, options.fontFamily(), {
      fixedHeight: true,
      scrollable: true,
    })
