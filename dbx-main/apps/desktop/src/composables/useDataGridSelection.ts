...
export function useDataGridSelection(options: UseDataGridSelectionOptions) {
  ...
  function toggleCellSelection(rowIndex: number, colIndex: number) {
    ...
    const next =
      selectedCellKeys.value.size > 0 ? new Set(selectedCellKeys.value) : selectedRangeToCellKeys(selectedRange.value)
    if (next.has(key)) {
      next.delete(key)
    } else {
      next.add(key)
...
function extractSelectedCellKeys(
  ...
  const positions = [...keys]
    .map(parseCellKey)
    .filter(
      (position): position is CellPosition =>
        !!position && position.rowIndex < rows.length && position.colIndex < columns.length,
    )
    .sort((a, b) => a.rowIndex - b.rowIndex || a.colIndex - b.colIndex)
  const selectedColumnIndexes = normalizeSelectedColumnIndexes(positions.map((position) => position.colIndex)).filter(
