...
export function useSqlExecution(deps: {
  ...
  async function resolvedExecutableSql(source?: SqlExecutionOverride): Promise<string> {
    ...
    if (isSqlExecutionSnapshot(source))
      return resolveExecutableSql(source.fullSql, source.selectedSql, { cursorPos: source.cursorPos })
  ...
  async function doExecute(sql?: string) {
    ...
    await queryStore.executeCurrentSql(
      sql,
      isRedis ? { skipRedisSafetyCheck: deps.blockDangerousRedisCommands?.value === false } : undefined,
    )
