...
type PendingChangesHistorySnapshot = Pick<
  PendingChangesSnapshot,
  "newRows" | "quickEntryDraftRow" | "dirtyRows" | "deletedRows" | "transactionActive"
>
...
export function useDataGridEditor(options: UseDataGridEditorOptions) {
  ...
  const resolvedDatabaseType = computed(
    () => databaseType.value ?? effectiveDatabaseTypeForConnection(connectionStore.getConfig(connectionId.value ?? "")),
  )
  ...
  const useTransaction = computed(
    () =>
      editable.value &&
      supportsDataGridTransaction(resolvedDatabaseType.value) &&
      (!!customSaveHandler?.value || hasBackendSaveTarget.value),
  )
  ...
  function focusEditInput(select = true) {
    const focusInput = () => {
      ...
      const input = (root ?? document).querySelector(".cell-edit-input") as
        | HTMLInputElement
        | HTMLTextAreaElement
        | null
      input?.focus()
  ...
  function onEditKeydown(e: KeyboardEvent) {
    const isExpandedTextarea =
      typeof HTMLTextAreaElement !== "undefined" &&
      e.target instanceof HTMLTextAreaElement &&
      e.target.dataset.expandedCellEditor === "true"
  ...
  function restoreRows(rowIds: number[]) {
    const sourceIndexes = rowIds
      .map((rowId) => getRowItem(rowId)?.sourceIndex)
      .filter((sourceIndex): sourceIndex is number => sourceIndex !== undefined && deletedRows.value.has(sourceIndex))
  ...
  function saveStatementOptions(snapshot = snapshotPendingSaveChanges()) {
    ...
    return {
      ...
      dirtyRows: [...snapshot.dirtyRows.entries()].map(
  ...
  function dataChangeOperation(snapshot: PendingSaveSnapshot) {
    const operations = [
      snapshot.newRows.length > 0 ? "INSERT" : "",
      snapshot.dirtyRows.size > 0 ? "UPDATE" : "",
      snapshot.deletedRows.size > 0 ? "DELETE" : "",
    ].filter(Boolean)
  ...
  async function saveChanges(saveOptions: SaveChangesOptions = {}) {
    ...
    if (
      ...
      customHandler.supportsInsert !== true &&
      customHandler.canInsert !== true
    ) {
    ...
    if (useTransaction.value && hasBackendSaveTarget.value) {
      ...
      try {
        apiResult = await api.executeBatch(
          connectionId.value!,
          database.value ?? "",
          stmts,
          preparedSave?.executionSchema,
