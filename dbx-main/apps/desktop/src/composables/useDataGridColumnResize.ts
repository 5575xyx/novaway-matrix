...
import {
  ...
  DATA_GRID_AUTO_FIT_VALUE_TEXT_LIMIT,
  DATA_GRID_COL_AUTO_FIT_MAX_WIDTH,
  DATA_GRID_COL_MIN_WIDTH,
  DATA_GRID_SAMPLE_ROWS,
} from "@/lib/dataGrid/dataGridColumnWidth"
...
export function useDataGridColumnResize(options: UseDataGridColumnResizeOptions) {
  ...
  function initColumnWidths() {
    ...
    if (
      columnWidths.value.length !== columns.value.length ||
      previousColumnIndexes.join("\0") !== nextColumnIndexes.join("\0")
    ) {
