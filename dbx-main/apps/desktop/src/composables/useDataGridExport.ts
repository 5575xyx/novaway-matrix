...
import {
  ...
  type SelectionData,
} from "@/lib/dataGrid/gridSelection"
...
export function useDataGridExport(options: UseDataGridExportOptions) {
  ...
  async function copySelectedRowsTsv() {
    ...
    const rows = displayItems.value
      .filter((item) => selectedRowIds.value.has(item.id) && !item.isDraft)
      .map((item) => item.data)
      ...
      try {
        ...
        const result = await resultToExport(rowIds, (info) => {
          if (needsFullExport && exportProgressState && exportProgressState.value.status === "Running") {
            ...
            const adjustedTotal =
              info.totalRows !== null && info.rowsExported > info.totalRows ? info.rowsExported : info.totalRows
            exportProgressState.value = {
              ...exportProgressState.value,
  ...
  async function exportQueryResultViaBackend(format: "csv" | "xlsx", rowIds?: number[]): Promise<boolean> {
    ...
    try {
      const terminalProgress = await api.startQueryResultExport(request, (progress) => {
        if (exportProgressState) {
          const adjustedTotal =
            progress.totalRows !== null && progress.rowsExported > progress.totalRows
  ...
  async function exportSql(rowIds?: number[]) {
    await runExclusiveExport(async () => {
      try {
        ...
        await saveTextFile(
          content,
          exportFileName(tableMeta.value?.tableName || "export", "sql", { preferFallback: true }),
          "SQL",
          "sql",
        )
        toast(t("grid.exported"))
  ...
  function exportFileName(
    ...
    extension: string,
    options: { page?: boolean; allResults?: boolean; preferFallback?: boolean } = {},
  ): string {
...
export function defaultDataGridExportFileName(
  baseName: string | undefined,
  fallbackBaseName: string,
  extension: string,
  options: { page?: boolean; allResults?: boolean } = {},
): string {
  const sanitizedBaseName =
    sanitizeExportBaseName(baseName || "") || sanitizeExportBaseName(fallbackBaseName) || "export"
...
function buildMongoCopyInsertStatement(options: {
  collection: string
  columns: string[]
  ...
  const columnIndexes = saveColumns
    .map((column, index) => ({ column, index }))
    .filter((item): item is { column: string; index: number } => !!item.column)
  ...
  const documents = options.rows.map((row) =>
    buildMongoCopyInsertDocument(columnIndexes.map((item) => row[item.index]) as MongoInputValue[], documentColumns, {
