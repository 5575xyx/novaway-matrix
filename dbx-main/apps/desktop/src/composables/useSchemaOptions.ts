...
import {
  isSchemaAware as isSchemaAwareType,
  isSingleDatabase,
  usesTreeSchemaMode,
...
export function schemaOptionsForConnection(
  schemaNames: string[],
  connection:
    | Pick<ConnectionConfig, "db_type" | "driver_profile" | "visible_databases" | "visible_schemas">
    | undefined,
  database = "",
): string[] {
...
export function useSchemaOptions() {
  ...
  async function loadSchemaOptions(connectionId: string, database: string) {
    ...
    try {
      ...
      schemaOptions.value[key] = schemaOptionsForConnection(
        await api.listSchemas(connectionId, database),
        connection,
        database,
      )
    } catch (e) {
