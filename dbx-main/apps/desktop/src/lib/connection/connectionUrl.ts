...
function parseJdbcInformixUrl(source: string): ParsedConnectionUrl | null {
  const match =
    /^jdbc:informix-sqli:\/\/(?:(?<userinfo>[^@/?#]*)@)?(?<host>\[[^\]]+\]|[^:/?#]+)(?::(?<port>\d+))?\/(?<database>[^:?#]*)(?::(?<params>[^?#]*))?/i.exec(
      source,
    )
  if (!match?.groups) return null
...
function parseJdbcDremioUrl(source: string): ParsedConnectionUrl | null {
  const match =
    /^jdbc:dremio:(?<mode>direct|zk)=(?<host>\[[^\]]+\]|[^:;]+)(?::(?<port>\d+))?(?:;(?<params>.*))?$/i.exec(source)
  if (!match?.groups) return null
...
export function parseConnectionUrl(value: string, preferredProfile?: string): ParsedConnectionUrl {
  ...
  const parsedUrlParams =
    profile.type === "redis" && normalizedFragment === "insecure"
      ? [urlParamsWithoutName, "insecure=true"].filter(Boolean).join("&")
      : urlParamsWithoutName
