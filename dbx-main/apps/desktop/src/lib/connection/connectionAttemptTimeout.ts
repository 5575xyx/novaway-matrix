...
export function connectionAttemptTimeoutMs(
  ...
  const agentMinTimeoutSecs =
    config.db_type === "access" ? ACCESS_AGENT_MIN_CONNECT_TIMEOUT_SECS : AGENT_DRIVER_MIN_CONNECT_TIMEOUT_SECS
  const timeouts = [
    DRIVER_STARTUP_FLOOR_TYPES.has(config.db_type as DatabaseType)
      ? Math.max(baseTimeoutSecs, agentMinTimeoutSecs)
      : baseTimeoutSecs,
  ]
