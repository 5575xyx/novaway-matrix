...
export function matchesTarget(node: TreeNode, target: ActiveTabSidebarTarget): boolean {
  ...
  if (target.type === "mongo-gridfs") {
    return (
      node.type === "mongo-gridfs" && node.connectionId === target.connectionId && node.database === target.database
    )
  }
  ...
  if (target.type === "mq-tenant") {
    return (
      node.type === "mq-tenant" &&
      node.connectionId === target.connectionId &&
      (node.mqTenant || node.label) === target.tenant
    )
  }

  if (target.type === "nacos-namespace") {
    return (
      node.type === "nacos-namespace" &&
      node.connectionId === target.connectionId &&
      (node.nacosNamespace || "") === target.namespace
    )
  }
...
export function findSidebarNodeForTarget(
...
export function findNodePathForTarget(
  target: ActiveTabSidebarTarget,
  treeNodes: readonly TreeNode[],
): TreeNode[] | null {
