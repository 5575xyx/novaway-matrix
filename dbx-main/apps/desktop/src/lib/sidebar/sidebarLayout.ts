...
export function reconcileLayout(connectionIds: string[], layout: SidebarLayout | null): SidebarLayout {
  ...
  const order = layout.order
    .map((entry) => normalizeEntry(entry, validIds, validGroups, seenConnections, seenGroups))
    .filter(Boolean) as SidebarOrderEntry[]
...
export function remapSidebarLayoutConnectionIds(
...
export function findConnectionLocation(
  layout: SidebarLayout,
  connectionId: string,
): { entries: SidebarOrderEntry[]; entryIndex: number; groupId?: string } | null {
  const visit = (
...
function findGroupEntry(
  entries: SidebarOrderEntry[],
  groupId: string,
): Extract<SidebarOrderEntry, { type: "group" }> | null {
  for (const entry of entries) {
    if (entry.type !== "group") continue
...
function cloneEntries(entries: SidebarOrderEntry[]): SidebarOrderEntry[] {
  return entries.map((entry) =>
    entry.type === "group"
      ? { type: "group", id: entry.id, children: cloneEntries(entryChildren(entry)) }
      : { ...entry },
  )
}
...
export function createGroup(
...
): { layout: SidebarLayout; groupId: string } {
  const groupId = uuid()
  ...
  return {
    groupId,
    layout: {
      groups: [...layout.groups, group].map((current) =>
        parentFound && current.id === parentGroupId ? { ...current, collapsed: false } : current,
      ),
      order,
    },
  }
