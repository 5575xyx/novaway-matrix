...
async function parseConnection(node: ParsedNode): Promise<ConnectionConfig | null> {
  ...
  if (serviceProvider) {
    ...
    if (sp.includes("oceanbase")) {
      ...
      effectiveProfile = oceanbaseOracleMode
        ? {
            ...profile,
        ...
        : { ...profile, dbType: "mysql", profile: "oceanbase", label: "OceanBase", port: 2881 }
  ...
  const host =
    ...
    (effectiveProfile.dbType === "sqlite" ? "" : "127.0.0.1")
  // Navicat exports OceanBase Oracle connections with Database="ORCL", but OceanBase resolves the target from the username.
  const database =
    effectiveProfile.dbType === "sqlite"
      ? sqlitePath
      : effectiveProfile.dbType === "oceanbase-oracle"
        ? ""
        : getAny(node.values, ["database", "databaseName", "initialDatabase", "serviceName", "sid", "schema"])
  const isOracleLike = effectiveProfile.dbType === "oracle" || effectiveProfile.dbType === "oceanbase-oracle"
  const oracleConnectionType =
    isOracleLike && getAny(node.values, ["sid"]) ? "sid" : isOracleLike ? "service_name" : undefined
  ...
  const keepaliveValue = Number(
    getAny(node.values, ["keepAliveInterval", "keepaliveInterval", "keepAliveTime", "keepaliveTime"]),
  )
  ...
  const keepaliveInterval =
    Number.isFinite(keepaliveValue) && keepaliveValue > 0 && keepaliveEnabled ? keepaliveValue : 0
