...
const KEY_ARGUMENT_GROUPS = new Set([
  "string",
  "generic",
  "list",
  "hash",
...
function buildSpecInfo(spec: RedisCommandSpec, label: string): string {
  return [
    `Command: ${label}`,
    `Group: ${spec.group}`,
    `Arity: ${describeArity(spec.arity)}`,
    `Safety: ${spec.safety}`,
  ].join("\n")
}
...
export function getRedisCompletionContext(text: string, cursor: number): RedisCompletionContext {
  ...
  const commandHeadTokens =
    typedTokens.length >= 2 && REDIS_COMMAND_TABLE[`${main} ${typedTokens[1]!.toUpperCase()}`] ? 2 : 1
...
export function buildRedisCompletionItemsFromContext(
  context: RedisCompletionContext,
  input: RedisCompletionInput = {},
): RedisCompletionItem[] {
  ...
  if (
    ...
    takesKeyArgument(context.mainCommand) &&
    shouldSuggestKeyAt(context.mainCommand, context.argumentIndex)
  ) {
...
export function buildRedisCompletionItems(
  text: string,
  cursor: number,
  input: RedisCompletionInput = {},
): RedisCompletionItem[] {
