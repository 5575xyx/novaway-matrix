...
export function buildNacosRawRequest(
  method: string,
  path: string,
  queryText: string,
  bodyText: string,
...
export function formatNacosConfigIdentity(
  item: Pick<NacosConfigItem, "namespace" | "dataId" | "group">,
  fallbackNamespace = "",
): string {
  return [
    `namespace=${item.namespace || fallbackNamespace || "public"}`,
    `dataId=${item.dataId}`,
...
export function buildNacosConfigExport(item: NacosConfigItem, content: string): string {
  return [
    ...
    `# dataId: ${item.dataId}`,
    `# group: ${item.group || "DEFAULT_GROUP"}`,
    item.configType ? `# type: ${item.configType}` : "",
    "",
    content,
  ]
...
export function resolveNacosConfigCopyText(
...
export function buildNacosConfigHistoryRollbackConfirm(item: NacosConfigHistoryItem, fallbackNamespace = ""): string {
  return [
    ...
    `dataId=${item.dataId}`,
    `group=${item.group || "DEFAULT_GROUP"}`,
    item.lastModifiedTime ? `historyTime=${item.lastModifiedTime}` : "",
...
export function buildNacosInstanceConfirm(
  service: NacosServiceInfo,
  instance: NacosInstanceInfo,
  patch: Partial<NacosInstanceInfo>,
  fallbackGroup = "",
  namespace = "",
): string {
