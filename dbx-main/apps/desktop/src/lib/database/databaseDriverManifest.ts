...
const DATABASE_DRIVER_BY_TYPE = new Map<DatabaseType, DatabaseDriverManifestEntry>(
  DATABASE_DRIVER_ENTRIES.map((entry) => [entry.dbType, entry]),
)
...
export function supportsDatabaseFeature(
  dbType: DatabaseType | undefined,
  capability: DatabaseProductCapability,
): boolean {
...
function productCapabilities(overrides: Partial<DatabaseProductCapabilities>): DatabaseProductCapabilities {
  const capabilities = Object.fromEntries(
    DATABASE_PRODUCT_CAPABILITY_KEYS.map((key) => [key, false]),
  ) as DatabaseProductCapabilities
