...
export function isSqlServerLinkedNode(
  node: Pick<TreeNode, "linkedServer" | "linkedCatalog" | "linkedSchema" | "schema">,
): boolean {
  return !!node.linkedServer || !!node.linkedCatalog || !!node.linkedSchema || !!parseSqlServerLinkedSchema(node.schema)
}

export function sqlServerLinkedTableSchema(
  node: Pick<TreeNode, "linkedServer" | "linkedCatalog" | "linkedSchema">,
): string | undefined {
