...
export const CREATE_DATABASE_CHARSET_OPTIONS = [
  "utf8mb4",
  "utf8",
  "gbk",
  "latin1",
...
export function createDatabaseCollationOptionsForCharset(
  charset: string,
  collationsByCharset: Record<string, string[]> = FALLBACK_COLLATION_OPTIONS,
): string[] {
  return collationsByCharset[normalizeCreateDatabaseCharsetKey(charset)] ?? []
}

export function defaultCreateDatabaseCollationForCharset(
  charset: string,
  collationsByCharset: Record<string, string[]> = FALLBACK_COLLATION_OPTIONS,
): string {
...
export function nextCreateDatabaseCollation(
  nextCharset: string,
  previousCharset: string,
...
export function parseCreateDatabaseCharsetMetadata(
  charsetResult: QueryLikeResult,
  collationResult: QueryLikeResult,
): CreateDatabaseCharsetMetadata {
  const charsetNameIndex = columnIndex(charsetResult.columns, ["charset", "character set", "character_set_name"])
  const charsetDefaultCollationIndex = columnIndex(charsetResult.columns, [
    "default collation",
  ...
  return {
    ...
    collationsByCharset: Object.keys(collationsByCharset).length
      ? collationsByCharset
      : { ...FALLBACK_COLLATION_OPTIONS },
  }
}
