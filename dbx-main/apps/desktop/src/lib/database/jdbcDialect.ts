...
type JdbcDialectConnection = Pick<ConnectionConfig, "db_type"> &
  Partial<Pick<ConnectionConfig, "driver_profile" | "connection_string" | "jdbc_driver_class" | "jdbc_driver_paths">>
...
const JDBC_DIALECT_MATCHERS: Array<{ type: DatabaseType; patterns: RegExp[] }> = [
  ...
  {
    type: "iris",
    patterns: [/jdbc:(?:iris|cache):/i, /com\.intersystems\.jdbc\.(?:IRIS|Cache)Driver/i, /intersystems-jdbc/i],
  },
]
...
export function inferJdbcDialect(connection?: JdbcDialectConnection): DatabaseType | undefined {
  ...
  const haystack = [
    connection.driver_profile,
    connection.connection_string,
    connection.jdbc_driver_class,
    ...(connection.jdbc_driver_paths ?? []),
  ]
    .filter(Boolean)
    .join("\n")
...
export function connectionObjectTreeQuerySchema(
  connection: JdbcDialectConnection | undefined,
  database: string,
  schema?: string,
): string {
...
export function metadataSchemaForConnection(
  connection: JdbcDialectConnection | undefined,
  database: string,
  schema?: string,
): string {
...
export function connectionObjectTreeNodeSchema(
  connection: JdbcDialectConnection | undefined,
  database: string,
  schema?: string,
): string | undefined {
