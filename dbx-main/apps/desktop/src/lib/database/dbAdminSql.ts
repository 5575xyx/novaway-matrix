...
export function buildGetSchemaCommentSql(options: SchemaNameSqlOptions): string {
  ...
  return [
    ...
    "LEFT JOIN pg_catalog.pg_description d ON d.objoid = n.oid AND d.objsubid = 0 AND d.classoid = 'pg_namespace'::regclass",
    `WHERE n.nspname = ${quoteSqlLiteral(options.name)};`,
  ].join("\n")
}
