...
export function replaceLazyKvChildren(
  state: LazyKvKeyTreeState,
  parentKey: string | null,
  keys: KvKeySummary[],
  ...
  const children = options.append
    ? mergeLazyChildren(parentKey ? state.nodeByKey.get(parentKey)?.children || [] : state.roots, nextChildren)
    : nextChildren
...
export function replaceLazyKvFocusedRoot(
  state: LazyKvKeyTreeState,
  root: KvKeySummary,
  keys: KvKeySummary[],
  continuation: string | null | undefined,
  ...
  for (const path of chain) {
    const node = lazyNodeFromSummary(
      path === focusedPath ? { ...root, key: focusedPath } : { key: path, numChildren: 1 },
      previous.get(path),
    )
...
function flattenLazyNodes(
  nodes: LazyKvKeyTreeNode[],
