...
export function serializeOpenTabs(tabs: QueryTab[]): SavedOpenTab[] {
  return tabs.map((tab) => ({
    ...
    ...(tab.mode === "query" && tab.resultRuns?.length
      ...
      : {}),
    ...(tab.mode === "query" && tab.activeResultRunId !== undefined
...
function isSavedOpenTab(value: unknown): value is SavedOpenTab {
  ...
  return (
    ...
    (typeof tab.sql === "string" || typeof tab.savedSqlId === "string")
  )
}

function restoreOpenTabsArray(
  parsed: unknown,
  rawActiveTabId: string | null,
  options: { queryOnly?: boolean; filter?: OpenTabsRestoreFilter } = {},
): RestoredOpenTabs {
  ...
  try {
    ...
    const tabs: QueryTab[] = filtered.map((tab) => {
      ...
      return {
        ...
        resultRuns,
        activeResultRunId: resultRuns?.some((run) => run.id === tab.activeResultRunId)
...
export function restoreOpenTabsPayload(
  payload: { tabs?: unknown; activeTabId?: unknown } | null | undefined,
  options: { queryOnly?: boolean; filter?: OpenTabsRestoreFilter } = {},
): RestoredOpenTabs {
  if (!payload) return { tabs: [], activeTabId: null }
  return restoreOpenTabsArray(
    payload.tabs,
    typeof payload.activeTabId === "string" ? payload.activeTabId : null,
    options,
  )
}
