...
const mongoDocumentProvider: DocumentStoreProvider = {
  ...
  sortInputForColumn: (column, direction) =>
    direction ? JSON.stringify({ [column]: direction === "asc" ? 1 : -1 }) : "",
}
...
export function buildDocumentFilterCondition(
  rule: DocumentFilterRule,
  options: DocumentFilterParseOptions = {},
...
export function combineDocumentFilterConditions(
  conditions: Record<string, unknown>[],
  rules: Pick<DocumentFilterRule, "conjunction">[],
): Record<string, unknown> | null {
...
export function parseDocumentFilterInput(
  input: string,
  options: DocumentFilterParseOptions = {},
...
export function currentDocumentFilterJson(
  input: string,
...
export function elasticsearchSearchBodyFromDocumentQuery(
  options: Pick<DocumentStoreQueryPreviewOptions, "filterJson" | "sortJson" | "skip" | "limit">,
): Record<string, unknown> {
...
function safeElasticsearchSearchBodyFromDocumentQuery(
  options: Pick<DocumentStoreQueryPreviewOptions, "filterJson" | "sortJson" | "skip" | "limit">,
): Record<string, unknown> {
  try {
...
function elasticsearchSortFromDocumentSort(sortJson?: string): unknown[] {
  ...
  return Object.entries(parsed).map(([field, direction]) => ({
    [field]: {
      order: direction === -1 || (typeof direction === "string" && direction.toLowerCase() === "desc") ? "desc" : "asc",
    },
  }))
}
