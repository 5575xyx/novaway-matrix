...
export function buildMongoCompletionItems(
  text: string,
  cursor: number,
  input: MongoCompletionInput = {},
): MongoCompletionItem[] {
  return buildMongoCompletionItemsFromContext(getMongoCompletionContext(text, cursor), input)
}

export function buildMongoCompletionItemsFromContext(
  context: MongoCompletionContext,
  input: MongoCompletionInput = {},
): MongoCompletionItem[] {
...
export function shouldAutoOpenMongoCompletion(text: string, cursor: number): boolean {
  ...
  if (/[{,[]/.test(previousChar))
    return isLikelyFieldPosition(text, cursor) || isLikelyAggregationPipeline(text, cursor)
...
function fieldItems(prefix: string, fields: MongoCompletionField[]): MongoCompletionItem[] {
  ...
  const snippets = FIELD_SNIPPETS.filter((snippet) => matchesFuzzyPrefix(snippet.label, normalizedPrefix)).map(
    (snippet) => ({
      label: snippet.label,
      type: "snippet" as const,
      detail: snippet.detail,
...
function stageItems(prefix: string): MongoCompletionItem[] {
  ...
  const snippets = [
    ...
    {
      label: "$lookup stage",
      apply: '$lookup: { from: "${collection}", localField: "${field}", foreignField: "_id", as: "${as}" }',
      detail: "Join another collection",
    },
  ]
...
function isAfterCursorMethodDot(beforeCursor: string): boolean {
  return /(?:^|[\s;(])db\.(?:[A-Za-z_][\w$-]*|getCollection\(["'][^"']+["']\))\.(?:find|aggregate)\s*\([\s\S]*\)(?:\s*\.\s*(?:sort|skip|limit)\s*\([\s\S]*\))*\s*\.\s*[\w$-]*$/.test(
    beforeCursor,
  )
}
...
function extractActiveCollection(text: string, cursor: number): string | undefined {
  ...
  const directMatches = [...before.matchAll(/db\.([A-Za-z_][\w$-]*)\s*\./g)].filter(
    (match) => match[1] !== "getCollection",
  )
