...
export function setMongoUrlParamBoolean(
...
export function normalizeMongoTlsFormState(
  ssl: boolean,
  urlParams: string | undefined,
  caCertPath: string | undefined,
): { urlParams: string; caCertPath: string } {
  if (ssl) {
  ...
  const next = setMongoUrlParam(
    setMongoUrlParam(setMongoUrlParam(urlParams, "tlsAllowInvalidCertificates", ""), "tlsAllowInvalidHostnames", ""),
    "tlsCAFile",
    "",
  )
...
export function mongodbAuthFailureHint(message: string): string {
  ...
  if (
    message.includes('certificate not valid for name "127.0.0.1"') ||
    (message.includes("invalid peer certificate") && message.includes("127.0.0.1") && message.includes("docdb"))
  ) {
