...
export function policyFormsFromEffectivePolicies(raw: unknown, fallback: MqPolicyForms): MqPolicyForms {
  ...
  const publish = firstRecord(
    topicPolicies.publishRate,
    root.publishRate,
    namespaceSource.publishRate,
    namespaceSource.publishMaxMessageRate,
  )
  ...
  const backlog = normalizeBacklogQuota(
    firstDefined(
      topicPolicies.backlogQuota,
      root.backlogQuota,
      namespaceSource.backlogQuota,
      namespaceSource.backlogQuotaMap,
      namespaceSource.backlog_quota_map,
    ),
  )
  const retention = firstRecord(
    topicPolicies.retention,
    root.retention,
    namespaceSource.retention,
    namespaceSource.retentionPolicies,
    namespaceSource.retention_policies,
  )
    ...
    publishForm: publish
      ? {
          publishThrottlingRateInMsg:
            numberField(publish.publishThrottlingRateInMsg) ?? fallback.publishForm.publishThrottlingRateInMsg,
          publishThrottlingRateInByte:
            numberField(publish.publishThrottlingRateInByte) ?? fallback.publishForm.publishThrottlingRateInByte,
        }
      : { ...fallback.publishForm },
    dispatchForm: dispatch
      ? {
          dispatchThrottlingRateInMsg:
            numberField(dispatch.dispatchThrottlingRateInMsg) ?? fallback.dispatchForm.dispatchThrottlingRateInMsg,
          dispatchThrottlingRateInByte:
            numberField(dispatch.dispatchThrottlingRateInByte) ?? fallback.dispatchForm.dispatchThrottlingRateInByte,
    ...
    subscribeForm: subscribe
      ? {
          subscribeThrottlingRatePerConsumer:
            numberField(subscribe.subscribeThrottlingRatePerConsumer) ??
            fallback.subscribeForm.subscribeThrottlingRatePerConsumer,
    ...
    retentionForm: retention
      ? {
          retentionTimeInMinutes:
            numberField(retention.retentionTimeInMinutes) ?? fallback.retentionForm.retentionTimeInMinutes,
          retentionSizeInMb:
            numberField(retention.retentionSizeInMb) ??
            numberField(retention.retentionSizeInMB) ??
            fallback.retentionForm.retentionSizeInMb,
        }
