...
export function formatMqTokenIssueError(error: unknown): MqTokenIssueErrorView {
  ...
  if (isMissingTokenSigningConfig(message)) {
    return {
      ...
      detail:
        "请编辑该连接，在 MQ 配置中设置“Broker Token 签发”为 HS256 SECRET 或 RS256 PRIVATE，并填写签发密钥后再生成。",
    }
  }
...
function isMissingTokenSigningConfig(message: string) {
  ...
  return (
    normalized.includes("token signing is not configured") ||
    normalized.includes("token signing key is required") ||
    normalized.includes("broker token signing key is required")
  )
}
