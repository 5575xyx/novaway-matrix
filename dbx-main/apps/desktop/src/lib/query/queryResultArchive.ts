...
export async function decodeQueryResultArchive(
  bytes: Uint8Array | ArrayBuffer,
): Promise<DecodedQueryResultArchive | undefined> {
  try {
    ...
    if (decoded.magic !== ARCHIVE_MAGIC || decoded.version !== ARCHIVE_VERSION || decoded.codec !== ARCHIVE_CODEC)
      return undefined
