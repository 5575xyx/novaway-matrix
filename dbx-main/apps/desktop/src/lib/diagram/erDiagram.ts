...
export function normalizeCustomDiagramRelationship(
  input: Omit<CustomDiagramRelationship, "id"> & { id?: string },
...
export function buildDiagramRelationships(
  tables: DiagramTable[],
  ...
  const custom = customRelationships
    ...
    .filter(
      (relationship) =>
        columnExists(tableMap.get(relationship.sourceTable), relationship.sourceColumn) &&
        columnExists(tableMap.get(relationship.targetTable), relationship.targetColumn),
    )
    .map((relationship) => ({
      ...relationship,
...
function nextJoinableRelationship(
  relationships: DiagramRelationship[],
  joinedTables: Set<string>,
  consumedRelationships: Set<string>,
): DiagramRelationship | undefined {
...
export function buildDiagramJoinSql(relationships: DiagramRelationship[], options: DiagramJoinSqlOptions = {}): string {
  const joinableRelationships = relationships.filter(
    (relationship) =>
  ...
  const rootTable =
    options.rootTable &&
    joinableRelationships.some(
      (relationship) =>
        relationship.sourceTable === options.rootTable || relationship.targetTable === options.rootTable,
    )
      ? options.rootTable
      : joinableRelationships[0].sourceTable
  ...
  while (consumedRelationships.size < joinableRelationships.length) {
    ...
    if (!joinedTables.has(tableToJoin)) {
      ...
      const joinConditions = joinableRelationships.filter(
        (item) =>
          !consumedRelationships.has(item.id) &&
          ((item.sourceTable === tableToJoin && previousJoinedTables.has(item.targetTable)) ||
            (item.targetTable === tableToJoin && previousJoinedTables.has(item.sourceTable))),
      )
      joinConditions.forEach((item) => consumedRelationships.add(item.id))
      joinLines.push(
        `${joinType} ${quoteIdentifier(tableToJoin)} ${aliases.get(tableToJoin)} ON ${joinConditions.map((item) => relationshipCondition(item, aliases)).join(" AND ")}`,
      )
    }
    consumedRelationships.add(relationship.id)
  }

  const whereRelationships = joinableRelationships.filter(
    (relationship) =>
      !consumedRelationships.has(relationship.id) &&
      joinedTables.has(relationship.sourceTable) &&
      joinedTables.has(relationship.targetTable),
  )
  whereRelationships.forEach((relationship) => consumedRelationships.add(relationship.id))
  const whereLine =
    whereRelationships.length > 0
      ? `WHERE ${whereRelationships.map((relationship) => relationshipCondition(relationship, aliases)).join(" AND ")}`
      : ""
  const selectList = [...joinedTables].map((table) => `  ${aliases.get(table)}.*`).join(",\n")
  const disconnectedRelationships = joinableRelationships.filter(
    (relationship) => !consumedRelationships.has(relationship.id),
  )
  const disconnectedNotes = disconnectedRelationships.map(
    (relationship) =>
      `-- Disconnected relationship skipped: ${relationship.sourceTable}.${relationship.sourceColumn} = ${relationship.targetTable}.${relationship.targetColumn}`,
  )

  return [
    `SELECT`,
    selectList,
    `FROM ${quoteIdentifier(rootTable)} ${aliases.get(rootTable)}`,
    ...joinLines,
    whereLine,
    ...disconnectedNotes,
  ]
