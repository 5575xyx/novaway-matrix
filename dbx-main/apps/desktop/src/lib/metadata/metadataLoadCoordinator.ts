...
export function logMetadataLoadTrace(
  logger: MetadataLoadTraceLogger | undefined,
  trace: MetadataLoadTrace,
  event: string,
  details?: MetadataLoadTraceDetails,
) {
...
export class MetadataLoadCoordinator {
  ...
  run<T>(
    scope: MetadataScopeInput | string,
    load: () => Promise<T>,
    options?: MetadataLoadCoordinatorOptions,
  ): Promise<T> {
