...
import {
  ...
  type MetadataLoadCacheStatus,
  type MetadataLoadTraceLogger,
} from "./metadataLoadCoordinator"
...
export function tableMetadataScope(
  request: Pick<
    TableMetadataRequest,
    "connectionId" | "database" | "schema" | "tableName" | "tableType" | "driverProfile" | "databaseType"
  >,
): MetadataScopeInput {
  return {
    kind: "table-metadata",
...
export function getCachedTableMetadata(
  request: Pick<
    TableMetadataRequest,
    "connectionId" | "database" | "schema" | "tableName" | "tableType" | "driverProfile" | "databaseType"
  >,
): TableMetadataLoadResult | undefined {
...
export function tableMetadataToDataTabMeta(
  metadata: TableMetadata,
  schema = metadata.schema,
): NonNullable<QueryTab["tableMeta"]> {
  return {
    schema,
...
export async function loadTableMetadata(request: TableMetadataRequest): Promise<TableMetadataLoadResult> {
  ...
  logMetadataLoadTrace(request.traceLogger, trace, "cache-miss", {
  ...
  const metadata = await tableMetadataCoordinator.run(
    ...
    async () => {
      ...
      const indexes = await api
        .listIndexes(request.connectionId, request.database, request.schema ?? "", request.tableName)
        .catch((): IndexInfo[] => [])
      const primaryKeys = editableRowIdentifierColumns(
        request.databaseType as DatabaseType,
        columns,
        indexes,
        request.tableType,
      )
