...
export function completionSchemasFromTree(
  ...
  visitTree(nodes, (node) => {
    if (node.connectionId !== connectionId || node.database !== database || node.type !== "schema" || !node.schema)
      return
...
export function completionTablesFromTree(
  nodes: readonly TreeNode[],
  connectionId: string,
  database: string,
  schema?: string,
