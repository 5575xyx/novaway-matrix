...
function resultStats(snapshot: TabResultSnapshot): { rowCount: number; columnCount: number } {
  const activeRun =
    snapshot.resultRuns?.find((run) => run.id === snapshot.activeResultRunId) ?? snapshot.resultRuns?.[0]
  const result =
    snapshot.result ??
    snapshot.results?.[snapshot.activeResultIndex ?? 0] ??
    snapshot.results?.[0] ??
    activeRun?.result ??
    activeRun?.results?.[activeRun.activeResultIndex ?? 0] ??
    activeRun?.results?.[0]
  return {
