...
function parseSelectProjections(
  ...
  let fromIndex = tokens.findIndex(
    (item, index) =>
      index > selectIndex && item.depth === selectDepth && item.kind === "word" && item.normalized === "from",
  )
...
function aliasAfter(
  ...
  index: number,
  dialect: SqlSemanticDialectAdapter,
): { alias?: string; aliasSpan?: SqlSemanticSpan; nextIndex: number } {
  let cursor = index
  ...
  return {
    source: {
      ...
      sourceSpan: {
        ...
        end: alias.aliasSpan?.end ?? state.tokens[close]?.span.end ?? 0,
      },
      columns,
    },
...
function parseTableFunctionSource(
  ...
  if (
    !nameToken ||
    nameToken.kind !== "word" ||
    !["table", "xmltable", "json_table", "the", "read_csv", "read_parquet", "read_json", "unnest"].includes(
  ...
  return {
    source: {
      ...
      sourceSpan: {
        ...
        end: alias.aliasSpan?.end ?? state.tokens[safeClose]?.span.end ?? nameToken.span.end,
      },
      unresolved: close < 0,
    },
    nextIndex: alias.nextIndex,
...
function parseTableSource(
  ...
  const kind = cte
    ? "cte"
    : introducer === "update" || introducer === "into" || (state.statement.kind === "delete" && introducer === "from")
      ? "mutation_target"
      : "table"
...
function isTableListContinuation(tokens: readonly SqlSemanticToken[], position: number): boolean {
  ...
  for (let index = commaIndex - 1; index >= 0; index -= 1) {
    ...
    if (TABLE_INTRODUCERS.has(token.normalized) || token.normalized === "from" || token.normalized === "join")
      return true
...
function buildCursorIntent(
  ...
  if (last?.text === "*" || trailing.prefix === "*") {
    ...
    const replacementRange =
      last?.text === "*" ? { start: last.span.start, end: Math.min(cursor, last.span.end) } : trailing.replacementRange
    return {
      kind: "star",
      prefix: "*",
      replacementRange,
...
export function buildSqlSemanticModel(
  ...
  const tokens = significantTokens(
    allTokens.filter((item) => item.span.end > statementSpan.start && item.span.start < statementSpan.end),
  )
