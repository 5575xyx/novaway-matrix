...
export function shouldRunSqlSemanticDiagnostics(
  ...
  if ((context.suggestTables || context.exclusiveTableSuggestions) && isCursorAfterTableTrigger(sql, cursor))
    return false
  return true
}
...
export function isSqlSemanticDiagnosticInputContext(
  ...
  return (
    ...
    !!context.qualifier ||
    ((context.suggestTables || context.exclusiveTableSuggestions) && isCursorAfterTableTrigger(sql, cursor))
  )
}
