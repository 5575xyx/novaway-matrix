...
class SqlCompletionProvider {
  ...
  build(): SqlCompletionItem[] {
    ...
    if (!context.exclusiveColumnSuggestions && context.suggestTables) {
      this.items.push(
        ...buildForeignKeyRelatedTableItems(context, this.input.tables, this.input.foreignKeysByTable, this.dialect),
      )
      this.items.push(
        ...buildTableItems(
          context.prefix,
...
export function getSqlCompletionContext(sql: string, cursor: number): SqlCompletionContext {
  ...
  const inPotentialPackageMemberContext =
    !!qualifier && !exclusiveTableSuggestions && !insertInfo && !oracleTableFunctionContext
  ...
  const suggestRoutines =
    ...
    (!preferColumnsOverGlobalRoutines &&
  ...
  const preferredKeywords = preferredKeywordsForCompletion(
    beforeCursor,
    beforeToken,
...
function detectUpdateCompletionContext(beforeCursor: string): {
  ...
  const cleaned = beforeCursor.replace(/'[^']*'/g, "''").replace(/"[^"]*"/g, '""')
  const match =
    /^\s*update\s+((?:"[^"]+"|`[^`]+`|[A-Za-z_][\w$]*)(?:\.(?:"[^"]+"|`[^`]+`|[A-Za-z_][\w$]*))?)(?:\s+(?:as\s+)?([A-Za-z_][\w$]*))?/i.exec(
      cleaned,
    )
  if (!match) return null
...
function buildTableItems(
  ...
  const existingAliases = new Set(
    referencedTables.map((ref) => ref.alias?.toLowerCase()).filter((alias): alias is string => !!alias),
  )
  return tables
...
function findCompletionTable(
  ...
  return tables.find(
    (table) =>
      ...
      (!normalizedSchema || !table.schema || normalizeIdentifierPart(table.schema) === normalizedSchema),
  )
}
...
function buildObjectItems(
  ...
  return objects
    .filter(
      (object) => (!onlyProcedures || object.type === "procedure") && objectMatchesCompletionContext(object, context),
    )
    .map((object) => {
      ...
      return {
        ...
        boost:
          computeBoost(object.name, context.prefix) +
          (object.type === "procedure" ? 1800 : object.type === "package" ? 1600 : 900),
      }
    })
    .sort(compareCompletionItems)
    .slice(0, MAX_TABLE_COMPLETION_ITEMS)
...
function objectIsQualifiedByContext(object: SqlCompletionObject, context: SqlCompletionContext): boolean {
  ...
  return (
    ...
    (!!qualifierPackage &&
      ...
      (!qualifierSchema || !object.parentSchema || object.parentSchema.toLowerCase() === qualifierSchema))
  )
}
...
function objectMatchesCompletionContext(object: SqlCompletionObject, context: SqlCompletionContext): boolean {
  ...
  if (context.qualifier) {
    ...
    if (object.parentSchema && `${object.parentSchema}.${object.parentName ?? ""}`.toLowerCase() === qualifier)
      return matchesPrefix(object.name, context.prefix)
  }
  return matchesPrefix(object.name, context.prefix)
}
...
function buildStarExpansionItem(
  ...
  const columns = context.qualifier
    ? referencedTablesForSelectAllColumns(context).flatMap((ref) =>
        columnsForSelectAllReferencedTable(ref, columnsByTable),
      )
  ...
  const expansion = context.qualifier
    ? buildSelectAllColumnExpansion(uniqueColumns, context.qualifier, true, dialect)
    : uniqueColumns.map((column) => quoteSqlIdentifier(column.name, dialect)).join(", ")
  return {
    label: "* → columns",
...
function buildSelectAllColumnItems(
  ...
  for (const ref of targetRefs) {
    ...
    const qualifier =
      context.qualifier || ref.alias || (shouldQualify ? quoteSqlIdentifier(ref.name, dialect) : undefined)
...
function buildAliasItems(context: SqlCompletionContext, databaseType?: DatabaseType): SqlCompletionItem[] {
  const items: SqlCompletionItem[] = []
  const existingAliases = new Set(
    context.referencedTables.map((ref) => ref.alias?.toLowerCase()).filter((alias): alias is string => !!alias),
  )
...
function buildColumnItems(
  ...
  if (context.insertTable) {
    ...
    relevantCols = allColumns.filter(
      (column) =>
        relatedTables.some((table) => columnMatchesReferencedTable(column, table)) ||
        (!!qualifiedTarget && columnMatchesQualifiedTable(column, qualifiedTarget)),
    )
  } else if (context.referencedTables.length > 0) {
    relevantCols = allColumns.filter((column) =>
      context.referencedTables.some((table) => columnMatchesReferencedTable(column, table)),
    )
  }
...
function referencedTableMatchesColumnQualifier(
  ...
  qualifiedTarget: { schema: string; table: string } | null,
): boolean {
