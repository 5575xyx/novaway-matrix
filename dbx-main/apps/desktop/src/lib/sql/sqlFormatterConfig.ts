...
function isSqlFormatterParamTypes(value: unknown): value is SqlFormatterParamTypes | null {
  ...
  if (!Object.keys(value).every((key) => ["positional", "numbered", "named", "quoted", "custom"].includes(key)))
    return false
  ...
  if (value.custom !== undefined && (!Array.isArray(value.custom) || !value.custom.every(isCustomParameter)))
    return false
  return true
}
