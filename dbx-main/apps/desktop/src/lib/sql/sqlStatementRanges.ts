...
const ORACLE_PL_SQL_CREATE_OBJECT_TYPES = new Set([
  "FUNCTION",
  "PROCEDURE",
  "TRIGGER",
  "PACKAGE",
  ...
  while (i < len) {
    ...
    if (customDelimiter) {
      ...
      const isOraclePlSql =
        ...
        statementStart !== -1 &&
        startsWithOraclePlSqlBlock(sql.slice(statementStart, i))
      if (isOraclePlSql) {
        markContent(i)
...
function splitStatementRangeAtSoftStarts(
  ...
  for (let index = 0; index < boundaries.length; index += 1) {
    ...
    const to = next
      ? trimRangeEndBeforeNextBoundary(sql, boundary.from, next.from)
...
export function executableStatementRanges(sql: string, databaseType?: DatabaseType): SqlTextRange[] {
  ...
  return splitSqlStatementRanges(sql, databaseType).flatMap((statement) =>
    splitStatementRangeAtSoftStarts(sql, statement, databaseType).map((range) => rangeFor(range, sql)),
  )
}
...
export function buildExecutionCandidates(
  ...
  const cursorStatement =
    databaseType === "redis"
      ? redisCommandRangeAtCursor(sql, cursorPos)
      : statementRangeAtCursor(sql, cursorPos, databaseType)
...
function candidateFromRange(
  ...
  return {
    kind,
    label:
      kind === "cursor" ? (isRedis ? "currentCommand" : "currentStatement") : isRedis ? "allCommands" : "allStatements",
    sql: range.sql,
    from: range.from,
    to: range.to,
  }
