...
function looksLikeSqlDefaultExpression(value: string): boolean {
  ...
  if (
    /^(current_timestamp|current_date|current_time|localtimestamp|localtime|sysdate|systimestamp)(\(\))?$/i.test(text)
  ) {
    return true
  }
...
const GeneratorFunctions: Record<string, (params?: GeneratorParams) => string> = {
  ...
  product_name: (params) => {
    ...
    if (keywords.length === 0)
      return `${pick(["Premium", "Basic", "Ultra", "Standard"])} ${pick(["Widget", "Gadget", "Tool", "Device"])}`
  ...
  product_category: (params) =>
    pick(productCategoryByLang[pickLang(params?.languages, "en")] ?? productCategoryByLang["en"]),
...
export function findGeneratorKey(columnName: string, dataType: string, isAutoIncrement?: boolean): string {
  ...
  const isBoolName = /^(is|has|had|can|did|enable|disable|allow|use|visible|deleted|active|flag)[_\-]?/i.test(
    columnName,
  )
...
export function defaultGeneratorParams(_columnName: string, attrs: ColumnAttrs, generatorKey: string): GeneratorParams {
  ...
  if (generatorKey === "number") {
    ...
    const isAmount = /price|amount|total|cost|fee|balance|salary|income|revenue|tax|discount|money|payment|price/i.test(
      col,
    )
...
export function generateValue(
  ...
  if ((key === "date" || key === "datetime") && rowIndex === 0) {
    console.log(
      "[dbx:gen] col=%s key=%s start=%j end=%j allDay=%j",
      columnName,
      key,
      params?.start,
      params?.end,
  ...
  if (key === "text") {
    ...
    if (fmt === "bcrypt") {
      ...
      const b64 = (n: number) =>
        Array.from({ length: n }, () => b64chars[Math.floor(Math.random() * b64chars.length)]).join("")
  ...
  if (key === "image")
    return `0x${Array.from({ length: 16 }, () => randInt(0, 255).toString(16).padStart(2, "0")).join("")}`
...
function generateByType(type: string, rowIndex: number): unknown {
  ...
  if (t.includes("int") || t.includes("serial") || t === "bigint" || t === "smallint" || t === "tinyint")
    return randInt(1, 1000)
  if (t.includes("decimal") || t.includes("numeric") || t.includes("float") || t.includes("double") || t === "real")
    return randDecimal(0, 10000, 2)
  ...
  for (let i = 0; i < config.rowCount; i++) {
    const row = config.columns.map((col) =>
      generateValue(
        ...
        i,
        col.generatorParams,
        col.isAutoIncrement ? null : col.columnDefault,
      ),
    )
    rows.push(row)
  }
