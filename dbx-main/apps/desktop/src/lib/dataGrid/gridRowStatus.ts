...
export function shouldShowQuickEntryDraftRow(options: {
  editable: boolean
  ...
  return (
    options.editable &&
    options.hasInsertTarget &&
    options.quickEntryEnabled &&
    !options.hasPendingChanges &&
    matchesRowStatusFilter("draft", options.rowStatusFilter)
  )
}

export function canDeleteGridRowItem(options: {
  editable: boolean
  isDraft: boolean
  isDeleted: boolean
  isNew: boolean
  canEditExistingRows: boolean
  ...
  return (
    ...
    !options.isDraft &&
    !options.isDeleted &&
    !options.isSavingNewRow &&
    (options.isNew || options.canEditExistingRows)
  )
}
