...
describe("Dremio JDBC connection URLs", () => {
  it("parses legacy direct JDBC URLs into a Dremio JDBC profile", () => {
    const parsed = parseConnectionUrl(
      "jdbc:dremio:direct=dremio.example.com:31010;schema=analytics;user=admin;password=secret;ssl=true",
    )
    ...
    expect(parsed.connectionString).toBe(
      "jdbc:dremio:direct=dremio.example.com:31010;schema=analytics;user=admin;password=secret;ssl=true",
    )
  })
  ...
  it("parses Arrow Flight SQL JDBC URLs into a Dremio JDBC profile", () => {
    const parsed = parseConnectionUrl(
      "jdbc:arrow-flight-sql://admin:secret@dremio.example.com:32010?schema=analytics&useEncryption=true",
    )
