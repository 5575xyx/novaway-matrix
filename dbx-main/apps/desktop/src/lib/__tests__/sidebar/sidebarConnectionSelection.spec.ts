...
describe("sidebar connection selection", () => {
  it("uses all selected connections when the current connection is selected", () => {
    ...
    expect(selectedConnectionDuplicateTargets(current, selected).map((item) => item.connectionId)).toEqual([
      "conn-1",
      "conn-2",
      "conn-3",
    ])
  ...
  it("keeps delete and duplicate target selection aligned", () => {
    ...
    expect(selectedConnectionDuplicateTargets(current, selected)).toEqual(
      selectedConnectionDeleteTargets(current, selected),
    )
  })
})
