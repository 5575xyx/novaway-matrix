...
describe("MCP config templates", () => {
  ...
  it("builds VS Code config with a direct node launch command", () => {
    const config = JSON.parse(
      buildMcpVsCodeConfig([["DBX_MCP_ALLOW_WRITES", "0"]], {
        command: "node",
  ...
  it("builds Codex TOML config with env entries", () => {
    expect(buildMcpCodexConfig([["DBX_MCP_ALLOW_WRITES", "0"]])).toBe(
      [
        "[mcp_servers.dbx]",
        'command = "dbx-mcp-server"',
        "",
  ...
  it("builds Codex TOML config with a direct node launch command", () => {
    expect(
      buildMcpCodexConfig([["DBX_MCP_ALLOW_WRITES", "0"]], { command: "node", args: ["C:\\dbx\\mcp\\dist\\index.js"] }),
    ).toBe(
      [
        "[mcp_servers.dbx]",
        'command = "node"',
