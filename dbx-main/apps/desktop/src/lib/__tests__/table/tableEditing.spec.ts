  ...
  it("uses unique indexes as row identifiers when primary keys are absent", () => {
    ...
    expect(
      editableRowIdentifierColumns(
        "postgres",
        [column("email"), column("name")],
        [index(["email"], true, "email IS NOT NULL")],
      ),
    ).toEqual([])
    expect(editableRowIdentifierColumns("postgres", [column("id", true), column("email")], [index(["email"])])).toEqual(
      ["id"],
    )
  })
  ...
  it("treats ClickHouse row identifier cells as readonly on existing rows", () => {
    ...
    expect(
      isClickHouseExistingRowReadonlyColumn(
        ...
        "event_date",
        ["id"],
        [{ ...column("event_date"), extra: "partition_key" }],
      ),
    ).toBe(true)
