...
describe("zookeeper lazy key tree", () => {
  it("stores only the current path direct children on reset", () => {
    ...
    expect(flattenLazyKvKeyTree(state, new Set()).map((row) => `${row.depth}:${row.node.label}`)).toEqual([
      "0:a",
      "0:folder",
    ])
  })
  ...
  it("does not expose grandchildren until their parent is loaded", () => {
    ...
    expect(flattenLazyKvKeyTree(state, new Set(["lazy:/app"])).map((row) => row.node.key)).toEqual([
      "/app",
      "/app/name",
    ])
  })
