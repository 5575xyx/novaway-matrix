...
import {
  ...
  defaultCreateDatabaseCollationForCharset,
...
describe("createDatabaseCharsetOptions", () => {
  it("offers common MySQL character sets with utf8mb4 first", () => {
    ...
    expect(CREATE_DATABASE_CHARSET_OPTIONS).toEqual(
      expect.arrayContaining(["ascii", "big5", "gb2312", "utf16", "utf32"]),
    )
  })
  ...
  it("offers common collations for the selected character set", () => {
    expect(createDatabaseCollationOptionsForCharset("utf8mb4").slice(0, 3)).toEqual([
      "utf8mb4_unicode_ci",
      "utf8mb4_general_ci",
      "utf8mb4_bin",
    ])
    expect(createDatabaseCollationOptionsForCharset("utf8mb4")).toEqual(expect.arrayContaining(["utf8mb4_bin"]))
    expect(createDatabaseCollationOptionsForCharset("utf8mb4").some((collation) => collation.includes("0900"))).toBe(
      false,
    )
    expect(createDatabaseCollationOptionsForCharset("gbk")).toEqual(["gbk_chinese_ci", "gbk_bin"])
    expect(createDatabaseCollationOptionsForCharset("utf8")).toEqual(
      createDatabaseCollationOptionsForCharset("utf8mb3"),
    )
  })
  ...
  it("keeps server-provided utf8mb4 collations while hiding 0900 entries from the dropdown", () => {
    ...
    expect(metadata.collationsByCharset.utf8mb4).toEqual([
      "utf8mb4_general_ci",
      "utf8mb4_unicode_ci",
