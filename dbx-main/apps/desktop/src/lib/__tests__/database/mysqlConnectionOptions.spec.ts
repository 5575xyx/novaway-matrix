...
describe("mysqlConnectionOptions", () => {
  ...
  it("enables the canonical cleartext password auth param and removes aliases", () => {
    expect(setMysqlCleartextPasswordAuthEnabled("allowCleartextPasswords=true&charset=utf8mb4", true)).toBe(
      "charset=utf8mb4&enable_cleartext_plugin=true",
    )
  })
  ...
  it("disables cleartext password auth params while preserving unrelated params", () => {
    expect(
      setMysqlCleartextPasswordAuthEnabled(
        "allowCleartextPasswords=true&enable_cleartext_plugin=true&charset=utf8mb4&connect_timeout=10",
        false,
      ),
    ).toBe("charset=utf8mb4&connect_timeout=10")
  })
})
