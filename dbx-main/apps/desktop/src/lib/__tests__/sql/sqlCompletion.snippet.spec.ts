...
const BUILTIN_SELECT: SqlSnippet = {
  id: "builtin-sel",
  label: "select *",
  prefix: "sel",
  body: "SELECT *\nFROM table\nLIMIT 100;",
}
const BUILTIN_CREATE_TABLE: SqlSnippet = {
  id: "builtin-ct",
  label: "create table",
  prefix: "ct",
  body: "CREATE TABLE table (\n  column type\n);",
}
const BUILTIN_UPDATE: SqlSnippet = {
  id: "builtin-upd",
  label: "update set",
  prefix: "upd",
  body: "UPDATE table\nSET column = value\nWHERE condition;",
}
const BUILTIN_CTE: SqlSnippet = {
  id: "builtin-cte",
  label: "common table expression",
  prefix: "cte",
  body: "WITH name AS (\n  SELECT columns\n  FROM table\n)\nSELECT *\nFROM name;",
}
const BUILTIN_INSERT: SqlSnippet = {
  id: "builtin-ins",
  label: "insert into",
  prefix: "ins",
  body: "INSERT INTO table (columns)\nVALUES (values);",
}
const BUILTIN_JOIN: SqlSnippet = {
  id: "builtin-join",
  label: "join",
  prefix: "join",
  body: "JOIN table ON left_column = right_column",
}
const BUILTIN_CASE: SqlSnippet = {
  id: "builtin-case",
  label: "case when",
  prefix: "case",
  body: "CASE\n  WHEN condition THEN value\n  ELSE default\nEND",
}
const BUILTIN_CREATE_INDEX: SqlSnippet = {
  id: "builtin-ci",
  label: "create index",
  prefix: "ci",
