...
import {
  ...
  type SqlCompletionProviderInput,
...
function mergeColumns(
  ...maps: Array<Map<string, SqlCompletionColumn[]> | undefined>
...
function semanticCompletion(
  markedSql: string,
  input: Partial<SqlCompletionProviderInput> = {},
  options: { databaseType?: DatabaseType; dialect?: "mysql" | "postgres" | "sqlserver" } = {},
) {
...
describe("semantic SQL completion candidates", () => {
  it("keeps alias-qualified column completion scoped to one row source", () => {
    ...
    const { items } = semanticCompletion("SELECT * FROM users u JOIN orders o ON o.user_id = u.id WHERE u.|", {
      columnsByTable,
    })
  ...
  it("uses CTE projected columns without remote metadata", () => {
    const { items, context } = semanticCompletion(
  ...
  it("expands alias star from only the qualified row source", () => {
    ...
    const { context, items } = semanticCompletion("SELECT u.*| FROM users u JOIN orders o ON o.user_id = u.id", {
      columnsByTable,
    })
  ...
  it("preserves dialect-aware identifier quoting in apply text", () => {
    const columnsByTable = new Map<string, SqlCompletionColumn[]>([
      ["Order Details", [{ name: "User Name", table: "Order Details" }]],
    ])

    const { items } = semanticCompletion(
      'SELECT od."User| FROM "Order Details" od',
      { columnsByTable },
