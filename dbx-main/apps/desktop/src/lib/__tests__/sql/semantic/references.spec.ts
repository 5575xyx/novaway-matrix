...
import {
  resolveSqlSemanticNavigationTarget,
  mergeSqlSemanticReferenceAnalysis,
  sqlSemanticCompletionReferenceTables,
  sqlSemanticTableReferences,
} from "@/lib/sql/semantic/references"
...
describe("sqlSemanticReferences shared consumers", () => {
  it("feeds CTE projected columns to diagnostics without physical table metadata", () => {
    const { sql, cursor } = sqlFixtureCursor(
      "WITH recent_orders(id, total) AS (SELECT id, total FROM orders) SELECT ro.missing FROM recent_orders ro WHERE ro.|",
    )
      ...
      columns: [
        {
          name: "missing",
          qualifier: "ro",
          span: span(sql.indexOf("missing") + 1, sql.indexOf("missing") + "missing".length),
        },
      ],
    }
  ...
  it("keeps completion diagnostics and navigation row-source resolution consistent", () => {
    const { sql, cursor } = sqlFixtureCursor(
      "WITH recent_orders(id, total) AS (SELECT id, total FROM orders) SELECT * FROM recent_orders ro WHERE ro.|",
    )
    const model = buildSqlSemanticModel(sql, cursor)
    ...
    expect(completionRefs).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "recent_orders", alias: "ro", columns: ["id", "total"] }),
      ]),
    )
    expect(diagnosticRefs).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "recent_orders", alias: "ro", columns: ["id", "total"] }),
      ]),
    )
