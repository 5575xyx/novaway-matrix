...
describe("sqlSemanticTokens", () => {
  it("tokenizes comments, strings, quoted identifiers, brackets, and backticks", () => {
    ...
    expect(
      tokens.some((token) => token.kind === "quoted_identifier" && unquoteSqlSemanticIdentifier(token) === "User Name"),
    ).toBe(true)
    expect(
      tokens.some((token) => token.kind === "quoted_identifier" && unquoteSqlSemanticIdentifier(token) === "order"),
    ).toBe(true)
    expect(
      tokens.some((token) => token.kind === "quoted_identifier" && unquoteSqlSemanticIdentifier(token) === "Mixed"),
    ).toBe(true)
