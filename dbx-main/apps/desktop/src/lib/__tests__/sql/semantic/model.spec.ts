...
import {
  sqlSemanticCompletionScope,
  sqlSemanticLocalColumnsByTable,
  sqlSemanticProjectionAliasColumns,
} from "@/lib/sql/semantic/completion"
...
describe("sqlSemanticModel baseline fixtures", () => {
    ...
    it(fixture.name, () => {
      ...
      for (const expectedSource of fixture.expected.rowSources ?? []) {
        const expectedObject = Object.fromEntries(
          Object.entries(expectedSource).filter(([, value]) => value !== undefined),
        )
  ...
  it("does not expose CTE body tables as outer query row sources", () => {
    const { sql, cursor } = sqlFixtureCursor(
      "WITH recent_orders AS (SELECT id FROM orders) SELECT * FROM recent_orders ro WHERE ro.|",
    )
    const model = buildSqlSemanticModel(sql, cursor)
  ...
  it("does not expose subquery body tables as outer query row sources", () => {
    ...
    expect(model.rowSources).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: "sq", kind: "subquery" })]),
    )
    expect(model.rowSources.some((source) => source.name === "users")).toBe(false)
  })
  ...
  it("exposes PostgreSQL projection aliases in ORDER BY but not WHERE", () => {
    ...
    expect(
      sqlSemanticProjectionAliasColumns(
        buildSqlSemanticModel(orderBy.sql, orderBy.cursor, { databaseType: "postgres" }),
      ).map((column) => column.name),
    ).toContain("total")
    expect(
      sqlSemanticProjectionAliasColumns(
        buildSqlSemanticModel(where.sql, where.cursor, { databaseType: "postgres" }),
      ).map((column) => column.name),
    ).not.toContain("total")
  })
  ...
  it("exposes MySQL projection aliases in GROUP BY and HAVING", () => {
    ...
    expect(
      sqlSemanticProjectionAliasColumns(
        buildSqlSemanticModel(groupBy.sql, groupBy.cursor, { databaseType: "mysql" }),
      ).map((column) => column.name),
    ).toContain("total")
    expect(
      sqlSemanticProjectionAliasColumns(
        buildSqlSemanticModel(having.sql, having.cursor, { databaseType: "mysql" }),
      ).map((column) => column.name),
    ).toContain("total")
  })
  ...
  it("keeps dialect-specific identifier normalization and qualifier scopes", () => {
    ...
    expect(buildSqlSemanticModel(sqlServer.sql, sqlServer.cursor, { databaseType: "sqlserver" }).rowSources[0]).toEqual(
      expect.objectContaining({ name: "Users", qualifierParts: ["dbo"], alias: "u" }),
    )
    expect(
      sqlSemanticProjectionAliasColumns(
        buildSqlSemanticModel(postgres.sql, postgres.cursor, { databaseType: "postgres" }),
      ).map((column) => column.name),
    ).toContain("Order Total")
    expect(buildSqlSemanticModel(mysql.sql, mysql.cursor, { databaseType: "mysql" }).rowSources[0]).toEqual(
      expect.objectContaining({ name: "events", qualifierParts: ["analytics"], alias: "e" }),
    )
    expect(buildSqlSemanticModel(sqlite.sql, sqlite.cursor, { databaseType: "sqlite" }).rowSources[0]).toEqual(
      expect.objectContaining({ name: "users", qualifierParts: ["main"], alias: "u" }),
    )
  })
  ...
  it("covers SQL Server case-insensitive bracket and multi-part qualifier contexts", () => {
    ...
    expect(model.rowSources[0]).toEqual(
      expect.objectContaining({ name: "Users", qualifierParts: ["ServerOne", "AppDb", "dbo"], alias: "U" }),
    )
  ...
  it("covers PostgreSQL lower-case folding with CTEs and ORDER BY projection aliases", () => {
    const { sql, cursor } = sqlFixtureCursor(
      "WITH RecentOrders AS (SELECT id, total FROM orders) SELECT total AS total_alias FROM RecentOrders ro ORDER BY total_|",
    )
    const model = buildSqlSemanticModel(sql, cursor, { databaseType: "postgres" })

    expect(model.rowSources).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "recentorders", alias: "ro", columns: ["id", "total"] }),
      ]),
    )
  ...
  it("covers MySQL database-qualified backticks and projection alias visibility", () => {
    ...
    expect(buildSqlSemanticModel(groupBy.sql, groupBy.cursor, { databaseType: "mysql" }).rowSources[0]).toEqual(
      expect.objectContaining({ name: "events", qualifierParts: ["analytics"], alias: "e" }),
    )
    expect(
      sqlSemanticProjectionAliasColumns(
    ...
    expect(
      sqlSemanticProjectionAliasColumns(buildSqlSemanticModel(where.sql, where.cursor, { databaseType: "mysql" })).map(
        (column) => column.name,
      ),
    ).not.toContain("total")
  })
  ...
  it("covers SQLite and DuckDB schema-light local row-source behavior", () => {
    ...
    expect(
      sqlSemanticCompletionScope(buildSqlSemanticModel(sqlite.sql, sqlite.cursor, { databaseType: "sqlite" }))
        .useRemoteMetadata,
    ).toBe(true)
    expect(buildSqlSemanticModel(duckdb.sql, duckdb.cursor, { databaseType: "duckdb" }).rowSources[0]).toEqual(
      expect.objectContaining({ kind: "table_function", name: "csv", alias: "csv" }),
    )
  })
})
