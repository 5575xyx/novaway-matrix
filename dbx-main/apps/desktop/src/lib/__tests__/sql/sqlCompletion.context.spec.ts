...
describe("sqlCompletion scoped context classification", () => {
  ...
  it("extracts statement-local table aliases", () => {
    ...
    expect(context.referencedTables).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ schema: "dbo", name: "Users", alias: "u" }),
        expect.objectContaining({ name: "Orders", alias: "o" }),
      ]),
    )
  })
  ...
  it("exposes CTEs as table-like referenced tables", () => {
    ...
    expect(context.referencedTables).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "recent_orders", columns: ["id", "total"] }),
        expect.objectContaining({ name: "recent_orders", alias: "ro" }),
      ]),
    )
  })
  ...
  it("extracts subquery aliases and projected columns", () => {
    ...
    expect(context.referencedTables).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: "sq", alias: "sq", columns: ["id", "user_name"] })]),
    )
  })
})
...
describe("sqlCompletion scoped metadata ranking", () => {
  ...
  it("keeps large table catalogs bounded", () => {
    const tables = Array.from({ length: 500 }, (_, index) => ({
      name: `TempTable_${String(index).padStart(3, "0")}`,
      schema: "dbo",
      type: "table" as const,
    }))
    const sql = "SELECT * FROM Temp"
