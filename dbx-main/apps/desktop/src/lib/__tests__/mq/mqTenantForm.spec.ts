...
describe("mqTenantForm", () => {
  it("extracts selectable cluster names from Pulsar cluster info extras", () => {
    expect(mqClusterOptionsFromExtra({ clusters: ["standalone", "ec-pulsar", "standalone", "", 42] })).toEqual([
      "standalone",
      "ec-pulsar",
    ])
  ...
  it("requires both a tenant name and at least one allowed cluster", () => {
    ...
    expect(validateTenantForm("tenant-a", { adminRoles: [], allowedClusters: [] })).toBe(
      "Allowed clusters are required",
    )
