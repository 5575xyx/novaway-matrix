...
export type ObjectBrowserSortKey =
  | "name"
  | "type"
  | "estimatedRows"
  | "totalBytes"
...
function routineSignatureForDisplay(
  type: ObjectBrowserRow["type"],
...
export function filterObjectBrowserRows(rows: ObjectBrowserRow[], query: string): ObjectBrowserRow[] {
  ...
  if (regex) {
    return rows.filter((row) =>
      [row.displayName, row.name, row.type, row.comment].filter(Boolean).some((value) => regex.test(String(value))),
    )
  }
  return rows.filter((row) =>
    [row.displayName, row.name, row.type, row.comment]
      .filter(Boolean)
...
export function initialObjectBrowserSortDirection(key: ObjectBrowserSortKey): ObjectBrowserSortDirection {
  return key === "created_at" || key === "updated_at" || key === "estimatedRows" || key === "totalBytes"
    ? "desc"
    : "asc"
}
...
function compareObjectBrowserNumber(
  left: string | number | null | undefined,
  right: string | number | null | undefined,
  direction: ObjectBrowserSortDirection,
): number {
