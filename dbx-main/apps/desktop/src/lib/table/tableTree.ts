...
export function mergeTableTreePageChildren(
...
export function buildSimpleObjectTreeNodes({
  ...
  for (const obj of objects) {
    ...
    if (
      !["TABLE", "VIEW", "MATERIALIZED_VIEW", "PROCEDURE", "FUNCTION", "SEQUENCE", "PACKAGE", "PACKAGE_BODY"].includes(
        objectType,
      )
    ) {
      continue
...
export function buildGroupedObjectTreeNodes({
  ...
  for (const def of groupDefs) {
    ...
    const isExpandable =
      def.nodeType === "group-tables" || def.nodeType === "group-views" || def.nodeType === "group-materialized-views"
