...
} from "@/lib/table/tableStructureEditorSql.ts"
...
export function canEditManticoreColumnProperties(
  dbType: DatabaseType | undefined,
  hasOriginalColumn: boolean,
): boolean {
...
export const ORACLE_LIKE_TYPE_LENGTH_DISABLES: string[] = [
  "binary_double",
  "binary_float",
  "bigint",
  "boolean",
...
function stripPostgresStringDefaultCast(defaultValue: string, dataType: string): string {
  ...
  const match = trimmed.match(
    /^('(?:''|[^'])*')::\s*((?:character\s+varying)|character|varchar|char|text|bpchar|name|jsonb?|xml|bytea|uuid)(?:\s*\(\s*\d+\s*\))?$/i,
  )
...
function findColumnDraftByName(
  ...
  for (const name of names) {
    ...
    const matches = columns
      ...
      .filter(({ column, index }) => !usedIndexes.has(index) && column.name.toLowerCase() === lowerName)
...
export function rehydrateColumnDraftsFromMetadata(
  draftColumns: EditableStructureColumn[],
  columns: ColumnInfo[],
...
export function generateIndexName(
  tableName: string,
  columns: string[],
  maxLength = AUTO_INDEX_NAME_MAX_LENGTH,
): string {
...
export function generateUniqueIndexName(
  ...
  for (let counter = 2; counter < 10_000; counter++) {
    ...
    const stem =
      base.length + suffix.length <= maxLength ? base : base.slice(0, maxLength - suffix.length).replace(/_+$/g, "")
...
export function splitDataType(raw: string): { baseType: string; params: string } {
  ...
  const suffix = trimmed
    ...
    .trim()
    .replace(/\s+/g, " ")
  const baseType = /^(?:signed|unsigned|zerofill)(?:\s+(?:signed|unsigned|zerofill))*$/i.test(suffix)
    ? `${baseTypePrefix} ${suffix}`.trim()
    : baseTypePrefix
...
function combineMysqlNumericAttributeType(
  dbType: DatabaseType | undefined,
  baseType: string,
  params: string,
): string | null {
  ...
  if (!parts.slice(attrIndex).every((part) => ["signed", "unsigned", "zerofill"].includes(part.toLowerCase())))
    return null
