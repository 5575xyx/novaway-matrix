...
export function editablePrimaryKeys(
...
export function editableRowIdentifierColumns(
  databaseType: DatabaseType | undefined,
  columns: ColumnInfo[],
  indexes?: IndexInfo[],
  tableType?: string,
  ...
  const uniqueIndex = indexes
    ?.filter((index) => !index.filter && index.columns.length > 0 && (index.is_primary || index.is_unique))
    .sort(
      (left, right) => Number(right.is_primary) - Number(left.is_primary) || left.columns.length - right.columns.length,
    )[0]
  return uniqueIndex?.columns ?? []
}

export function isTableDataEditable(
  databaseType: DatabaseType | undefined,
  primaryKeys: string[],
  tableType?: string,
): boolean {
...
export function canUseKeylessRowPredicate(
  databaseType: DatabaseType | undefined,
...
export function usesSyntheticRowIdKey(
  databaseType: DatabaseType | undefined,
  primaryKeys: string[],
  tableType?: string,
): boolean {
...
export function isClickHouseExistingRowReadonlyColumn(
  databaseType: DatabaseType | undefined,
  column: string,
  primaryKeys: readonly string[],
  columns: ColumnInfo[] = [],
