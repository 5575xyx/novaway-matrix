...
function buildTdengineInsertTemplate(options: TableSqlTemplateOptions): string {
  const columns = options.columns ?? []
  const isTagColumn = (column: ColumnInfo) =>
    (column.comment ?? "").toUpperCase() === "TAG" || (column.extra ?? "").toUpperCase().includes("TAG")
  ...
  const valueColumns = columns
    .filter((column) => !isTagColumn(column))
  ...
  const tagsSql =
    tagPlaceholders.length > 0 ? `TAGS (${tagPlaceholders.join(", ")})` : "TAGS (/* TODO: add tag values */)"
...
export function buildTableUpdateTemplate(options: TableSqlTemplateOptions): string {
  ...
  const setClause = updateColumns.length
    ? updateColumns
        .map(
          (column) =>
            `${quoteTableIdentifier(options.databaseType, column.name)} = ${columnPlaceholderValue(column, options.databaseType)}`,
        )
        .join(",\n    ")
...
function buildWhereClause(databaseType: DatabaseType | undefined, primaryKeys: ColumnInfo[]): string {
  ...
  const conditions = primaryKeys.map(
    (column) => `${quoteTableIdentifier(databaseType, column.name)} = ${columnPlaceholderValue(column, databaseType)}`,
  )
...
function columnPlaceholderValue(column: ColumnInfo, databaseType?: DatabaseType): string {
  ...
  if (typeLooksBoolean(dataType))
    return databaseType === "mysql" || databaseType === "sqlite" || databaseType === "rqlite" ? "1" : "TRUE"
  ...
  if (typeLooksJson(dataType))
    return databaseType === "postgres" || databaseType === "redshift" ? "'{}'::jsonb" : "'{}'"
  if (typeLooksBinary(dataType)) return "''"
  if (typeLooksUuid(dataType))
    return databaseType === "postgres" || databaseType === "redshift"
...
function typeLooksNumeric(dataType: string): boolean {
  return /^(tinyint|smallint|mediumint|int\d*|integer\d*|bigint|smallserial|serial\d*|bigserial|number|numeric|decimal|dec|float\d*|double|real|double\s+precision|money|smallmoney|fixed)/i.test(
    dataType.trim(),
  )
}
...
function typeLooksTimestamp(dataType: string): boolean {
  return /^(datetime|timestamp|timestamptz|timestamp\s+with\s+time\s+zone|timestamp\s+without\s+time\s+zone|smalldatetime|datetime2|datetimeoffset)/i.test(
    dataType.trim(),
  )
}
...
function typeLooksBinary(dataType: string): boolean {
  return /^(bytea|blob|binary|varbinary|image|raw|longblob|mediumblob|tinyblob|long\s+raw|bfile)$/i.test(
    dataType.trim(),
  )
}
