...
export const TABLE_COLUMN_TEMPLATE_DATABASE_TYPES: DatabaseType[] = manifestDatabaseTypes().filter(
  isTableColumnTemplateDatabaseType,
)
...
export function tableColumnTemplates(
  columnNames: readonly string[] = DEFAULT_TABLE_COLUMN_TEMPLATE_FIELDS,
): TableColumnTemplate[] {
...
export function createTableColumnTemplateDrafts(options: {
  templateId: string
  ...
  return presetFieldColumns(
    options.databaseType,
    parseTableColumnTemplateFields([...(options.columnNames ?? DEFAULT_TABLE_COLUMN_TEMPLATE_FIELDS)]),
    options.createId,
  ).filter((column) => !existingNames.has(column.name.toLowerCase()))
}

function presetFieldColumns(
  databaseType: DatabaseType | undefined,
  fields: readonly TableColumnTemplateField[],
  createId: () => string,
): EditableStructureColumn[] {
  return fields
    ...
    .map((field) => {
      return templateColumn(
        createId,
        field.name,
        configuredFieldDataType(field, databaseType) ?? "",
...
function configuredFieldDataType(
  field: TableColumnTemplateField,
  databaseType: DatabaseType | undefined,
): string | undefined {
  const configuredDataType = databaseType ? field.dataTypesByDatabase[databaseType] : undefined
  return configuredDataType && configuredDataType !== EMPTY_TABLE_COLUMN_TEMPLATE_DATA_TYPE
    ? configuredDataType
    : undefined
}

function isTableColumnTemplateFieldApplicable(
  field: TableColumnTemplateField,
  databaseType: DatabaseType | undefined,
): boolean {
