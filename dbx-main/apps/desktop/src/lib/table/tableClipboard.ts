...
export function tableClipboardEntryMatchesTarget(entry: TableClipboardContext, target: TableClipboardContext): boolean {
  return (
    entry.connectionId === target.connectionId &&
    entry.database === target.database &&
    normalizeSchema(entry.schema) === normalizeSchema(target.schema)
  )
}

export function tableClipboardMatchesTarget(
  entries: TableClipboardContext[],
  target: TableClipboardContext | null,
): boolean {
...
export function tableDataCopyColumnOptions(
