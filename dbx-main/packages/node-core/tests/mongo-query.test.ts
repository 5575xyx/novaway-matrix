...
test("parseMongoFindCommand accepts shell-style find commands", () => {
  assert.deepEqual(
    parseMongoFindCommand(
      'db.getCollection("operation_logs").find({"level":"info"}).sort({"ts":-1}).skip(5).limit(10)',
    ),
    {
...
test("parseMongoWriteCommand accepts unquoted update operator keys", () => {
  assert.deepEqual(
    parseMongoWriteCommand(
      "db.projects.updateOne({_id: ObjectId('507f1f77bcf86cd799439011')}, {$set: {name: 'next'}})",
    ),
    {
      kind: "update",
      collection: "projects",
...
test("parseMongoAggregateCommand accepts aggregate pipelines", () => {
  assert.deepEqual(
    parseMongoAggregateCommand(
      'db.projects.aggregate([{"$match":{"active":true}},{"$group":{"_id":"$owner","total":{"$sum":1}}}])',
    ),
    {
      collection: "projects",
...
test("parseMongoWriteCommand accepts supported write commands", () => {
  ...
  assert.deepEqual(
    parseMongoWriteCommand('db.projects.createIndex({"email":1},{"unique":true,"name":"projects_email_unique"})'),
    {
      kind: "createIndex",
      collection: "projects",
      keys: '{"email":1}',
