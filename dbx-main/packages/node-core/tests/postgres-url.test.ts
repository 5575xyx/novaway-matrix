...
test("postgres MCP URL keeps explicit sslmode", () => {
  const url = buildConnectionUrl(
    postgresConfig({ ssl: true, url_params: "sslmode=verify-full&application_name=dbx" }),
    {
      host: "pg.example.com",
      port: 5432,
    },
...
test("postgres MCP URL drops MySQL-style TLS params", () => {
  const url = buildConnectionUrl(
    postgresConfig({
      url_params:
        "ssl-mode=required&verify_ca=false&verify_identity=false&require_ssl=true&charset=utf8mb4&application_name=dbx",
    }),
...
test("postgres MCP URL maps schema and timezone to options", () => {
  ...
  assert.equal(
    url,
    "postgres://postgres:secret@pg.example.com:5432/app?options=-c%20search_path%3Dapp%20-c%20TimeZone%3DUTC",
  )
})
...
test("pooled URL builder rejects non-pooled direct types", () => {
  assert.throws(
    () =>
      buildConnectionUrl(postgresConfig({ db_type: "sqlite", host: "/tmp/app.db", port: 0 }), {
        host: "/tmp/app.db",
        port: 0,
      }),
