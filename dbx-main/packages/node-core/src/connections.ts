...
function openDb(readonly = false, dbPath = defaultDbPath()): Database.Database {
  if (!readonly) {
  ...
  config.transport_layers.forEach((layer, index) => {
    if (layer.type === "ssh") {
      layer.password ||=
        ...
        (layer.id === "legacy"
          ? getSecret(db, connectionId, "ssh_password")
          : getSecret(db, connectionId, `ssh_tunnels.${layer.id || index}.password`))
...
export async function inspectConnectionStore(
