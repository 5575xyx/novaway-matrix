...
import {
  parseRedisCommandArgv,
  classifyRedisCommand,
  type RedisCommandOptions,
  type RedisCommandResult,
  type RedisCommandSafety,
} from "./redis-command.js"
...
function hasActiveSshLayer(config: ConnectionConfig): boolean {
  return (
    config.transport_layers?.some((layer) => layer.type === "ssh" && layer.enabled !== false && !!layer.host) ?? false
  )
}

function firstProxyLayer(config: ConnectionConfig): ProxyLayer | undefined {
  return config.transport_layers?.find(
    (layer): layer is ProxyLayer => layer.type === "proxy" && layer.enabled !== false && !!layer.host,
  )
}
...
function normalizePostgresUrlParams(value: string, forceTls: boolean): string {
  ...
  for (const part of value.trim().replace(/^\?/, "").split("&")) {
    ...
    if (
      lowerKey === "charset" ||
      lowerKey === "require_ssl" ||
      lowerKey === "verify_ca" ||
      lowerKey === "verify_identity"
    ) {
      continue
    }
    parts.push(part)
...
function normalizeMysqlUrlParams(value: string, forceTls: boolean, acceptInvalidCerts: boolean): string {
  ...
  if (
    !parts.some(
      (part) => urlParamKeyIs(part, "ssl-mode") || urlParamKeyIs(part, "sslmode") || urlParamKeyIs(part, "require_ssl"),
    )
  ) {
...
async function bridgeDataRequest<T>(path: string, body: Record<string, unknown>): Promise<T> {
  ...
  try {
    ...
    throw new Error(
      "DBX desktop app is not running. This database type requires DBX to be running for query execution.",
    )
  }
...
async function mysqlQuery(
  ...
  return queryWithRetry(
    ...
    async () => {
      ...
      const rows = (Array.isArray(results) ? results : []).slice(0, resolveMaxRows(options)) as Record<
        string,
        unknown
      >[]
...
export async function executeQuery(
  ...
  if (config.db_type === "mongodb") {
    ...
    if (count) {
      const result = await withTimeout(
        mongoFindDocuments(config, count.collection, 0, 1, count.filter),
        resolveTimeoutMs(options),
      )
    ...
    if (find) {
      const result = await withTimeout(
        mongoFindDocuments(config, find.collection, find.skip, find.limit, find.filter, find.projection, find.sort),
        resolveTimeoutMs(options),
      )
    ...
    if (collectionStats) {
      const result = await withTimeout(
        mongoCollectionStats(config, collectionStats.collection, collectionStats.scale),
        resolveTimeoutMs(options),
      )
...
async function executeRedisCommandDirect(
  config: ConnectionConfig,
  db: number,
  commandText: string,
  options?: RedisCommandOptions,
...
async function executeMongoWrite(
  ...
  if (command.kind === "dropIndex" || command.kind === "dropIndexes") {
    const result = await bridgeDataRequest<{ dropped_names: string[]; affected_rows: number }>(
...
function parseCollectionCountCommand(
  source: string,
  method: "countDocuments" | "count",
): MongoCountDocumentsCommand | null {
...
export function parseMongoWriteCommand(input: string): MongoWriteCommand | null {
  ...
  if (dropIndexes) {
    ...
    return indexes !== null
      ? { kind: "dropIndexes", collection: dropIndexes.collection, ...(indexes ? { indexes } : {}) }
      : null
  }
...
export function evaluateMongoWriteSafety(
  ...
  if (
    !options.allowDangerous &&
    (command.kind === "update" || command.kind === "delete") &&
    isEmptyJsonObject(command.filter)
  ) {
    return {
      allowed: false,
      reason:
