...
} from "../../apps/desktop/src/lib/dataGrid/columnFormatter.ts"
...
test("formats unix timestamps in seconds, milliseconds, and auto mode", () => {
  ...
  assert.equal(
    applyColumnFormatter(1715758200, { kind: "datetime", unit: "seconds", pattern: "YYYY-MM-DD HH:mm:ssZ" }),
    "2024-05-15 15:30:00+08:00",
  )
  assert.equal(
    applyColumnFormatter(1715758200001, {
      kind: "datetime",
      unit: "milliseconds",
      pattern: "YYYY-MM-DD HH:mm:ss.SSSZ",
    }),
    "2024-05-15 15:30:00.001+08:00",
  )
  assert.equal(
    applyColumnFormatter(1715758200001, { kind: "datetime", unit: "auto", pattern: "YYYY-MM-DD HH:mm:ss.SSSZ" }),
    "2024-05-15 15:30:00.001+08:00",
  )
  assert.equal(
    applyColumnFormatter("1715758200001", { kind: "datetime", unit: "auto", pattern: "YYYY-MM-DD HH:mm:ss.SSSZ" }),
  ...
  assert.equal(
    applyColumnFormatter(1715758200, { kind: "datetime", unit: "auto", pattern: "YYYY-MM-DD HH:mm:ssZ" }),
    "2024-05-15 15:30:00+08:00",
  )
  assert.equal(
    applyColumnFormatter("1715758200", { kind: "datetime", unit: "auto", pattern: "YYYY-MM-DD HH:mm:ssZ" }),
...
test("does not treat compact date strings as unix timestamps", () => {
  ...
  assert.equal(
    applyColumnFormatter("20260514", { kind: "datetime", unit: "auto", pattern: "YYYY-MM-DD HH:mm:ss" }),
    "20260514",
  )
  assert.equal(applyColumnFormatter(20260514, { kind: "datetime", unit: "auto", pattern: "YYYYMMDD" }), "20260514")
  assert.equal(
    applyColumnFormatter("2026-05-14T16:00:00Z", { kind: "datetime", unit: "auto", pattern: "YYYY-MM-DDTHH:mm:ssZ" }),
    "2026-05-15T00:00:00+08:00",
  )
  assert.equal(
    applyColumnFormatter("2026-05-14", { kind: "datetime", unit: "auto", pattern: "YYYY-MM-DD" }),
    "2026-05-14",
  )
})
...
test("falls back to normal display for nulls and invalid formatter input", () => {
  ...
  assert.equal(
    applyColumnFormatter("2022-01-33", { kind: "datetime", unit: "auto", pattern: "YYYY-MM-DD" }),
    "2022-01-33",
  )
  ...
  assert.deepEqual(normalizeColumnFormatter({ kind: "datetime", unit: "auto", pattern: 123 }), {
    kind: "datetime",
    unit: "auto",
