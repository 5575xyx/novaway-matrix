...
} from "../../apps/desktop/src/lib/mongo/mongoConnectionOptions.ts"
...
test("sets MongoDB boolean URL params", () => {
  assert.equal(
    setMongoUrlParamBoolean("replicaSet=rs0", "retryWrites", false, true),
...
test("clears MongoDB TLS form state when SSL is disabled", () => {
  assert.deepEqual(
    normalizeMongoTlsFormState(
      false,
      "replicaSet=rs0&tlsAllowInvalidCertificates=true&tlsAllowInvalidHostnames=true&tlsCAFile=%2Ftmp%2Fca.pem",
      "/tmp/ca.pem",
    ),
...
test("form caCertPath overrides tlsCAFile in URL params when set", () => {
  assert.deepEqual(
    normalizeMongoTlsFormState(true, "replicaSet=rs0&tlsCAFile=%2Ftmp%2Flegacy-ca.pem", "/tmp/explicit-ca.pem"),
    {
      urlParams: "replicaSet=rs0",
...
test("adds a MongoDB directConnection multi-host hint", () => {
  const message =
    "MongoDB connection failed: Kind: An invalid argument was provided: cannot specify multiple seeds with directConnection=true"
...
test("adds a MongoDB SRV DNS hint for IP endpoints", () => {
  const message =
    'MongoDB connection failed: Kind: An error occurred during DNS resolution: DNS error: no records found for Query { name: Name("_mongodb._tcp.172.17.0.1."), query_type: SRV }'
...
test("adds a MongoDB replica set localhost advertisement hint", () => {
  const message =
    "MongoDB connection failed: Kind: Server selection timeout: No available servers. Topology: { Type: ReplicaSetNoPrimary, Set Name: LIMLIU, Servers: [ { Address: 127.0.0.1:27017, Type: Unknown, Error: Kind: I/O error: Connection refused (os error 111) } ] }"
...
test("adds a MongoDB rustls hint for tlsAllowInvalidHostnames", () => {
  const message =
    "MongoDB connection failed: Kind: An invalid argument was provided: tlsAllowInvalidHostnames is an invalid option"
