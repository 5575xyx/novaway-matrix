...
import {
  canExecuteWithoutSelectedDatabase,
  requiresSqlFileTargetDatabaseSelection,
  supportsConnectionLevelDatabaseBootstrap,
} from "../../apps/desktop/src/lib/connection/connectionLevelDatabaseBootstrap.ts"
import type { ConnectionConfig } from "../../apps/desktop/src/types/database.ts"

function connection(
  dbType: ConnectionConfig["db_type"],
  driverProfile?: string,
): Pick<ConnectionConfig, "db_type" | "driver_profile"> {
  return {
    db_type: dbType,
...
test("allows install scripts that create and switch databases before table DDL", () => {
  const sql =
    "SET NAMES utf8mb4; DROP DATABASE IF EXISTS app_db; CREATE DATABASE app_db; USE app_db; CREATE TABLE users(id INT)"
