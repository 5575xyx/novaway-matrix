...
import {
  ...
  filterSchemaDiffTables,
} from "../../apps/desktop/src/lib/schema/schemaDiffTableFilter.ts"
...
test("filters schema diff tables before detail loading", () => {
  ...
  const result = filterSchemaDiffTables(
    [table("user_profile"), table("user_profile_bak"), table("orders"), table("audit_log")],
    [table("user_profile"), table("orders_bak"), table("orders")],
    filter,
  )
