...
test("defaults query-result export row limit settings", () => {
  ...
  assert.equal(
    normalizeEditorSettings({ queryExportKeysetOptimizationEnabled: false }).queryExportKeysetOptimizationEnabled,
    false,
  )
  assert.equal(
    normalizeEditorSettings({ queryExportKeysetOptimizationEnabled: "nope" as any })
...
test("keeps saved UI font family", () => {
  ...
  assert.equal(
    normalizeEditorSettings({ uiFontFamily: SYSTEM_UI_FONT_FAMILY } as any).uiFontFamily,
...
test("normalizes table column template fields", () => {
  ...
  const normalizedTemplateFields = normalizeEditorSettings({
  ...
  } as any).tableColumnTemplateFields
  assert.equal(
  ...
  assert.deepEqual(
    normalizeEditorSettings({ tableColumnTemplateFields: [] } as any).tableColumnTemplateFields,
