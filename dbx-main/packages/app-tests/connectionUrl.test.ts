...
import {
  ...
  parseConnectionUrl,
} from "../../apps/desktop/src/lib/connection/connectionUrl.ts"
...
test("parses mysql URL name as decoded connection name", () => {
  const parsed = parseConnectionUrl(
    "mysql://root:123456@localhost/?name=%E5%85%AC%E5%8F%B8+-+%E6%9C%AC%E5%9C%B0Docker&charset=utf8mb4",
  )
...
test("parses MySQL JDBC URL params with ProxySQL multi-at usernames", () => {
  const parsed = parseConnectionUrl(
    "jdbc:mysql://127.0.0.1:6033/example?user=xxxxx%40db_readonly%40127.0.0.1&password=p%40wd&useSSL=false",
  )
...
test("parses Informix JDBC URLs with multiple parameters", () => {
  const parsed = parseConnectionUrl(
    "jdbc:informix-sqli://192.168.1.1:9088/mydb:INFORMIXSERVER=ol_informix;DB_LOCALE=en_US.UTF8",
  )
...
test("parses Informix JDBC URLs with credentials", () => {
  const parsed = parseConnectionUrl(
    "jdbc:informix-sqli://user:p%40ss@db.example.com:1533/testdb:INFORMIXSERVER=myserver",
  )
...
test("keeps typed H2 credentials when JDBC URL does not include them", () => {
  ...
  const applied = applyParsedConnectionUrl(
    { name: "", db_type: "h2", username: "typed-user", password: "typed-secret" } as any,
    parsed,
  )
...
test("uses H2 JDBC URL credentials when they are included", () => {
  const parsed = parseConnectionUrl(
    "jdbc:h2:split:28:C:/dbx-test/h2/sample-db;USER=url-user;PASSWORD=url-secret;AUTO_SERVER=TRUE",
  )
  const applied = applyParsedConnectionUrl(
    { name: "", db_type: "h2", username: "typed-user", password: "typed-secret" } as any,
    parsed,
...
test("rebuilds H2 split JDBC URLs with an edited file path", () => {
  assert.equal(
    h2FileJdbcUrlWithPath("jdbc:h2:split:28:C:/dbx-test/h2/sample-db;AUTO_SERVER=TRUE", "D:/dbx/new-sample.mv.db"),
    "jdbc:h2:split:28:D:/dbx/new-sample;AUTO_SERVER=TRUE",
  )
})
