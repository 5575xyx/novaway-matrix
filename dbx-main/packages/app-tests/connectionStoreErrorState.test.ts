...
test("late original connect errors replace the generic timeout detail", async () => {
  ...
  try {
    ...
    assert.equal(
      store.connectionErrors["conn-1"],
      "Connection attempt timed out after 3s. Please check the network or VPN and try again.",
    )
    ...
    if (String(input) === "/api/connection/connect") {
      return new Promise<Response>((resolve) => {
        setTimeout(
          () =>
            resolve(
              new Response(JSON.stringify("conn-1"), { status: 200, headers: { "Content-Type": "application/json" } }),
            ),
          3500,
        )
      })
...
test.each([
  ...
  "hanging %s root metadata load times out and clears loading state",
  async (dbType, loader, endpoint, label) => {
    vi.useFakeTimers()
    const restoreStorage = installMemoryStorage()
    const originalFetch = globalThis.fetch
    globalThis.fetch = (async (input) => {
    ...
    try {
      ...
      const timeoutRejection = assert.rejects(
        () => loadPromise,
        new RegExp(`Connection timed out while loading ${label} after 35s`),
      )
