...
} from "../../apps/desktop/src/lib/database/dbAdminSql.ts"
...
test("postgres schema comment SQL quotes identifiers and literals", () => {
  ...
  assert.equal(
    buildSetSchemaCommentSql({ databaseType: "postgres", name: 'tenant"one', comment: "Owner's area" }),
    `COMMENT ON SCHEMA "tenant""one" IS 'Owner''s area';`,
  )
})

test("postgres schema comment SQL clears blank comments", () => {
  assert.equal(
    buildSetSchemaCommentSql({ databaseType: "postgres", name: "analytics", comment: "  " }),
