...
} from "../../apps/desktop/src/lib/sql/semantic/diagnostics.ts"
...
test("flags where-clause columns missing from a single referenced table", () => {
  ...
  const diagnostics = buildSqlSemanticDiagnostics(analysis, {
    ...
    columnsByTable: new Map([
      [
        "t_0001",
        ["id", "image_name", "image_mime", "image_data", "image_url"].map((name) => ({ name, table: "t_0001" })),
      ],
    ]),
  })
...
test("resolves correlated subquery columns against the visible query scopes", () => {
  ...
  const diagnostics = buildSqlSemanticDiagnostics(analysis, {
    ...
    columnsByTable: new Map([
      [
        "mds_base_house",
        ["house_id", "hou_add", "contract_code", "pay_type", "HOU_PAY_AMT"].map((name) => ({
          name,
          table: "mds_base_house",
        })),
      ],
...
test("defers diagnostics while the cursor is in table completion context", () => {
  ...
  assert.equal(
    shouldRunSqlSemanticDiagnostics("SELECT * FROM `t_19991` LIMIT 100", "SELECT * FROM `t_19991` LIMIT 100".length, {
      databaseType: "mysql",
    }),
    true,
  )
  assert.equal(
    shouldRunSqlSemanticDiagnostics(
      "SELECT * FROM `t_0001` where ids > 1 LIMIT 50;",
      "SELECT * FROM `t_0001` where ids > 1 LIMIT 50;".length,
      { databaseType: "mysql" },
    ),
    true,
  )
