...
} from "../../apps/desktop/src/lib/connection/connectionVisibleDatabases.ts"
import {
  connectionUsesVisibleSchemaFilter,
  filterDatabaseNamesForConnection,
  filterDatabaseNamesForVisiblePicker,
} from "../../apps/desktop/src/lib/database/visibleDatabases.ts"
...
test("connection database filtering still applies saved visible database filters for sidebar display", () => {
  assert.deepEqual(
    filterDatabaseNamesForConnection(["app", "analytics", "mysql", "sys"], config({ visible_databases: ["app"] })),
    ["app"],
  )
})
...
test("Dameng default SYSDBA user remains selectable", () => {
  assert.deepEqual(filterDatabaseNamesForConnection(["SYS", "SYSDBA", "SYSAUDITOR"], config({ db_type: "dameng" })), [
    "SYSDBA",
  ])
})
