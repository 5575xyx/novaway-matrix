...
} from "../../apps/desktop/src/lib/table/objectBrowserRows.ts"
...
test("builds unique row ids for overloaded routines with the same visible name", () => {
  ...
  assert.deepEqual(
    rows.map((row) => row.id),
    [
      "dbms_pipe:list_pipes:FUNCTION::0",
      "dbms_pipe:list_pipes:FUNCTION:name text:0",
      "dbms_pipe:create_pipe:FUNCTION::0",
    ],
  )
  assert.deepEqual(
