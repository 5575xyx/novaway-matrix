...
import {
  buildMongoUpdateDocument as buildMongoDocumentUpdate,
  formatMongoShellLiteral as formatMongoDocumentShellLiteral,
...
test("parseMongoWriteCommand parses createIndex with optional options", () => {
  assert.deepEqual(
    parseMongoWriteCommand("db.users.createIndex({email: 1}, {unique: true, name: 'users_email_unique'})"),
    {
      kind: "createIndex",
      collection: "users",
      keys: '{"email": 1}',
...
test("buildMongoUpdateDocument ignores _id and preserves typed values", () => {
  ...
  assert.equal(
    formatMongoDocumentShellLiteral(update),
    '{"$set":{"age":42,"profile":{"role":"admin"}},"$unset":{"nickname":""}}',
  )
})
