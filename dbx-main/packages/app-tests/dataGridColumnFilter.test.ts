...
function installFilterFetchMock() {
  globalThis.fetch = (async (input, init) => {
    if (String(input) === "/api/query/build-data-grid-column-values-filter-condition") {
      ...
      const valueClause = nonNull.length
        ? `${quote(options.columnName)} IN (${nonNull.map((value: unknown) => (typeof value === "number" ? value : `'${value}'`)).join(", ")})`
        : ""
...
test("quotes text server-side column filters and appends them to existing WHERE input", async () => {
  ...
  assert.equal(
    appendColumnValueFilterCondition("deleted_at IS NULL", condition),
