...
test("detects obvious remote image URLs", () => {
  assert.equal(
    cellImagePreviewUrl("https://cdn.example.com/avatar/user.png"),
    "https://cdn.example.com/avatar/user.png",
  )
  assert.equal(
...
test("can skip generated binary image previews while keeping URL previews", () => {
  ...
  assert.equal(
    cellImagePreviewUrl("https://cdn.example.com/avatar.png", "longblob", { binary: false }),
...
test("detects binary image values from byte arrays and buffer-like values", () => {
  ...
  assert.match(
    cellImagePreviewUrl({ type: "Buffer", data: [0xff, 0xd8, 0xff, 0xe0] }) ?? "",
