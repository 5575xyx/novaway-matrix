...
} from "../../apps/desktop/src/lib/table/routineExecutionSql.ts"
import {
  routineParametersFromResult,
  routineParametersQuery,
} from "../../apps/desktop/src/lib/table/routineParameters.ts"
...
test("builds procedure execution templates for common dialect families", () => {
  ...
  assert.equal(
    buildProcedureExecutionSql({ databaseType: "databend", schema: "default", routineName: "refresh_stats" }),
    "CALL PROCEDURE refresh_stats();",
  )
})
...
test("builds parameter metadata queries for supported dialects", () => {
  ...
  assert.match(
    routineParametersQuery({ database: "default", databaseType: "databend", schema: "default", routineName: "demo" }) ??
      "",
    /system\.procedures/,
  )
  assert.match(
