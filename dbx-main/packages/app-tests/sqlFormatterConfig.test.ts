...
} from "../../apps/desktop/src/lib/sql/sqlFormatterConfig.ts"
...
test("keeps valid formatter settings and clamps invalid values", () => {
  ...
  assert.deepEqual(
    normalizeSqlFormatterSettings({
      ...
      editor: {
        shortcuts: [
          { id: "duplicateLine", keys: { windows: "Ctrl+W", linux: "Ctrl+W", macos: "Cmd+W" }, enabled: false },
        ],
      },
    }),
    DEFAULT_SQL_FORMATTER_SETTINGS,
  ...
  const result = parseSqlFormatterConfig(
    JSON.stringify({
      ...
      editor: {
        ...
        shortcuts: [
          {
            id: "unknownLegacyShortcut",
            action: "copyLineDown",
            keys: { windows: "", linux: "Ctrl+W", macos: "Cmd+W" },
            enabled: "yes",
          },
        ],
      },
...
test("rejects malformed formatter config files", () => {
  ...
  assert.deepEqual(
    parseSqlFormatterConfig(JSON.stringify({ version: 2, formatter: "sql-formatter", options: {} })).ok,
    false,
  )
  assert.deepEqual(
    parseSqlFormatterConfig(JSON.stringify({ version: 1, formatter: "prettier", options: {} })).ok,
    false,
  )
  assert.deepEqual(
...
test("rejects invalid known formatter option values when parsing config files", () => {
  ...
  const invalidParams = parseSqlFormatterConfig(
    JSON.stringify({ version: 1, formatter: "sql-formatter", options: { params: ["42"] } }),
  )
  assert.equal(invalidParams.ok, false)
  if (!invalidParams.ok) assert.match(invalidParams.message, /params/)

  const legacyNullParams = parseSqlFormatterConfig(
    JSON.stringify({ version: 1, formatter: "sql-formatter", options: { params: null } }),
  )
  assert.equal(legacyNullParams.ok, true)

  const invalidParamTypes = parseSqlFormatterConfig(
    JSON.stringify({ version: 1, formatter: "sql-formatter", options: { paramTypes: { custom: [{ regex: "" }] } } }),
  )
