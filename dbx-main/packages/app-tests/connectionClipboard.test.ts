...
async function withConnectionStore(
  initialConnections: ConnectionConfig[],
  initialLayout: SidebarLayout | null,
  run: (store: ReturnType<typeof useConnectionStore>) => Promise<void>,
) {
  ...
  globalThis.fetch = (async (input, init) => {
    ...
    if (url === "/api/connection/list") {
      return new Response(JSON.stringify(savedConnections), {
        status: 200,
...
test("copies and pastes a root connection without writing to the OS clipboard", async () => {
  await withConnectionStore(
    [conn("conn-1", "Main MySQL")],
    { groups: [], order: [{ type: "connection", id: "conn-1" }] },
    async (store) => {
