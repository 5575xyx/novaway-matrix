...
import {
  ...
  normalizeUpdateDownloadSource,
  resolveUpdateReleaseUrl,
  tagVersion,
} from "../../apps/desktop/src/composables/useAppUpdater.ts"
...
test("resolves release page URL from update download source", () => {
  ...
  assert.equal(
    ...
    "https://cnb.cool/dbxio.com/dbx/-/releases/tag/v0.5.39",
  )
  assert.equal(
    resolveUpdateReleaseUrl(updateInfo({ latest_version: "0.5.44" }), "atomgit", fallbackUrl),
    "https://atomgit.com/t8y2/dbx/releases/v0.5.44",
  )
  assert.equal(
    resolveUpdateReleaseUrl(
      updateInfo({ release_url: "https://github.com/t8y2/dbx/releases/tag/v0.5.39" }),
      "official",
      fallbackUrl,
    ),
    "https://github.com/t8y2/dbx/releases/tag/v0.5.39",
  )
