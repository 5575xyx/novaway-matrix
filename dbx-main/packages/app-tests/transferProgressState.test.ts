...
import {
  isTransferRunActive,
  nextTransferTerminalState,
  shouldKeepTransferDraftOnOpen,
  shouldResetTransferDialogOnOpen,
} from "../../apps/desktop/src/lib/app/transferProgressState.ts"
...
test("keeps terminal flags for done and cancelled progress events", () => {
  assert.deepEqual(nextTransferTerminalState({ done: false, cancelled: false, error: false }, { status: "done" }), {
    done: true,
    cancelled: false,
    error: false,
  })
...
test("keeps running transfer dialog state but resets terminal result on reopen", () => {
  ...
  assert.equal(
    shouldResetTransferDialogOnOpen({ transferring: true, done: false, cancelled: false, error: false }),
    false,
  )
  assert.equal(
    shouldResetTransferDialogOnOpen({ transferring: true, done: false, cancelled: false, error: true }),
    true,
  )
  assert.equal(
    shouldResetTransferDialogOnOpen({ transferring: true, done: true, cancelled: false, error: false }),
    true,
  )
  assert.equal(
    shouldResetTransferDialogOnOpen({ transferring: true, done: false, cancelled: true, error: false }),
    true,
  )
})
...
test("keeps editable transfer draft across repeated dialog opens until user starts a new task", () => {
  assert.equal(
    shouldKeepTransferDraftOnOpen(true, { transferring: false, done: false, cancelled: false, error: false }),
    true,
  )
  assert.equal(
    shouldKeepTransferDraftOnOpen(false, { transferring: true, done: false, cancelled: false, error: true }),
    true,
  )
  assert.equal(
    shouldKeepTransferDraftOnOpen(false, { transferring: false, done: false, cancelled: false, error: false }),
    false,
  )
})
