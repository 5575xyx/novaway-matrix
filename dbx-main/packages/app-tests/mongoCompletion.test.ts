...
test("uses getCollection apply text for unsafe collection names", () => {
  const item = buildMongoCompletionItems("db.order", "db.order".length, { collections }).find(
    (candidate) => candidate.label === "order-items",
  )
...
test("suggests collection stats methods after a collection reference", () => {
  ...
  const item = buildMongoCompletionItems("db.users.stat", "db.users.stat".length).find(
    (candidate) => candidate.label === "stats",
  )
...
test("suggests cursor methods after find result chains", () => {
  ...
  const formattedChainItems = buildMongoCompletionItems(
    "db.characters.find({\n  name: 'Ada'\n})\n  .",
    "db.characters.find({\n  name: 'Ada'\n})\n  .".length,
  )
  const formattedPrefixedItems = buildMongoCompletionItems(
    "db.characters.find({\n  name: 'Ada'\n})\n  .li",
    "db.characters.find({\n  name: 'Ada'\n})\n  .li".length,
  )
...
test("suggests query fields at object starts and after commas", () => {
  ...
  const afterComma = buildMongoCompletionItems(
    "db.users.find({ name: 'Ada', ",
    "db.users.find({ name: 'Ada', ".length,
    { fields },
  )
...
test("completion context is tolerant of unfinished input", () => {
  const context = getMongoCompletionContext(
    'db.getCollection("users").find({ "',
    'db.getCollection("users").find({ "'.length,
  )
