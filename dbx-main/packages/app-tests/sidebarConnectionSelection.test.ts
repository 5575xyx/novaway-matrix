...
} from "../../apps/desktop/src/lib/sidebar/sidebarConnectionSelection.ts"
...
test("uses selected connections as clipboard targets only when the selection is connection-only", () => {
  ...
  assert.deepEqual(
    selectedConnectionClipboardNodes([first, second]).map((node) => node.connectionId),
    ["conn-1", "conn-2"],
  )
  assert.deepEqual(selectedConnectionClipboardNodes([first, table]), [])
  assert.deepEqual(
    selectedConnectionClipboardTargets(first, [first, second]).map((node) => node.connectionId),
    ["conn-1", "conn-2"],
  )
  assert.deepEqual(
    selectedConnectionClipboardTargets(first, [first, table]).map((node) => node.connectionId),
    ["conn-1"],
  )
})
