...
test("table metadata loader deduplicates equivalent in-flight requests and caches the result", async () => {
  ...
  const { clearTableMetadataCache, loadTableMetadata } = await import(
    "../../apps/desktop/src/lib/metadata/tableMetadataCache.ts"
  )
...
test("table metadata invalidation keeps unrelated schemas cached", async () => {
  ...
  const { clearTableMetadataCache, getCachedTableMetadata, invalidateTableMetadataCache, loadTableMetadata } =
    await import("../../apps/desktop/src/lib/metadata/tableMetadataCache.ts")
  clearTableMetadataCache()
