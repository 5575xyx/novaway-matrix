...
test("parses standard enum values", () => {
  assert.deepEqual(enumValuesForColumn({ data_type: "enum('pending','active','archived')" }), [
    "pending",
    "active",
    "archived",
  ])
...
test("parses MySQL enum values with charset and collation suffixes", () => {
  assert.deepEqual(
    enumValuesForColumn({
      data_type:
        "enum('id','string','text','long','double','bool','date','datetime','object','secret') CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci",
    }),
