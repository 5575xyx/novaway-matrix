...
} from "../../apps/desktop/src/lib/metadata/metadataLoadCoordinator.ts"
...
test("metadata scope key separates result-affecting fields", () => {
  ...
  assert.notEqual(
    metadataScopeKey({ ...base, searchFilter: "user" }),
    metadataScopeKey({ ...base, searchFilter: "order" }),
  )
  assert.notEqual(metadataScopeKey({ ...base, offset: 0 }), metadataScopeKey({ ...base, offset: 500 }))
  assert.notEqual(
    metadataScopeKey({ ...base, objectTypes: ["TABLE"] }),
    metadataScopeKey({ ...base, objectTypes: ["VIEW"] }),
  )
  assert.notEqual(
    metadataScopeKey({ ...base, sidebarDisplayMode: "simple" }),
    metadataScopeKey({ ...base, sidebarDisplayMode: "grouped" }),
  )
})
...
test("metadata load coordinator keeps different filters independent", async () => {
  ...
  const user = coordinator.run(
    { kind: "sidebar-table-search", connectionId: "conn", database: "app", schema: "public", searchFilter: "user" },
    async () => {
      calls++
      return first.promise
    },
  )
  const order = coordinator.run(
    { kind: "sidebar-table-search", connectionId: "conn", database: "app", schema: "public", searchFilter: "order" },
    async () => {
      calls++
