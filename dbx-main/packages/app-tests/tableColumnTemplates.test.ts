...
test("filters configured fields by current database type", () => {
  const columns = createTableColumnTemplateDrafts({
    ...
    databaseType: "postgres",
    columnNames: [
      "mysql_only | mysql:bigint",
      "postgres_only | postgres:uuid",
      "common_name | mysql:<empty> | postgres:<empty>",
      "common_code | mysql:varchar(64) | postgres:varchar(32)",
    ],
    createId: () => "id",
  })
...
test("normalizes configured preset fields without adding built-in fields", () => {
  const [tenantId, requestId] = normalizeTableColumnTemplateFields([
    " tenant_id | mysql:bigint | postgres:uuid | default:0 ",
    "tenant_id | mysql:int",
    "",
  ...
  const parsed = parseTableColumnTemplateFields([
    "tenant_id | mysql:bigint | postgres:uuid | nullable:true | default:0 | comment:Tenant",
  ])
  assert.deepEqual(parsed[0], {
    name: "tenant_id",
    dataTypesByDatabase: { mysql: "bigint", postgres: "uuid" },
    defaultValue: "0",
    isNullable: true,
    comment: "Tenant",
  })
