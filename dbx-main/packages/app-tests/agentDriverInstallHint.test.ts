...
} from "../../apps/desktop/src/lib/connection/agentDriverInstallHint.ts"
...
test("detects available updates for the selected agent driver", () => {
  assert.equal(
    hasAgentDriverUpdate("dameng", [{ db_type: "dameng", installed: true, update_available: true }], "dm"),
    true,
  )
  assert.equal(
    hasAgentDriverUpdate("dameng", [{ db_type: "dameng", installed: true, update_available: false }], "dm"),
    false,
  )
...
test("uses unified Oracle and selected profile keys for update hints", () => {
  assert.equal(
    hasAgentDriverUpdate("oracle", [{ db_type: "oracle", installed: true, update_available: true }], "oracle-10g"),
    true,
  )
  assert.equal(
    hasAgentDriverUpdate(
...
test("appends agent driver update hints once", () => {
  ...
  assert.equal(
    appendAgentDriverUpdateHint("Original error\n\nDriver update available.", hint),
    "Original error\n\nDriver update available.",
  )
