...
test("completed query executions append result runs and select the latest run", async () => {
  ...
  globalThis.fetch = withConnectionHealthMock(async (input, init) => {
    ...
    if (url === "/api/query/execute-multi") {
      ...
      return new Response(
        JSON.stringify([
          { columns: [`run_${executeCount}`], rows: [[executeCount]], affected_rows: 0, execution_time_ms: 1 },
        ]),
...
test("table data export fetches every filtered page", async () => {
  ...
  globalThis.fetch = withConnectionHealthMock(async (input, init) => {
    ...
    if (url === "/api/query/build-table-select-sql") {
      ...
      return new Response(
        JSON.stringify(
          `SELECT * FROM "public"."users" WHERE (status = 'active') ORDER BY "id" DESC LIMIT ${limit} OFFSET ${offset ?? 0};`,
        ),
        { status: 200, headers: { "Content-Type": "application/json" } },
      )
    }
    if (url === "/api/query/execute-multi") {
      ...
      return new Response(
        JSON.stringify([{ columns: ["id", "status"], rows, affected_rows: 0, execution_time_ms: 1 }]),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        },
...
test("agent-session query export raises maxRows to the configured export limit", async () => {
  ...
  globalThis.fetch = withConnectionHealthMock(async (input, init) => {
    ...
    if (url === "/api/query/prepare-pagination-plan") {
      return new Response(
        JSON.stringify({
          sqlToExecute: "SELECT * FROM big_table",
          pageLimit: 10_000,
          pageOffset: 0,
          useAgentResultSession: true,
        }),
