...
import {
  rankSavedSqlHistory,
  savedSqlHistoryScore,
  savedSqlMatchesHistoryScope,
} from "../../apps/desktop/src/lib/savedSql/savedSqlHistory.ts"
import type { SavedSqlFile } from "../../apps/desktop/src/types/database.ts"

function file(
  input: Partial<SavedSqlFile> & Pick<SavedSqlFile, "id" | "name" | "connectionId" | "database">,
): SavedSqlFile {
  return {
    folderId: undefined,
    schema: undefined,
...
test("SQL 库历史按连接和数据库过滤", () => {
  const files = [
    file({ id: "a", name: "a.sql", connectionId: "conn-1", database: "app" }),
    file({ id: "b", name: "b.sql", connectionId: "conn-1", database: "analytics" }),
    file({ id: "c", name: "c.sql", connectionId: "conn-2", database: "app" }),
  ]
...
test("SQL 库历史综合打开次数和最近打开时间排序", () => {
  ...
  const files = [
    file({
      ...
      database: "app",
      openCount: 2,
      openedAt: "2026-06-11T11:00:00.000Z",
    }),
    file({
      id: "frequent",
      name: "frequent.sql",
