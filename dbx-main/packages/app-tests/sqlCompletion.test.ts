...
test("suggests Manticore Search SQL functions and command snippets", () => {
  const matchItems = buildSqlCompletionItems(
    "select * from products where mat",
    "select * from products where mat".length,
    {
      tables,
      columnsByTable,
  ...
  assert.ok(
    showItems.some((item) => item.type === "snippet" && item.label === "show meta" && item.apply === "SHOW META;"),
  )
  assert.ok(
    showTablesItems.some(
      (item) => item.type === "snippet" && item.label === "show tables" && item.apply === "SHOW TABLES;",
    ),
  )
  assert.ok(
    callPqItems.some(
      (item) =>
        item.type === "snippet" &&
        item.label === "call pq" &&
        item.apply === "CALL PQ ('pq', ('{\"title\":\"query\"}'));",
    ),
  )
...
test("suggests all columns expansion for each joined table", () => {
  ...
  assert.equal(
    items.find((item) => item.type === "snippet" && item.label === "o.*")?.apply,
...
test("does not auto-open completion inside SQL comments", () => {
  ...
  assert.equal(
    shouldAutoOpenSqlCompletion("select '-- not comment' as value", "select '-- not comment' as value".length),
    true,
  )
...
test("table alias suggestions avoid existing aliases", () => {
  const items = buildSqlCompletionItems(
    "select * from customer_orders co join customer_orders ",
    "select * from customer_orders co join customer_orders ".length,
    {
...
test("boosts foreign-key related table candidates in JOIN table context", () => {
  const foreignKeysByTable = new Map<string, SqlCompletionForeignKey[]>([
    ["public.orders", [{ name: "orders_user_id_fkey", column: "user_id", ref_table: "users", ref_column: "id" }]],
  ])
