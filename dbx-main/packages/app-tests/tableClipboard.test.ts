...
} from "../../apps/desktop/src/lib/table/tableClipboard.ts"
...
test("table clipboard entries must match the paste target context", () => {
  ...
  assert.equal(
    tableClipboardMatchesTarget([{ connectionId: "c1", database: "other", schema: "public" }], target),
    false,
  )
...
test("table data copy uses only writable columns for first-class databases", () => {
  ...
  assert.deepEqual(
    tableDataCopyColumnOptions("postgres", [
      { ...columns[0], extra: "generated always as identity" },
      { ...columns[1] },
      { ...columns[2], extra: "generated always as (name) stored" },
    ]),
    {
      columns: ["id", "name"],
  ...
  assert.deepEqual(
    tableDataCopyColumnOptions("mysql", [
      { ...columns[0], extra: "auto_increment" },
      { ...columns[1] },
      { ...columns[2], extra: "STORED GENERATED" },
    ]),
    {
      columns: ["id", "name"],
      postgresOverridingSystemValue: false,
