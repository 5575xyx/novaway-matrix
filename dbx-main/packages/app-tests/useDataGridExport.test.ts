...
const { defaultDataGridExportFileName, useDataGridExport } = await import(
...
function buildExportHarness(options: { currentResultLabel?: string; exportFileBaseName?: string } = {}) {
  ...
  const queryResultExportRequest = vi.fn(
    async (options: { exportId: string; filePath: string; format: "csv" | "xlsx" }) => ({
...
test("copy row JSON expands nested JSON strings", async () => {
  ...
  const jsonString =
    '{"endingBalance":{"beginningBalance":"0","endingBalance":"20000","endingDate":"2024-10-30"},"financeChargeInfo":null,"interestChargeInfo":null,"Line":[]}'
  const row = {
    id: 1,
...
test("copy MongoDB row as INSERT uses Mongo shell insert syntax", async () => {
  const contextCell = ref({ rowId: 1, rowIndex: 0, col: 0 })
  const jsonString =
    '{"endingBalance":{"beginningBalance":"0","endingBalance":"100","endingDate":"2024-11-25"},"Line":[]}'
  const row = {
    id: 1,
...
test("copy MongoDB rows as INSERT excludes _id for insert without primary keys", async () => {
  ...
  const rows = [
    {
      id: 1,
      data: ["6743e4bfa3f6f84bc3fff6c8", "done"],
      isNew: false,
      isDeleted: false,
      isDirtyCol: [false, false],
      status: "",
    ...
    {
      id: 2,
      data: ["6743e4bfa3f6f84bc3fff6c9", "draft"],
      isNew: false,
      isDeleted: false,
      isDirtyCol: [false, false],
      status: "",
  ...
  assert.equal(
    clipboardMock.copyToClipboard.mock.calls[0][0],
    'db.getCollection("accounting_reconciliations").insertMany([{"status":"done"},{"status":"draft"}]);',
  )
})
...
test("default data grid export file names use sanitized base names and compact local timestamps", () => {
  ...
  try {
    ...
    assert.equal(
      defaultDataGridExportFileName("daily/report.sql", "export", "xlsx", { page: true }),
      "daily_report_page_260602150405.xlsx",
    )
...
test("full query result CSV export streams through the backend without loading all rows", async () => {
  const { composable, fullExportResult, queryResultExportRequest, exportProgressDialog, exportProgressState } =
    buildExportHarness()
