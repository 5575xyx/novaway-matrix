...
import {
  buildSingleDdlExportFileContent,
  ensureSqlStatementTerminator,
  joinExportedDdls,
} from "../../apps/desktop/src/lib/export/ddlExport.ts"

test("ensureSqlStatementTerminator appends a trailing semicolon when missing", () => {
  assert.equal(
    ensureSqlStatementTerminator("CREATE TABLE `users` (\n  `id` int\n) ENGINE=InnoDB"),
    "CREATE TABLE `users` (\n  `id` int\n) ENGINE=InnoDB;",
  )
})
...
test("joinExportedDdls separates exported objects with blank lines and terminates each statement", () => {
  assert.equal(
    joinExportedDdls(["CREATE TABLE `users` (`id` int)", "CREATE TABLE `posts` (`id` int);"]),
