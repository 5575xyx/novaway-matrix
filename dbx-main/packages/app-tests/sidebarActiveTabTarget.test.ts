...
} from "../../apps/desktop/src/lib/sidebar/sidebarActiveTabTarget.ts"
...
test("sidebar node scrolling keeps visible rows in place and reveals hidden rows", () => {
  ...
  assert.equal(
    scrollTopForSidebarNode({ index: 11, currentScrollTop: 300, viewportHeight: 140, topOcclusionHeight: 28 }),
    280,
  )
})
