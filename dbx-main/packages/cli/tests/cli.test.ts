  ...
  const result = await runCli(["connections", "list", "--json"], {
    backend: fakeBackend({
      loadConnections: async () => {
        throw Object.assign(
          new Error("Failed to load DBX connections from /tmp/dbx/dbx.db: NODE_MODULE_VERSION 127 mismatch"),
          {
...
test("rejects invalid query timeouts", async () => {
  const result = await runCli(["query", "local", "select 1", "--timeout", "forever", "--json"], {
    backend: fakeBackend(),
  })
...
test("allows non-dangerous writes when explicitly enabled", async () => {
  ...
  const result = await runCli(
    ["query", "local", "update users set name = 'x' where id = 1", "--allow-writes", "--json"],
    {
      backend: fakeBackend({
        executeQuery: async () => {
          executed = true
