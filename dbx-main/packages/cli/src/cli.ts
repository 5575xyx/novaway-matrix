...
export async function runCli(argv: string[], options: RunOptions = {}): Promise<CliResult> {
  ...
  try {
    ...
    if (args[0] === "schema" && args[1] === "describe") {
      ...
      if (flags.format === "csv") {
        return ok(
          csvTable(["name", "data_type", "is_nullable", "is_primary_key", "column_default", "comment"], columns),
        )
      }
      return ok(
        `${mdTable(
    ...
    if (args[0] === "query") {
      ...
      const connectionName = usesDefaultConnection
        ? env.DBX_CONNECTION!
        : required(args[1], "Connection name is required.")
      ...
      const sql = flags.file
        ? await readFile(flags.file, "utf-8")
        : required(sqlArg, "SQL string or --file is required.")
...
function sqlSafetyFromCliEnv(
  env: NodeJS.ProcessEnv,
): Required<Pick<SqlSafetyOptions, "allowWrites" | "allowDangerous">> {
  return {
...
function fail(code: string, message: string, json: boolean): CliResult {
  const text = json
    ? `${JSON.stringify(errorPayload(code, message), null, 2)}\n`
...
function formatDoctor(diagnostics: DbxDiagnostics): string {
  const rows = [
    ...
    [
      "Desktop bridge",
      diagnostics.bridgePortFileExists
        ? `available (${diagnostics.bridgeUrl ?? diagnostics.bridgePortFile})`
        : "not running",
    ],
