...
function connectionMatchesScope(config: ConnectionConfig, scope: McpScope): boolean {
  return (
    (!!scope.connectionId && config.id === scope.connectionId) ||
    (!!scope.connectionName && config.name === scope.connectionName)
  )
}
...
async function resolveConnection(
  ...
  if (!scopeEnabled(scope)) {
    ...
    return config
      ? { config }
      : { error: toolError("CONNECTION_NOT_FOUND", `Connection "${requestedName}" not found.`) }
  }
  ...
  if (requestedName?.trim() && requestedName !== scopedConfig.name && requestedName !== scopedConfig.id) {
    return {
      error: toolError(
        ...
        `Connection "${requestedName}" is outside this DBX AI session scope.`,
      ),
    }
  }
  ...
  server.tool(
    ...
    "Execute a Redis command on a Redis connection",
    {
      ...
      db: z
        ...
        .min(0)
        .optional()
        .describe("Redis logical database number (default: scoped/default database or 0)"),
    ...
    async ({ connection_name, db, command }) => {
      ...
      if (scopedConfig.db_type !== "redis") {
        return toolError(
          ...
          `Connection "${scopedConfig.name}" is ${scopedConfig.db_type}, not Redis.`,
        )
      }
      ...
      try {
        const result = await backend.executeRedisCommand(
          scopedConfig,
          defaultRedisDb(scopedConfig, scope, db),
          command,
          {
