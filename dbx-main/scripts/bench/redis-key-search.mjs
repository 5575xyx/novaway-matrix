...
function printReport(result) {
  ...
  console.log(
    `- Seed: ${result.seed.skipped ? "reused existing dataset" : `loaded in ${formatMs(result.seed.elapsedMs)}`}`,
  )
  console.log("")
  ...
  for (const row of result.measurements) {
    console.log(
      `| ${row.label} | ${row.keys} | ${row.calls ?? "-"} | ${row.payloadBytes ? formatBytes(row.payloadBytes) : "-"} | ${formatMs(row.elapsedMs)} |`,
    )
  }
}
