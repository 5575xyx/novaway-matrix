...
async function confirmOrExit(message) {
  ...
  if (!process.stdin.isTTY) {
    fail(
      "Refusing to trigger release without confirmation in a non-interactive shell. Re-run with --yes if this is intentional.",
    )
  }
...
function getLatestAgentTag() {
  ...
  if (legacyRepoCheck.status === 0) {
    const legacyTags = run("git", ["-C", legacyRepo, "tag", "--list", "v*"])
      .stdout.split(/\r?\n/)
  ...
  if (status.needed) {
    const detail =
      status.changedFiles.length > 0
        ? `${status.changedFiles.length} changed file${status.changedFiles.length === 1 ? "" : "s"}`
        : "release baseline missing"
...
function getLatestSemverTag(prefix) {
  const tags = run("git", ["tag", "--list", `${prefix}*`])
    .stdout.split(/\r?\n/)
    .map((tag) => tag.trim())
    .filter(Boolean)
...
function normalizeExplicitVersion(value, explicitTagPrefix) {
  const trimmed = value
    .trim()
    .replace(new RegExp(`^${escapeRegExp(explicitTagPrefix)}`), "")
    .replace(/^v/, "")
