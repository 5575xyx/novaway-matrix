...
assertMatches(
  ...
  /mq-admin\s*=\s*\[[^\]]*"dbx-core\/mq-admin"[^\]]*\]/,
  "Tauri crate must expose an mq-admin feature that enables dbx-core/mq-admin.",
)
assertMatches(
  "src-tauri/Cargo.toml",
  /default\s*=\s*\[[^\]]*"mq-admin"[^\]]*\]/,
  "Tauri default features must include mq-admin so desktop commands are registered.",
)
assertMatches(
  "crates/dbx-web/Cargo.toml",
  /\[features\][\s\S]*mq-admin\s*=\s*\[[^\]]*"dbx-core\/mq-admin"[^\]]*\]/,
  "Web crate must expose an mq-admin feature that enables dbx-core/mq-admin.",
)
assertMatches(
  "crates/dbx-web/Cargo.toml",
  /\[features\][\s\S]*default\s*=\s*\[[^\]]*"mq-admin"[^\]]*\]/,
  "Web default features must include mq-admin so web routes are registered.",
)

assertIncludes(
  "apps/desktop/src/components/layout/ContentArea.vue",
  "MqAdminConsole",
  "Main content area must import and render the MQ admin console.",
)
assertIncludes(
  "apps/desktop/src/components/layout/ContentArea.vue",
  "activeTab.mode === 'mq'",
  "Main content area must have an mq mode render branch.",
)
assertIncludes(
  "apps/desktop/src/components/sidebar/TreeItem.vue",
  '"mq"',
  "Sidebar connection handling must be aware of MQ connections.",
)
assertIncludes(
  "apps/desktop/src/stores/queryStore.ts",
  "openMqAdmin",
  "Query store must expose an MQ admin tab opener.",
)
...
assert.ok(
  drivers?.some((driver) => driver.dbType === "mq"),
  "Driver manifest must include dbType=mq.",
)
...
assertIncludes(
  ...
  'mqTestConnection = forward("mqTestConnection")',
  "MQ frontend calls must use the shared forward() API layer.",
)
assertIncludes(
  "apps/desktop/src/components/connection/ConnectionDialog.vue",
  "mqAdminUrl",
  "Connection dialog must include MQ admin URL fields.",
)
assertIncludes(
  "apps/desktop/src/components/connection/ConnectionDialog.vue",
  "external_config",
  "Connection dialog must submit MQ external_config.",
)
...
for (const panel of ["TenantsPanel.vue", "NamespacesPanel.vue", "TopicsPanel.vue", "SubscriptionsPanel.vue"]) {
  assertIncludes(
    `apps/desktop/src/components/mq/${panel}`,
    "readOnly",
    `${panel} must disable mutating actions in read-only mode.`,
  )
}
