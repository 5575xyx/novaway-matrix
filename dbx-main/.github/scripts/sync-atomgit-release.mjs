...
function atomGitAssetNames(release) {
  const names = new Set()
  const groups = [release?.assets, release?.attach_files, release?.attachFiles, release?.files, release?.attachments]
...
class AtomGitClient {
  ...
  async getRelease(tag) {
    const res = await this.request(
      "GET",
      `/repos/${this.owner}/${this.repo}/releases/${encodeURIComponent(tag)}`,
      null,
      {
        allow404: true,
      },
  ...
  async getUploadTarget(tag, fileName) {
    ...
    const res = await this.request(
      "GET",
      `/repos/${this.owner}/${this.repo}/releases/${encodeURIComponent(tag)}/upload_url?${params}`,
    )
