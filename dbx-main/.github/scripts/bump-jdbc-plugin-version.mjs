...
export function evaluateJdbcPluginReleaseBump({ changedFiles, pomXml, manifestJson }) {
  ...
  if (pomVersion !== currentManifestVersion) {
    throw new Error(
      `JDBC plugin version mismatch: pom.xml is ${pomVersion} but manifest.json is ${currentManifestVersion}.`,
    )
  }

  const shouldBump =
    changedFiles.some(isReleaseBumpRelevantJdbcPluginChange) && !changedFiles.some(hasJdbcPluginVersionChange)
