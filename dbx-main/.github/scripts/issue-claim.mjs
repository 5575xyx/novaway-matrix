...
const assignees = await ghJson(["issue", "view", issueNumber, "--json", "assignees", "-q", ".assignees"])

if (assignees.length > 0) {
  const names = assignees.map((a) => `@${a.login}`).join(", ")
  await gh(["issue", "comment", issueNumber, "--body", `❌ @${commentUser} 这个 issue 已经有人认领了：${names}`])
  process.exit(0)
}
...
// Confirm
await gh(["issue", "comment", issueNumber, "--body", `✅ @${commentUser} 已认领 #${issueNumber}，开始处理吧！`])
