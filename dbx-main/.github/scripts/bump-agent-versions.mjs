...
function parseLegacyStandaloneProjects(buildGradle) {
  ...
  return new Set([...match[1].matchAll(/['"]([^'"]+)['"]/g)].map((entry) => entry[1]).filter(Boolean))
}
...
export function evaluateAgentVersionBump({
  ...
  if (manualVersionsChanged && !skipBump) {
    logs.push(
      "Manual agents/versions.json changes detected; preserving manually changed module versions and auto-bumping the rest.",
    )
  }

  if (skipBump) {
    logs.push(
      "Skipping automatic module version bump for migrated first release; versions.json was carried over from dbx-agents.",
    )
  ...
  if (commonChanged) {
    logs.push(
      "Common agent runtime changes detected; common-triggered bumps are limited to modules that package agents/common.",
    )
  }
...
function main() {
  ...
  const changedFiles = options.skipBump
    ? []
    : git(["diff", "--name-only", `${options.prevTag}..HEAD`])
        .split("\n")
        .filter(Boolean)
