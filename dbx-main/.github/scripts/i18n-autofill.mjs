...
function groupMissingKeys(targetRoot, missingKeys, translations, sourceRoot) {
  ...
  for (const key of missingKeys) {
    ...
    if (!byParent.has(parentKey))
      byParent.set(parentKey, { objectNode: parentNode, sourceObjectNode: sourceParent, entries: [] })
  ...
  return [...byParent.values()].map((group) => ({
    ...
    entries: group.entries.sort(
      (left, right) => left.order - right.order || left.path.join(".").localeCompare(right.path.join(".")),
    ),
  }))
}
...
function findInsertionParent(root, fullPath) {
  ...
  for (const segment of fullPath.slice(0, -1)) {
    ...
    if (property.value.type !== "object") {
      throw new Error(
        `${fullPath.join(".")}: cannot insert under non-object locale key ${[...existingPath, segment].join(".")}`,
      )
    }
...
function buildInsertion(text, objectNode, entries, sourceObjectNode) {
  ...
  const childIndent =
    objectNode.properties.length > 0 ? lineIndentBefore(text, objectNode.properties[0].start) : `${parentIndent}  `
...
function renderEntryTree(tree, sourceObjectNode, indent) {
  const lines = []
  const entries = [...tree.children.entries()].sort(([left], [right]) =>
    compareSourceOrder(sourceObjectNode, left, right),
  )
  ...
  for (const [key, child] of entries) {
    ...
    const childSourceObject = sourceObjectNode?.properties.find(
      (property) => property.key === key && property.value.type === "object",
    )?.value
