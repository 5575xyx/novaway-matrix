...
- `bunfig.toml` sets `minimumReleaseAge = 259200`: newly published dependency versions are frozen for 3 days (opentui packages exempted). Check this first when a new dep version fails to install.
- `./script/generate.ts` runs SDK build → `bun dev generate > openapi.json` → auto-format.
