...
// ../../../presenton-preview-build/stubs/chart-data.mjs
function markdownToPlainChartText(value) {
  return String(value ?? "")
}

// ../../../presenton-preview-build/stubs/template-v2-text.mjs
function normalizeRawTextMarkdownElement(element) {
...
// ../../../presenton-preview-build/stubs/math.mjs
...
// ../../../presenton-preview-build/stubs/chart-browser.mjs
var CHART_BROWSER_SCRIPT_URL = "/vendor/chart-4.5.1.umd.min.js"
var CHART_DATALABELS_SCRIPT_URL = "/vendor/chartjs-plugin-datalabels-2.2.0.min.js"
...
var DEFAULT_CHART_COLORS = ["#7F22FE", "#155DFC", "#F59E0B", "#12B76A", "#EF4444", "#06B6D4", "#8B5CF6", "#64748B"]
...
function templateV2UiToHtml(ui, options = {}) {
  ...
  return jsonToHtml(payload.items, payload.width, payload.height, payload.fonts, payload.background)
}
function templateV2UiToHtmlFragment(ui, options = {}) {
  const payload = templateV2RenderPayload(ui, options)
  if (!payload) return null
  return jsonToHtmlFragment(payload.items, payload.width, payload.height, payload.fonts, payload.background)
}
function templateV2RenderPayload(ui, options) {
  ...
  const items = [...rootElements, ...components].map((item) => readRecord(normalizeTemplateV2AssetUrls(item)))
  if (items.length === 0) {
    return null
  }
...
function normalizeTemplateV2AssetUrls(value) {
  ...
  const normalized = Object.fromEntries(
    Object.entries(record).map(([key, child]) => [key, normalizeTemplateV2AssetUrls(child)]),
  )
...
function jsonToHtmlFragment(items, width, height, fonts = {}, background = "#FFFFFF") {
  const records = items.map(readRecord)
  const bg = escapeCssColor(background)
  return `${renderFontAssetTags(fonts)}${renderSlideRoot(records, width, height, bg)}`
}
...
function renderFontAssetTags(fonts) {
  ...
  if (embeddedCss) {
    tags.push(`<style>${escapeStyleText(embeddedCss)}${fontCssFamilyAliases(embeddedCss)}</style>`)
  }
  ...
  tags.push(
    ...stylesheets.map((url) => `<link rel="stylesheet" href="${escapeAttribute(resolveBackendAssetUrl(url))}">`),
  )
  return tags.join("")
}
function normalizeFontFaces(fonts) {
  ...
  return Object.entries(readRecord(fonts)).flatMap(([family, value]) =>
    family === "css" || family === "font_css" || family === "fonts" ? [] : normalizeFontFaceEntry(family, value),
  )
}
function normalizeFontFaceEntry(fallbackFamily, value) {
  ...
  const record = readRecord(value)
  const family =
    readString(record.family ?? record.name ?? record.fontFamily ?? record.font_family) ?? fallbackFamily?.trim()
  const source = readString(record.url ?? record.src ?? record.href ?? record.data ?? record.source)
  ...
  return [
    {
      ...
      style:
        readFontStyle(record.style ?? record.fontStyle ?? record.font_style) ?? inferFontStyle(`${family} ${source}`),
    },
  ]
}
...
function renderImage(item, mode) {
  ...
  if (color && readBoolean(item.isIcon ?? item.is_icon)) {
    ...
    return `<div style="${frameStyle(item, mode)}${boxStyle(item)}color:${escapeCssColor(
      color,
  ...
  return `<img alt="" src="${escapeAttribute(source)}" style="${frameStyle(item, mode)}${boxStyle(
    item,
...
function renderTable(item, mode) {
  ...
  if (!rows.length) {
    return `<div style="${frameStyle(item, mode)}${transformStyle(item)}overflow:hidden"></div>`
  }
  ...
  const cells = rows
    .flatMap((row, rowIndex) =>
      Array.from({ length: colCount }, (_, colIndex) => {
        ...
        return `<div style="${tableCellStyle(cell, isHeader, tableFont)}">${cellText(cell, tableFont, isHeader)}</div>`
      }),
    )
    .join("")
...
function renderContainer(item, mode) {
  ...
  )}display:flex;align-items:${verticalAlign(readString(alignment.vertical))};justify-content:${horizontalAlign(
...
function renderPolygon(item, mode) {
  ...
  return `<div style="${frameStyleFromBox(box, mode)}${transformStyle(
    item,
  )}overflow:visible"><svg width="100%" height="100%" viewBox="0 0 ${cssNumber(box.width ?? 1)} ${cssNumber(
    box.height ?? 1,
...
function renderEllipseVector(item, mode) {
  ...
  return `<div style="${frameStyleFromBox(box, mode)}${transformStyle(
    item,
  )}overflow:visible"><svg width="100%" height="100%" viewBox="0 0 ${cssNumber(width)} ${cssNumber(
    height,
  )}" preserveAspectRatio="none" style="display:block;overflow:visible">${shape}</svg></div>`
}
...
function renderProgressBarInfographic(item, mode) {
  ...
  const label = showLabel
    ? `<div style="color:#111827;font-size:${cssNumber(
        Math.max(10, Math.min(16, Math.round((box.height ?? fallbackSize.height) * 0.3))),
      )}px;font-weight:700;line-height:1;text-align:right">${escapeHtml(metrics.label)}</div>`
    : ""
  return `<div style="${frameStyle(item, mode, fallbackSize)}${transformStyle(
    ...
    Math.max(6, Math.min(18, Math.round((box.height ?? fallbackSize.height) * 0.35))),
...
function chartConfig(item, height) {
  ...
  const showLegend = readOptionalBoolean(item.legend ?? item.showLegend, autoShowLegend)
  ...
  const xAxisGrid = readOptionalBoolean(item.x_axis_grid ?? item.xAxisGrid ?? item.grid, true)
  const yAxisGrid = readOptionalBoolean(item.y_axis_grid ?? item.yAxisGrid ?? item.grid, true)
  ...
  const xAxisTitle = markdownToPlainChartText(
    readString(Object.prototype.hasOwnProperty.call(item, "x_axis_title") ? item.x_axis_title : item.xAxisTitle) ?? "",
  )
  const yAxisTitle = markdownToPlainChartText(
    readString(Object.prototype.hasOwnProperty.call(item, "y_axis_title") ? item.y_axis_title : item.yAxisTitle) ?? "",
  )
  const config = {
...
function chartDatasets(chartKind, data) {
    ...
    return [
      {
        ...
        backgroundColor: series.values.map(
          (_, index) =>
            data.colors[index % data.colors.length] ?? DEFAULT_CHART_COLORS[index % DEFAULT_CHART_COLORS.length],
        ),
        borderColor: "#FFFFFF",
        borderWidth: 1,
        hoverOffset: 0,
  ...
  if (chartKind === "scatter" || chartKind === "bubble") {
    return data.series.map((seriesItem) => {
      const colors =
        data.series.length === 1 ? categoryColors(seriesItem, data.colors) : [seriesColor(seriesItem, data)]
      return {
        label: seriesItem.name,
        data:
          chartKind === "bubble"
  ...
  return data.series.map((series, index) => {
    ...
    const perCategoryColors =
      data.series.length === 1 && data.colors.length ? categoryColors(series, data.colors) : null
    ...
    const dataset = {
      ...
      maxBarThickness: 62,
      presentonBarRadius:
        barChart && !stackedBarChart ? { horizontal: isHorizontalChart(chartKind), radius: 7 } : void 0,
...
function normalizeChartData(item, chartKind) {
  ...
  const series = readArray(item.series)
    ...
    .map((series2, index) => {
      ...
      return {
        ...
        points: rawValues.map((value, valueIndex) => chartPoint(value, valueIndex)),
        values,
      }
    })
  ...
  const maxLength = Math.min(24, Math.max(1, ...series.map((item2) => item2.values.length), points.length))
...
function describeGaugeArc(centerX, centerY, radius, ratio) {
  ...
  const end = gaugePoint(centerX, centerY, radius, 180 + clamp(ratio, 0, 1) * 180)
...
function readBox(item, fallbackSize) {
  ...
  if (type === "vector") {
    return polygonBox(item, vectorShape(item) === "ellipse" ? polygonSourcePoints(item) : polygonPoints(item))
  }
  return {
...
function sampleSmoothCurve(points, closed, tension, segments) {
  ...
  for (let index = 0; index < segmentCount; index += 1) {
    ...
    for (let step = 1; step <= segments; step += 1) {
      sampled.push(hermitePoint(p1, p2, startTangent, endTangent, step / segments))
    }
  }
  return sampled
}
...
function boxStyle(item) {
  ...
  if (fillColor) {
    style += `background-color:${escapeCssColor(colorWithOpacity(fillColor, readNumber(fill.opacity)))};`
  }
...
function tableRows(item) {
  ...
  return (columns.length ? [columns, ...bodyRows] : bodyRows).filter((row) => Array.isArray(row))
}
...
function tableCellStyle(cellValue, header, tableFont) {
  ...
  let style = `${fontStyle(cellFont, {
    includeTextDecoration: false,
  })}display:flex;align-items:center;justify-content:${horizontalAlign(alignment)};border:${cssNumber(
...
function cellText(cellValue, tableFont, header) {
  ...
  if (typeof text === "string") {
    return `<span style="${fontStyle(
      readableTableFont({ ...tableFont, ...readRecord(cell.font), ...headerFontPatch }, fillColor, header),
...
function parseRgbColor(color) {
  ...
  if (!rgb) return null
  return [clamp(Number(rgb[1]), 0, 255), clamp(Number(rgb[2]), 0, 255), clamp(Number(rgb[3]), 0, 255)]
}
...
function paddingStyle(padding) {
  ...
  )}px ${cssNumber(readNumber(padding.bottom) ?? 0)}px ${cssNumber(readNumber(padding.left) ?? 0)}px;`
}
...
function clipPathStyle(item) {
  const value = normalizeCssClipPath(readString(item.clippath ?? item.clipPath ?? item.clip_path))
