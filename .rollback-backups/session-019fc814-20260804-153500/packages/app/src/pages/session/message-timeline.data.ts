...
import { AssistantMessage, Part, SessionStatus, SnapshotFileDiff, UserMessage } from "@novaway/sdk/v2"
import { groupParts, PartGroup, renderable } from "@novaway/ui/message-part"
...
export type TimelineRowMap = {
  ...
  DiffSummary: { userMessageID: string; diffs: SummaryDiff[]; fallback: boolean }
...
export namespace Timeline {
  export function constructMessageRows(
    ...
    const visibleUserContent = userParts.some((part) => {
    ...
    if (visibleUserContent || comments.length > 0 || compaction) {
      rows.push(
    ...
    const sourceDiffs =
      userMessage.summary?.diffs && userMessage.summary.diffs.length > 0
        ? userMessage.summary.diffs
        : [
            ...fallbackDiffs,
            ...assistantMessages.flatMap((message) =>
              getMessageParts(message.id)
                .filter((part): part is Part & { files: string[] } => part.type === "patch")
                .flatMap((part) => part.files.map((file) => ({ file, additions: 0, deletions: 0 }))),
            ),
          ]
    const usingFallback =
      !(userMessage.summary?.diffs && userMessage.summary.diffs.length > 0) && sourceDiffs.length > 0
    const diffs = sourceDiffs
